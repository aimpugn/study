...
### 라이브 코딩 1차 대비 (활성 스케줄 · 지원사 · 2026-07-03 금 10:00 · HackerRank CodePair · Java)

**확정 사항(2026-06-29)**: 1차 = 지원사, **HackerRank CodePair**(LeetCode 아님), 금 10시, 최대 1시간, Google Meet 음성/화상, 다른 직군 인터뷰어 가능, CS 기초도 평가 가능. 평가 무게중심은 *정답 < 접근·사고 과정·think-aloud*이고 7단계 흐름(명료화 → 접근+복잡도 → 구현 → 테스트+엣지 → 최적화)을 명시 안내받음. 아래 D-표의 LeetCode 번호는 **패턴 지도**로만 쓰고, 실제 연습은 `examples/hackerrank/jvm` 트랙에서 동급 HackerRank 문제로 한다. 루프: HR 사이트에서 풀기(적으며 발화) → `Solution.java` 복붙 → `algorithm-annotate` 첨삭. 복잡도 발화 기준은 `algorithms/complexity_analysis_playbook.md`. 언어 Java 고정. CS 기초는 study/interview에서 사용자가 별도 정리(원하면 모의 중 1~2개 삽입).

1차가 라이브 코딩으로 확정이라, 이 절이 1차까지의 활성 스케줄이다. 아래 2주 압축/5주 표는 문제 풀 + 1차 이후 계속용으로 내려간다. 핵심 판정(2026-06-26): **"어려운 문제만 풀기"는 1차 라이브에 역효과** — 라이브는 정답 여부만큼 소통·접근·테스트를 보고, Hard-only는 인식 폭을 좁혀 예상 밖 Medium에서 더 잘 얼어붙는다. 그래서 Hard-only가 아니라 `매일 라이브 모의 + Medium 유창성 + canonical Hard 노출 + 라이브 빈출 설계형(LRU) + 전날 taper`. 단, 난이도가 높은 만큼 Hard·설계 비중은 평소보다 올린다.
