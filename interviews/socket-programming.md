...
- [소켓이란 무엇이고 소켓 프로그래밍이란 무엇인가](#소켓이란-무엇이고-소켓-프로그래밍이란-무엇인가)
  ...
  - [OS 중립 모델과 OS별 API 차이](#os-중립-모델과-os별-api-차이)
...
소켓(socket)은 애플리케이션이 운영체제의 네트워크 기능을 쓰기 위해 여는 통신 끝점입니다. OS 중립적으로 말하면, 소켓은 "내 프로그램이 특정 전송 방식(TCP/UDP 등)으로 상대와 데이터를 주고받기 위해 OS에 등록한 통로"입니다. Unix/Linux/macOS 같은 POSIX 계열에서는 이 통로가 파일 디스크립터(file descriptor)라는 정수 손잡이로 보이는 경우가 많고, Windows에서는 Winsock의 `SOCKET` 핸들처럼 다른 형태로 보입니다. 겉모양은 달라도 커널 또는 OS 네트워크 스택 안에서 주소, 프로토콜 상태, 송수신 버퍼, 오류와 종료 상태가 관리된다는 점은 같습니다.
...
## OS 중립 모델과 OS별 API 차이

소켓을 OS 중립적으로 설명할 때는 함수 이름보다 아래 불변식을 먼저 잡는 편이 안전합니다.
...
운영체제마다 API 이름, 핸들 타입, 비동기 I/O 모델, 옵션 이름은 달라질 수 있습니다. 그래도 대부분의 네트워크 프로그래밍은 다음 질문을 공유합니다.
...
4. 기다림을 어떻게 표현할 것인가
    ...
    blocking call로 기다릴 수도 있고, `select`/`poll`/`epoll`/`kqueue`, Windows IOCP, Apple Network framework의 callback/queue처럼 OS별 이벤트 모델을 쓸 수도 있습니다. 이 차이는 구현 방식의 차이이지, "소켓은 OS 네트워크 스택에 등록된 통신 끝점"이라는 핵심 모델을 바꾸지는 않습니다.

면접에서는 따라서 먼저 OS 중립 모델로 답하고, 그다음 대표 구현을 붙이면 좋습니다.

> 개념적으로 소켓은 OS 네트워크 스택에 만든 통신 끝점입니다. Unix 계열에서는 그 끝점을 파일 디스크립터로 다루고, Windows에서는 Winsock `SOCKET` 핸들로 다루며, Java나 Go 같은 언어는 다시 `Socket`, `ServerSocket`, `Listener`, `Conn` 같은 객체로 감쌉니다. API 모양은 달라도 주소를 붙이고, 연결을 맺거나 받고, 바이트나 데이터그램을 주고받고, 오류와 종료를 처리한다는 생명주기는 거의 같습니다.
