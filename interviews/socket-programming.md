...
여기서 중요한 점은 오류 메시지 하나만 보고 근본 원인을 확정하면 안 된다는 것입니다. 예를 들어 `Connection reset`은 Java 예외 이름이 아니라 TCP 사건이 애플리케이션에 드러난 것입니다. 패킷을 보면 서버가 보냈을 수도 있고, 로드밸런서가 보냈을 수도 있고, 클라이언트 쪽 종료가 서버 로그에 반대로 찍힌 것일 수도 있습니다.
