# 데이터베이스 운영, 보안, 장애 분석은 어떻게 증상에서 원인으로 좁혀 가는가?
...
이 문서는 느린 SQL, lock 대기, 복제 지연, 자원 포화, 관측 지표, 권한, 행 단위 보안, 암호화, 감사 로그, secret 관리를 하나의 운영 흐름으로 묶습니다. 핵심은 "DB가 느리다"라는 흐린 증상을 요청 식별자, 실행 계획, wait event, blocker graph, OS 자원, 권한 경계, 감사 기록 같은 확인 가능한 증거로 좁혀 가는 능력입니다.
...
보안은 `GRANT` 문 몇 개가 아니라 역할과 소유권을 나누는 설계입니다. 예를 들어 `app_runtime`은 주문 조회와 결제 상태 갱신에 필요한 DML만 갖고, `app_migration`은 schema 변경을 맡으며, `report_user`는 읽기 전용으로 제한하고, table owner는 runtime 계정이 되지 않게 분리합니다. 행 단위 보안(row-level security, RLS)은 같은 table 안에서도 tenant별로 볼 수 있는 row를 제한하는 장치지만, owner나 우회 권한, 정책 누락을 조심해야 합니다. 암호화도 저장 시 암호화, 전송 구간 TLS, 애플리케이션 필드 암호화, key 관리 위치를 나누어 봐야 합니다. 감사 로그는 누가 언제 무엇을 봤고 바꿨는지 재구성할 수 있어야 하지만, 민감값 자체를 로그로 남기면 안 됩니다.
...
이런 관측 방식이 필요한 배경은 현대 서비스에서 DB가 더 이상 한 사람이 접속해 보는 단일 서버가 아니기 때문입니다. 애플리케이션 instance가 여러 대이고, connection pool이 각 instance마다 있으며, primary/replica, cache, queue, search index가 같은 요청 경로에 붙습니다. 사용자는 "결제 화면이 느리다"고 말하지만, 실제로는 app thread pool, DB pool, DB lock, WAL fsync, replica replay, 외부 PG timeout 중 하나가 시간을 잡아먹을 수 있습니다. 그래서 운영자는 먼저 모든 층에 같은 요청 식별자를 붙이고, 증상을 하나의 시간선으로 끼워 맞춥니다.

```text
request_id=pay-777
  12:00:00.010 app received
  12:00:00.020 pool borrow start
  12:00:03.900 pool borrow end
  12:00:03.910 SQL start
  12:00:03.980 SQL end
  12:00:04.000 response
```

이 시간선에서는 DB SQL 자체가 70ms만 걸렸습니다. 느린 것은 DB 내부 실행이 아니라 connection을 빌리기 전의 queue입니다. 같은 "4초 지연"이라도 이 한 줄 시간선이 있으면 인덱스 추가라는 잘못된 처방을 피할 수 있습니다.
...
Observability가 단순 monitoring보다 더 넓게 쓰이는 이유는 dashboard의 숫자를 모으는 것만으로는 원인과 행동이 연결되지 않기 때문입니다. Monitoring은 "CPU 80%, slow query 20건"처럼 상태를 보여 줄 수 있습니다. Observability는 그 숫자가 어떤 요청, 어떤 query fingerprint, 어떤 wait, 어떤 업무 실패와 연결되는지 추적하게 합니다. 장애 중에는 이 차이가 큽니다. 숫자가 많아도 서로 이어지지 않으면 운영자는 화면을 옮겨 다니며 추측해야 하고, 숫자가 적어도 같은 사건을 관통하면 원인 후보를 빠르게 줄일 수 있습니다.
...
이 구분을 놓치면 위험한 조치를 하게 됩니다. `fsync` 지연이 보인다고 durability 설정을 바로 낮추면 장애는 줄어들 수 있지만 crash safety를 잃을 수 있습니다. Connection pool 대기가 보인다고 pool size를 키우면 DB process는 실행 가능한 작업(runnable task)과 lock contention을 더 많이 떠안을 수 있습니다. Replica lag가 보인다고 replica만 재시작하면 실제 원인인 primary WAL 폭증이나 replica disk flush 병목은 그대로 남습니다. 면접 답변에서는 DB 지표를 먼저 잡고, 그 지표가 운영체제의 CPU 스케줄링(CPU scheduling), 메모리 회수(memory reclaim), 블록 I/O 대기열(block I/O queue), 저장 장치 flush 중 어디와 만나는지까지 한 단계 더 내려가야 합니다.
...
많은 시스템은 처음에는 하나의 계정으로 시작합니다. 개발과 운영이 작을 때는 `app_user` 하나가 table을 만들고, 읽고, 쓰고, batch도 돌리고, 장애 때 수동 수정까지 하는 편이 빠릅니다. 하지만 서비스가 커지면 같은 계정이 너무 많은 일을 할 수 있다는 사실이 사고 표면이 됩니다. 애플리케이션 SQL injection이나 잘못된 배치가 schema 변경, 대량 삭제, 민감 테이블 조회까지 이어질 수 있습니다. Role 분리는 속도를 늦추기 위한 절차가 아니라, 실패가 퍼지는 반경을 줄이는 구조입니다.

```text
single account phase
  app_user = runtime + migration + batch + manual fix
  one leaked password -> 모든 권한 노출

separated roles
  app_runtime  -> 필요한 DML만
  app_migration -> DDL과 grant 변경
  app_readonly -> reporting SELECT
  dba_admin    -> break-glass 절차로 제한
```

이렇게 나누면 권한 오류를 볼 때도 "어떤 계정에 무엇을 더 줄까"가 아니라 "이 작업이 어느 역할의 책임인가"를 먼저 묻게 됩니다.
...
암호화가 자주 오해되는 이유는 "데이터가 암호화되어 있다"는 문장이 공격 경로를 하나로 줄여 말하기 때문입니다. 디스크 스냅샷을 훔쳐 간 공격자와, 정상 DB 계정으로 접속한 과권한 애플리케이션과, 로그 저장소에서 민감값을 읽은 내부자는 서로 다른 경로를 탑니다. At-rest encryption은 첫 번째 경로에는 도움이 되지만, 두 번째와 세 번째 경로를 자동으로 막지 않습니다. 그래서 암호화 설명은 항상 저장 위치, 접근 주체, 키 위치, 로그 redaction, 감사 추적과 함께 닫혀야 합니다.
...
이 섹션의 `account_pg`와 `account_innodb`는 11번 문서의 engine 실험과 같은 이름을 쓰지만, 여기서는 재생 가능성을 위해 disposable table을 다시 만듭니다. 운영 DB가 아니라 로컬 실험 DB에서만 실행합니다.

PostgreSQL에서는 다음 table을 준비합니다.
...
CREATE INDEX ix_account_pg_status_updated_at
ON account_pg(status, updated_at DESC);

INSERT INTO account_pg(id, status, balance, updated_at) VALUES
(1, 'ACTIVE', 1000.00, now() - interval '3 minutes'),
(2, 'ACTIVE', 2000.00, now() - interval '2 minutes'),
(3, 'CLOSED', 0.00, now() - interval '1 day');
```

MySQL/InnoDB에서는 다음 table을 준비합니다.
...
INDEX ix_account_innodb_status_updated_at(status, updated_at)
) ENGINE = InnoDB;

INSERT INTO account_innodb(id, status, balance, updated_at) VALUES
(1, 'ACTIVE', 1000.00, CURRENT_TIMESTAMP),
(2, 'ACTIVE', 2000.00, CURRENT_TIMESTAMP),
(3, 'CLOSED', 0.00, CURRENT_TIMESTAMP);
```

PASS 신호는 이후 blocker, plan, slow query 실험이 "이미 존재한다고 가정한 table"에 기대지 않고 바로 실행 가능한 것입니다. FAIL 신호는 table이 없어서 실험이 중단되거나, 실험 table을 운영 schema와 섞는 것입니다.
...
INSERT INTO app.orders(id, tenant_id, amount) VALUES
(1, 'tenant-a', 100.00),
(2, 'tenant-b', 200.00);

GRANT USAGE ON SCHEMA app TO app_runtime;
GRANT SELECT ON app.orders TO app_runtime;
...
PASS 신호는 `app_runtime`으로 볼 때 `tenant-a` 설정에서는 tenant-a row만, `tenant-b` 설정에서는 tenant-b row만 보이는 것입니다. 마지막 `RESET ROLE` 뒤 table owner나 superuser 계정으로 실행한 조회가 모든 row를 볼 수 있다면, 그것은 RLS가 "모든 계정에 자동 적용되는 마법"이 아니라 role과 owner 경계에 의존한다는 증거입니다. PostgreSQL 공식 문서는 table owner가 보통 row security를 우회하고, 필요하면 `ALTER TABLE ... FORCE ROW LEVEL SECURITY`로 owner도 정책 대상이 되게 할 수 있다고 설명합니다.

FAIL 신호는 connection pool에서 `app.tenant_id`가 다음 요청으로 새어 나가거나, admin/owner path를 runtime path와 같은 검증으로 착각하는 것입니다. 이 실험은 반드시 `SET LOCAL`처럼 transaction-local setting을 쓰는 경로, pool reset, owner/admin 조회 경로를 따로 확인해야 합니다.
...
RLS는 강한 보호 장치가 될 수 있지만 완전 자동 보장은 아닙니다. 예를 들어 `orders`에 tenant-a와 tenant-b row가 있고, runtime role이 `current_setting('app.tenant_id')`로 tenant-a만 보도록 정책을 받는다고 하겠습니다. 이때 runtime role이 tenant-b row를 못 보는 것은 정상입니다. 하지만 table owner, superuser, `BYPASSRLS` 권한을 가진 role, 잘못 만든 `SECURITY DEFINER` function, tenant setting이 남은 connection pool은 다른 경로를 만들 수 있습니다.

그래서 RLS 답변은 "켜면 됩니다"가 아니라 네 가지를 말해야 합니다. 첫째, runtime role이 실제로 RLS 대상인지 확인합니다. 둘째, owner/admin path가 정책을 우회할 수 있는지 별도 테스트합니다. 셋째, tenant id를 session 변수로 넣는다면 `SET LOCAL`과 transaction/pool reset으로 다음 요청 오염을 막습니다. 넷째, tenant-a 계정으로 tenant-b row를 읽는 cross-tenant test를 자동화합니다. RLS는 application predicate의 대체품이라기보다, 애플리케이션 실수와 raw SQL 경로를 줄이는 DB 단의 방어선입니다.
...
대표 반례는 `느린 API를 보고 바로 index를 추가했지만 실제 원인은 long transaction lock wait인 상황`입니다. 요청 로그에는 5초가 보이고 SQL 문장은 평범해 보이지만, DB 안에서는 session B가 row lock을 기다리고 session A가 `idle in transaction`으로 멈춰 있을 수 있습니다. 이때 index를 추가하면 write 비용만 늘고 blocker는 그대로 남습니다.

DBMS별로는 PostgreSQL은 `pg_stat_activity`, `pg_locks`, `pg_blocking_pids`, `EXPLAIN (ANALYZE, BUFFERS)`를 보고, MySQL/InnoDB는 processlist, Performance Schema, InnoDB status, slow log를 봅니다. 이 차이는 도구 이름 암기가 아니라 장애 대응 순서를 바꿉니다. PostgreSQL에서 blocker PID를 찾았다면 application request id와 연결해 어떤 transaction이 열려 있는지 봅니다. MySQL에서 metadata lock 대기가 보인다면 긴 SELECT 뒤에 DDL이 막히고, 그 뒤의 DML까지 줄줄이 대기하는 chain을 의심합니다.

이 흐름은 `p95/p99 latency, pool wait time, lock wait seconds, blocker age, active connection count, replica seconds behind source, failed login/audit events` 같은 지표로 검증합니다. 예를 들어 p99 latency와 pool wait이 같이 오르면 DB 내부 query보다 connection 점유 시간이 먼저 의심됩니다. Lock wait seconds와 blocker age가 같이 튀면 transaction 경계를 봅니다. Failed login이나 broad grant 변경이 늘면 보안 이벤트를 성능 장애와 별도로 추적해야 합니다. 지표가 예상과 다르면 처음 세운 원인 가설을 다시 열어야 합니다.
...
면접에서는 최종적으로 이렇게 압축할 수 있습니다. 운영 대응은 먼저 read-only 관측으로 증상, 직접 원인, 근본 원인을 나눈 뒤 안전한 완화와 재발 방지를 분리한다고 말합니다. 그 다음 꼬리 질문이 오면 `request id`, `wait state`, `blocker`, `replica lag`, `privilege` 중 하나를 골라 실제 확인 명령과 위험한 오판을 함께 설명하면 됩니다.
