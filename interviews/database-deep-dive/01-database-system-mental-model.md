...
데이터베이스를 면접에서 설명할 때 가장 위험한 답변은 "SQL을 받아서 데이터를 저장하고 조회합니다"에서 멈추는 답변입니다. 이 말은 틀리지는 않지만, 데이터베이스가 실제로 어려워지는 지점을 거의 설명하지 못합니다.

조금 더 정확히 말하면 DBMS는 한 SQL 문장에 대해 네 가지 판단을 이어 붙입니다. 먼저 문장의 구조와 실행 때 들어오는 값을 나누고, 그 문장이 요구하는 결과 의미를 정하고, 같은 결과를 만들 물리 경로를 고르고, 동시에 실행되거나 장애가 나도 어느 시점의 결과와 commit 보장을 지킬지 결정합니다.
...
아래 그림은 SQL 한 줄이 DBMS 프로세스, 운영체제 커널, 저장 장치를 차례로 지나갈 수 있다는 사실을 보여 줍니다. 데이터베이스 요청은 DBMS 내부에서만 끝나지 않고, 필요할 때 system call, page cache, filesystem, block layer, device driver, storage cache까지 내려갑니다.

![DBMS process, OS kernel, and storage I/O stack](assets/db-os-io-stack.svg)

## 처음 잡을 지도

이 문서를 읽을 때는 DBMS를 "SQL을 실행하는 프로그램"이 아니라, 네 가지 판단을 계속 이어 붙이는 시스템으로 봅니다.

| 판단 축 | DBMS가 묻는 질문 | 다음에 이어지는 문서 |
| --- | --- | --- |
| 구조와 값 | 이 SQL에서 고정된 문법 구조는 무엇이고, 실행 때 들어오는 값은 무엇인가 | 이 문서의 prepared statement, quoting, injection 경계 |
| 논리 의미 | 이 요청은 어떤 관계(relation), 행(row), NULL, 문자열 비교 규칙(collation), 정렬 의미를 요구하는가 | 이 문서의 관계 모델, SQL 의미, NULL/collation |
| 물리 경로 | 같은 결과를 어떤 인덱스(index), 페이지(page), 행 흐름(row stream) 경로로 만들 것인가 | [02](02-storage-pages-buffer-io.md), [04](04-index-query-optimizer.md) |
| 시간 속 보장 | 동시에 실행되거나 crash가 나도 어떤 시점(snapshot)과 commit 결과를 지킬 것인가 | [03](03-wal-redo-undo-crash-recovery-pitr.md), [07](07-mvcc-snapshot-visibility.md), [08](08-isolation-lock-deadlock.md) |

이 네 축은 하나의 SQL에서 동시에 움직입니다.

```text
애플리케이션 요청
  -> SQL text와 bind value를 분리한다
  -> relation, column, predicate, NULL, collation 의미를 정한다
  -> optimizer가 후보 실행 경로를 비교한다
  -> executor가 row stream을 만들며 page와 index를 읽는다
  -> transaction snapshot과 recovery log가 결과의 시간 경계를 지킨다
  -> client는 result set을 받는다
```

같은 지도를 UPDATE 하나에 적용하면, 면접에서 자주 나오는 lock, version, page, log, replication 질문이 한 timeline 위에 놓입니다.

```text
UPDATE accounts SET balance = balance - 100 WHERE id = 42;

T1. 구조와 값
    SQL 구조: accounts table의 id=42 row를 찾아 balance를 바꾼다
    bind value가 있다면 id와 amount 같은 값만 들어온다

T2. 잠금과 현재 version
    executor는 id=42 후보 row를 찾고, write conflict를 막기 위해 필요한 row lock을 잡는다
    MVCC 엔진은 이 statement가 볼 수 있는 version과 새로 만들 version을 구분한다

T3. page와 buffer
    row가 들어 있는 data page를 buffer에 올린다
    memory 안의 page 또는 새 tuple/version이 바뀌고 dirty 상태가 된다

T4. WAL/redo
    crash 뒤 이 변경을 다시 만들 수 있는 log record를 먼저 남긴다
    commit 성공을 말하기 전에는 제품 설정에 맞는 durable log 경계가 닫혀야 한다

T5. commit과 lock release
    commit record가 안정화되면 client는 성공 응답을 받는다
    lock은 풀리지만 data file page는 아직 disk에 내려가지 않았을 수 있다

T6. replica와 backup/PITR
    replica는 WAL 또는 binary log 계열 변경 기록을 받아 receive/apply 위치를 따라간다
    PITR은 backup 이후 log chain을 목표 시점까지 replay하고 멈추는 별도 질문이다
```

이 timeline은 제품별 세부를 하나로 뭉개려는 표가 아닙니다. PostgreSQL은 heap tuple version과 WAL, InnoDB는 clustered index page, undo/redo, binary log 경계가 따로 있습니다. 그래도 면접에서 먼저 말해야 할 공통 구조는 같습니다. "한 UPDATE는 값을 바꾸는 문장"이 아니라 "잠금과 version을 정하고, page를 바꾸고, log를 안정화하고, commit과 replica/PITR 경계를 남기는 사건"입니다.

처음에는 모든 제품별 세부를 외우려고 하기보다, "이 문장은 어떤 의미를 요구하고, 엔진은 어떤 물리 경로로 그 의미를 만들며, 시간 속에서 어떤 보장을 지키는가"를 먼저 말할 수 있으면 됩니다.
...
```text
애플리케이션 코드(client code)
  SQL text: SELECT id, display_name ... WHERE email = ? ...
  바인딩할 값(bind value): "kim@example.com"

SQL 인터페이스(SQL interface)
  token: SELECT / FROM / users / email / ? / ORDER BY / LIMIT
  parameter: ?는 값 자리이지 column name이나 table name이 아니다

논리 의미(logical meaning)
  users 테이블/관계에서 email 값이 bind value와 같고
  deleted_at이 NULL인 row를 고른다
  created_at 내림차순으로 정렬한 뒤 첫 row만 반환한다

실행 계획 선택(planner)
  후보 경로 A: users 전체 scan -> filter -> sort -> limit
  후보 경로 B: email index lookup -> deleted_at filter -> sort/limit
  후보 경로 C: composite index(email, deleted_at, created_at) range scan

실행기와 저장소(executor/storage)
...
같은 trace를 실제 row 세 개로 더 작게 보면 상태 이동이 보입니다.

```text
users snapshot at statement start
  id=7,  email='kim@example.com', deleted_at=NULL,        created_at=10:00, visible=yes
  id=8,  email='kim@example.com', deleted_at='09:00',     created_at=11:00, visible=yes
  id=9,  email='lee@example.com', deleted_at=NULL,        created_at=12:00, visible=yes
...
filter deleted_at IS NULL
  keep id=7
  drop id=8 because it is logically deleted

ORDER BY created_at DESC + LIMIT 1
  only id=7 remains, so no competing row is returned

snapshot visibility check
  if id=7 were not visible in this statement's snapshot, it would be skipped

result set
  id=7, display_name=...
```

이 작은 표는 "행을 찾는다"가 실제로는 여러 판정의 합이라는 점을 보여 줍니다. 먼저 값이 맞는지 보고, 그다음 업무상 삭제된 행인지 보고, 정렬과 개수 제한을 적용하고, 마지막으로 현재 statement가 볼 수 있는 version인지 확인합니다. 저장 엔진과 실행 계획은 이 의미를 깨지 않는 범위에서 더 싼 물리 경로를 찾습니다.
...
경로 A: table scan
  많은 table page를 읽는다
  1,000,000 rows 각각의 email을 검사한다
  후보가 여러 개 남을 때만 sort가 의미를 가진다

경로 B: email index
  몇 개의 index page를 읽는다
  매칭된 data page를 방문한다
  후보 row의 deleted_at을 검사한다

같은 논리 답:
  kim@example.com 사용자의 id/display_name

서로 다른 물리 작업:
  경로 A는 999,999 rows가 후보가 아님을 증명하는 데 일을 쓴다.
  경로 B는 index 경로를 따라 작은 후보 집합으로 바로 좁혀 들어간다.
```
...
관계 모델에서 table은 관계(relation)에 가깝고 row는 튜플(tuple)에 가깝습니다. 여기서 중요한 것은 relation이 "사실의 집합"이라는 사고방식입니다. `orders` relation은 주문이라는 사실들을 담고, primary key는 그 사실 하나를 식별하는 후보를 고릅니다.

다만 SQL에서 표는 수학의 집합과 닮았지만, 실제 `SELECT` 결과는 중복 행을 남길 수 있습니다. 이 성질을 공식 용어로는 bag 또는 multiset이라고 부릅니다. `SELECT status FROM orders`는 status가 같은 주문이 여러 개라면 같은 status 값을 여러 번 반환할 수 있습니다. `DISTINCT`를 명시해야 중복 제거가 의미 계약이 됩니다.
...
prepared statement의 상태를 connection pool과 함께 보면 더 현실적입니다. 여기서 **session은 DB 서버가 한 연결에 붙여 둔 상태**입니다. PostgreSQL server-side prepared statement처럼 session에 묶이는 객체는 같은 애플리케이션 프로세스 안에서도 어떤 physical connection을 빌렸는지에 따라 보이거나 보이지 않을 수 있습니다.
...
여기까지의 기억 지도는 네 줄로 줄일 수 있습니다. SQL 인터페이스는 구조와 값을 먼저 나누고, 관계 모델은 결과의 의미를 정하며, 실행 계획은 그 의미를 만들 물리 경로를 고릅니다. DBMS마다 heap, clustered index, prepared statement session, collation 같은 경계가 다르므로 같은 SQL 이름만 보고 같은 내부 구조라고 말하면 안 됩니다. 아래 실험은 이 네 줄이 실제로 어떻게 관찰되는지 확인하는 용도입니다.
...
2. "prepared statement는 query를 미리 컴파일하니까 항상 빠르죠?"
    ...
    항상은 아닙니다. Prepared statement는 SQL text를 매번 새로 해석하는 비용을 줄이고, 값 바인딩으로 SQL 코드와 데이터를 분리하는 데 도움이 됩니다. 하지만 query 시간이 대부분 page read, join, sort, lock wait에서 나오면 parse 비용을 줄여도 체감이 작을 수 있습니다.

    PostgreSQL에서는 parameter 값에 따라 custom plan과 generic plan의 선택이 문제가 될 수 있습니다. 예를 들어 `orders(status, created_at)` index가 있고, `status = 'RARE'`는 100건, `status = 'PAID'`는 1억 건이라고 하겠습니다.

    ```sql
    PREPARE recent_orders(text) AS
    SELECT *
    FROM orders
    WHERE status = $1
    ORDER BY created_at DESC
    ...
    EXECUTE recent_orders('RARE');
    EXECUTE recent_orders('PAID');
    ```

    `RARE` 값은 index로 바로 좁히는 계획이 좋을 수 있고, `PAID` 값은 너무 많은 row를 만나 다른 계획이 나을 수 있습니다. 그런데 generic plan은 특정 parameter 값 하나에 맞춘 계획이 아니라 여러 실행에 공통으로 쓸 계획입니다. 값 분포가 심하게 기울어져 있으면 어떤 값에는 generic plan이 나쁜 선택이 될 수 있습니다. 이 문제는 [인덱스와 optimizer](04-index-query-optimizer.md)의 selectivity와 parameter skew 설명과 이어집니다.

    MySQL도 server-side prepared statement가 metadata change 뒤 다시 준비되는 경우, session 범위, prepared statement 개수 제한 같은 운영 경계를 가집니다. 따라서 좋은 답변은 "prepared statement는 보안과 반복 실행 비용에 도움이 되지만, 항상 더 빠른 것은 아니다. 실제 plan과 parameter 분포를 봐야 한다"입니다.
