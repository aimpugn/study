...
복제가 변경 로그를 중심으로 움직이는 이유는 DBMS가 이미 durability를 위해 변경 순서를 기록하고 있기 때문입니다. Primary가 crash recovery를 위해 남긴 순서 있는 기록은 "다른 노드가 같은 변화를 따라가게 하는 재료"로도 쓸 수 있습니다. 그래서 복제는 table 파일을 매번 통째로 복사하는 기능이 아니라, commit history를 다른 노드가 소비하게 만드는 흐름에 가깝습니다. 이 관점이 있으면 replica lag도 "복사본이 느림"이 아니라 "변경 기록의 생성, 전송, 저장, 적용 중 어디가 뒤처졌는가"로 나눌 수 있습니다.

이 흐름에서 primary의 commit과 replica의 apply는 같은 순간이 아닐 수 있습니다. 비동기 복제에서는 primary가 replica apply를 기다리지 않고 client에게 commit 성공을 반환할 수 있습니다. 동기 또는 semi-sync 계열은 replica 확인을 더 기다리므로 데이터 손실 창을 줄일 수 있지만, replica가 느리거나 네트워크가 끊겼을 때 write 응답이 늦어지거나 멈출 수 있습니다. Consensus 기반 distributed SQL은 또 다른 모델로, leader가 복제본 다수의 확인(quorum)을 받은 뒤 commit을 인정합니다. 이 문서에서는 전통적인 primary-replica 복제와 backup/failover 경계를 중심으로 다루고, consensus와 shard/partition 경계는 [파티션, 샤딩, 분산 SQL](10-partition-sharding-distributed-sql.md)로 넘깁니다.
...
Semi-sync나 synchronous replication은 이 창을 줄일 수 있습니다. 하지만 commit 성공을 돌려주기 전에 더 많은 replica 확인을 기다릴수록 write 응답이 느려지고, replica 장애나 network partition 때 primary가 write를 계속 받을 수 없을 수도 있습니다. 면접에서 `동기로 하면 안전합니다`라고만 답하면 데이터 손실 위험을 줄이는 대신 무엇을 비용으로 치르는지 놓칩니다. 안전성, 지연 시간, 고가용성, 운영 복잡도를 같이 말해야 합니다.
...
Backup이 replica와 별도로 필요한 이유는 장애가 항상 서버 고장 형태로 오지 않기 때문입니다. 사람이 잘못된 DELETE를 실행하거나, application bug가 잘못된 값을 정상 commit하거나, migration이 의미를 잘못 바꾸면 replica는 그 실수를 성실하게 따라갈 수 있습니다. Replica는 현재 primary의 history를 이어받는 장치이고, backup은 "그 history의 어느 과거 지점으로 돌아갈 재료"입니다. 그래서 백업은 고가용성의 부록이 아니라, 잘못된 정상 변경을 되돌릴 시간을 사는 장치입니다.
...
PITR이 "되감기"처럼 보이지만 실제 운영에서는 새 history를 시작하는 일에 가깝습니다. 14:02:59 상태로 복구한 뒤 그 DB가 새 주문을 받기 시작하면, 14:03 이후 원래 primary에서 이어졌던 history와 복구 DB의 history는 서로 다른 가지가 됩니다. 이 차이를 구분하지 않으면, 복구 전 history의 log를 복구 후 DB에 잘못 이어 붙이거나, 원래 replica를 아무 검증 없이 새 primary에 붙이는 사고가 날 수 있습니다. 그래서 목표 시점 복구는 데이터만 맞추는 작업이 아니라 "앞으로 어느 history를 서비스의 정식 history로 삼을지"를 정하는 절차입니다.
...
Fencing이 필요한 배경은 "죽은 것처럼 보이는 서버"와 "정말로 다시는 쓰기를 받을 수 없는 서버"가 다르기 때문입니다. Network partition에서는 primary가 일부 관측자에게만 보이지 않을 수 있고, 잠깐 멈춘 노드가 뒤늦게 살아나 기존 endpoint로 쓰기를 받을 수도 있습니다. Failover 자동화가 빠를수록 이 차이를 더 엄격하게 다뤄야 합니다. 새 primary를 세우는 속도보다 old primary가 더 이상 history를 만들 수 없다는 보장이 먼저 닫혀야 split-brain을 피할 수 있습니다.
...
Distributed SQL은 단순 async primary-replica와 다를 수 있습니다. Raft/Paxos 계열 consensus를 쓰면 leader가 복제본 다수(quorum)의 확인을 받은 뒤 commit을 인정합니다. 이 경우 failover와 replication lag의 의미가 전통적인 async replica와 다르게 나타납니다. 하지만 consensus는 node 사이 왕복 시간과 quorum을 유지해야 하는 가용성 비용을 만듭니다.
...
3. Backup restore drill을 수행합니다.
    ...
    작은 DB에서 base backup을 만들고, 몇 개 transaction을 실행한 뒤, 특정 transaction 직전으로 복구하는 절차를 문서화합니다. PostgreSQL이라면 `pg_basebackup`과 WAL archive가 최소 골격입니다. 공식 문서는 `pg_basebackup`이 point-in-time recovery의 출발점이 될 수 있고, PITR은 base backup 뒤의 WAL을 목표 시점까지 replay하는 방식이라고 설명합니다.

    ```text
    PostgreSQL PITR drill skeleton

    1. WAL archive가 켜진 disposable cluster를 준비합니다.
       archive_mode = on
       archive_command = 'cp %p /safe_wal_archive/%f'

    2. 대표 row를 만듭니다.
       INSERT INTO orders(id, status) VALUES (100, 'BEFORE_BACKUP');

    3. base backup을 별도 위치에 만듭니다.
       pg_basebackup -D /restore_lab/base -Fp -Xs -P

    4. backup 이후에 정상 transaction과 실수 transaction을 순서대로 실행합니다.
       INSERT INTO orders(id, status) VALUES (101, 'AFTER_BACKUP_OK');
       -- 목표 시점 기록
       SELECT now();
       DELETE FROM orders WHERE id = 100;

    5. restore directory에 base backup을 풀고 WAL archive를 restore_command로 연결합니다.
       restore_command = 'cp /safe_wal_archive/%f %p'
       recovery_target_time = '목표 시점'

    6. recovery가 끝난 DB에서 대표 row를 확인합니다.
       id=100은 살아 있어야 하고,
       목표 시점 이후 DELETE는 반영되면 안 됩니다.
    ```

    PASS 신호는 복구 후 row count와 대표 query가 목표 시점과 일치하고, 복구에 걸린 시간이 기록되는 것입니다. FAIL 신호는 backup file 생성만 확인하고 restore를 해 보지 않는 것입니다. 더 중요한 FAIL은 WAL archive가 중간에 끊겨 목표 시점까지 replay하지 못하는데도 "backup이 있으니 괜찮다"고 말하는 것입니다.

4. Log chain 누락을 일부러 가정합니다.
...
- "복제가 있으니 고가용성과 데이터 보호가 모두 해결됩니다"라고 말하는 것
    ...
    복제는 primary에서 commit된 변경을 replica가 따라가게 하는 장치입니다. 그래서 운영자가 `DELETE FROM orders`를 잘못 실행해도, 그 삭제가 정상 commit이었다면 replica도 삭제를 따라갈 수 있습니다. 이 경우 replica는 실수 삭제를 되돌리는 저장소가 아닙니다. 서비스가 계속 열려 있는가를 보는 고가용성(availability)과, 과거의 올바른 시점으로 돌아갈 수 있는가를 보는 복구 가능성(recoverability)은 분리해야 합니다. 논리적 실수, 데이터 오염, 늦게 발견한 장애, ransomware 같은 문제에는 backup과 PITR이 필요합니다.

- "Replication lag는 단지 성능 지연입니다"라고 말하는 것

    Lag는 사용자가 어떤 시점의 데이터를 읽는지 바꾸는 일관성 문제입니다. 사용자가 방금 주문을 만들었는데 replica가 아직 그 log를 적용하지 못했다면 주문 상세 화면에는 `주문이 없습니다`가 나올 수 있습니다. Primary가 성공 응답을 돌려준 주문도, replica가 그 log를 받기 전에 primary가 영구 장애를 만나면 failover 뒤 새 primary에는 없을 수 있습니다. 관리자 리포트가 몇 분 전 상태를 보여 주거나, 위험 알림이 늦게 울리는 문제도 같은 지연에서 나올 수 있습니다.

- "PITR은 full backup을 복원하는 것입니다"라고 말하는 것

    Full/base backup은 시작점일 뿐입니다. 예를 들어 12:00에 base backup을 만들고, 12:30에 정상 주문 100건이 들어왔고, 12:35에 운영자가 잘못된 `DELETE FROM orders WHERE tenant_id = 'A'`를 실행했다고 하겠습니다. 목표는 12:34:59로 돌아가는 것입니다. 이때 12:00 backup만 복원하면 12:30의 정상 주문까지 사라집니다. 12:00 backup 위에 12:34:59까지의 연속 WAL/binlog를 replay해야 목표 시점이 됩니다. Log chain이 12:20에서 끊기면 12:34:59로 갈 수 없습니다.
...
- "동기 복제를 켜면 단점 없이 RPO 0이 됩니다"라고 말하는 것
    ...
    Commit 성공을 돌려주기 전에 replica 확인을 기다리면 primary만 성공하고 replica가 놓치는 창을 줄일 수 있습니다. 하지만 그만큼 commit 응답이 느려지고, replica 장애나 network partition 때 쓰기를 계속 받을지 멈출지 결정해야 합니다. 즉 동기 복제는 데이터 손실 가능성을 줄이는 대신 지연 시간(latency)과 고가용성(availability) 비용을 commit 경로에 넣는 선택입니다.
