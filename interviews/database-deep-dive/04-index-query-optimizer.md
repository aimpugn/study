...
각 index가 row stream을 얼마나 빨리 원하는 모양으로 만드는지 숫자로 적어 보면 더 분명합니다.

```text
orders rows for user_id=42: 50,000
status='PAID' rows among them: 8,000
first page needs: newest 20 rows

index A (user_id)
  access range: 50,000 entries
  residual filter: status
  order work: created_at/id sort may be needed
  table lookup: total_amount를 얻으려면 row 방문 가능

index B (user_id, status)
  access range: 8,000 entries
  residual filter: much smaller
  order work: still may need sort

index C (user_id, status, created_at DESC, id DESC) INCLUDE(total_amount)
  access range: newest PAID entries in order
  output can stop after 20 visible rows
  table lookup may be reduced if covering/index-only conditions hold
```

이 숫자는 실제 제품의 cost 공식이 아니라 사고 실험입니다. 좋은 index를 고르는 첫 질문은 "몇 row를 찾을 수 있는가"가 아니라 "몇 page와 몇 row 후보를 읽고, 그 뒤 sort와 table lookup을 얼마나 줄이는가"입니다.
...
실제 lookup은 "계산식 O(log N)"보다 page 이동으로 느끼는 편이 실무에 가깝습니다.
...
1. root page
   ...
   choose child pointer covering 40..80

2. internal page
   compare separator keys again
   choose leaf page covering 42..48

3. leaf page
   find key 42
   return row pointer, primary key, or indexed payload

4. data access if needed
   PostgreSQL-like heap: visit heap page/item
...
find first matching leaf entry
  -> read leaf entries in key order
  -> stop when key leaves the range
  -> fetch table rows only when needed
```

그래서 B-tree 계열은 equality 하나뿐 아니라 range와 order에도 강합니다. 반대로 key 순서와 맞지 않는 함수, 앞 column이 비어 있는 composite 조건, collation이 다른 비교는 이 leaf 순서를 그대로 활용하기 어렵게 만듭니다.
...
이 정렬을 범위로 잘라 보면 leftmost prefix가 왜 중요한지 더 작게 보입니다.

| 조건 | index 안에서 보이는 모양 | scan 감각 |
| --- | --- | --- |
| `user_id = 42` | `(42, *, *)`가 한 덩어리 | 한 연속 구간으로 좁힙니다. |
| `user_id = 42 AND status = 'PAID'` | `(42, PAID, *)`가 더 작은 덩어리 | 더 짧은 연속 구간으로 좁힙니다. |
| `user_id = 42 AND status = 'PAID' ORDER BY created_at` | 같은 덩어리 안에서 `created_at` 순서 | sort를 줄이거나 피할 수 있습니다. |
| `status = 'PAID'`만 있음 | `(1, PAID, *)`, `(2, PAID, *)`, `(42, PAID, *)`처럼 user별로 흩어짐 | leading column이 없어서 기본적으로 여러 구간을 봐야 합니다. |

즉 composite index는 "여러 index를 하나로 묶은 것"이 아니라 "왼쪽 column부터 사전식으로 정렬된 하나의 key 공간"입니다. `status`가 두 번째 column이면 `status='PAID'` 값들이 table 전체에서 한 덩어리로 모여 있지 않습니다. 이 감각을 잡으면 column 순서를 query shape와 함께 고민하게 됩니다.
...
covering을 판단할 때는 "필요한 column이 index에 있다"와 "table/heap을 정말 방문하지 않는다"를 분리해야 합니다.

| 질문 | PostgreSQL에서의 경계 | InnoDB에서의 경계 |
| --- | --- | --- |
| 필요한 값이 index에 있는가 | key column 또는 `INCLUDE` column에 있어야 합니다. | secondary index leaf에 index column과 primary key가 있고, 필요한 column이 모두 있어야 합니다. |
| row visibility를 index만으로 알 수 있는가 | visibility map이 all-visible이라고 알려 주는 heap page일 때 heap fetch를 줄입니다. | consistent read와 clustered record 접근 경계가 다르며, secondary index만으로 끝나는 query인지 봅니다. |
| write 비용은 어떤가 | index entry가 커지고 update 때 유지 비용이 늘 수 있습니다. | secondary index가 커지고 primary key 길이도 영향을 줍니다. |

따라서 "covering index를 만들면 항상 빠르다"는 말은 약합니다. 더 정확한 답은 "table 방문을 줄일 수 있는 후보를 만들지만, MVCC visibility, index width, write workload, DBMS 구현 경계가 함께 맞아야 이득이 난다"입니다.
...
partial index가 쓰이는지 여부는 query가 predicate를 어떻게 드러내는지에 크게 묶입니다.
...
query A
  ...
  planner can see the predicate shape directly

query B
  WHERE user_id = 42
    AND coalesce(deleted_at, 'infinity') = 'infinity'
  logically similar in the application author's head
  but planner may not prove it implies deleted_at IS NULL

query C
  WHERE user_id = $1
    AND deleted_at = $2
  if $2 happens to be NULL, equality is not IS NULL
  predicate does not match the intended partial index
```

이 예시는 partial index가 나쁘다는 뜻이 아닙니다. 오히려 active row만 자주 읽는 서비스에서는 아주 강력할 수 있습니다. 다만 index predicate와 query predicate가 함께 관리되어야 하며, ORM이나 query builder가 표현을 바꾸면 index가 갑자기 안 쓰일 수 있습니다.
...
row stream은 각 node를 지나며 모양이 바뀝니다.

```text
input rows
  50,000 orders for user 42

Filter(status='PAID')
  8,000 rows remain

Join(payments)
  8,000 order rows + payment rows
  maybe 8,200 joined rows if some orders have multiple payments

Aggregate(by order)
  back to 8,000 order groups

Sort(created_at DESC)
  8,000 groups ordered

Limit(20)
  only 20 rows returned to client
```

이 trace에서 병목은 여러 곳에 생길 수 있습니다. Filter가 너무 늦으면 불필요한 join 입력이 커지고, join이 잘못된 순서로 잡히면 inner lookup이 반복되며, aggregate나 sort가 memory를 넘으면 temp file로 spill됩니다. 그래서 `EXPLAIN`은 plan node 이름을 외우는 도구가 아니라 "어느 단계에서 row stream이 너무 커졌는가"를 찾는 도구입니다.
...
통계 오차는 plan node를 통해 증상으로 드러납니다.

| 관측 | 의미 | 다음 확인 |
| --- | --- | --- |
| estimated rows 100, actual rows 100,000 | selectivity 추정이 크게 틀렸습니다. | 통계 freshness, histogram, most common values, parameter skew |
| chosen index가 있지만 filter에서 대부분 버림 | access path가 조건을 충분히 줄이지 못했습니다. | composite index 순서, predicate가 index condition에 들어갔는지 |
| nested loop의 inner scan이 수십만 번 반복 | outer row 수 추정이 틀렸거나 join order가 나쁩니다. | join cardinality, extended statistics, alternative join |
| sort/hash가 disk로 spill | memory 안에서 처리한다는 가정이 깨졌습니다. | work memory, result size, temp read/write |

이런 표를 머리에 두면 "통계가 중요합니다"라는 문장이 실제 진단 질문으로 바뀝니다. 통계는 옵티마이저의 눈이고, `EXPLAIN ANALYZE`의 actual 값은 그 눈이 얼마나 빗나갔는지 확인하는 시력 검사에 가깝습니다.
...
실전에서는 다음 순서로 읽으면 덜 흔들립니다.

1. 가장 아래 scan node부터 본다.

    table 또는 index에서 몇 row를 만들었는지 봅니다. 여기서 이미 예상보다 너무 많은 row가 나오면 위 node는 대부분 그 비용을 떠안습니다.

2. estimated rows와 actual rows를 비교한다.

    추정과 실제가 크게 다르면 optimizer가 잘못된 세계를 보고 plan을 골랐을 가능성이 큽니다. 통계, parameter skew, correlation, predicate 표현을 확인합니다.

3. filter와 index condition을 나눈다.

    조건이 index 범위를 줄였는지, 아니면 row를 가져온 뒤 residual filter로 버렸는지 봅니다. 같은 `WHERE`라도 비용은 완전히 다릅니다.

4. sort, aggregate, join의 메모리 사용을 본다.

    temp file, filesort, disk spill, hash batch 같은 단서가 있으면 CPU 문제가 아니라 memory/I/O 문제가 섞입니다.

5. runtime 관측과 연결한다.

    PostgreSQL이면 `BUFFERS`, MySQL이면 `EXPLAIN ANALYZE`, slow query log, performance schema, OS I/O 지표를 함께 봅니다. `EXPLAIN`은 가설이고, runtime 관측은 그 가설의 실험 결과입니다.
...
작은 목록으로 보면 offset과 keyset의 차이가 더 선명합니다.

```text
ordered rows for user 42
  r1  created_at=10:00 id=100
  r2  created_at=09:59 id=99
  r3  created_at=09:58 id=98
  ...
  r100000
  r100001
  r100002
...
OFFSET 100000 LIMIT 20
  keep order from r1
  count and discard first 100000
  return r100001..r100020

keyset after cursor (created_at, id) = r100000's key
  seek to rows older than the cursor
  return next 20
```

동시 insert가 있을 때도 차이가 납니다. 사용자가 1페이지를 본 뒤 더 최신 row가 앞에 삽입되면 offset 방식의 2페이지는 원래 보았던 row를 다시 보거나 일부 row를 건너뛸 수 있습니다. keyset 방식은 "마지막으로 본 key보다 뒤"라는 조건으로 다음 범위를 잡기 때문에 next page 흐름에서는 더 안정적입니다. 대신 "37페이지로 바로 이동" 같은 요구에는 맞지 않습니다. 그래서 pagination 설계는 UI 요구와 index order, cursor 안정성을 함께 봐야 합니다.
