...
MySQL binary log도 항상 같은 모양의 사건을 적는 것은 아닙니다. MySQL 8.4 문서는 binary logging format을 statement-based, row-based, mixed로 나누고, row-based가 기본값이라고 설명합니다. statement-based logging은 실행된 SQL statement를 중심으로 남기고, row-based logging은 개별 table row가 어떻게 영향을 받았는지를 event로 남깁니다. mixed는 상황에 따라 statement 방식에서 row 방식으로 바뀔 수 있습니다. 그래서 replica가 "binary log event를 적용했다"는 말은 InnoDB redo log처럼 page 변경을 물리적으로 replay했다는 뜻이 아닙니다. replica와 PITR 질문에서는 `GTID`, binary log position, event time처럼 server 계층의 적용 경계를 보고, crash recovery 질문에서는 redo/WAL과 checkpoint 이후 page replay 경계를 따로 봐야 합니다.
...
- [MySQL 8.4 Binary Logging Formats](https://dev.mysql.com/doc/refman/8.4/en/binary-log-formats.html): statement-based, row-based, mixed logging의 차이와 row-based default를 확인합니다.
