...
본문은 PostgreSQL과 MySQL/InnoDB를 중심으로 쓴다. Oracle, SQL Server, RocksDB 계열처럼 다른 저장 엔진과 DBMS에도 비슷한 원리가 있지만, 같은 이름을 그대로 옮기면 틀릴 수 있다. 이 파일의 목표는 모든 제품의 내부 구조를 다 외우는 것이 아니라, 면접에서 "로그로 복구한다"는 말을 했을 때 그 말이 어떤 계층의 어떤 소비자를 가리키는지 끝까지 설명할 수 있게 만드는 것이다.
...
복제도 로그를 읽는다. 하지만 복제를 설명하는 log 소비자와 PITR을 설명하는 log 소비자는 같은 질문에 답하지 않는다. PostgreSQL streaming replication은 WAL record를 standby로 보내 replay하게 할 수 있다. MySQL replication은 source의 binary log event를 replica가 relay log로 받아 적용한다. 이때 질문은 "primary의 변경을 다른 서버가 얼마나 늦게 따라가는가"다. PITR의 질문은 "기준 backup 이후 어느 시점까지 replay하고 멈출 것인가"다. crash recovery의 질문은 "data file과 log를 보고 현재 cluster를 transaction-consistent 상태로 회복할 수 있는가"다.
...
DB를 특정 시점으로 되돌리는 일은 DB 안에서만 끝나지 않는다. 결제 승인, 외부 메시지 발행, 검색 색인, 캐시, 파일 업로드, 이메일 발송처럼 DB 밖에 이미 나간 side effect가 있으면, DB만 10:30:59로 되돌려도 시스템 전체가 10:30:59로 돌아가지 않는다. PITR은 database state를 되돌리는 강력한 도구지만, 외부 시스템과의 reconciliation을 자동으로 해결하지 않는다.
