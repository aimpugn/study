...
이 문서는 OS와 컴퓨터 구조를 암기용 단어 목록으로 보지 않고, 프로그램 한 줄이 CPU 시간, 가상 메모리, 파일 디스크립터, 커널 큐(queue), 장치와 네트워크를 어떻게 지나가는지 설명합니다. 목차가 길어도 먼저 `프로세스 -> 시스템 콜 -> 커널 객체 -> 큐/버퍼 -> 장치 또는 네트워크` 흐름을 잡고, 그다음 부팅, CPU, 메모리, 파일 시스템, I/O, 소켓, 컨테이너(container), 관측 도구 질문으로 내려가면 됩니다.
...
- [OS, 커널, 컴퓨터 구조](#os-커널-컴퓨터-구조)
    ...
    - [CPU와 숫자 표현](#cpu와-숫자-표현)
        - [부동소수점](#부동소수점)
            - [원문: 부동소수점](#원문-부동소수점)
                - [부동소수점](#부동소수점-1)
    - [부팅과 init 시스템](#부팅과-init-시스템)
        - [systemd가 다른 시스템 데몬들을 실행하는 과정](#systemd가-다른-시스템-데몬들을-실행하는-과정)
            - [원문: systemd가 다른 시스템 데몬들을 실행하는 과정](#원문-systemd가-다른-시스템-데몬들을-실행하는-과정)
                - [systemd가 다른 시스템 데몬들을 실행하는 과정](#systemd가-다른-시스템-데몬들을-실행하는-과정-1)
        - [서버 부팅 과정](#서버-부팅-과정)
            - [원문: 서버 부팅 과정](#원문-서버-부팅-과정)
                - [서버 부팅 과정](#서버-부팅-과정-1)
    - [서버 하드웨어와 운영 환경](#서버-하드웨어와-운영-환경)
        - [노트북도 서버가 될 수 있는데 왜 굳이 서버 호스팅을 받나요?](#노트북도-서버가-될-수-있는데-왜-굳이-서버-호스팅을-받나요)
            - [원문: 노트북도 서버가 될 수 있는데 왜 굳이 서버 호스팅을 받나요?](#원문-노트북도-서버가-될-수-있는데-왜-굳이-서버-호스팅을-받나요)
                - [노트북도 서버가 될 수 있는데 왜 굳이 서버 호스팅을 받나요?](#노트북도-서버가-될-수-있는데-왜-굳이-서버-호스팅을-받나요-1)
        - [왜 맥북이나 가정용 PC로 서비스하지 않고 별도의 서버용 컴퓨터를 사용하나요?](#왜-맥북이나-가정용-pc로-서비스하지-않고-별도의-서버용-컴퓨터를-사용하나요)
            - [원문: 왜 맥북이나 가정용 PC로 서비스하지 않고 별도의 서버용 컴퓨터를 사용하나요?](#원문-왜-맥북이나-가정용-pc로-서비스하지-않고-별도의-서버용-컴퓨터를-사용하나요)
                - [왜 맥북이나 가정용 PC로 서비스하지 않고 별도의 서버용 컴퓨터를 사용하나요?](#왜-맥북이나-가정용-pc로-서비스하지-않고-별도의-서버용-컴퓨터를-사용하나요-1)
    - [스케줄링과 선점](#스케줄링과-선점)
        - [선점형(Pre-emption)OS와 비선점형(Nonpre-emption)OS 차이](#선점형pre-emptionos와-비선점형nonpre-emptionos-차이)
            - [원문: 선점형(Pre-emption)OS와 비선점형(Nonpre-emption)OS 차이](#원문-선점형pre-emptionos와-비선점형nonpre-emptionos-차이)
                - [선점형(Pre-emption)OS와 비선점형(Nonpre-emption)OS 차이](#선점형pre-emptionos와-비선점형nonpre-emptionos-차이-1)
    - [프로세스 생성과 실행](#프로세스-생성과-실행)
        - [`&`와 background process](#와-background-process)
            - [원문: `&`와 background process](#원문-와-background-process)
                - [`&`와 background process](#와-background-process-1)
        - [exec](#exec)
            - [원문: exec](#원문-exec)
                - [exec](#exec-1)
        - [fork](#fork)
        ...
        - [java 프로그램이 프로세스로 실행되는 과정: 시스템 레벨부터 JVM까지](#java-프로그램이-프로세스로-실행되는-과정-시스템-레벨부터-jvm까지)
            - [원문: java 프로그램이 프로세스로 실행되는 과정: 시스템 레벨부터 JVM까지](#원문-java-프로그램이-프로세스로-실행되는-과정-시스템-레벨부터-jvm까지)
                - [java 프로그램이 프로세스로 실행되는 과정: 시스템 레벨부터 JVM까지](#java-프로그램이-프로세스로-실행되는-과정-시스템-레벨부터-jvm까지-1)
    - [면접에서 OS 질문을 30초 안에 답하는 기본 구조](#면접에서-os-질문을-30초-안에-답하는-기본-구조)
    - [컴퓨터는 전원이 들어온 뒤 첫 명령을 어떻게 실행하는가](#컴퓨터는-전원이-들어온-뒤-첫-명령을-어떻게-실행하는가)
    - [BIOS와 UEFI는 무엇이 다르고 왜 UEFI가 등장했는가](#bios와-uefi는-무엇이-다르고-왜-uefi가-등장했는가)
    - [부트로더는 커널을 어떻게 찾아 메모리에 올리는가](#부트로더는-커널을-어떻게-찾아-메모리에-올리는가)
    - [커널은 부팅 직후 어떤 순서로 자기 자신을 초기화하는가](#커널은-부팅-직후-어떤-순서로-자기-자신을-초기화하는가)
    - [systemd는 왜 PID 1로 실행되고 무엇을 책임지는가](#systemd는-왜-pid-1로-실행되고-무엇을-책임지는가)
    - [데몬은 일반 프로세스와 무엇이 다르고 어떻게 관리되는가](#데몬은-일반-프로세스와-무엇이-다르고-어떻게-관리되는가)
    - [CPU는 명령어를 어떤 주기로 실행하는가](#cpu는-명령어를-어떤-주기로-실행하는가)
    - [ISA와 마이크로아키텍처는 어떻게 다른가](#isa와-마이크로아키텍처는-어떻게-다른가)
    - [레지스터는 왜 필요하고 메모리보다 왜 빠른가](#레지스터는-왜-필요하고-메모리보다-왜-빠른가)
    - [CPU 캐시는 왜 필요하고 어떤 계층으로 구성되는가](#cpu-캐시는-왜-필요하고-어떤-계층으로-구성되는가)
    - [캐시 일관성은 멀티코어에서 어떤 문제를 해결하는가](#캐시-일관성은-멀티코어에서-어떤-문제를-해결하는가)
    - [메모리 계층 구조는 성능 병목을 어떻게 만든다](#메모리-계층-구조는-성능-병목을-어떻게-만든다)
    - [정수는 컴퓨터 안에서 어떻게 표현되고 overflow는 왜 생기는가](#정수는-컴퓨터-안에서-어떻게-표현되고-overflow는-왜-생기는가)
    - [부동소수점은 왜 정확한 십진수가 아니고 언제 위험한가](#부동소수점은-왜-정확한-십진수가-아니고-언제-위험한가)
    - [엔디언은 무엇이고 네트워크 바이트 오더와 어떻게 연결되는가](#엔디언은-무엇이고-네트워크-바이트-오더와-어떻게-연결되는가)
    - [alignment와 padding은 메모리 구조에 어떤 영향을 주는가](#alignment와-padding은-메모리-구조에-어떤-영향을-주는가)
    - [사용자 모드와 커널 모드는 왜 분리되어 있는가](#사용자-모드와-커널-모드는-왜-분리되어-있는가)
    - [시스템 콜은 함수 호출과 무엇이 다른가](#시스템-콜은-함수-호출과-무엇이-다른가)
    - [interrupt, trap, exception은 어떻게 구분하는가](#interrupt-trap-exception은-어떻게-구분하는가)
    - [커널은 하드웨어 장치를 어떻게 추상화하는가](#커널은-하드웨어-장치를-어떻게-추상화하는가)
    - [프로세스는 실행 중인 프로그램 이상의 무엇인가](#프로세스는-실행-중인-프로그램-이상의-무엇인가)
    - [프로세스 주소 공간은 어떤 구역으로 나뉘는가](#프로세스-주소-공간은-어떤-구역으로-나뉘는가)
    - [fork는 무엇을 복사하고 무엇을 공유하는가](#fork는-무엇을-복사하고-무엇을-공유하는가)
    - [exec는 왜 새 프로세스를 만드는 것이 아니라 현재 프로세스를 갈아끼우는가](#exec는-왜-새-프로세스를-만드는-것이-아니라-현재-프로세스를-갈아끼우는가)
    - [wait와 exit는 부모-자식 프로세스 관계를 어떻게 닫는가](#wait와-exit는-부모-자식-프로세스-관계를-어떻게-닫는가)
    - [shell에서 `&`를 붙이면 실제로 무엇이 달라지는가](#shell에서-를-붙이면-실제로-무엇이-달라지는가)
    - [프로세스와 스레드는 무엇을 공유하고 무엇을 따로 가지는가](#프로세스와-스레드는-무엇을-공유하고-무엇을-따로-가지는가)
    - [스레드가 많아지면 왜 context switch 비용이 문제가 되는가](#스레드가-많아지면-왜-context-switch-비용이-문제가-되는가)
    - [선점형 스케줄링은 왜 필요하고 어떤 위험을 만든다](#선점형-스케줄링은-왜-필요하고-어떤-위험을-만든다)
    - [스케줄러는 어떤 기준으로 다음 실행 대상을 고르는가](#스케줄러는-어떤-기준으로-다음-실행-대상을-고르는가)
    - [load average는 CPU 사용률과 무엇이 다른가](#load-average는-cpu-사용률과-무엇이-다른가)
    - [가상 메모리는 왜 필요하고 실제 메모리와 어떻게 연결되는가](#가상-메모리는-왜-필요하고-실제-메모리와-어떻게-연결되는가)
    - [MMU와 TLB는 주소 변환에서 어떤 역할을 하는가](#mmu와-tlb는-주소-변환에서-어떤-역할을-하는가)
    - [page table은 프로세스마다 왜 따로 필요한가](#page-table은-프로세스마다-왜-따로-필요한가)
    - [page fault는 항상 장애인가](#page-fault는-항상-장애인가)
    - [copy-on-write는 fork 비용을 어떻게 줄이는가](#copy-on-write는-fork-비용을-어떻게-줄이는가)
    - [swap은 메모리 부족을 어떻게 완화하고 왜 성능을 무너뜨릴 수 있는가](#swap은-메모리-부족을-어떻게-완화하고-왜-성능을-무너뜨릴-수-있는가)
    - [OOM Killer는 언제 프로세스를 죽이는가](#oom-killer는-언제-프로세스를-죽이는가)
    - [커널 메모리와 사용자 메모리는 왜 분리해서 봐야 하는가](#커널-메모리와-사용자-메모리는-왜-분리해서-봐야-하는가)
    - [메모리 할당기는 malloc 요청을 실제 메모리와 어떻게 연결하는가](#메모리-할당기는-malloc-요청을-실제-메모리와-어떻게-연결하는가)
    - [파일 디스크립터는 파일이 아니라 무엇을 가리키는가](#파일-디스크립터는-파일이-아니라-무엇을-가리키는가)
    - [inode, dentry, file object는 파일 시스템에서 어떻게 연결되는가](#inode-dentry-file-object는-파일-시스템에서-어떻게-연결되는가)
    - [VFS는 왜 여러 파일 시스템을 하나처럼 보이게 하는가](#vfs는-왜-여러-파일-시스템을-하나처럼-보이게-하는가)
    - [page cache는 파일 I/O 성능을 어떻게 바꾸는가](#page-cache는-파일-io-성능을-어떻게-바꾸는가)
    - [`write()`가 성공하면 데이터는 디스크에 저장된 것인가](#write가-성공하면-데이터는-디스크에-저장된-것인가)
    - [파일 시스템 journaling은 어떤 장애를 막고 무엇을 보장하지 않는가](#파일-시스템-journaling은-어떤-장애를-막고-무엇을-보장하지-않는가)
    - [block device와 character device는 어떻게 다른가](#block-device와-character-device는-어떻게-다른가)
    - [I/O scheduler는 디스크 요청을 왜 재정렬하는가](#io-scheduler는-디스크-요청을-왜-재정렬하는가)
    - [DMA는 CPU 개입 없이 데이터를 어떻게 옮기는가](#dma는-cpu-개입-없이-데이터를-어떻게-옮기는가)
    - [zero-copy는 실제로 어떤 복사를 줄이는가](#zero-copy는-실제로-어떤-복사를-줄이는가)
    - [blocking I/O는 스레드를 정확히 어디에서 멈추게 하는가](#blocking-io는-스레드를-정확히-어디에서-멈추게-하는가)
    - [non-blocking I/O는 blocking I/O와 무엇이 다른가](#non-blocking-io는-blocking-io와-무엇이-다른가)
    - [select, poll, epoll은 왜 등장했고 무엇이 달라졌는가](#select-poll-epoll은-왜-등장했고-무엇이-달라졌는가)
    - [kqueue와 io_uring은 epoll과 어떤 관점에서 비교해야 하는가](#kqueue와-io_uring은-epoll과-어떤-관점에서-비교해야-하는가)
    - [소켓은 파일 디스크립터인데 왜 네트워크 통신이 되는가](#소켓은-파일-디스크립터인데-왜-네트워크-통신이-되는가)
    - [네트워크 패킷은 NIC에서 애플리케이션 버퍼까지 어떻게 이동하는가](#네트워크-패킷은-nic에서-애플리케이션-버퍼까지-어떻게-이동하는가)
    - [TCP 연결 상태는 커널 안에서 어떻게 관리되는가](#tcp-연결-상태는-커널-안에서-어떻게-관리되는가)
    - [pipe와 redirect는 shell 명령 사이에 어떤 커널 객체를 만든다](#pipe와-redirect는-shell-명령-사이에-어떤-커널-객체를-만든다)
    - [signal은 프로세스에게 어떤 방식으로 비동기 이벤트를 전달하는가](#signal은-프로세스에게-어떤-방식으로-비동기-이벤트를-전달하는가)
    - [IPC는 프로세스 사이의 격리를 어떻게 우회해서 협력하게 하는가](#ipc는-프로세스-사이의-격리를-어떻게-우회해서-협력하게-하는가)
    - [mutex, spinlock, futex는 어느 계층의 동기화 도구인가](#mutex-spinlock-futex는-어느-계층의-동기화-도구인가)
    - [RCU는 커널에서 읽기 많은 자료구조를 어떻게 보호하는가](#rcu는-커널에서-읽기-많은-자료구조를-어떻게-보호하는가)
    - [atomic operation과 memory barrier는 왜 하드웨어 지식이 필요한가](#atomic-operation과-memory-barrier는-왜-하드웨어-지식이-필요한가)
    - [NUMA는 서버 메모리 접근 비용을 어떻게 바꾸는가](#numa는-서버-메모리-접근-비용을-어떻게-바꾸는가)
    - [SMT와 물리 코어는 성능 설명에서 어떻게 구분해야 하는가](#smt와-물리-코어는-성능-설명에서-어떻게-구분해야-하는가)
    - [서버용 하드웨어는 가정용 PC와 어떤 장애 모델이 다른가](#서버용-하드웨어는-가정용-pc와-어떤-장애-모델이-다른가)
    - [RAID와 백업은 왜 같은 말이 아닌가](#raid와-백업은-왜-같은-말이-아닌가)
    - [가상화는 OS 위에 또 다른 OS를 어떻게 실행하게 하는가](#가상화는-os-위에-또-다른-os를-어떻게-실행하게-하는가)
    - [container는 VM과 무엇이 다르고 커널을 어떻게 공유하는가](#container는-vm과-무엇이-다르고-커널을-어떻게-공유하는가)
    - [namespace는 프로세스가 보는 세계를 어떻게 분리하는가](#namespace는-프로세스가-보는-세계를-어떻게-분리하는가)
    - [cgroup은 리소스 사용량을 어떻게 제한하고 관측하는가](#cgroup은-리소스-사용량을-어떻게-제한하고-관측하는가)
    - [container에서 PID 1 문제는 왜 생기는가](#container에서-pid-1-문제는-왜-생기는가)
    - [권한 모델은 UID, GID, capability로 어떻게 나뉘는가](#권한-모델은-uid-gid-capability로-어떻게-나뉘는가)
    - [SELinux와 AppArmor는 일반 파일 권한과 무엇이 다른가](#selinux와-apparmor는-일반-파일-권한과-무엇이-다른가)
    - [ASLR, NX, stack canary는 실행 파일 보안을 어떻게 강화하는가](#aslr-nx-stack-canary는-실행-파일-보안을-어떻게-강화하는가)
    - [ELF 실행 파일은 어떤 구조를 가지고 커널에 로딩되는가](#elf-실행-파일은-어떤-구조를-가지고-커널에-로딩되는가)
    - [동적 링커는 프로그램 시작 전에 무엇을 해석하는가](#동적-링커는-프로그램-시작-전에-무엇을-해석하는가)
    - [ABI와 calling convention은 함수 호출을 어떤 규칙으로 고정하는가](#abi와-calling-convention은-함수-호출을-어떤-규칙으로-고정하는가)
    - [core dump는 프로세스가 죽을 때 어떤 정보를 남기는가](#core-dump는-프로세스가-죽을-때-어떤-정보를-남기는가)
    - [strace는 시스템 콜 관점에서 프로그램을 어떻게 보여 주는가](#strace는-시스템-콜-관점에서-프로그램을-어떻게-보여-주는가)
    - [lsof는 열린 파일과 포트를 어떤 기준으로 보여 주는가](#lsof는-열린-파일과-포트를-어떤-기준으로-보여-주는가)
    - [perf는 CPU 병목을 어떤 샘플링 정보로 설명하는가](#perf는-cpu-병목을-어떤-샘플링-정보로-설명하는가)
    - [vmstat, iostat, pidstat는 각각 어떤 병목을 드러내는가](#vmstat-iostat-pidstat는-각각-어떤-병목을-드러내는가)
    - [로그와 메트릭만으로 OS 병목을 오판하기 쉬운 경우는 무엇인가](#로그와-메트릭만으로-os-병목을-오판하기-쉬운-경우는-무엇인가)
    - [Linux와 macOS, Windows의 OS 개념은 어디까지 공통이고 어디서 달라지는가](#linux와-macos-windows의-os-개념은-어디까지-공통이고-어디서-달라지는가)
    - [POSIX는 운영체제 지식을 어떤 공통 인터페이스로 묶는가](#posix는-운영체제-지식을-어떤-공통-인터페이스로-묶는가)
    - [OS 지식은 Java, Spring, Netty 설명으로 어떻게 이어지는가](#os-지식은-java-spring-netty-설명으로-어떻게-이어지는가)
    - [OS 지식은 데이터베이스 성능 설명으로 어떻게 이어지는가](#os-지식은-데이터베이스-성능-설명으로-어떻게-이어지는가)
    - [OS 지식은 시스템 아키텍처 답변에서 어떤 판단 축이 되는가](#os-지식은-시스템-아키텍처-답변에서-어떤-판단-축이-되는가)
...
확인 방법은 `/proc`, `strace`, `lsof`, `ss`, `vmstat`, `iostat`, scheduler/load 지표, kernel log입니다. 빠르게 반환된 호출도 page cache, dirty page, socket buffer, device queue에 남아 나중의 지연이나 실패로 보일 수 있습니다.
...
2. IEEE 754 표준
   ...
   IEEE 754는 부동 소수점 표현의 국제 표준으로, 다음과 같은 형식을 정의합니다:
    ...
    8비트 지수부는 비트 패턴만 보면 00000000부터 11111111, 즉 0~255입니다.
    그런데 실제 지수는 아래와 같은 값들이 필요합니다:
    - 2³  -> 실제 지수 +3
    - 2⁰  -> 실제 지수  0
    - 2⁻³ -> 실제 지수 -3

    이때 그냥 8비트에 음수 지수를 직접 넣으려면 "지수부 안에 또 부호 비트를 둘 것인가?" 같은 문제가 생깁니다.
    IEEE 754는 대신 기준점을 가운데쯤에 두고, 그 기준점보다 작으면 음수 지수, 크면 양수 지수로 해석하는데, 단정밀도에서는 그 기준점이 127입니다.
    왜 하필 127이냐면, 단정밀도 지수부가 8비트이기 때문입니다.
    8비트는 총 256개 패턴을 만들 수 있는데, IEEE 754는 그중 00000000(0 또는 아주 작은 비정규화 수)과 11111111(무한대 또는 NaN)을 일반 정규화 수가 아니라 특수 용도로 남겨 둡니다.

    - 실제 지수 +3 -> 저장 지수 130 -> 10000010
    - 실제 지수  0 -> 저장 지수 127 -> 01111111
    - 실제 지수 -3 -> 저장 지수 124 -> 01111100

    그러면 일반 정규화 수의 저장 지수는 1~254를 씁니다. 이 범위를 실제 지수로 바꾸면:
    - 저장 지수 1   -> 실제 지수 1 - 127   = -126
    - 저장 지수 127 -> 실제 지수 127 - 127 = 0
    - 저장 지수 254 -> 실제 지수 254 - 127 = +127
    ...
    - 지수부(Exponent):
      ...
      bias를 사용하여 음수 지수 표현합니다. 바이어스(bias)는 실제 지수를 저장용 숫자로 바꾸기 위해 더해 두는 기준값입니다. 즉, 음수 지수와 양수 지수를 한 지수 필드 안에 저장하기 위한 인코딩 규칙입니다.
...
4. 부동 소수점 연산의 구현
    ...
    - 덧셈/뺄셈의 경우:
      ...
      '12.375 + 3.25' 덧셈 과정의 비트 단위 연산은 다음과 같습니다.
        ...
        2. 가수부 덧셈 준비
            ...
            두 수의 지수를 먼저 맞추기 위해 작은 지수를 가진 수의 가수 조정합니다.

            더 큰 지수인 2³에 맞추기 위해 3.25의 유효숫자를 오른쪽으로 2비트 시프트합니다.
        ...
        3. 이진 덧셈 수행
              ...
              1.100011 00000000000000000
            + 0.011010 00000000000000000
            -------------------------
              1.111101 00000000000000000

        4. 결과 정규화 확인

            결과가 이미 1.xxx 형태이므로 추가 정규화는 필요 없습니다.

            1.111101 × 2³ = 1111.101₂ = 15.625₁₀
                                ^ 우측으로 세 칸 이동

        5. 최종 비트 패턴 구성
            - 부호: 0 (양수)
            - 지수: 10000010 (130₁₀, 즉 3 + 127)
            - 가수: 11110100000000000000000
    ...
    - 곱셈의 경우:
      ...
      original: 40.218750 0 10000100 01000001110000000000000

      IEEE 754 분석:
      Sign bit: 0 Exponent: 10000100 (132)
...
systemd가 다른 시스템 데몬들을 실행하는 과정은 기본적으로 fork와 exec의 조합을 사용합니다. 하지만 systemd는 전통적인 Unix 방식에 몇 가지 최적화와 추가 기능을 더했습니다.
...
이렇게 systemd는 기본적인 fork-exec 모델을 사용하면서도, 추가적인 기능과 최적화를 통해 더 효율적이고 유연한 서비스 관리를 제공합니다. 소켓 활성화, D-Bus 활성화, cgroups 사용 등의 기능은 전통적인 init 시스템에서는 볼 수 없었던 systemd의 특징적인 기능들입니다.
...
3. 부트 로더 로딩
   ...
   부트 로더는 운영 체제를 로드하는 소프트웨어 프로그램입니다. 운영 체제가 올바르게 시작될 수 있도록 필요한 초기 환경을 제공합니다.

   디스크는 MBR이나 GPT 중 하나의 파티션 테이블만 사용합니다. UEFI 시스템은 주로 GPT를 사용하며, 레거시 BIOS 시스템은 MBR을 사용합니다.
...
4. 부트 로더 실행
   ...
   부트 로더는 다양한 커널 버전이나 운영 체제 중 선택할 수 있는 메뉴를 제공하고, 선택된 커널에 필요한 파라미터를 전달합니다 부트 로더의 작동 원리는 다음과 같습니다.
    ...
    3. 초기 RAM 디스크 (initrd/initramfs) 로딩:
       ...
       임시 루트 파일 시스템을 로드합니다. 대부분의 현대 시스템은 initramfs를 사용하며, initrd는 레거시 시스템에서 사용됩니다.
...
5. 시스템 초기화 및 서비스 시작
    ...
    - systemd:
      ...
      systemd는 많은 Linux 배포판에서 사용하는 초기화 시스템(init system)이자 시스템 및 서비스 관리자입니다. 이전의 SysV init 시스템을 대체했습니다.

      시스템 부팅 시 프로세스 ID 1로 실행되며, 다른 모든 프로세스의 부모 프로세스 역할을 합니다. systemd의 주요 역할은 시스템 데몬을 포함한 다양한 서비스를 관리하고 시작/중지하는 것입니다.
    ...
    - 시스템 데몬 시작:
      ...
      시스템 데몬은 백그라운드에서 실행되는 프로세스로, 특정 서비스나 기능을 제공합니다. 예를 들어 sshd (SSH 서버), httpd (웹 서버), crond (작업 스케줄러) 등이 있습니다. systemd가 이러한 네트워크, 로깅, 보안 등 기본 시스템 서비스(데몬)를 시작합니다.
...
서버용 컴퓨터와 가정용 PC는 하드웨어 설계 목적에 따라 많은 차이점을 가지고 있습니다. 서버는 안정성, 확장성, 고가용성, 성능을 목표로 설계되었으며, 가정용 PC는 일반적으로 비용과 성능의 균형을 중시합니다.
...
- 프로세서 (CPU)
  ...
  가정용 PC는 일반적으로 더 높은 클럭 속도를 제공하여 단일 스레드 성능에 유리하지만, 서버용 CPU는 다중 스레드 처리와 안정성을 중시합니다. 서버용 CPU는 가정용 PC보다 훨씬 많은 코어를 지원하여 멀티태스킹 성능이 뛰어납니다. 서버용 CPU는 수십 개의 메모리 슬롯을 지원하며, 수 TB의 ECC 메모리를 사용할 수 있습니다.
...
- 메모리 (RAM)
  ...
  서버 메모리는 ECC 기능을 통해 메모리 오류를 감지하고 수정하여 데이터 무결성이 보장되지만, 가정용 PC 메모리는 ECC를 지원하지 않아 안정성 면에서 차이가 있습니다. 서버는 수 TB의 메모리를 지원하는 반면, 가정용 PC는 128GB 정도가 최대입니다.
...
- 저장 장치 (Storage)
  ...
  서버용 저장 장치는 높은 TBW 수명과 강력한 내구성을 제공하여 데이터 무결성을 유지합니다. PC용 저장 장치는 성능은 뛰어나지만, 내구성에서 서버급 장치보다 낮습니다.
...
- 전원 공급 장치 (PSU)
  ...
  서버는 PSU가 중복되어 전원 문제 시에도 지속적으로 운영될 수 있습니다. 가정용 PC는 단일 PSU만 사용하므로 PSU 고장 시 시스템이 중단될 위험이 있습니다.
...
- 네트워크 인터페이스
  ...
  서버 네트워크 인터페이스는 PC에 비해 훨씬 빠른 네트워크 속도와 대역폭을 제공합니다. 서버는 다중 네트워크 포트를 사용하여 더 높은 가용성과 안정성을 제공하지만, PC는 단일 기가비트 이더넷만을 지원합니다.

같은 코어 수와 RAM 크기를 가진 PC와 서버 컴퓨터가 있다고 가정해도, 서버 컴퓨터를 선택하는 데에는 여러 중요한 이유가 있습니다:

1. 프로세서 아키텍처의 차이:
   서버용 프로세서(예: Intel Xeon)는 데스크톱 프로세서와 다른 아키텍처를 가집니다. 이들은 다음과 같은 특징을 갖습니다:
...
2. ECC (Error-Correcting Code) 메모리:
   서버용 시스템은 ECC 메모리를 사용합니다. ECC 메모리는 다음과 같은 이점을 제공합니다:
...
`exec()` 호출은 메모리 페이지를 복사하는 것이 아니라, 기존의 메모리 맵을 지우고 새로운 메모리 맵을 로드하기 때문에, 기존의 부모 프로세스 메모리를 재사용하지 않습니다. 따라서 `exec()` 호출 후에는 이전 프로세스의 메모리 상태가 남아 있지 않고, 새로운 프로그램이 해당 프로세스의 메모리 공간을 차지하게 됩니다.
...
`fork` 호출 시, 자식 프로세스는 부모 프로세스의 메모리 공간을 그대로 공유합니다. 즉, 자식 프로세스는 부모 프로세스와 동일한 메모리 맵을 가지고 있지만, 메모리 페이지는 실제로 복사되지 않습니다. 이때의 복사는 논리적인 복사로, 실제 메모리 페이지는 자식 프로세스가 해당 페이지를 수정하려고 할 때까지 물리적으로 복사되지 않습니다.

COW 방식으로 동작하므로, 자식 프로세스가 메모리 내용을 수정할 때까지는 부모와 자식 프로세스가 동일한 메모리 페이지를 공유합니다. 그러나 자식 프로세스가 메모리를 수정하려고 하면, 해당 메모리 페이지가 실제로 복사되어 자식 프로세스에서만 수정됩니다. 자식 프로세스가 처음 생성되었을 때는 Apache 마스터 프로세스의 메모리 상태를 공유합니다. 하지만 PHP를 처리하는 동안 PHP 코드가 실행되면서 메모리 수정이 일어나면, 해당 메모리 페이지는 COW에 의해 독립적으로 복사됩니다.

Apache 마스터 프로세스는 미리 설정된 수만큼의 자식 프로세스를 생성합니다. 이때, 자식 프로세스는 Apache 마스터 프로세스의 상태를 그대로 상속받습니다. 이 프로세스들은 PHP를 처리하기 위한 프로세스로 동작합니다. mod_php 모듈을 사용할 경우, PHP 인터프리터가 Apache 마스터 프로세스에 이미 로드되어 있으면, 자식 프로세스가 부모 프로세스의 메모리 공간을 상속하게 됩니다.
...
1. 시스템 레벨 프로세스 실행 메커니즘
    ...
    1. 셸의 명령어 처리 시스템
       ...
       `bash -c "java -jar some_boot_app.jar"`가 실행될 때, 다음과 같은 과정이 발생합니다:
        ...
        2. Java 실행 파일 경로 해석 시스템은 다음과 같은 계층적 구조로 Java 실행 파일을 찾습니다:
           ...
           이 과정에서 각 심볼릭 링크는 다음과 같은 메타데이터를 포함합니다:
            ...
            - ASLR 활성화:
              ...
              ASLR이 활성화되어 있어 JVM의 베이스 주소가 매번 달라집니다. 가령 아래 ELF 구조 분석에서 나오는 Entry point address(0x1040)는 파일 내에서의 상대적인 오프셋입니다. ASLR이 적용되는 실제 베이스 주소는 프로세스가 메모리에 로드될 때 결정됩니다. 실제 실행 시 Entry point의 절대 주소는 `베이스 주소 + 0x1040`이 됩니다.
           ...
           ELF(Executable and Linkable Format)는 실행 파일, 객체 파일, 공유 라이브러리, 코어 덤프 등을 위한 표준 파일 포맷입니다. Unix 계열 시스템에서 가장 널리 사용되는 실행 파일 포맷입니다.
    ...
    2. ELF 바이너리 구조 분석
       ...
       Java 실행 파일은 ELF(Executable and Linkable Format) 형식을 따릅니다. 다음과 같은 ELF 구조의 특징들은 JVM이 시작될 때 기반이 되는 환경을 제공하며, JVM의 성능과 보안에 직접적인 영향을 미칩니다. 예를 들어, JIT 컴파일러가 생성하는 네이티브 코드는 이러한 ELF 구조가 제공하는 제약 조건 내에서 동작해야 합니다.
        ...
        2. 주요 세그먼트
           ...
           Java 실행 파일(예: /usr/bin/java)의 ELF 구조는 일반적인 네이티브 실행 파일과 다른 몇 가지 특징적인 구조를 가집니다:
            ...
            - `LOAD` 세그먼트들
              ...
              `LOAD` 세그먼트들의 메모리 매핑이 완료된 후에야 JVM이 자신의 힙 공간을 할당할 수 있습니다 PLT/GOT를 통한 동적 심볼 해석으로 인해 JNI 호출 시 약간의 오버헤드가 발생합니다
            ...
            - `DYNAMIC` 세그먼트
              ...
              Java 실행 파일은 매우 많은 공유 라이브러리 의존성을 가집니다. 다음과 같은 공유 라이브러리들이 `DYNAMIC` 세그먼트에 명시되어 있습니다.
                - libjvm.so (JVM 코어)
...
2. 프로세스 생성 및 메모리 초기화
    ...
    1. Fork-Exec 메커니즘:
       ...
       운영체제는 새로운 프로세스를 생성하기 위해 Fork-Exec 패턴을 사용합니다. 이 과정은 다음과 같은 단계로 이루어집니다:
...
7. 메모리 관리 상세 분석
    ...
    1. 세대별 가비지 컬렉션 구현
       ...
       JVM의 가비지 컬렉션은 "Generational Hypothesis"에 기반하여 설계되었습니다. 이는 다음과 같은 특성을 가집니다:
...
아래 항목들은 현재 원문 배치 내용과 별개로 앞으로 정식 답변 자산으로 준비할 중주제 후보입니다. 이미 정리된 원문은 위에 그대로 두고, 누락 위험을 줄이기 위해 OS, 커널, 하드웨어, 실행 파일, 관측, 운영 질문까지 넓게 펼쳐 둡니다.

## 면접에서 OS 질문을 30초 안에 답하는 기본 구조

이 절에서 다룰 핵심: 짧은 직답, 핵심 구분, 아래층으로 내려가는 첫 기술 단위, 꼬리 질문을 받았을 때의 전개 순서.

## 컴퓨터는 전원이 들어온 뒤 첫 명령을 어떻게 실행하는가

이 절에서 다룰 핵심: 전원 공급, reset vector, firmware, BIOS/UEFI, bootloader, kernel entry까지 이어지는 첫 실행 경로.

## BIOS와 UEFI는 무엇이 다르고 왜 UEFI가 등장했는가

이 절에서 다룰 핵심: legacy BIOS 한계, GPT, EFI System Partition, Secure Boot, firmware와 OS 사이의 계약.

## 부트로더는 커널을 어떻게 찾아 메모리에 올리는가

이 절에서 다룰 핵심: GRUB, boot entry, kernel image, kernel parameter, initramfs, root filesystem 전환.

## 커널은 부팅 직후 어떤 순서로 자기 자신을 초기화하는가

이 절에서 다룰 핵심: 압축 해제, 초기 page table, interrupt 설정, scheduler 준비, init 프로세스 실행.

## systemd는 왜 PID 1로 실행되고 무엇을 책임지는가

이 절에서 다룰 핵심: init system, service dependency, target, unit, socket activation, process reaping.

## 데몬은 일반 프로세스와 무엇이 다르고 어떻게 관리되는가

이 절에서 다룰 핵심: session, controlling terminal, background service, restart policy, log, signal handling.

## CPU는 명령어를 어떤 주기로 실행하는가

이 절에서 다룰 핵심: fetch-decode-execute cycle, program counter, register, ALU, pipeline의 기본 흐름.

## ISA와 마이크로아키텍처는 어떻게 다른가

이 절에서 다룰 핵심: x86-64, ARM64 같은 명령어 집합과 실제 CPU 내부 구현의 분리.

## 레지스터는 왜 필요하고 메모리보다 왜 빠른가

이 절에서 다룰 핵심: general-purpose register, instruction operand, stack pointer, frame pointer, syscall register convention.

## CPU 캐시는 왜 필요하고 어떤 계층으로 구성되는가

이 절에서 다룰 핵심: L1/L2/L3, cache line, locality, cache miss, 백엔드 성능 설명에서 캐시가 등장하는 지점.

## 캐시 일관성은 멀티코어에서 어떤 문제를 해결하는가

이 절에서 다룰 핵심: core별 cache, coherence protocol, invalidation, false sharing, memory barrier와의 연결.

## 메모리 계층 구조는 성능 병목을 어떻게 만든다

이 절에서 다룰 핵심: register, cache, RAM, SSD, network storage 사이의 지연 시간 차이와 설계 판단.

## 정수는 컴퓨터 안에서 어떻게 표현되고 overflow는 왜 생기는가

이 절에서 다룰 핵심: two's complement, signed/unsigned, overflow, underflow, 언어별 정수 연산 차이.

## 부동소수점은 왜 정확한 십진수가 아니고 언제 위험한가

이 절에서 다룰 핵심: IEEE 754, binary fraction, rounding, NaN, infinity, 금융/정산 코드에서의 위험.

## 엔디언은 무엇이고 네트워크 바이트 오더와 어떻게 연결되는가

이 절에서 다룰 핵심: little-endian, big-endian, serialization, packet parsing, binary protocol.

## alignment와 padding은 메모리 구조에 어떤 영향을 주는가

이 절에서 다룰 핵심: CPU 접근 단위, struct layout, padding, cache line, native interop에서의 위험.

## 사용자 모드와 커널 모드는 왜 분리되어 있는가

이 절에서 다룰 핵심: privilege level, protection ring, unsafe instruction, kernel boundary, 보안과 안정성.

## 시스템 콜은 함수 호출과 무엇이 다른가

이 절에서 다룰 핵심: user mode to kernel mode 전환, syscall ABI, trap instruction, context 저장과 복원.

## interrupt, trap, exception은 어떻게 구분하는가

이 절에서 다룰 핵심: 외부 장치 interrupt, 의도적 trap, CPU exception, page fault, syscall과의 관계.

## 커널은 하드웨어 장치를 어떻게 추상화하는가

이 절에서 다룰 핵심: device driver, block device, character device, major/minor number, `/dev`.

## 프로세스는 실행 중인 프로그램 이상의 무엇인가

이 절에서 다룰 핵심: address space, file descriptor table, signal handler, credentials, resource limit, kernel task 구조.

## 프로세스 주소 공간은 어떤 구역으로 나뉘는가

이 절에서 다룰 핵심: text, data, bss, heap, stack, mmap region, shared library mapping.

## fork는 무엇을 복사하고 무엇을 공유하는가

이 절에서 다룰 핵심: process table entry, virtual memory, file descriptor, copy-on-write, parent-child 관계.

## exec는 왜 새 프로세스를 만드는 것이 아니라 현재 프로세스를 갈아끼우는가

이 절에서 다룰 핵심: execve, ELF loader, argv/envp, address space replacement, PID 유지.

## wait와 exit는 부모-자식 프로세스 관계를 어떻게 닫는가

이 절에서 다룰 핵심: exit status, zombie process, orphan process, reaping, PID 1의 역할.

## shell에서 `&`를 붙이면 실제로 무엇이 달라지는가

이 절에서 다룰 핵심: foreground/background job, process group, terminal control, job control signal.

## 프로세스와 스레드는 무엇을 공유하고 무엇을 따로 가지는가

이 절에서 다룰 핵심: address space, file descriptor, stack, register context, scheduling unit.

## 스레드가 많아지면 왜 context switch 비용이 문제가 되는가

이 절에서 다룰 핵심: CPU register save/restore, scheduler, cache pollution, run queue, latency.

## 선점형 스케줄링은 왜 필요하고 어떤 위험을 만든다

이 절에서 다룰 핵심: time slice, preemption, fairness, starvation, race condition, kernel preemption.

## 스케줄러는 어떤 기준으로 다음 실행 대상을 고르는가

이 절에서 다룰 핵심: runnable state, priority, nice, CFS, run queue, CPU affinity.

## load average는 CPU 사용률과 무엇이 다른가

이 절에서 다룰 핵심: runnable task, uninterruptible sleep, CPU saturation, I/O wait, 운영 관측 해석.

## 가상 메모리는 왜 필요하고 실제 메모리와 어떻게 연결되는가

이 절에서 다룰 핵심: virtual address, physical address, address translation, isolation, overcommit.

## MMU와 TLB는 주소 변환에서 어떤 역할을 하는가

이 절에서 다룰 핵심: page table walk, TLB hit/miss, context switch와 TLB flush.

## page table은 프로세스마다 왜 따로 필요한가

이 절에서 다룰 핵심: per-process address space, permission bit, user/kernel mapping, page table hierarchy.

## page fault는 항상 장애인가

이 절에서 다룰 핵심: demand paging, copy-on-write, swapped-out page, invalid access, SIGSEGV.

## copy-on-write는 fork 비용을 어떻게 줄이는가

이 절에서 다룰 핵심: shared physical page, write fault, page duplication, fork-heavy 서버와의 연결.

## swap은 메모리 부족을 어떻게 완화하고 왜 성능을 무너뜨릴 수 있는가

이 절에서 다룰 핵심: swap out/in, page reclaim, thrashing, memory pressure, latency spike.

## OOM Killer는 언제 프로세스를 죽이는가

이 절에서 다룰 핵심: overcommit, memory cgroup, oom_score, kill decision, 컨테이너(container) 환경의 차이.

## 커널 메모리와 사용자 메모리는 왜 분리해서 봐야 하는가

이 절에서 다룰 핵심: kernel heap, slab allocator, page cache, pinned memory, user copy.

## 메모리 할당기는 malloc 요청을 실제 메모리와 어떻게 연결하는가

이 절에서 다룰 핵심: brk, mmap, arena, fragmentation, allocator와 OS 경계.

## 파일 디스크립터는 파일이 아니라 무엇을 가리키는가

이 절에서 다룰 핵심: descriptor table, open file description, inode, offset, dup, close.

## inode, dentry, file object는 파일 시스템에서 어떻게 연결되는가

이 절에서 다룰 핵심: path lookup, directory entry, metadata, hard link, open file handle.

## VFS는 왜 여러 파일 시스템을 하나처럼 보이게 하는가

이 절에서 다룰 핵심: ext4, xfs, tmpfs, procfs, sysfs를 공통 API로 다루는 구조.

## page cache는 파일 I/O 성능을 어떻게 바꾸는가

이 절에서 다룰 핵심: read cache, write-back, dirty page, cache hit, 메모리 사용량 해석.

## `write()`가 성공하면 데이터는 디스크에 저장된 것인가

이 절에서 다룰 핵심: buffer, page cache, flush, fsync, fdatasync, durability와 성능 tradeoff.

## 파일 시스템 journaling은 어떤 장애를 막고 무엇을 보장하지 않는가

이 절에서 다룰 핵심: metadata consistency, crash recovery, ordered/writeback mode, application-level durability.

## block device와 character device는 어떻게 다른가

이 절에서 다룰 핵심: disk, terminal, pipe, socket, buffering, random access 가능 여부.

## I/O scheduler는 디스크 요청을 왜 재정렬하는가

이 절에서 다룰 핵심: HDD seek, SSD/NVMe queue, fairness, latency, throughput.

## DMA는 CPU 개입 없이 데이터를 어떻게 옮기는가

이 절에서 다룰 핵심: device memory access, interrupt completion, network/disk I/O, zero-copy의 기반.

## zero-copy는 실제로 어떤 복사를 줄이는가

이 절에서 다룰 핵심: user-kernel copy, sendfile, mmap, splice, NIC 전송 경로.

## blocking I/O는 스레드를 정확히 어디에서 멈추게 하는가

이 절에서 다룰 핵심: syscall wait, sleep state, wait queue, wakeup, scheduler handoff.

## non-blocking I/O는 blocking I/O와 무엇이 다른가

이 절에서 다룰 핵심: O_NONBLOCK, EAGAIN/EWOULDBLOCK, readiness, retry loop.

## select, poll, epoll은 왜 등장했고 무엇이 달라졌는가

이 절에서 다룰 핵심: fd set scan, interest list, ready list, level-triggered, edge-triggered.

## kqueue와 io_uring은 epoll과 어떤 관점에서 비교해야 하는가

이 절에서 다룰 핵심: BSD event queue, Linux submission/completion queue, readiness vs completion.

## 소켓은 파일 디스크립터인데 왜 네트워크 통신이 되는가

이 절에서 다룰 핵심: socket object, protocol stack, send/receive buffer, TCP state.

## 네트워크 패킷은 NIC에서 애플리케이션 버퍼까지 어떻게 이동하는가

이 절에서 다룰 핵심: interrupt, softirq, kernel network stack, socket buffer, copy to user.

## TCP 연결 상태는 커널 안에서 어떻게 관리되는가

이 절에서 다룰 핵심: SYN backlog, accept queue, ESTABLISHED, TIME_WAIT, keepalive.

## pipe와 redirect는 shell 명령 사이에 어떤 커널 객체를 만든다

이 절에서 다룰 핵심: anonymous pipe, stdin/stdout/stderr, file descriptor inheritance.

## signal은 프로세스에게 어떤 방식으로 비동기 이벤트를 전달하는가

이 절에서 다룰 핵심: SIGTERM, SIGKILL, SIGCHLD, signal mask, handler, async-signal safety.

## IPC는 프로세스 사이의 격리를 어떻게 우회해서 협력하게 하는가

이 절에서 다룰 핵심: pipe, Unix domain socket, shared memory, message queue, semaphore.

## mutex, spinlock, futex는 어느 계층의 동기화 도구인가

이 절에서 다룰 핵심: user-space lock, kernel wait, busy waiting, futex fast path/slow path.

## RCU는 커널에서 읽기 많은 자료구조를 어떻게 보호하는가

이 절에서 다룰 핵심: read-copy-update, grace period, lock-free read path, kernel data structure.

## atomic operation과 memory barrier는 왜 하드웨어 지식이 필요한가

이 절에서 다룰 핵심: compare-and-swap, instruction reordering, visibility, CPU memory model.

## NUMA는 서버 메모리 접근 비용을 어떻게 바꾸는가

이 절에서 다룰 핵심: socket, local/remote memory, NUMA node, CPU pinning, database 성능.

## SMT와 물리 코어는 성능 설명에서 어떻게 구분해야 하는가

이 절에서 다룰 핵심: hyper-threading, execution unit sharing, throughput, latency-sensitive workload.

## 서버용 하드웨어는 가정용 PC와 어떤 장애 모델이 다른가

이 절에서 다룰 핵심: ECC, redundant PSU, hot swap, RAID, BMC, IPMI, data center 운영.

## RAID와 백업은 왜 같은 말이 아닌가

이 절에서 다룰 핵심: disk redundancy, availability, accidental deletion, corruption, restore test.

## 가상화는 OS 위에 또 다른 OS를 어떻게 실행하게 하는가

이 절에서 다룰 핵심: hypervisor, VM, trap and emulate, hardware virtualization, guest kernel.

## container는 VM과 무엇이 다르고 커널을 어떻게 공유하는가

이 절에서 다룰 핵심: namespace, cgroup, image filesystem, process isolation, shared kernel.

## namespace는 프로세스가 보는 세계를 어떻게 분리하는가

이 절에서 다룰 핵심: PID, mount, network, user, IPC namespace.

## cgroup은 리소스 사용량을 어떻게 제한하고 관측하는가

이 절에서 다룰 핵심: CPU quota, memory limit, blkio, pids, 컨테이너(container) OOM.

## container에서 PID 1 문제는 왜 생기는가

이 절에서 다룰 핵심: signal forwarding, zombie reaping, init process behavior, Docker entrypoint.

## 권한 모델은 UID, GID, capability로 어떻게 나뉘는가

이 절에서 다룰 핵심: root privilege, setuid, file permission, Linux capability, least privilege.

## SELinux와 AppArmor는 일반 파일 권한과 무엇이 다른가

이 절에서 다룰 핵심: mandatory access control, policy, profile, denial log, 운영 장애 진단.

## ASLR, NX, stack canary는 실행 파일 보안을 어떻게 강화하는가

이 절에서 다룰 핵심: address randomization, non-executable memory, stack overwrite detection.

## ELF 실행 파일은 어떤 구조를 가지고 커널에 로딩되는가

이 절에서 다룰 핵심: ELF header, program header, section, segment, interpreter, dynamic linking.

## 동적 링커는 프로그램 시작 전에 무엇을 해석하는가

이 절에서 다룰 핵심: shared library, PLT/GOT, relocation, symbol resolution, `ld.so`.

## ABI와 calling convention은 함수 호출을 어떤 규칙으로 고정하는가

이 절에서 다룰 핵심: argument register, stack alignment, return value, syscall ABI, FFI/JNI.

## core dump는 프로세스가 죽을 때 어떤 정보를 남기는가

이 절에서 다룰 핵심: memory image, register, stack trace, ulimit, gdb 분석.

## strace는 시스템 콜 관점에서 프로그램을 어떻게 보여 주는가

이 절에서 다룰 핵심: syscall sequence, blocking point, errno, file/network troubleshooting.

## lsof는 열린 파일과 포트를 어떤 기준으로 보여 주는가

이 절에서 다룰 핵심: PID, FD, inode, socket, deleted file, port owner.

## perf는 CPU 병목을 어떤 샘플링 정보로 설명하는가

이 절에서 다룰 핵심: sampling profiler, call graph, kernel/user stack, CPU cycles.

## vmstat, iostat, pidstat는 각각 어떤 병목을 드러내는가

이 절에서 다룰 핵심: run queue, context switch, page fault, disk utilization, per-process CPU/I/O.

## 로그와 메트릭만으로 OS 병목을 오판하기 쉬운 경우는 무엇인가

이 절에서 다룰 핵심: CPU busy vs I/O wait, memory used vs page cache, load average vs throughput.

## Linux와 macOS, Windows의 OS 개념은 어디까지 공통이고 어디서 달라지는가

이 절에서 다룰 핵심: POSIX, syscall 차이, Mach/BSD, NT kernel, 개발자가 말할 수 있는 공통 추상화.

## POSIX는 운영체제 지식을 어떤 공통 인터페이스로 묶는가

이 절에서 다룰 핵심: process, file, signal, pipe, socket API, Unix-like portability.

## OS 지식은 Java, Spring, Netty 설명으로 어떻게 이어지는가

이 절에서 다룰 핵심: JVM process, native thread, file descriptor, socket, epoll, GC pause와 OS scheduling.

## OS 지식은 데이터베이스 성능 설명으로 어떻게 이어지는가

이 절에서 다룰 핵심: page cache, fsync, lock wait, context switch, NUMA, disk I/O.

## OS 지식은 시스템 아키텍처 답변에서 어떤 판단 축이 되는가

이 절에서 다룰 핵심: connection limit, process/thread model, resource isolation, failure domain, capacity planning.
