...
이 문서는 긴 질문 모음처럼 보이지만, 처음 붙잡을 장면은 하나입니다. 서버가 요청을 받았고, 그 요청이 스레드에서 바로 기다리는지, 커널 소켓 버퍼와 이벤트 루프에 남는지, 코루틴이나 worker queue에 이어지는지를 구분하는 문서입니다. 목차가 길어도 먼저 `blocking / non-blocking`, `sync / async`, `concurrency / parallelism`의 축을 잡고, 그다음 각 원문 질문으로 내려가면 됩니다.
    ...
    - [Blocking, non-blocking, async 구분](#blocking-non-blocking-async-구분)
        - [HTTP 요청을 Async로 처리했을 때의 이점](#http-요청을-async로-처리했을-때의-이점)
            - [원문: HTTP 요청을 Async로 처리했을 때의 이점](#원문-http-요청을-async로-처리했을-때의-이점)
                - [HTTP 요청을 Async로 처리했을 때의 이점](#http-요청을-async로-처리했을-때의-이점-1)
        ...
        - [I/O 서브시스템](#io-서브시스템)
            - [원문: I/O 서브시스템](#원문-io-서브시스템)
                - [I/O 서브시스템](#io-서브시스템-1)
        - [Kotlin의 async HTTP 요청과 코루틴 비교](#kotlin의-async-http-요청과-코루틴-비교)
            - [원문: Kotlin의 async HTTP 요청과 코루틴 비교](#원문-kotlin의-async-http-요청과-코루틴-비교)
                - [Kotlin의 async HTTP 요청과 코루틴 비교](#kotlin의-async-http-요청과-코루틴-비교-1)
                    - [Async HTTP 요청 (예: OkHttp, Retrofit)](#async-http-요청-예-okhttp-retrofit)
        - [논블로킹 vs 비동기 차이](#논블로킹-vs-비동기-차이)
            - [원문: 논블로킹 vs 비동기 차이](#원문-논블로킹-vs-비동기-차이)
                - [논블로킹 vs 비동기 차이](#논블로킹-vs-비동기-차이-1)
        - [블로킹 vs 논블로킹, 동기 vs 비동기, 동시성 vs 병렬성 개념 정리](#블로킹-vs-논블로킹-동기-vs-비동기-동시성-vs-병렬성-개념-정리)
            - [원문: 블로킹 vs 논블로킹, 동기 vs 비동기, 동시성 vs 병렬성 개념 정리](#원문-블로킹-vs-논블로킹-동기-vs-비동기-동시성-vs-병렬성-개념-정리)
                - [블로킹 vs 논블로킹, 동기 vs 비동기, 동시성 vs 병렬성 개념 정리](#블로킹-vs-논블로킹-동기-vs-비동기-동시성-vs-병렬성-개념-정리-1)
        - [비동기 프로그래밍](#비동기-프로그래밍)
            - [원문: 비동기 프로그래밍](#원문-비동기-프로그래밍)
                - [비동기 프로그래밍](#비동기-프로그래밍-1)
        - [싱글 코어 \& 싱글 스레드 경우 커널에서의 비동기와 멀티플렉싱](#싱글-코어--싱글-스레드-경우-커널에서의-비동기와-멀티플렉싱)
            - [원문: 싱글 코어 \& 싱글 스레드 경우 커널에서의 비동기와 멀티플렉싱](#원문-싱글-코어--싱글-스레드-경우-커널에서의-비동기와-멀티플렉싱)
                - [싱글 코어 \& 싱글 스레드 경우 커널에서의 비동기와 멀티플렉싱](#싱글-코어--싱글-스레드-경우-커널에서의-비동기와-멀티플렉싱-1)
    - [Coroutine과 협력형 실행](#coroutine과-협력형-실행)
        - [Kotlin Coroutine](#kotlin-coroutine)
            - [원문: Kotlin Coroutine](#원문-kotlin-coroutine)
                - [Kotlin Coroutine](#kotlin-coroutine-1)
        - [코루틴 제어의 양보](#코루틴-제어의-양보)
            - [원문: 코루틴 제어의 양보](#원문-코루틴-제어의-양보)
                - [코루틴 제어의 양보](#코루틴-제어의-양보-1)
        - [코루틴을 Cooperative Multitasking 이라고 하는 이유?](#코루틴을-cooperative-multitasking-이라고-하는-이유)
            - [원문: 코루틴을 Cooperative Multitasking 이라고 하는 이유?](#원문-코루틴을-cooperative-multitasking-이라고-하는-이유)
                - [코루틴을 Cooperative Multitasking 이라고 하는 이유?](#코루틴을-cooperative-multitasking-이라고-하는-이유-1)
                    - [Kotlin Coroutine](#kotlin-coroutine-2)
                    - [일반적인 Java의 멀티스레딩](#일반적인-java의-멀티스레딩)
    - [Event loop와 네트워크 런타임](#event-loop와-네트워크-런타임)
        - [4. Netty 관련 질문](#4-netty-관련-질문)
            - [원문: 4. Netty 관련 질문](#원문-4-netty-관련-질문)
                - [4. Netty 관련 질문](#4-netty-관련-질문-1)
        ...
        - [Netty의 이벤트 루프(Event Loop)와 epoll 기반 논블로킹 I/O 구현](#netty의-이벤트-루프event-loop와-epoll-기반-논블로킹-io-구현)
            - [원문: Netty의 이벤트 루프(Event Loop)와 epoll 기반 논블로킹 I/O 구현](#원문-netty의-이벤트-루프event-loop와-epoll-기반-논블로킹-io-구현)
                - [Netty의 이벤트 루프(Event Loop)와 epoll 기반 논블로킹 I/O 구현](#netty의-이벤트-루프event-loop와-epoll-기반-논블로킹-io-구현-1)
    - [Goroutine과 런타임 스케줄링](#goroutine과-런타임-스케줄링)
        - [Go의 Goroutine은 M 대 N 모델 멀티스레딩](#go의-goroutine은-m-대-n-모델-멀티스레딩)
            - [원문: Go의 Goroutine은 M 대 N 모델 멀티스레딩](#원문-go의-goroutine은-m-대-n-모델-멀티스레딩)
                - [Go의 Goroutine은 M 대 N 모델 멀티스레딩](#go의-goroutine은-m-대-n-모델-멀티스레딩-1)
    - [Java monitor와 wait/notify](#java-monitor와-waitnotify)
        - [1. 자바에서의 쓰레드와 모니터 개념](#1-자바에서의-쓰레드와-모니터-개념)
            - [원문: 1. 자바에서의 쓰레드와 모니터 개념](#원문-1-자바에서의-쓰레드와-모니터-개념)
                - [1. 자바에서의 쓰레드와 모니터 개념](#1-자바에서의-쓰레드와-모니터-개념-1)
        - [10. 핵심 요약 및 결론](#10-핵심-요약-및-결론)
            - [원문: 10. 핵심 요약 및 결론](#원문-10-핵심-요약-및-결론)
                - [10. 핵심 요약 및 결론](#10-핵심-요약-및-결론-1)
        - [3. synchronized 블록과 모니터 잠금](#3-synchronized-블록과-모니터-잠금)
            - [원문: 3. synchronized 블록과 모니터 잠금](#원문-3-synchronized-블록과-모니터-잠금)
                - [3. synchronized 블록과 모니터 잠금](#3-synchronized-블록과-모니터-잠금-1)
                    - [3.1 진입 과정](#31-진입-과정)
                    - [3.2 탈출 과정](#32-탈출-과정)
        - [4. wait()와 notify() 메서드의 메커니즘](#4-wait와-notify-메서드의-메커니즘)
            - [원문: 4. wait()와 notify() 메서드의 메커니즘](#원문-4-wait와-notify-메서드의-메커니즘)
        ...
        - [5. wait set의 작동 원리](#5-wait-set의-작동-원리)
            - [원문: 5. wait set의 작동 원리](#원문-5-wait-set의-작동-원리)
                - [5. wait set의 작동 원리](#5-wait-set의-작동-원리-1)
        - [6. 쓰레드 상태: BLOCKED vs WAITING](#6-쓰레드-상태-blocked-vs-waiting)
            - [원문: 6. 쓰레드 상태: BLOCKED vs WAITING](#원문-6-쓰레드-상태-blocked-vs-waiting)
        ...
        - [7. notify가 없으면 wait한 스레드는 어떻게 되는가?](#7-notify가-없으면-wait한-스레드는-어떻게-되는가)
            - [원문: 7. notify가 없으면 wait한 스레드는 어떻게 되는가?](#원문-7-notify가-없으면-wait한-스레드는-어떻게-되는가)
                - [7. notify가 없으면 wait한 스레드는 어떻게 되는가?](#7-notify가-없으면-wait한-스레드는-어떻게-되는가-1)
        - [7. 결론 정리](#7-결론-정리)
            - [원문: 7. 결론 정리](#원문-7-결론-정리)
                - [7. 결론 정리](#7-결론-정리-1)
        - [8. JVM 레벨에서의 synchronized 작동 방식](#8-jvm-레벨에서의-synchronized-작동-방식)
            - [원문: 8. JVM 레벨에서의 synchronized 작동 방식](#원문-8-jvm-레벨에서의-synchronized-작동-방식)
        ...
        - [9. 실제 동작 시나리오 예시](#9-실제-동작-시나리오-예시)
            - [원문: 9. 실제 동작 시나리오 예시](#원문-9-실제-동작-시나리오-예시)
                - [9. 실제 동작 시나리오 예시](#9-실제-동작-시나리오-예시-1)
        - [쓰레드 wait와 notify 예제](#쓰레드-wait와-notify-예제)
            - [원문: 쓰레드 wait와 notify 예제](#원문-쓰레드-wait와-notify-예제)
                - [쓰레드 wait와 notify 예제](#쓰레드-wait와-notify-예제-1)
        ...
        - [자바 쓰레드, 상태, 그리고 모니터 등](#자바-쓰레드-상태-그리고-모니터-등)
            - [원문: 자바 쓰레드, 상태, 그리고 모니터 등](#원문-자바-쓰레드-상태-그리고-모니터-등)
                - [자바 쓰레드, 상태, 그리고 모니터 등](#자바-쓰레드-상태-그리고-모니터-등-1)
        - [자바에서 wait/notify 패턴\*\*을 조금 더 현실감 있게 적용한 \*\*프로덕션 코드 예시](#자바에서-waitnotify-패턴을-조금-더-현실감-있게-적용한-프로덕션-코드-예시)
            - [원문: 자바에서 wait/notify 패턴\*\*을 조금 더 현실감 있게 적용한 \*\*프로덕션 코드 예시](#원문-자바에서-waitnotify-패턴을-조금-더-현실감-있게-적용한-프로덕션-코드-예시)
                - [**자바에서 wait/notify 패턴**을 조금 더 현실감 있게 적용한 **프로덕션 코드 예시**](#자바에서-waitnotify-패턴을-조금-더-현실감-있게-적용한-프로덕션-코드-예시-1)
                    ...
                    - [6. 마지막 정리](#6-마지막-정리)
    - [OS I/O multiplexing](#os-io-multiplexing)
        - [Node.js의 싱글 스레드와 libuv의 멀티 스레딩](#nodejs의-싱글-스레드와-libuv의-멀티-스레딩)
            - [원문: Node.js의 싱글 스레드와 libuv의 멀티 스레딩](#원문-nodejs의-싱글-스레드와-libuv의-멀티-스레딩)
                - [Node.js의 싱글 스레드와 libuv의 멀티 스레딩](#nodejs의-싱글-스레드와-libuv의-멀티-스레딩-1)
        - [epoll 상세](#epoll-상세)
            - [원문: epoll 상세](#원문-epoll-상세)
                - [epoll 상세](#epoll-상세-1)
        - [libuv, aio\_\*, io\_uring](#libuv-aio_-io_uring)
            - [원문: libuv, aio\_\*, io\_uring](#원문-libuv-aio_-io_uring)
                - [libuv, aio\_\*, io\_uring](#libuv-aio_-io_uring-1)
        - [네트워크 통신 시 epoll 동작 방식](#네트워크-통신-시-epoll-동작-방식)
            - [원문: 네트워크 통신 시 epoll 동작 방식](#원문-네트워크-통신-시-epoll-동작-방식)
                - [네트워크 통신 시 epoll 동작 방식](#네트워크-통신-시-epoll-동작-방식-1)
        - [동시 요청이 많아 소켓이 몇 만 개 이상 필요한 상황에서 File Descriptor 설정을 수정하지 않은 경우](#동시-요청이-많아-소켓이-몇-만-개-이상-필요한-상황에서-file-descriptor-설정을-수정하지-않은-경우)
            - [원문: 동시 요청이 많아 소켓이 몇 만 개 이상 필요한 상황에서 File Descriptor 설정을 수정하지 않은 경우](#원문-동시-요청이-많아-소켓이-몇-만-개-이상-필요한-상황에서-file-descriptor-설정을-수정하지-않은-경우)
                - [동시 요청이 많아 소켓이 몇 만 개 이상 필요한 상황에서 File Descriptor 설정을 수정하지 않은 경우](#동시-요청이-많아-소켓이-몇-만-개-이상-필요한-상황에서-file-descriptor-설정을-수정하지-않은-경우-1)
        - [소켓, epoll, nginx, 과다한 요청](#소켓-epoll-nginx-과다한-요청)
            - [원문: 소켓, epoll, nginx, 과다한 요청](#원문-소켓-epoll-nginx-과다한-요청)
                - [소켓, epoll, nginx, 과다한 요청](#소켓-epoll-nginx-과다한-요청-1)
    - [공유 상태와 경쟁 조건](#공유-상태와-경쟁-조건)
        - [경쟁 조건 (Race Condition)](#경쟁-조건-race-condition)
            - [원문: 경쟁 조건 (Race Condition)](#원문-경쟁-조건-race-condition)
                - [경쟁 조건 (Race Condition)](#경쟁-조건-race-condition-1)
        - [멀티스레딩에서 어떻게 읽기 전용 속성을 동시에 사용할 수 있는지](#멀티스레딩에서-어떻게-읽기-전용-속성을-동시에-사용할-수-있는지)
            - [원문: 멀티스레딩에서 어떻게 읽기 전용 속성을 동시에 사용할 수 있는지](#원문-멀티스레딩에서-어떻게-읽기-전용-속성을-동시에-사용할-수-있는지)
                - [멀티스레딩에서 어떻게 읽기 전용 속성을 동시에 사용할 수 있는지](#멀티스레딩에서-어떻게-읽기-전용-속성을-동시에-사용할-수-있는지-1)
        ...
        - [자바 쓰레드 동기화](#자바-쓰레드-동기화)
            - [원문: 자바 쓰레드 동기화](#원문-자바-쓰레드-동기화)
                - [자바 쓰레드 동기화](#자바-쓰레드-동기화-1)
                    - [1. 자바에서의 쓰레드와 모니터 개념](#1-자바에서의-쓰레드와-모니터-개념-2)
                    - [2. 쓰레드 상태(States) 개념](#2-쓰레드-상태states-개념)
                    - [3. synchronized 블록과 모니터 잠금](#3-synchronized-블록과-모니터-잠금-2)
                    - [3.1 진입 과정](#31-진입-과정-1)
                    - [3.2 탈출 과정](#32-탈출-과정-1)
                    - [4. wait()와 notify() 메서드의 메커니즘](#4-wait와-notify-메서드의-메커니즘-2)
                    - [4.1 wait()가 하는 일](#41-wait가-하는-일-1)
                    - [4.2 notify()와 notifyAll()](#42-notify와-notifyall-1)
                    - [5. wait set의 작동 원리](#5-wait-set의-작동-원리-2)
                    - [6. 쓰레드 상태: BLOCKED vs WAITING](#6-쓰레드-상태-blocked-vs-waiting-2)
                    - [6.1 BLOCKED 상태](#61-blocked-상태-1)
                    - [6.2 WAITING 상태](#62-waiting-상태-1)
                    - [7. notify가 없으면 wait한 스레드는 어떻게 되는가?](#7-notify가-없으면-wait한-스레드는-어떻게-되는가-2)
                    - [8. JVM 레벨에서의 synchronized 작동 방식](#8-jvm-레벨에서의-synchronized-작동-방식-2)
                    ...
                    - [9. 실제 동작 시나리오 예시](#9-실제-동작-시나리오-예시-2)
                    - [10. 핵심 요약 및 결론](#10-핵심-요약-및-결론-2)
    - [교착과 진행 보장](#교착과-진행-보장)
        - [교착 상태 (Deadlock)](#교착-상태-deadlock)
            - [원문: 교착 상태 (Deadlock)](#원문-교착-상태-deadlock)
                - [교착 상태 (Deadlock)](#교착-상태-deadlock-1)
    - [스레드 상태와 스케줄링](#스레드-상태와-스케줄링)
        - [2. 쓰레드 상태(States) 개념](#2-쓰레드-상태states-개념-1)
            - [원문: 2. 쓰레드 상태(States) 개념](#원문-2-쓰레드-상태states-개념)
                - [2. 쓰레드 상태(States) 개념](#2-쓰레드-상태states-개념-2)
    - [스레드 생성과 스케줄링](#스레드-생성과-스케줄링)
        - [`pthread_create` 이후 커널에서 발생하는 작업들](#pthread_create-이후-커널에서-발생하는-작업들)
            - [원문: `pthread_create` 이후 커널에서 발생하는 작업들](#원문-pthread_create-이후-커널에서-발생하는-작업들)
                - [`pthread_create` 이후 커널에서 발생하는 작업들](#pthread_create-이후-커널에서-발생하는-작업들-1)
...
확인 방법은 thread dump, scheduler queue, event loop lag, socket 상태, `epoll_wait`/`kqueue` 관측, lock wait 로그입니다. "CPU를 안 쓴다"와 "진행이 안전하다"는 같은 말이 아니므로, 어떤 상태가 어디서 기다리는지까지 확인해야 합니다.
...
<!-- curriculum-chunk: sha256=ba19407a52452a019f15b8d1a639fe4ec588c6cc10c49fe5dcbcfe504cdf57eb major=concurrency-async-io mid=Blocking, non-blocking, async 구분 sub=HTTP 요청을 Async로 처리했을 때의 이점 sources=source/interview_questions2.md:439-541, source/interviews2.md:439-541 -->

> Source: `source/interview_questions2.md:439-541`
> Classification reason: async/non-blocking
> Duplicate source aliases: `source/interview_questions2.md:439-541, source/interviews2.md:439-541`
...
<!-- curriculum-chunk: sha256=91014364d9fab87a6f61ccb6eda97432a5593d7d2479beb11f549912c396daa4 major=concurrency-async-io mid=Blocking, non-blocking, async 구분 sub=I/O 서브시스템 sources=source/interview_questions.md:4524-4582, source/interviews.md:4524-4582 -->

> Source: `source/interview_questions.md:4524-4582`
> Classification reason: async/non-blocking
> Duplicate source aliases: `source/interview_questions.md:4524-4582, source/interviews.md:4524-4582`
...
<!-- curriculum-chunk: sha256=0bc1c669012932f489656618f282d0df26cf0193c741b0224f33c6efe970cb91 major=concurrency-async-io mid=Blocking, non-blocking, async 구분 sub=Kotlin의 async HTTP 요청과 코루틴 비교 sources=source/interview_questions2.md:409-438, source/interviews2.md:409-438 -->

> Source: `source/interview_questions2.md:409-438`
> Classification reason: async/non-blocking
> Duplicate source aliases: `source/interview_questions2.md:409-438, source/interviews2.md:409-438`
...
<!-- curriculum-chunk: sha256=29a146ab57b3d26f1db3c13d61aa65dd932ef48b76355a34aff2986ea856d01a major=concurrency-async-io mid=Blocking, non-blocking, async 구분 sub=논블로킹 vs 비동기 차이 sources=source/interview_questions.md:4470-4523, source/interviews.md:4470-4523 -->

> Source: `source/interview_questions.md:4470-4523`
> Classification reason: async/non-blocking
> Duplicate source aliases: `source/interview_questions.md:4470-4523, source/interviews.md:4470-4523`
...
| 개념                  | 설명                                                |
|---------------------|---------------------------------------------------|
| 논블로킹(Non-blocking) | 호출한 스레드가 준비되지 않은 I/O에서 오래 잠들지 않고 즉시 돌아옵니다. 다만 완료 여부는 readiness 알림이나 재시도 루프로 다시 확인해야 합니다. |
| 비동기(Asynchronous) | 호출한 흐름이 결과를 즉시 받지 않고, 완료 결과를 callback, future, coroutine continuation, event로 나중에 받습니다. |

핵심 차이는 "작업 위치" 하나로만 닫히지 않습니다. 논블로킹은 기다림 방식의 문제이고, 비동기는 완료 결과를 받는 방식의 문제입니다. 실제 서버에서는 둘이 함께 쓰일 수 있습니다. 예를 들어 event loop는 non-blocking socket을 등록해 두고, 준비 이벤트가 오면 callback을 실행해 async 흐름을 이어 갑니다.
...
- 논블로킹(Non-blocking) 예제: 논블로킹은 "결과를 직접 확인해야 하는 방식"
  ...
  ✅ 예제: 논블로킹 소켓 읽기
    ...
    int bytesRead = socket.read(buffer);  // 데이터가 없으면 즉시 0 반환, peer가 정상 종료하면 -1 반환
    ...
    - Java NIO `SocketChannel.read()`는 non-blocking mode에서 지금 읽을 byte가 없으면 보통 `0`을 반환하고, peer가 stream을 닫아 더 읽을 data가 없으면 `-1`을 반환합니다.
    - 즉, 스레드는 readiness 알림이나 재시도 루프를 통해 나중에 다시 확인해야 합니다.
...
<!-- curriculum-chunk: sha256=6f99ae3cef9ed51b575f4112e6885dd1c67496a61e106dc32ae241e52e03c9c3 major=concurrency-async-io mid=Blocking, non-blocking, async 구분 sub=블로킹 vs 논블로킹, 동기 vs 비동기, 동시성 vs 병렬성 개념 정리 sources=source/interview_questions.md:4155-4469, source/interviews.md:4155-4469 -->

> Source: `source/interview_questions.md:4155-4469`
> Classification reason: async/non-blocking
> Duplicate source aliases: `source/interview_questions.md:4155-4469, source/interviews.md:4155-4469`
...
<!-- curriculum-chunk: sha256=e61baa9c0afee796de80c74db875cd8798270502a2b58dc2f30e15a090b71552 major=concurrency-async-io mid=Blocking, non-blocking, async 구분 sub=비동기 프로그래밍 sources=source/interview_questions.md:3812-3873, source/interviews.md:3812-3873 -->

> Source: `source/interview_questions.md:3812-3873`
> Classification reason: async/non-blocking
> Duplicate source aliases: `source/interview_questions.md:3812-3873, source/interviews.md:3812-3873`
...
<!-- curriculum-chunk: sha256=f3cf345881e3dda85aa07e8cf735cc91c2c2a4c2821b187523513ac97d5263c9 major=concurrency-async-io mid=Blocking, non-blocking, async 구분 sub=싱글 코어 & 싱글 스레드 경우 커널에서의 비동기와 멀티플렉싱 sources=source/interview_questions.md:4104-4154, source/interviews.md:4104-4154 -->

> Source: `source/interview_questions.md:4104-4154`
> Classification reason: async/non-blocking
> Duplicate source aliases: `source/interview_questions.md:4104-4154, source/interviews.md:4104-4154`
...
<!-- curriculum-chunk: sha256=83b85432cf78a549a38f93d84e6930c9985f9e0630c6075330de38a2c337c8d9 major=concurrency-async-io mid=Coroutine과 협력형 실행 sub=Kotlin Coroutine sources=source/interview_questions.md:3747-3770, source/interviews.md:3747-3770 -->

> Source: `source/interview_questions.md:3747-3770`
> Classification reason: coroutine
> Duplicate source aliases: `source/interview_questions.md:3747-3770, source/interviews.md:3747-3770`
...
<!-- curriculum-chunk: sha256=ffbe6742cabd9b009c81d5584ca5541c668bd6e0aac7616a146862a6bc079ae2 major=concurrency-async-io mid=Coroutine과 협력형 실행 sub=코루틴 제어의 양보 sources=source/interview_questions2.md:357-408, source/interviews2.md:357-408 -->

> Source: `source/interview_questions2.md:357-408`
> Classification reason: coroutine
> Duplicate source aliases: `source/interview_questions2.md:357-408, source/interviews2.md:357-408`
...
<!-- curriculum-chunk: sha256=21d3928f49c0b7c1820fb38a5130d9de92f36c9f5d35820a269fe916564f3242 major=concurrency-async-io mid=Coroutine과 협력형 실행 sub=코루틴을 Cooperative Multitasking 이라고 하는 이유? sources=source/interview_questions2.md:250-356, source/interviews2.md:250-356 -->

> Source: `source/interview_questions2.md:250-356`
> Classification reason: coroutine
> Duplicate source aliases: `source/interview_questions2.md:250-356, source/interviews2.md:250-356`
...
<!-- curriculum-chunk: sha256=f594edbc04d54389d2f0ff4fbb940d7d898d04a74c7e888a06a818910ad5b7c4 major=concurrency-async-io mid=Event loop와 네트워크 런타임 sub=4. Netty 관련 질문 sources=source/interview_questions3.md:189-212 -->

> Source: `source/interview_questions3.md:189-212`
...
<!-- curriculum-chunk: sha256=649cf073128bff3d4874f03aabb397d6d79635628df319cc500895d3d14f1408 major=concurrency-async-io mid=Event loop와 네트워크 런타임 sub=Netty의 이벤트 루프(Event Loop)와 epoll 기반 논블로킹 I/O 구현 sources=source/interview_questions.md:10855-10992, source/interviews.md:10803-10940 -->

> Source: `source/interview_questions.md:10855-10992`
> Classification reason: event loop
> Duplicate source aliases: `source/interview_questions.md:10855-10992, source/interviews.md:10803-10940`
...
<!-- curriculum-chunk: sha256=c6902e7e8ee1488f24a206675e072db5143558634bb85ac1846f259b15ca0fa2 major=concurrency-async-io mid=Goroutine과 런타임 스케줄링 sub=Go의 Goroutine은 M 대 N 모델 멀티스레딩 sources=source/interview_questions.md:3735-3746, source/interviews.md:3735-3746 -->

> Source: `source/interview_questions.md:3735-3746`
> Classification reason: goroutine
> Duplicate source aliases: `source/interview_questions.md:3735-3746, source/interviews.md:3735-3746`
...
<!-- curriculum-chunk: sha256=b3d7caa2f7f8d0678fa28e3765b52b5efb83e3f23ca512a3a0d2529c0f3a1633 major=concurrency-async-io mid=Java monitor와 wait/notify sub=1. 자바에서의 쓰레드와 모니터 개념 sources=source/interview_questions3.md:396-405 -->

> Source: `source/interview_questions3.md:396-405`
...
<!-- curriculum-chunk: sha256=26965e7485369ce237a93404fac05c64e47f232e3eb2dd542d7c14641823122d major=concurrency-async-io mid=Java monitor와 wait/notify sub=10. 핵심 요약 및 결론 sources=source/interview_questions3.md:535-550 -->

> Source: `source/interview_questions3.md:535-550`
> Classification reason: monitor/wait-notify
...
<!-- curriculum-chunk: sha256=e8e1b25a1a710a3be97449a3edf42a13868ca90e6cd8f8386e686b99988d4088 major=concurrency-async-io mid=Java monitor와 wait/notify sub=3. synchronized 블록과 모니터 잠금 sources=source/interview_questions3.md:421-440 -->

> Source: `source/interview_questions3.md:421-440`
> Classification reason: monitor/wait-notify
...
<!-- curriculum-chunk: sha256=b9855554ec7f8548ecdfec4dec7ea48dba53f891accde28c7f65db90496a97f2 major=concurrency-async-io mid=Java monitor와 wait/notify sub=4. wait()와 notify() 메서드의 메커니즘 sources=source/interview_questions3.md:441-462 -->

> Source: `source/interview_questions3.md:441-462`
> Classification reason: monitor/wait-notify
...
<!-- curriculum-chunk: sha256=dc98dc5c9e117401b065ca46145124d4b24f30c8034b0ea23700a1db9a230978 major=concurrency-async-io mid=Java monitor와 wait/notify sub=5. wait set의 작동 원리 sources=source/interview_questions3.md:463-472 -->

> Source: `source/interview_questions3.md:463-472`
> Classification reason: monitor/wait-notify
...
<!-- curriculum-chunk: sha256=510429daa623c32b746152808abece85f146922230224c7349ebf7f93a811ab3 major=concurrency-async-io mid=Java monitor와 wait/notify sub=6. 쓰레드 상태: BLOCKED vs WAITING sources=source/interview_questions3.md:473-490 -->

> Source: `source/interview_questions3.md:473-490`
> Classification reason: monitor/wait-notify
...
<!-- curriculum-chunk: sha256=5bb4abec82ee2b4472b53da72b32794b50f166bdf26f85b5e1db0623bbb7c809 major=concurrency-async-io mid=Java monitor와 wait/notify sub=7. notify가 없으면 wait한 스레드는 어떻게 되는가? sources=source/interview_questions3.md:491-498 -->

> Source: `source/interview_questions3.md:491-498`
> Classification reason: monitor/wait-notify
...
<!-- curriculum-chunk: sha256=d8916525ad5ed1cbf51d60cdbb6bd2469af888eef730d7b00abbfbb1fb8ef8cd major=concurrency-async-io mid=Java monitor와 wait/notify sub=7. 결론 정리 sources=source/interview_questions3.md:933-947 -->

> Source: `source/interview_questions3.md:933-947`
> Classification reason: monitor/wait-notify
...
<!-- curriculum-chunk: sha256=06a678eddb2cae922c806faacf62f24c3f926def22401f979690f9191ccb1ad7 major=concurrency-async-io mid=Java monitor와 wait/notify sub=8. JVM 레벨에서의 synchronized 작동 방식 sources=source/interview_questions3.md:499-515 -->

> Source: `source/interview_questions3.md:499-515`
> Classification reason: monitor/wait-notify
...
2. 실제 실행 시, JVM은 객체 헤더(mark word)와 모니터 구조를 이용해 락 상태, 소유 스레드, 대기 큐(queue) 등을 관리한다(작동).
...
<!-- curriculum-chunk: sha256=f0cc627e4757c059dc91c52d2e7e4557d9eb23b009c2bd6dfbfa35698f124a93 major=concurrency-async-io mid=Java monitor와 wait/notify sub=9. 실제 동작 시나리오 예시 sources=source/interview_questions3.md:516-534 -->

> Source: `source/interview_questions3.md:516-534`
...
<!-- curriculum-chunk: sha256=81a56e3576b3a49162d40755c09be27585f58f529d702af36e6d3ff954e0df7b major=concurrency-async-io mid=Java monitor와 wait/notify sub=쓰레드 wait와 notify 예제 sources=source/interview_questions3.md:731-932 -->

> Source: `source/interview_questions3.md:731-932`
> Classification reason: monitor/wait-notify
...
<!-- curriculum-chunk: sha256=0021bab710687cef9aa4a021f615003eeb3c2129a1665f90deb1f5f0e092509f major=concurrency-async-io mid=Java monitor와 wait/notify sub=자바 쓰레드, 상태, 그리고 모니터 등 sources=source/interview_questions3.md:394-395 -->

> Source: `source/interview_questions3.md:394-395`
> Classification reason: monitor/wait-notify
...
<!-- curriculum-chunk: sha256=3f7a46d63875ce1f079336d99f76be43ca8b15c26d2cbb3c43740558cb1fddf1 major=concurrency-async-io mid=Java monitor와 wait/notify sub=자바에서 wait/notify 패턴**을 조금 더 현실감 있게 적용한 **프로덕션 코드 예시 sources=source/interview_questions3.md:551-730 -->

> Source: `source/interview_questions3.md:551-730`
> Classification reason: monitor/wait-notify
...
<!-- curriculum-chunk: sha256=f7f95ffe9d57af36d470d8c74dabbcc0a72646d43e7e056faffc0f03cc7c6fac major=concurrency-async-io mid=OS I/O multiplexing sub=Node.js의 싱글 스레드와 libuv의 멀티 스레딩 sources=source/interview_questions.md:8897-9127, source/interviews.md:8845-9075 -->

> Source: `source/interview_questions.md:8897-9127`
> Classification reason: io multiplexing
> Duplicate source aliases: `source/interview_questions.md:8897-9127, source/interviews.md:8845-9075`
...
<!-- curriculum-chunk: sha256=9f6edefaad1c2f587d5d7f19477786fedf9b0c38cca16269960736f4fa108db3 major=concurrency-async-io mid=OS I/O multiplexing sub=epoll 상세 sources=source/interview_questions.md:9128-9213, source/interviews.md:9076-9161 -->

> Source: `source/interview_questions.md:9128-9213`
> Classification reason: io multiplexing
> Duplicate source aliases: `source/interview_questions.md:9128-9213, source/interviews.md:9076-9161`
...
<!-- curriculum-chunk: sha256=3f5711fdf977cc4175636a2dc6393efe4eba9f2754124183105205148b4a0b0e major=concurrency-async-io mid=OS I/O multiplexing sub=libuv, aio_*, io_uring sources=source/interview_questions.md:3874-4103, source/interviews.md:3874-4103 -->

> Source: `source/interview_questions.md:3874-4103`
> Classification reason: io multiplexing
> Duplicate source aliases: `source/interview_questions.md:3874-4103, source/interviews.md:3874-4103`
...
<!-- curriculum-chunk: sha256=280db8c4d50445da1912415592bf4b630881eaab1204fe71cb5d759cbc6bee9b major=concurrency-async-io mid=OS I/O multiplexing sub=네트워크 통신 시 epoll 동작 방식 sources=source/interview_questions.md:6499-6611, source/interviews.md:6499-6611 -->

> Source: `source/interview_questions.md:6499-6611`
> Classification reason: io multiplexing
> Duplicate source aliases: `source/interview_questions.md:6499-6611, source/interviews.md:6499-6611`
...
<!-- curriculum-chunk: sha256=2c1a02421e0d534b966ab604c5e02388f2793d2a5108d0b48362f368be1d9488 major=concurrency-async-io mid=OS I/O multiplexing sub=동시 요청이 많아 소켓이 몇 만 개 이상 필요한 상황에서 File Descriptor 설정을 수정하지 않은 경우 sources=source/interview_questions2.md:542-701, source/interviews2.md:542-701 -->

> Source: `source/interview_questions2.md:542-701`
> Classification reason: io multiplexing
> Duplicate source aliases: `source/interview_questions2.md:542-701, source/interviews2.md:542-701`
...
- 네트워크 큐의 포화
    - TCP 소켓 대기 큐(queue, backlog)
        - 새로운 연결 요청은 소켓의 대기 큐(queue, 예: `backlog`)에 추가됩니다.
        - 파일 디스크립터 한계를 초과하면 대기 큐(queue)가 가득 찹니다. 커널은 더 이상 요청을 대기 큐(queue)에 넣지 못하고 새 연결 요청을 보류합니다.
        - 요청 처리 속도가 느려지고 대기 큐(queue)가 가득 차면:
            - 새로운 요청은 대기 큐(queue)에 추가되지 못하고, 클라이언트에게 RST(Reset) 패킷을 전송하여 연결을 종료시킵니다.
            - 클라이언트는 "연결 거부(Connection Refused)"를 받습니다.

    - 대기 큐(queue) 초과 시 OS의 동작
        - 기존 연결 중 일부는 여전히 활성화 상태이나, 새로운 연결을 추가할 공간이 없습니다.
        - OS는 대기 큐(queue) 초과 상태를 로그에 기록합니다
...
2. 네트워크 큐 크기 조정
    ...
    - `backlog` 크기를 늘려 네트워크 대기 큐(queue)의 용량을 증가시킵니다:
...
<!-- curriculum-chunk: sha256=0190f89b047fc24a65efa059996d2d9fddd431e24923ec23c1ccbf223c3bc117 major=concurrency-async-io mid=OS I/O multiplexing sub=소켓, epoll, nginx, 과다한 요청 sources=source/interview_questions.md:7294-7330, source/interviews.md:7242-7278 -->

> Source: `source/interview_questions.md:7294-7330`
> Classification reason: io multiplexing
> Duplicate source aliases: `source/interview_questions.md:7294-7330, source/interviews.md:7242-7278`
...
<!-- curriculum-chunk: sha256=1be2b1764a1b77373e661d7765db5573e62ab2cc4fbb4ab1d92f3783d6e69161 major=concurrency-async-io mid=공유 상태와 경쟁 조건 sub=경쟁 조건 (Race Condition) sources=source/interview_questions.md:635-1098, source/interviews.md:635-1098 -->

> Source: `source/interview_questions.md:635-1098`
> Classification reason: race condition
> Duplicate source aliases: `source/interview_questions.md:635-1098, source/interviews.md:635-1098`
...
<!-- curriculum-chunk: sha256=8c0646c3c8f7ac0313a8d98d13373b16f8e4c97169740009ab0ec6bad7d91a0d major=concurrency-async-io mid=공유 상태와 경쟁 조건 sub=멀티스레딩에서 어떻게 읽기 전용 속성을 동시에 사용할 수 있는지 sources=source/interview_questions2.md:102-249, source/interviews2.md:102-249 -->

> Source: `source/interview_questions2.md:102-249`
> Classification reason: shared state
> Duplicate source aliases: `source/interview_questions2.md:102-249, source/interviews2.md:102-249`
...
<!-- curriculum-chunk: sha256=212bbb821bac41121fd8d7e12be8c6fbbd28f163d24f2a056ad19e0718285fcb major=concurrency-async-io mid=공유 상태와 경쟁 조건 sub=자바 쓰레드 동기화 sources=source/interview_questions3.md:237-393 -->

> Source: `source/interview_questions3.md:237-393`
> Classification reason: shared state
...
<!-- curriculum-chunk: sha256=1c70931f2f4b3c5973967b55f6d677113322336f6857ee222f40eb43b36dff26 major=concurrency-async-io mid=교착과 진행 보장 sub=교착 상태 (Deadlock) sources=source/interview_questions.md:1099-1288, source/interviews.md:1099-1288 -->

> Source: `source/interview_questions.md:1099-1288`
> Classification reason: deadlock
> Duplicate source aliases: `source/interview_questions.md:1099-1288, source/interviews.md:1099-1288`
...
<!-- curriculum-chunk: sha256=6862e633a746c7b7eb9332cad0cc0d7d2f4c50bf50108719ec80a31cd85d72ec major=concurrency-async-io mid=스레드 상태와 스케줄링 sub=2. 쓰레드 상태(States) 개념 sources=source/interview_questions3.md:406-420 -->

> Source: `source/interview_questions3.md:406-420`
...
<!-- curriculum-chunk: sha256=1f6c20060905e8f0f49500dac33bc6e7f21447d534cb445e562fa75bffb38fc3 major=concurrency-async-io mid=스레드 생성과 스케줄링 sub=`pthread_create` 이후 커널에서 발생하는 작업들 sources=source/interview_questions.md:3771-3811, source/interviews.md:3771-3811 -->

> Source: `source/interview_questions.md:3771-3811`
> Classification reason: thread creation
> Duplicate source aliases: `source/interview_questions.md:3771-3811, source/interviews.md:3771-3811`
