...
2. 서버 커널은 NIC에서 받은 패킷을 TCP/IP 스택으로 처리하고, 포트에 바인딩된 리스닝 소켓과 연결별 소켓으로 나눕니다.
    ...
    서버에 패킷이 도착하면 가장 먼저 NIC가 전기 신호나 가상 네트워크 프레임을 받아 Ethernet frame 같은 링크 계층 단위로 처리합니다.
    NIC는 수신 큐에 데이터를 올리고, 보통 `DMA`(Direct Memory Access)로 커널 메모리 쪽 버퍼에 데이터를 옮긴 뒤 interrupt나 polling 방식으로 커널 드라이버에 "새 패킷이 왔다"고 알립니다.
    여기서 NIC는 선로와 서버 메모리 사이의 입구 역할을 하지만, "이 요청은 Nginx로 보내야 한다" 같은 애플리케이션 판단을 하지는 않습니다.

    > - `DMA`(Direct Memory Access)는 장치가 CPU를 매번 거치지 않고 메모리와 직접 데이터를 주고받는 방식입니다.
    >   네트워크 수신에서는 NIC가 받은 frame을 커널이 볼 수 있는 메모리 버퍼로 옮기는 데 쓰입니다.
    >
    > - `TCP/IP stack`(Transmission Control Protocol / Internet Protocol stack)은 IP, TCP, UDP 같은 인터넷 프로토콜을 처리하는 코드 묶음입니다.
    >   일반적인 서버에서는 이 스택의 중심이 NIC가 아니라 OS 커널 안에 있습니다.

    커널 TCP/IP 스택은 NIC 드라이버가 넘긴 packet buffer를 IP 계층에서 목적지 IP와 프로토콜로 확인하고, TCP 계층에서 checksum, port, sequence number, connection state를 봅니다.
    NIC가 checksum offload(검사값 계산 보조), segmentation offload(큰 데이터를 여러 TCP segment로 나누는 작업 보조), RSS(Receive Side Scaling, 수신 패킷을 여러 CPU queue로 나누는 기능)처럼 일부 반복 작업을 도와줄 수는 있지만, 보통 TCP 연결 상태, 소켓 버퍼, 재전송 타이머, listen backlog, accept queue의 주인은 커널 TCP/IP 스택입니다.
    ...
    `epoll`과 `kqueue`는 바로 이 소켓 file descriptor의 준비 상태(readiness)를 Nginx나 Apache 같은 사용자 공간 프로세스에 알려 주는 인터페이스입니다.
    ...
    ```text
    NIC
    -> interrupt 또는 driver polling
    -> NIC driver
    -> kernel TCP/IP stack
    -> socket receive buffer 또는 accept queue 상태 변경
    -> epoll/kqueue readiness event
    -> Nginx/Apache가 accept(), read(), recv() 호출
    ```

    리스닝 소켓과 연결별 소켓을 나누는 이유는 "서비스 입구"와 "개별 손님과의 대화 상태"가 서로 다르기 때문입니다.
    리스닝 소켓은 `0.0.0.0:443`이나 `127.0.0.1:8080`처럼 특정 IP와 port에 바인딩되어 새 연결 요청을 기다리는 입구입니다.
    이 소켓은 클라이언트 한 명의 요청 본문을 읽는 소켓이 아니라, SYN backlog와 accept queue를 통해 새 TCP 연결을 받아들이는 역할을 합니다.

    연결별 소켓은 TCP 3-way handshake가 끝난 뒤 `client IP:client port -> server IP:server port`라는 4-tuple로 식별되는 개별 연결입니다.
    이 연결별 소켓마다 TCP sequence number, receive buffer, send buffer, window, timeout, `ESTABLISHED`나 `TIME_WAIT` 같은 상태가 따로 필요합니다.
    서버 port는 모두 443으로 같아도 클라이언트 IP와 client port가 다르기 때문에, 수천 개 클라이언트가 같은 리스닝 소켓을 통해 들어와도 커널은 각각을 별도 연결별 소켓으로 관리할 수 있습니다.

    그래서 Nginx가 `accept()`를 호출하면 리스닝 소켓 자체가 바뀌는 것이 아니라, accept queue에 올라온 연결 하나에 대한 새 file descriptor가 반환됩니다.
    이후 Nginx는 그 연결별 file descriptor에서 HTTP 요청 byte를 읽고 응답 byte를 씁니다.
    이 분리가 있어야 한 클라이언트의 느린 전송, 재전송, 연결 종료, TLS 처리, keep-alive 상태가 다른 클라이언트의 연결 상태와 섞이지 않습니다.
...
- 공식 기준: [RFC 1034](https://www.rfc-editor.org/rfc/rfc1034), [RFC 1035](https://www.rfc-editor.org/rfc/rfc1035), [RFC 9293](https://datatracker.ietf.org/doc/rfc9293/), [RFC 5246](https://www.rfc-editor.org/rfc/rfc5246), [RFC 8446](https://www.rfc-editor.org/rfc/rfc8446)
