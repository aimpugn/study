...
- [핵심 인터뷰 정리](#핵심-인터뷰-정리)
    - [목차](#목차)
    - [이 문서는 어떻게 읽어야 하나](#이-문서는-어떻게-읽어야-하나)
    - [빠른 사용법](#빠른-사용법)
...
이 문서는 면접 답변을 외우기 위한 문장 모음이 아니라, 낯선 질문을 받았을 때 답변을 조립하는 지도입니다.
각 절은 하나의 정답을 주기보다, 질문이 어떤 문제를 묻고 있고 그 문제가 어떤 상태 이동으로 설명되는지 보여 주기 위해 존재합니다.
...
예를 들어 `@Transactional` 질문은 annotation 이름을 아는지 묻는 데서 끝나지 않습니다.
실제로는 "어떤 호출이 proxy를 지나 트랜잭션 경계에 들어가고, 어떤 DB 연결과 격리 수준 위에서 commit/rollback이 결정되며, self-invocation이나 checked exception에서 그 경계가 어떻게 깨질 수 있는가"를 묻습니다.
이 문서를 읽을 때도 같은 방식으로, 키워드 하나를 볼 때마다 그 키워드가 소유하거나 바꾸는 상태를 함께 떠올려야 합니다.

처음에는 아래 다섯 축만 기억하면 됩니다.

| 축 | 면접 질문에서 자주 보이는 말 | 실제로 따라가야 할 상태 |
| --- | --- | --- |
| 요청 경로 | HTTP, Nginx, Tomcat, Spring, DB | packet, 소켓 버퍼(socket buffer), 작업 스레드(worker thread), 트랜잭션 경계, DB 연결 |
| 데이터 보존 | transaction, WAL, replication, backup | dirty page, log record, commit point, replica lag, recovery point |
| 동시성 | thread, lock, async, event loop | 공유 상태, 실행 대기 큐(run queue), wait set, 준비 완료 이벤트(readiness event), blocking call |
| 확장성 | index, shard, partition, cache | page, fan-out, routing key, working set, cache invalidation |
| 운영 장애 | p99, OOM, timeout, lag, failover | 큐 길이(queue length), cgroup limit, heap/off-heap, socket backlog, health signal |

한 절을 읽고 나면 "이 주제의 정의"보다 "어떤 상태가 어디에서 생기고, 누가 그 상태를 소비하며, 실패하면 어떤 증거로 좁힐 수 있는가"를 먼저 말할 수 있어야 합니다.
...
3. 실제 경로를 낮은 층부터 위로 올립니다.
    ...
    면접관이 깊게 들어오면 "애플리케이션이 이렇게 합니다"에서 끝내지 말고, 소켓, 파일 디스크립터, 커널 큐(queue), 런타임 스레드, 커넥션 풀, WAL, 인덱스 페이지처럼 실제로 상태가 바뀌는 지점을 짚습니다.
...
먼저 아래 질문들을 "정답 문장"으로 외우지 말고, 각 질문이 어느 상태 이동을 묻는지 확인합니다.
웹 요청은 socket과 worker thread를 움직이고, 트랜잭션은 commit 가능한 변경 기록과 격리된 읽기 상태를 만들며, 인덱스는 테이블 전체 읽기를 page 단위 탐색으로 바꿉니다.
이렇게 문제를 상태 이동으로 바꾸면 처음 보는 꼬리 질문도 같은 지도 위에 놓을 수 있습니다.

| 먼저 볼 질문 | 질문이 실제로 묻는 상태 이동 | 답변에서 바로 확인할 증거 |
| --- | --- | --- |
| 웹 요청 경로 | 클라이언트 요청이 소켓 버퍼(socket buffer), 프록시 upstream, 작업 스레드(worker thread), 트랜잭션, DB 연결을 지나 응답으로 돌아오는 이동 | access log, thread dump, connection pool metric, DB slow query |
| 트랜잭션 | 여러 row 변경이 commit 가능한 로그와 격리된 읽기 상태로 묶이는 과정 | DB transaction log, lock wait, deadlock log, Spring transaction debug log |
| 인덱스 | 전체 테이블 읽기(table scan)를 B+Tree page 탐색과 실제 row fetch로 바꾸는 과정 | `EXPLAIN`, handler read count, buffer pool hit, covering index 여부 |
| blocking / async | 기다리는 주체가 호출 스레드(caller thread)인지, 커널의 준비 완료 이벤트(readiness event)인지, callback/future 완료인지 나뉘는 과정 | thread state, event loop block warning, socket state, p99 latency |
| `@Transactional` | method call이 proxy를 지나 DB 연결과 commit/rollback 경계에 들어가는 과정 | proxy 여부, self-invocation, rollback rule, connection auto-commit |
| 메시징 | producer와 consumer의 시간을 로그나 큐(queue)가 분리하고, 재처리와 순서 보장이 비용으로 남는 과정 | broker lag, offset, ack/nack, dead letter queue |
| TLS | 서버 신원 확인과 암호화에 쓸 통신 키 합의가 HTTP 전송 전에 끝나는 과정 | certificate chain, cipher suite, handshake capture |
| 장애 분석 | 증상 지표를 queue, CPU, memory, GC, DB, network, broker 중 한 병목 상태로 좁히는 과정 | p99, CPU run queue, GC log, slow query, packet loss, broker lag |
...
이 문서에서 1번 웹 요청 절은 확장 예시입니다.
초보자가 한 요청을 낮은 층부터 끝까지 따라갈 수 있도록 DNS, TCP, TLS, Nginx, Tomcat, Spring, DB를 길게 풀었습니다.
2번부터 23번까지는 같은 방식으로 답변을 조립하기 위한 골격이므로, 깊이가 더 필요하면 각 절의 관련 대주제 문서와 deep dive 문서로 이동해야 합니다.
...
먼저 전체 상태 이동을 한 번 잡고, 그 다음 DNS처럼 세부 단계로 내려갑니다.

```text
HTTP 요청 의도
  -> DNS 응답이 접속할 IP 후보를 준다
  -> 양쪽 커널에 TCP 연결 상태가 생긴다
  -> TLS 처리 지점(endpoint)이 서버를 인증하고 암호화에 쓸 통신 키를 만든다
  -> Nginx가 accept한 소켓이 암호화된 byte를 받는다
  -> Nginx가 HTTP를 해석하고 upstream 서버로 가는 소켓을 열거나 재사용한다
  -> Tomcat connector가 요청 처리 스레드(worker thread)에 요청을 넘긴다
  -> Spring이 controller 인자를 만들고 transaction / service 코드로 들어간다
  -> DB 연결(connection)이 SQL을 실행하고 row를 돌려준다
  -> response byte가 Tomcat, Nginx, TCP, TLS, client 쪽으로 되돌아간다
```

아래 DNS 레코드 표는 이 전체 흐름 중 "어느 IP 후보로 접속할 것인가"를 결정하는 첫 단계입니다.
면접에서 시간이 짧다면 표 전체를 외우기보다 `A/AAAA`, `CNAME`, TTL, cache가 연결 대상 선택과 장애 전파 시간에 어떤 영향을 주는지만 먼저 말하면 됩니다.

**이어 말할 순서**

1. 클라이언트는 DNS, TCP 3-way handshake, TLS 핸드셰이크를 거쳐 서버와 연결을 만듭니다.

    이 한 문장은 실제로는 세 가지 다른 문제를 순서대로 푸는 과정입니다.
    DNS는 `www.example.com` 같은 이름을 접속 가능한 IP 주소로 바꾸고, TCP는 그 IP와 port에 대해 양쪽 커널이 데이터를 주고받을 연결 상태를 만들며, TLS는 그 TCP 연결 위에 서버 인증과 암호화 키를 올립니다.
    그래서 HTTP 요청은 이 셋이 끝난 뒤에야 "어느 서버로, 어떤 연결로, 어떤 보안 채널에서" 보낼지 알게 됩니다.

    1. DNS resolve는 이름을 IP 주소로 바꾸는 단계입니다.

        브라우저나 HTTP client는 먼저 local cache, OS resolver cache, `/etc/hosts` 같은 로컬 단서를 확인합니다.
        없으면 보통 OS의 stub resolver가 회사 DNS, 공유기 DNS, 통신사 DNS, 또는 `8.8.8.8` 같은 recursive resolver에 질의합니다.
        recursive resolver는 자신이 답을 캐시하고 있지 않으면 root name server, TLD name server, authoritative name server를 따라가며 `A` record나 `AAAA` record를 찾습니다.

        DNS record는 "이 이름에 대해 어떤 종류의 답을 줄 것인가"를 나타내는 항목입니다.
        웹 요청에서 IP 주소를 직접 얻는 데 가장 자주 쓰는 것은 `A`와 `AAAA`이고, `CNAME`은 정식 이름(canonical name)으로 한 번 우회하게 만드는 별칭입니다.
        나머지 record는 메일, 도메인 권한, 인증 정책, 역방향 조회, 서비스 위치 탐색처럼 다른 목적을 담당합니다.

        | Record | 뜻 | 예시 | 웹 요청에서 기억할 점 |
        | --- | --- | --- | --- |
        | `A` | 도메인 이름(domain name)을 IPv4 주소로 연결합니다. | `www.example.com. 300 IN A 93.184.216.34` | 브라우저가 IPv4로 접속할 후보 IP를 얻습니다. |
        | `AAAA` | 도메인 이름을 IPv6 주소로 연결합니다. | `www.example.com. 300 IN AAAA 2606:2800:220:1:248:1893:25c8:1946` | IPv6 환경에서는 이 주소로 TCP 연결을 시도할 수 있습니다. |
        | `CNAME` | 한 이름을 다른 정식 이름으로 넘깁니다. | `www.example.com. 300 IN CNAME example.cdn.net.` | CDN이나 외부 서비스로 위임할 때 자주 보이며, 최종적으로는 그 이름의 `A`나 `AAAA`를 다시 찾습니다. |
        | `NS` | 해당 DNS 관리 구역(zone)의 authoritative name server를 지정합니다. | `example.com. 86400 IN NS ns1.example-dns.com.` | recursive resolver가 "이 도메인의 정답은 누구에게 물어야 하는가"를 찾는 데 씁니다. |
        | `SOA` | DNS 관리 구역(zone)의 시작 정보와 일련번호(serial), 갱신 주기(refresh) 같은 관리 메타데이터를 담습니다. | `example.com. 86400 IN SOA ns1.example.com. admin.example.com. 2026050601 7200 3600 1209600 300` | 운영자가 DNS 변경 전파와 zone 버전을 판단할 때 봅니다. |
        | `MX` | 메일을 받을 메일 서버(mail server)를 지정합니다. | `example.com. 300 IN MX 10 mail.example.com.` | 웹 접속 자체보다는 이메일 수신 경로에 쓰입니다. |
        | `TXT` | 자유 형식 문자열을 저장합니다. | `example.com. 300 IN TXT "v=spf1 include:_spf.example.com ~all"` | SPF, DKIM, DMARC 같은 메일 인증이나 도메인 소유권 검증에 자주 쓰입니다. |
        | `PTR` | IP 주소를 도메인 이름으로 역조회합니다. | `34.216.184.93.in-addr.arpa. 300 IN PTR www.example.com.` | 서버 진단, 메일 서버 신뢰도 확인, 로그 분석에서 보입니다. |
        | `SRV` | 특정 서비스의 대상 서버(host)와 포트(port)를 알려 줍니다. | `_sip._tcp.example.com. 300 IN SRV 10 5 5060 sipserver.example.com.` | HTTP 일반 접속에는 흔하지 않지만, 서비스 탐색이 필요한 프로토콜에서 씁니다. |
        | `CAA` | 어떤 인증 기관이 이 도메인의 TLS 인증서를 발급할 수 있는지 제한합니다. | `example.com. 300 IN CAA 0 issue "letsencrypt.org"` | 인증서 발급 정책과 관련이 있어 HTTPS 운영에서 중요합니다. |

        중간에 `CNAME`이 있으면 별칭을 따라 최종 이름의 주소 record를 다시 찾습니다.
        DNS 응답 안의 resource record에는 TTL이 붙고, 이 TTL 동안은 같은 이름과 같은 record type에 대해 authoritative name server까지 다시 물어보지 않고 cache된 record를 재사용할 수 있습니다.

        아래는 `dig` 출력과 비슷하게 줄인 DNS 응답 예시입니다.
        실제 운영 응답이 아니라 구조를 보여 주기 위한 예시이며, `300`과 `60`이 각 record의 TTL입니다.
        ...
        web-cdn.example.net.  60   IN  A      203.0.113.11
        ```

        이 응답은 "`www.example.com`의 `A` record를 물었더니, 먼저 `web-cdn.example.net`이라는 별칭이 나왔고, 그 별칭을 따라가 보니 IPv4 주소 두 개가 나왔다"는 뜻입니다.

        이 단계가 끝나면 클라이언트는 "접속할 IP 후보"를 얻습니다.
        아직 서버와 연결된 것은 아닙니다.
        DNS는 주소를 찾는 과정이지, 소켓을 여는 과정이 아닙니다.

    2. TCP 3-way handshake는 IP와 port 위에 신뢰성 있는 byte stream을 만들기 위한 단계입니다.

        클라이언트 커널은 임시 source port를 고르고, 서버의 `IP:443` 같은 목적지로 `SYN`을 보냅니다.
        `SYN`은 "이 연결에서 사용할 내 초기 sequence number는 여기서 시작합니다"라는 동기화 요청입니다.
        서버 커널은 해당 port에서 listen 중인 socket이 있으면 `SYN-ACK`로 응답합니다.
        이 응답은 "네 sequence number를 확인했고, 내 sequence number도 여기서 시작합니다"라는 뜻입니다.
        마지막으로 클라이언트가 `ACK`를 보내면 양쪽 TCP 상태가 `ESTABLISHED`가 되고, 서버 쪽에서는 accept 가능한 연결로 올라옵니다.

        이 단계가 끝나면 양쪽 커널은 source IP, source port, destination IP, destination port로 식별되는 하나의 TCP connection을 공유합니다.
        그래도 아직 HTTP를 안전하게 보내도 된다는 뜻은 아닙니다.
        TCP는 순서, 재전송, 흐름 제어를 제공하지만, 상대가 진짜 내가 기대한 서버인지나 중간에서 내용을 볼 수 없는지는 보장하지 않습니다.

    3. TLS 핸드셰이크는 이미 만들어진 TCP 연결 위에서 서버 인증과 암호화 키를 준비하는 단계입니다.

        HTTPS라면 클라이언트는 HTTP request line을 바로 보내지 않고 먼저 `ClientHello`를 보냅니다.
        여기에는 지원하는 TLS 버전, cipher suite 후보, 난수, SNI, ALPN, TLS 1.3 기준 key share 같은 정보가 들어갑니다.
        SNI는 하나의 IP에서 여러 도메인을 서비스할 때 서버가 어떤 인증서를 보여 줄지 고르는 힌트이고, ALPN은 HTTP/1.1과 HTTP/2 같은 애플리케이션 프로토콜을 협상하는 데 쓰입니다.
        서버는 `ServerHello`로 선택한 TLS 버전과 암호 설정, key share를 돌려주고, 인증서와 `CertificateVerify`, `Finished` 메시지로 "이 인증서의 private key를 가진 서버가 이 핸드셰이크에 참여했다"는 점을 증명합니다.
        클라이언트는 인증서 체인, hostname, 만료일, trust store를 검증하고, 양쪽은 ECDHE 같은 키 교환 결과로 같은 traffic key를 계산합니다.

        이 단계가 끝나면 클라이언트와 서버는 같은 대칭키 계열의 working key를 갖고, 이후 HTTP 요청과 응답은 TLS record 안에서 암호화되고 무결성 검증됩니다.
        여기까지 끝나야 브라우저는 `GET /... HTTP/1.1` 또는 HTTP/2 frame을 안전한 채널로 보냅니다.

    핵심은 `TCP 3-way handshake`는 연결 수립, `TCP 4-way handshake`는 연결 종료, `TLS handshake`는 이미 만들어진 TCP 연결 위의 보안 채널 수립이라는 점입니다.

    **TCP 3-way handshake는 TCP 연결을 만드는 과정입니다.**

    TCP 3-way handshake는 클라이언트와 서버의 커널이 서로의 초기 sequence number를 확인하고, 양방향 byte stream을 주고받을 연결 상태를 만드는 과정입니다.
    이 과정은 HTTP나 TLS보다 아래 계층에서 일어나며, 성공하면 양쪽 커널에는 `client IP:client port -> server IP:server port`로 식별되는 TCP 연결 상태가 생깁니다.
    ...
    1. 클라이언트는 `SYN`을 보내며 자기 초기 sequence number인 `x`를 알립니다.
        sequence number는 TCP가 byte 순서, 재전송, 중복 제거를 관리할 때 기준으로 삼는 번호입니다.
    2. 서버는 listen 중인 port에 대해 연결을 받을 수 있으면 `SYN-ACK`를 보냅니다.
        이 메시지는 클라이언트의 `SYN`을 확인했다는 `ack = x + 1`과 서버 자신의 초기 sequence number인 `y`를 함께 담습니다.
    3. 클라이언트가 `ACK`로 `ack = y + 1`을 보내면 양쪽이 서로의 초기 sequence number를 확인한 상태가 됩니다.
        이때 연결은 `ESTABLISHED`가 되고, 서버 쪽에서는 accept 가능한 연결로 올라옵니다.

    세 번이 필요한 이유는 양쪽 방향의 sequence number를 모두 확인해야 하기 때문입니다.
    한쪽만 확인하면 오래된 중복 packet과 새 연결의 packet을 구분하기 어렵고, 반대 방향으로도 데이터를 보낼 준비가 됐는지 확정할 수 없습니다.
    다만 TCP 3-way handshake는 암호화나 서버 신원 검증을 제공하지 않습니다.
    HTTPS에서는 이 TCP 연결이 만들어진 뒤, 그 byte stream 위에서 TLS 핸드셰이크가 별도로 이어집니다.

    **TCP 4-way handshake는 TCP 연결을 닫는 과정입니다.**

    TCP 4-way handshake는 연결 수립이 아니라 연결 종료 흐름입니다.
    TCP는 full-duplex, 즉 양방향 byte stream이므로 한쪽이 "나는 더 이상 보낼 데이터가 없습니다"라고 말해도, 반대쪽은 아직 남은 데이터를 보낼 수 있습니다.

    > `duplex`는 통신 방향을 말할 때 쓰는 용어입니다. 핵심은 "양쪽 방향이 가능한가"와 "양쪽 방향을 동시에 쓸 수 있는가"입니다.

    | 구분 | 뜻 | 예시 | TCP 종료에서 기억할 점 |
    | --- | --- | --- | --- |
    | simplex | 한 방향으로만 보낼 수 있습니다. | 방송 송출, 센서가 서버로만 보내는 단방향 링크 | 엄밀히 말하면 duplex가 아닙니다. 한쪽 방향만 있으므로 양방향 종료라는 개념이 약합니다. |
    | half-duplex | 양쪽 방향 모두 가능하지만, 같은 순간에는 한쪽만 보낼 수 있습니다. | 무전기처럼 한쪽이 말할 때 다른 쪽은 듣는 방식 | 양방향 통신은 가능하지만 동시에 흐르지 않으므로 TCP의 기본 모델과 다릅니다. |
    | full-duplex | 양쪽이 동시에 보낼 수 있습니다. | 일반적인 TCP 연결, 전화 통화, WebSocket 위의 양방향 메시지 | 한쪽이 `FIN`을 보내도 그쪽 송신 방향만 닫히고, 반대 방향은 아직 데이터를 보낼 수 있습니다. |

    TCP가 full-duplex라는 말은 애플리케이션이 반드시 양쪽에서 동시에 데이터를 보내야 한다는 뜻은 아닙니다.
    예를 들어 HTTP/1.1 요청과 응답은 보통 "클라이언트가 요청을 보내고 서버가 응답한다"는 순서로 보이지만, 그 아래의 TCP 연결 자체는 양방향 byte stream입니다.
    그래서 TCP는 한쪽 송신 방향만 먼저 닫는 half-close가 가능하고, 이 성질 때문에 정상 종료가 `FIN -> ACK -> FIN -> ACK`처럼 두 방향을 따로 닫는 흐름으로 설명됩니다.

    구체적인 packet 흐름은 보통 `FIN -> ACK -> FIN -> ACK`처럼 각 방향을 따로 닫는 모습으로 설명합니다.
    ...
    active closer: TIME_WAIT
    passive closer: CLOSED
    ```

    1. 먼저 닫는 쪽이 `FIN`을 보냅니다.
        `FIN`은 "내가 보내는 방향의 byte stream은 여기서 끝입니다"라는 뜻입니다.
    2. 상대는 `ACK`로 그 종료 요청을 확인합니다.
        이 시점에도 상대가 보낼 데이터가 남아 있으면 계속 보낼 수 있으므로, 연결 전체가 곧바로 닫힌 것은 아닙니다.
    3. 상대도 더 보낼 데이터가 없어지면 자기 방향의 종료를 뜻하는 `FIN`을 보냅니다.
    4. 처음 닫기 시작한 쪽이 마지막 `ACK`를 보내면 양방향 byte stream이 모두 닫힙니다.

    먼저 종료를 시작한 쪽은 보통 `TIME_WAIT` 상태에 잠시 머뭅니다.
    이 시간은 마지막 `ACK`가 유실됐을 때 다시 보낼 수 있게 하고, 네트워크 어딘가에 늦게 도착하는 예전 packet이 새 연결에 섞이지 않게 하려는 안전 장치입니다.
    그래서 짧은 연결을 너무 많이 만들면 `TIME_WAIT`가 많이 보일 수 있고, keep-alive가 handshake 비용뿐 아니라 종료 상태 부담을 줄이는 데도 도움이 됩니다.

    실제 packet이 항상 네 줄로만 보이는 것은 아닙니다.
    구현과 상태에 따라 `FIN`과 `ACK`가 한 packet에 합쳐질 수 있고, 비정상 종료에서는 `RST`로 끊길 수도 있습니다.
    그래도 면접에서는 "3-way는 연결 수립, 4-way는 정상 종료, TIME_WAIT는 늦은 packet과 마지막 ACK 재전송을 위한 안전 상태"라고 정리하면 충분히 단단합니다.

    **TLS handshake는 TCP 연결 위에 인증과 암호화 채널을 만드는 과정입니다.**

    TLS handshake는 TCP handshake를 대체하지 않습니다.
    먼저 TCP 3-way handshake로 `ESTABLISHED` 상태의 byte stream을 만든 뒤, TLS가 그 byte stream 안에 `ClientHello`, `ServerHello`, 인증서, 키 교환, `Finished` 같은 handshake message를 실어 보냅니다.
    이 과정이 끝나야 HTTP request와 response가 TLS record 안에서 암호화되어 흐릅니다.
    ...
    TLS 1.3 기준의 일반적인 전체 흐름은 아래처럼 설명할 수 있습니다.

    1. 클라이언트는 `ClientHello`를 보냅니다.
        이 메시지에는 지원 TLS 버전, cipher suite 후보, client random, SNI(Server Name Indication), ALPN(Application-Layer Protocol Negotiation), key share, 지원 signature algorithm, 필요하면 세션 재개용 PSK(Pre-Shared Key) 정보가 들어갑니다.
        여기서 "지원하는 TLS 버전 목록을 보낸다"는 말은 TLS 핸드셰이크 밖의 별도 단계가 아니라, TLS 핸드셰이크 첫 메시지 안의 협상 정보라는 뜻입니다.
    2. 서버는 `ServerHello`를 보냅니다.
        서버는 클라이언트 후보 중 사용할 TLS 버전, cipher suite, key share를 선택합니다.
        TLS 1.3에서는 이 시점 이후 양쪽이 ECDHE 같은 임시 키 교환 결과와 handshake transcript를 사용해 handshake traffic key를 계산하고, 뒤따르는 많은 handshake message가 암호화됩니다.
    3. 서버는 `EncryptedExtensions`로 ALPN 같은 추가 협상 결과를 보냅니다.
        예를 들어 같은 TLS 연결 위에서 HTTP/1.1을 쓸지 HTTP/2를 쓸지 정한 결과가 여기에 담길 수 있습니다.
    4. 서버는 `Certificate`, `CertificateVerify`, `Finished`를 보냅니다.
        `Certificate`는 서버 인증서 체인을 전달하고, `CertificateVerify`는 서버가 인증서의 private key를 가진 주체임을 지금 핸드셰이크 transcript에 대한 서명으로 증명합니다.
        `Finished`는 지금까지의 handshake message가 중간에서 바뀌지 않았고 양쪽이 같은 비밀값을 계산했다는 것을 검증하는 메시지입니다.
    5. 클라이언트는 인증서 체인, hostname, 만료일, trust store를 검증합니다.
        이 검증이 실패하면 사용자가 기대한 서버와 안전하게 연결됐다고 볼 수 없으므로 TLS 연결을 중단해야 합니다.
    6. 클라이언트는 `Finished`를 보냅니다.
        서버가 클라이언트 인증서를 요구하는 mTLS(mutual TLS) 환경이라면, 클라이언트도 `Certificate`와 `CertificateVerify`를 함께 보내 자기 신원을 증명할 수 있습니다.
    7. 양쪽은 application traffic key를 사용해 HTTP 데이터를 암호화합니다.
        이때부터 브라우저의 `GET /...` 요청이나 HTTP/2 frame은 평문 TCP data가 아니라 TLS record 안의 암호문으로 전송됩니다.

    TLS 1.2를 함께 설명해야 한다면 차이는 이렇게 잡으면 됩니다.
    TLS 1.2에서는 보통 `ClientHello -> ServerHello -> Certificate -> ServerKeyExchange(DHE/ECDHE일 때) -> ServerHelloDone -> ClientKeyExchange -> ChangeCipherSpec -> Finished -> server ChangeCipherSpec -> server Finished` 흐름으로 설명합니다.
    RSA key exchange를 쓰던 과거 구성에서는 클라이언트가 pre-master secret을 서버 공개키로 암호화해 보내는 방식도 가능했지만, 전방 비밀성(Forward Secrecy)을 제공하지 못하므로 현대 구성에서는 ECDHE 같은 임시 Diffie-Hellman 계열을 중심으로 설명하는 편이 안전합니다.
    TLS 1.3은 RSA key exchange를 제거하고, cipher suite 구성을 단순화했으며, 일반적인 새 연결을 1-RTT로 줄였습니다.
    0-RTT는 재개 연결에서 첫 메시지와 함께 애플리케이션 데이터를 보낼 수 있게 하지만, 재생 공격(replay attack) 위험 때문에 모든 요청에 무조건 쓰면 안 됩니다.

    세 흐름을 한 문장으로 묶으면, TCP 3-way handshake는 연결을 만들고, TCP 4-way handshake는 연결을 닫고, TLS handshake는 만들어진 TCP 연결 위에서 신원 확인과 암호화 키 합의를 수행합니다.
    따라서 `4-way handshake`를 TLS 핸드셰이크의 다른 이름처럼 부르면 안 됩니다.

2. 서버 프로세스는 요청이 오기 전에 이미 리스닝 소켓을 열어 둡니다.

    Nginx나 Tomcat이 "요청이 오면 그때 소켓을 처음 연다"고 생각하면 이후 흐름이 헷갈립니다.
    서버는 보통 시작할 때 먼저 커널에 소켓을 만들고, 그 소켓을 특정 주소와 port에 묶은 뒤, 새 연결을 받을 준비가 됐다고 표시합니다.

    ```text
    Nginx 또는 Tomcat 시작
    -> socket(AF_INET, SOCK_STREAM)
    -> bind(0.0.0.0:443 또는 127.0.0.1:8080)
    -> listen(backlog)
    -> accept(), epoll/kqueue, connector thread가 대기
    ```

    `socket()`은 통신 endpoint를 만들고 file descriptor를 돌려줍니다.
    `bind()`는 그 endpoint를 `0.0.0.0:443` 같은 local address와 port에 묶습니다.
    `listen()`은 이 socket을 "새 TCP 연결을 받을 입구"로 바꾸고, 아직 애플리케이션이 가져가지 않은 연결을 잠시 담을 queue 한계를 정합니다.
    이때 만들어진 것이 리스닝 소켓입니다.
    리스닝 소켓은 클라이언트 한 명의 요청 body를 읽는 소켓이 아니라, 여러 클라이언트의 연결 시작을 받는 입구입니다.

    그래서 "서버가 무작위 port로 소켓을 하나 여나요?"에 대한 기본 답은 아닙니다.
    서버는 서비스 port인 `443`, `80`, `8080` 같은 port에 리스닝 소켓을 열어 둡니다.
    무작위처럼 보이는 port는 보통 연결을 먼저 거는 쪽, 즉 active opener가 고르는 임시 source port(ephemeral port)입니다.
    브라우저가 Nginx에 연결할 때는 브라우저 쪽 source port가 임시 port이고, Nginx가 Tomcat upstream에 다시 연결할 때는 Nginx 쪽 source port가 임시 port가 됩니다.

3. TCP 3-way handshake는 애플리케이션이 아니라 양쪽 커널의 TCP/IP 스택이 수행합니다.

    브라우저나 HTTP client가 `connect()` 계열 호출을 하면 클라이언트 커널은 임시 source port를 고르고 `SYN`을 보냅니다.
    서버 NIC는 이 packet을 전기 신호나 가상 네트워크 frame으로 받고, driver가 커널 네트워킹 경로로 넘깁니다.
    일반적인 Linux 서버에서는 NIC가 HTTP나 Tomcat을 이해하는 것이 아니라, frame 수신, queue 분배, checksum 계산 보조, RSS(Receive Side Scaling, 수신 packet을 여러 queue/CPU로 나누는 기능) 같은 하드웨어 보조를 맡습니다.
    TCP 연결 상태, sequence number, 재전송, receive buffer, listen queue의 주인은 커널 TCP/IP 스택입니다.

    > NIC(Network Interface Card)는 서버가 네트워크에 연결되도록 해 주는 네트워크 인터페이스 장치입니다. 물리 서버에서는 랜 카드나 온보드 네트워크 어댑터이고, 클라우드나 컨테이너 환경에서는 가상 NIC처럼 보이는 장치일 수 있습니다.
    >
    > `DMA`(Direct Memory Access)는 장치가 CPU를 매번 거치지 않고 메모리와 직접 데이터를 주고받는 방식입니다. 네트워크 수신에서는 NIC가 받은 frame을 커널이 볼 수 있는 메모리 버퍼로 옮기는 데 쓰입니다.
    >
    > `TCP/IP stack`(Transmission Control Protocol / Internet Protocol stack)은 IP, TCP, UDP 같은 인터넷 프로토콜을 처리하는 코드 묶음입니다. 일반적인 서버에서는 이 스택의 중심이 NIC가 아니라 OS 커널 안에 있습니다.

    서버 커널은 packet의 목적지 IP, 목적지 port, protocol, TCP flag를 보고 "이 SYN은 `0.0.0.0:443`에 listen 중인 socket으로 온 연결 요청"처럼 분류합니다.
    listen 중인 socket이 있으면 커널은 반쯤 열린 연결 상태를 SYN backlog 쪽에 두고 `SYN-ACK`를 보냅니다.
    클라이언트의 마지막 `ACK`가 오면 연결은 `ESTABLISHED`가 되고, 서버 커널은 이 연결을 accept queue에 올립니다.
    이 시점부터 Nginx나 Tomcat 같은 사용자 공간 프로세스가 `accept()`로 가져갈 수 있는 연결별 소켓이 생깁니다.
    ...
    -> SYN
    -> Server NIC
    -> NIC driver / NAPI / interrupt or polling
    -> Server kernel TCP/IP stack
    -> listening socket의 SYN backlog
    -> SYN-ACK
    -> Client ACK
    -> established connection이 accept queue에 올라감
    -> Nginx/Tomcat accept()가 connected socket fd를 받음
    ```

    여기서 말한 interrupt나 polling은 Nginx가 직접 받는 알림이 아닙니다.
    이 알림은 NIC와 커널 드라이버 사이에서 "하드웨어 수신 큐에 새 frame이 왔다"고 알려 주는 낮은 층의 신호입니다.
    Nginx나 Apache가 사용하는 `epoll`, `kqueue`는 그보다 위에서 동작합니다.
    커널 TCP/IP 스택이 packet을 처리해 특정 소켓의 receive buffer에 읽을 byte를 쌓거나, 리스닝 소켓의 accept queue에 새 연결을 올리면, 그 소켓 file descriptor가 "읽을 수 있는 상태"가 됩니다.
    `epoll`과 `kqueue`는 바로 이 소켓 file descriptor의 준비 상태(readiness)를 사용자 공간 프로세스에 알려 주는 인터페이스입니다.

    > `polling`이라는 단어가 두 층에서 다르게 쓰일 수 있습니다. NIC 문맥의 polling은 커널 드라이버가 하드웨어 수신 큐를 확인하는 방식이고, `poll`/`epoll`/`kqueue` 문맥은 애플리케이션이 커널에 등록한 file descriptor의 준비 상태를 기다리는 방식입니다.

4. `accept()`는 리스닝 소켓을 바꾸지 않고, 연결별 소켓 file descriptor를 하나 꺼내 줍니다.

    `accept()`를 호출하면 리스닝 소켓 자체가 클라이언트 전용 소켓으로 변하는 것이 아닙니다.
    커널은 accept queue에 올라온 연결 하나를 꺼내 새 connected socket file descriptor를 사용자 공간 프로세스에 반환합니다.
    원래의 리스닝 소켓은 계속 `443`이나 `8080`에서 다음 연결을 기다립니다.

    이 연결별 소켓은 아래 4-tuple로 식별됩니다.

    ```text
    (source IP, source port, destination IP, destination port)
    ```

    예를 들어 브라우저가 Nginx에 붙고, Nginx가 Tomcat으로 다시 프록시하면 같은 사용자 요청 안에서도 TCP 연결이 두 개일 수 있습니다.

    | 구간 | local endpoint | remote endpoint | 누가 active opener인가 | 기억할 점 |
    | --- | --- | --- | --- | --- |
    | 브라우저 -> Nginx | Nginx `203.0.113.20:443` | 브라우저 `198.51.100.7:53144` | 브라우저 | Nginx의 accepted socket은 local port가 계속 `443`입니다. |
    | Nginx -> Tomcat | Nginx `127.0.0.1:42118` | Tomcat `127.0.0.1:8080` | Nginx | 여기서는 Nginx가 클라이언트 역할을 하므로 Nginx 쪽 source port가 임시 port입니다. |
    | Tomcat이 본 연결 | Tomcat `127.0.0.1:8080` | Nginx `127.0.0.1:42118` | Nginx | Tomcat의 accepted socket도 local port가 계속 `8080`입니다. |

    이 표에서 `53144`나 `42118` 같은 값이 임시 port입니다.
    서버가 "새 무작위 server port를 열었다"는 뜻이 아니라, 연결을 먼저 건 쪽의 source port가 연결마다 달라진 것입니다.
    Nginx가 upstream keep-alive connection을 재사용하면 Tomcat 쪽 새 TCP handshake가 매 요청마다 생기지 않을 수도 있지만, 클라이언트 쪽 연결과 upstream 연결이 별개의 연결이라는 구조는 그대로입니다.

5. Nginx는 연결별 소켓에서 TLS와 HTTP byte를 읽고, HTTP 메시지 경계를 애플리케이션 프로토콜로 판단합니다.

    TLS를 Nginx에서 종료하는 HTTPS 구성이라면, TCP 3-way handshake는 커널이 끝낸 뒤 Nginx의 TLS 라이브러리가 그 connected socket 위에서 `ClientHello`, `ServerHello`, 인증서, key exchange, `Finished` 메시지를 읽고 씁니다.
    NIC와 커널은 이 TLS 메시지를 "해석"하지 않고 byte로 운반합니다.
    인증서 선택, SNI 처리, ALPN 협상, 암호화 키 계산은 TLS endpoint인 Nginx 프로세스 쪽 책임입니다.
    TLS를 Tomcat까지 넘기는 구성이라면 이 TLS endpoint가 Tomcat 쪽으로 이동하고, Nginx가 TLS를 종료한 뒤 plain HTTP로 Tomcat에 전달하는 구성이라면 브라우저와 Nginx 사이에서만 TLS가 끝납니다.

    HTTP는 TCP가 만든 byte stream 위에서 메시지 경계를 정합니다.
    TCP는 "이 byte들이 POST body 10MB 전체다" 같은 HTTP 의미를 모릅니다.
    TCP가 제공하는 것은 순서가 맞고 누락이 있으면 재전송되는 byte stream입니다.
    Nginx의 HTTP parser는 start line, header, 빈 줄을 읽고, `Content-Length`가 있으면 그 길이만큼 body byte를 stream에서 더 읽습니다.
    `Transfer-Encoding: chunked`라면 chunk size를 읽어가며 body 경계를 판단하고, HTTP/2라면 frame 단위 규칙을 따릅니다.

    그래서 "컨텐츠 길이만큼 소켓이 모든 바이트를 수신한다"는 표현은 절반만 맞습니다.
    최종적으로 HTTP parser는 `Content-Length`만큼의 body를 읽어야 하지만, 커널 receive buffer가 그 전체를 한 번에 다 받아 둔 뒤 애플리케이션이 시작하는 것은 아닙니다.
    packet은 여러 TCP segment로 나뉘어 도착하고, 커널은 순서와 재전송을 관리하며 receive buffer에 byte를 쌓고, Nginx나 Tomcat은 `read()`/`recv()`를 반복해 가능한 만큼 가져갑니다.
    애플리케이션이 느리게 읽으면 receive buffer와 TCP window가 압박을 받아 송신자 쪽 전송도 느려질 수 있습니다.
    이 현상이 backpressure, 즉 받는 쪽 처리 속도가 보내는 쪽 속도에 영향을 주는 흐름 제어의 한 모습입니다.

6. Nginx는 client-facing 요청을 처리한 뒤 upstream인 Tomcat으로 별도 요청을 보냅니다.

    Nginx의 `location`과 `proxy_pass` 설정이 `http://127.0.0.1:8080`을 가리키면, Nginx는 Tomcat을 향해 HTTP 요청을 새로 구성합니다.
    이때 Nginx는 server가 아니라 Tomcat에 대한 TCP client입니다.
    커널에 `connect()`를 요청하고, 커널은 Nginx 쪽 임시 source port와 Tomcat의 `127.0.0.1:8080` destination port로 TCP 연결을 만들거나, 이미 살아 있는 upstream keep-alive 연결을 재사용합니다.

    Nginx는 원본 요청을 byte 그대로 tunnel처럼 넘길 수도 있고, 일반 reverse proxy처럼 HTTP request line과 header를 다시 만들어 보낼 수도 있습니다.
    예를 들어 `Host`, `X-Forwarded-For`, `X-Forwarded-Proto` 같은 header가 이 경계에서 조정될 수 있습니다.
    또한 설정에 따라 request body를 먼저 buffering한 뒤 Tomcat에 넘길 수도 있고, streaming에 가깝게 넘길 수도 있습니다.
    이 차이 때문에 느린 클라이언트가 Tomcat worker를 오래 붙잡는지, Nginx buffer가 중간에서 흡수하는지가 달라집니다.

7. Tomcat은 connector에서 요청을 받아 Coyote, servlet pipeline, Spring MVC로 넘깁니다.

    Tomcat도 시작할 때 connector가 `8080` 같은 port에 리스닝 소켓을 열어 둡니다.
    Nginx가 upstream 연결을 만들면 Tomcat 쪽 커널도 같은 TCP 3-way handshake와 accept queue 흐름을 거치고, Tomcat connector의 endpoint가 연결별 socket을 가져갑니다.
    Tomcat 공식 구조로는 request가 endpoint에서 시작해 protocol 처리로 넘어가고, servlet request processing의 공통 진입점인 Coyote adapter를 거칩니다.
    그 뒤 Catalina engine, host, context, filter chain, servlet으로 올라가고, Spring Boot 애플리케이션에서는 보통 `DispatcherServlet`이 그 servlet 역할을 맡습니다.

    여기서 thread 모델은 connector 설정에 따라 다르지만, 전통적인 Spring MVC/Tomcat 흐름에서는 요청 하나가 worker thread 하나를 점유해 controller, service, repository 호출을 이어 갑니다.
    request body는 이미 Tomcat 메모리에 완전히 올라온 값일 수도 있고, servlet input stream을 통해 애플리케이션이 읽는 시점에 socket에서 더 당겨질 수도 있습니다.
    multipart upload, 큰 JSON body, streaming API에서는 이 차이가 메모리 사용량과 timeout, backpressure에 직접 연결됩니다.

8. Spring MVC는 handler mapping, argument binding, validation, controller, service, repository 흐름으로 이동합니다.

    `DispatcherServlet`은 URL과 HTTP method를 기준으로 handler를 찾고, argument resolver가 path variable, query parameter, header, body를 controller method 인자로 바꿉니다.
    JSON body라면 Jackson 같은 message converter가 byte나 character stream을 Java object로 역직렬화합니다.
    validation이 통과하면 controller가 service를 호출하고, service는 transaction boundary 안에서 repository나 mapper를 호출합니다.

9. DB 접근이 필요하면 커넥션 풀에서 DB 연결을 빌리고, 응답은 반대 방향으로 흐릅니다.

    HikariCP 같은 connection pool에서 빌린 DB connection도 결국 DB 서버와의 TCP socket 위에서 동작합니다.
    SQL 실행 결과는 DB result set, domain object, DTO, HTTP response body로 바뀌고, Tomcat은 response header와 body byte를 connected socket에 씁니다.
    그 byte는 Tomcat 커널 send buffer, Nginx upstream receive buffer, Nginx proxy response, client-facing send buffer, 클라이언트 커널 receive buffer를 거쳐 브라우저로 돌아갑니다.

    TCP가 full-duplex라는 말은 이 전체 시간 동안 양쪽 방향 byte stream이 독립적으로 열려 있다는 뜻입니다.
    HTTP/1.1의 일반 요청/응답은 보통 클라이언트가 요청을 보내고 서버가 응답하는 순서로 보이지만, TCP 자체는 동시에 양방향 전송이 가능합니다.
    HTTP/2 stream multiplexing, WebSocket, 긴 upload를 끝까지 읽기 전에 서버가 보내는 오류 응답 같은 상황에서는 이 full-duplex 성질이 더 눈에 잘 보입니다.
    다만 애플리케이션 프로토콜과 서버 구현이 어떤 순서로 읽고 쓸지는 TCP가 아니라 HTTP, TLS, 프록시, servlet container의 규칙이 정합니다.
...
- DNS가 끝나면 바로 HTTP 요청을 보내나요: 아닙니다. DNS는 IP 후보만 알려 줍니다. 그 IP와 port에 TCP 연결을 만들고, HTTPS라면 TLS 핸드셰이크까지 끝낸 뒤 HTTP 요청을 보냅니다.
- recursive DNS와 authoritative DNS는 무엇이 다른가요: recursive resolver는 클라이언트를 대신해 답을 찾아 주는 질의자이고, authoritative name server는 특정 domain zone의 정답 record를 가지고 있는 서버입니다.
- TCP 3-way handshake는 왜 세 번 오가나요: 양쪽이 서로의 초기 sequence number를 확인해야 오래된 중복 segment와 새 연결을 구분할 수 있고, 양방향 송수신 준비가 됐다는 상태를 맞출 수 있기 때문입니다.
- TLS는 TCP와 무엇이 다른가요: TCP는 신뢰성 있는 byte stream을 만들지만 암호화와 서버 신원 검증을 제공하지 않습니다. TLS는 그 TCP 연결 위에서 인증서 검증, 키 교환, 암호화, 무결성 검증을 담당합니다.
- SNI와 ALPN은 왜 나오나요: SNI는 같은 IP 뒤의 여러 도메인 중 어떤 인증서를 쓸지 알려 주고, ALPN은 TLS 위에서 HTTP/1.1을 쓸지 HTTP/2를 쓸지 협상하게 해 줍니다.
- TCP 4-way handshake는 TLS와 관련 있나요: 아닙니다. TCP 4-way handshake는 보통 `FIN -> ACK -> FIN -> ACK`로 TCP 연결을 정상 종료하는 흐름을 말합니다. TLS 핸드셰이크는 TCP 연결 위에서 인증과 암호화 키를 준비하는 별도 흐름입니다.
- TLS 1.2와 TLS 1.3의 큰 차이는 무엇인가요: TLS 1.3은 RSA key exchange를 제거하고 ECDHE 기반 전방 비밀성을 기본 전제로 두며, 일반적인 새 연결의 handshake를 1-RTT로 줄였습니다.
- TCP/IP 스택은 NIC 안에 있나요: 일반적인 서버에서는 중심이 OS 커널 안에 있습니다. NIC는 frame 수신, DMA, checksum offload 같은 하드웨어 보조를 맡고, TCP 연결 상태와 소켓 큐는 커널이 관리합니다.
- 리스닝 소켓과 연결별 소켓은 왜 나누나요: 하나의 서버 port로 들어오는 많은 클라이언트를 받는 입구와, 각 클라이언트별 sequence number, buffer, timeout, 연결 상태를 분리해서 관리해야 하기 때문입니다.
- `accept()`는 TCP 핸드셰이크 전에 일어나나요: 서버 프로세스는 미리 `accept()`에서 기다릴 수 있지만, `accept()`가 반환하는 connected socket은 TCP 3-way handshake가 끝나 accept queue에 올라온 연결입니다. 애플리케이션이 바로 `accept()`를 호출하지 않아도 커널은 backlog 한도 안에서 handshake를 끝낸 연결을 잠시 보관할 수 있습니다.
- 서버가 클라이언트마다 새 무작위 port를 여나요: 아닙니다. 서버의 accepted socket은 local port가 계속 `443`이나 `8080`입니다. 연결을 먼저 거는 쪽의 source port가 임시 port이고, Nginx가 Tomcat에 연결할 때는 Nginx가 active opener라서 Nginx 쪽 source port가 임시 port가 됩니다.
- 핸드셰이크 당사자는 누구인가요: TCP 3-way handshake는 보통 양쪽 커널 TCP/IP 스택이 수행합니다. TLS handshake는 그 TCP 연결 위에서 TLS endpoint인 브라우저, Nginx, Tomcat 같은 사용자 공간 프로세스의 TLS 라이브러리가 수행합니다. NIC는 packet을 운반하고 보조할 뿐, 인증서나 HTTP 요청을 판단하지 않습니다.
- `Content-Length`만큼 커널이 먼저 다 받아 놓나요: 아닙니다. HTTP parser가 `Content-Length`를 보고 body 경계를 판단할 뿐이고, TCP는 byte stream을 segment 단위로 받아 receive buffer에 쌓습니다. Nginx나 Tomcat은 `read()`를 반복해 가능한 만큼 읽고, 느리게 읽으면 TCP window와 backpressure가 전송 속도에 영향을 줍니다.
- Nginx와 Tomcat 사이에도 TCP handshake가 있나요: 별도 upstream TCP 연결을 새로 만들면 있습니다. 다만 upstream keep-alive를 재사용하면 매 요청마다 새 handshake가 생기지는 않습니다. 어느 쪽이든 브라우저와 Nginx 사이 연결과 Nginx와 Tomcat 사이 연결은 서로 다른 TCP 연결입니다.
- NIC interrupt와 epoll은 같은 알림인가요: 아닙니다. NIC interrupt나 driver polling은 하드웨어와 커널 사이의 packet 도착 알림이고, `epoll`/`kqueue`는 커널이 사용자 공간 프로세스에 소켓 fd의 읽기 가능 상태를 알려 주는 인터페이스입니다.
...
첫 연결 구간에서는 DNS lookup 시간, TCP connect time, TLS handshake time, first byte time을 나누어 봅니다.
예를 들어 DNS가 느리면 recursive resolver, authoritative server, TTL, CNAME chain을 보고, TCP connect가 느리면 SYN 재전송, listen backlog, firewall, routing을 봅니다.
TLS가 느리거나 실패하면 인증서 만료, hostname mismatch, SNI 누락, cipher suite 불일치, trust store 문제를 봅니다.
...
- 관련 문서: [network-web-protocols.md](network-web-protocols.md), [security-cryptography.md](security-cryptography.md), [spring-backend-frameworks.md](spring-backend-frameworks.md), [concurrency-async-io.md](concurrency-async-io.md)
- 공식 기준: [RFC 1034](https://www.rfc-editor.org/rfc/rfc1034), [RFC 1035](https://www.rfc-editor.org/rfc/rfc1035), [RFC 9293](https://datatracker.ietf.org/doc/rfc9293/), [RFC 9112](https://www.rfc-editor.org/rfc/rfc9112), [RFC 5246](https://www.rfc-editor.org/rfc/rfc5246), [RFC 8446](https://www.rfc-editor.org/rfc/rfc8446), [Linux `socket(2)`](https://man7.org/linux/man-pages/man2/socket.2.html), [Linux `listen(2)`](https://man7.org/linux/man-pages/man2/listen.2.html), [Linux `accept(2)`](https://man7.org/linux/man-pages/man2/accept.2.html), [Linux networking scaling](https://docs.kernel.org/networking/scaling.html), [Linux NAPI](https://docs.kernel.org/networking/napi.html), [NGINX reverse proxy](https://docs.nginx.com/nginx/admin-guide/web-server/reverse-proxy/), [Apache Tomcat request process](https://tomcat.apache.org/tomcat-10.1-doc/architecture/requestProcess.html)
- 확인 명령 후보: `dig +trace www.example.com`, `dig www.example.com A`, `curl -v --http2 https://example.com`, `openssl s_client -connect example.com:443 -servername example.com -alpn h2`, `tcpdump -nn host <ip> and port 443`, `ip -s link`, `ethtool -k <iface>`, `ss -ltn`, `ss -ant state established`, `lsof -i :443`, Nginx access/error log, Tomcat access log, Spring actuator metrics, DB slow query log
...
3. 클라이언트는 인증서 chain, hostname, 유효 기간(validity), 신뢰 기준점(trust anchor)을 확인합니다.
...
Spring 서비스가 컨테이너 안에서 느리거나 죽으면 애플리케이션 로그만 보지 않고 CPU quota, memory limit, cgroup OOM, PID 1 signal 처리, health check, restart 정책, 컨테이너 네트워크를 함께 확인해야 합니다.
...
4. JVM은 컨테이너 메모리 제한(container memory limit)을 기준으로 heap, metaspace, direct buffer, thread stack을 함께 써야 합니다.
...
- 컨테이너와 VM은 무엇이 다른가요: VM은 guest kernel을 따로 실행하고, 컨테이너는 host kernel을 공유합니다.
- 컨테이너 메모리를 넘으면 JVM OOM인가 cgroup OOM인가요: heap OOM일 수도 있고, cgroup limit 초과로 kernel이 프로세스를 죽이는 OOM kill일 수도 있습니다.
...
- 확인 명령 후보: `docker inspect`, `docker stats`, 컨테이너 로그, `/sys/fs/cgroup` 지표, actuator health, orchestrator event log
...
2. Spring 컨테이너는 bean을 생성하고 constructor, field, method 등을 통해 dependency를 연결합니다.
...
따라서 컨테이너 이벤트, previous logs, heap dump, GC log, memory limit, CPU throttling, readiness 실패를 함께 봅니다.
...
4. 확인 경로를 제안합니다.
    ...
    `EXPLAIN`, thread dump, GC log, packet capture, broker lag, 컨테이너 이벤트처럼 어느 증거가 어떤 가설을 통과 또는 탈락시키는지 말합니다.
...
마지막 기억 지도는 네 문장으로 압축됩니다.
첫째, 면접 질문은 용어의 뜻을 묻는 것처럼 보여도 실제로는 어떤 상태가 어디서 바뀌는지 묻는 경우가 많습니다.
둘째, 좋은 답변은 문제와 불변식을 먼저 말하고, 그다음 낮은 계층의 실행 경로와 실패 신호로 내려갑니다.
셋째, 시스템 설계 답변은 기술 이름을 나열하는 것이 아니라 어떤 보장을 어디에 맡기고 어떤 비용을 받아들이는지 설명해야 합니다.
넷째, 장애 분석 답변은 관측값 하나를 결론으로 삼지 않고, 여러 증거를 나란히 놓아 가설을 통과시키거나 탈락시킵니다.

이 지도를 잡은 뒤 문서를 덮고 아래 질문에 자기 말로 답해 봅니다.
