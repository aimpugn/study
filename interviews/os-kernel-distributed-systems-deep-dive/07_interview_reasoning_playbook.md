...
- [1. 모르는 질문을 받으면 상태부터 찾는다](#1-모르는-질문을-받으면-상태부터-찾는다)
- [2. 장애 질문은 "무엇이 죽었나"보다 "무엇을 관측했나"로 시작한다](#2-장애-질문은-무엇이-죽었나보다-무엇을-관측했나로-시작한다)
- [3. 성능 질문은 queue와 병목을 찾는 문제다](#3-성능-질문은-queue와-병목을-찾는-문제다)
- [4. 일관성 질문은 read/write 계약으로 번역한다](#4-일관성-질문은-readwrite-계약으로-번역한다)
- [5. 확장성 질문은 partitioning과 coordination 비용으로 답한다](#5-확장성-질문은-partitioning과-coordination-비용으로-답한다)
- [자주 나오는 질문에 대한 압축 답변과 꼬리 경로](#자주-나오는-질문에-대한-압축-답변과-꼬리-경로)
    - [blocking, non-blocking, synchronous, asynchronous는 어떻게 다르나요?](#blocking-non-blocking-synchronous-asynchronous는-어떻게-다르나요)
    - [패킷 하나는 어떻게 request가 되나요?](#패킷-하나는-어떻게-request가-되나요)
    - [Kafka는 왜 빠른가요?](#kafka는-왜-빠른가요)
    - [Cassandra는 왜 write가 빠른가요?](#cassandra는-왜-write가-빠른가요)
    - [Spark shuffle은 왜 비싼가요?](#spark-shuffle은-왜-비싼가요)
    - [quorum과 consensus는 무엇이 다른가요?](#quorum과-consensus는-무엇이-다른가요)
...
- [관측 질문으로 답변을 마무리하기](#관측-질문으로-답변을-마무리하기)
- [제품별 꼬리 질문 대응](#제품별-꼬리-질문-대응)
- [마지막 연습: 하나의 현상을 세 층으로 답하기](#마지막-연습-하나의-현상을-세-층으로-답하기)
- [꼬리 질문 1: `write()`와 durability](#꼬리-질문-1-write와-durability)
- [꼬리 질문 2: `epoll`과 async](#꼬리-질문-2-epoll과-async)
- [꼬리 질문 3: QUORUM과 consensus](#꼬리-질문-3-quorum과-consensus)
- [꼬리 질문 4: GC와 OS 메모리](#꼬리-질문-4-gc와-os-메모리)
- [꼬리 질문 5: retry와 idempotency](#꼬리-질문-5-retry와-idempotency)
- [꼬리 질문 6: CAP와 PACELC](#꼬리-질문-6-cap와-pacelc)
- [답변을 망치는 습관과 수리법](#답변을-망치는-습관과-수리법)
- [마지막 replay: 긴 답변을 짧게 접는 법](#마지막-replay-긴-답변을-짧게-접는-법)
- [압박 질문을 받았을 때의 방어선](#압박-질문을-받았을-때의-방어선)
- [시스템 설계 질문으로 확장될 때](#시스템-설계-질문으로-확장될-때)
- [마지막 자기 점검](#마지막-자기-점검)
