...
- [NIC에서 application thread까지](#nic에서-application-thread까지)
- [interrupt만으로는 고속 network를 감당하기 어렵다](#interrupt만으로는-고속-network를-감당하기-어렵다)
- [TCP는 packet이 아니라 신뢰할 수 있는 byte stream을 제공하려 한다](#tcp는-packet이-아니라-신뢰할-수-있는-byte-stream을-제공하려-한다)
...
- [NIC queue, RSS, interrupt affinity는 request가 처음 줄을 서는 자리다](#nic-queue-rss-interrupt-affinity는-request가-처음-줄을-서는-자리다)
- [TCP는 순서를 만들지만 지연도 만든다](#tcp는-순서를-만들지만-지연도-만든다)
- [Listen backlog와 accept queue는 서버가 연결을 받기 전의 대기실이다](#listen-backlog와-accept-queue는-서버가-연결을-받기-전의-대기실이다)
- [Readiness와 completion은 다른 약속이다](#readiness와-completion은-다른-약속이다)
- [Event loop는 기다림을 줄이지만 막히면 전체가 막힌다](#event-loop는-기다림을-줄이지만-막히면-전체가-막힌다)
- [Send path와 qdisc는 응답도 줄을 세운다](#send-path와-qdisc는-응답도-줄을-세운다)
- [Network backpressure는 timeout보다 먼저 읽어야 한다](#network-backpressure는-timeout보다-먼저-읽어야-한다)
- [Interview replay: packet에서 request까지, response에서 timeout까지](#interview-replay-packet에서-request까지-response에서-timeout까지)
- [UDP와 TCP를 제품 의미로 비교하기](#udp와-tcp를-제품-의미로-비교하기)
