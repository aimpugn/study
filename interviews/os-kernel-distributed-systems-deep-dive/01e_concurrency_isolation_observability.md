...
- [race는 두 실행 흐름이 같은 상태를 다른 순서로 본다는 문제다](#race는-두-실행-흐름이-같은-상태를-다른-순서로-본다는-문제다)
- [futex는 user-space lock과 kernel sleep 사이의 다리다](#futex는-user-space-lock과-kernel-sleep-사이의-다리다)
- [lost wakeup은 조건 확인과 잠들기를 따로 하면 생긴다](#lost-wakeup은-조건-확인과-잠들기를-따로-하면-생긴다)
- [memory ordering은 "썼다"와 "다른 core가 그 순서로 보았다"를 분리한다](#memory-ordering은-썼다와-다른-core가-그-순서로-보았다를-분리한다)
- [cgroup과 namespace는 컨테이너를 process처럼 보이게 만드는 장치다](#cgroup과-namespace는-컨테이너를-process처럼-보이게-만드는-장치다)
- [observability는 tool 이름이 아니라 상태 질문이다](#observability는-tool-이름이-아니라-상태-질문이다)
- [perf와 eBPF는 "왜 느린지"를 함수와 event로 좁힌다](#perf와-ebpf는-왜-느린지를-함수와-event로-좁힌다)
- [현실 시나리오: CPU는 낮고 Kafka consumer lag는 증가한다](#현실-시나리오-cpu는-낮고-kafka-consumer-lag는-증가한다)
- [현실 시나리오: 컨테이너 메모리 제한 안에서 Spark executor가 죽는다](#현실-시나리오-컨테이너-메모리-제한-안에서-spark-executor가-죽는다)
- [문서를 덮고 확인할 것](#문서를-덮고-확인할-것)
- [Deadlock, livelock, starvation은 같은 정지가 아니다](#deadlock-livelock-starvation은-같은-정지가-아니다)
- [Futex와 park/unpark는 user-space와 kernel wait를 잇는다](#futex와-parkunpark는-user-space와-kernel-wait를-잇는다)
- [Memory ordering과 cache coherence는 lock 밖의 정확성을 다룬다](#memory-ordering과-cache-coherence는-lock-밖의-정확성을-다룬다)
- [IPC는 process isolation을 넘는 의도적인 통로다](#ipc는-process-isolation을-넘는-의도적인-통로다)
- [Namespace, cgroup, capability, seccomp는 container의 보이는 세계와 가능한 행동을 나눈다](#namespace-cgroup-capability-seccomp는-container의-보이는-세계와-가능한-행동을-나눈다)
- [Observability는 증상을 kernel 질문으로 바꾸는 일이다](#observability는-증상을-kernel-질문으로-바꾸는-일이다)
- [Flame graph와 tail latency를 함께 읽기](#flame-graph와-tail-latency를-함께-읽기)
- [Interview replay: 동시성과 격리, 관측을 한 번에 묶기](#interview-replay-동시성과-격리-관측을-한-번에-묶기)
- [보안과 보호는 성능과 별개가 아니다](#보안과-보호는-성능과-별개가-아니다)
- [Small experiment: lock wait와 CPU 사용률을 분리하기](#small-experiment-lock-wait와-cpu-사용률을-분리하기)
- [Product bridge: 하나의 증상을 세 갈래로 쪼개기](#product-bridge-하나의-증상을-세-갈래로-쪼개기)
...
초보자가 먼저 붙잡을 질문은 세 가지입니다.

- **무엇을 함께 바꾸는가:** counter, queue, metadata, file offset, socket state처럼 둘 이상의 실행 흐름이 만지는 공유 상태를 찾습니다.
- **누가 기다리는가:** lock owner, futex waiter, run queue의 runnable task, cgroup quota에 막힌 process처럼 대기 주체를 찾습니다.
- **어디서 증거를 볼 수 있는가:** thread dump, `/proc`, `ss`, `iostat`, perf/eBPF, cgroup event처럼 관측 표면을 정합니다.

```text
symptom: p99 latency grows
  -> shared state: queue or lock?
  -> waiter: runnable, sleeping, futex wait, I/O wait?
  -> evidence: thread dump + off-CPU profile + kernel/cgroup metric
  -> repair target: lock, queue, quota, I/O, retry, or backpressure
```

이 순서를 지키면 "느리다"는 감상에서 바로 제품 설정을 바꾸지 않고, 어느 계층의 상태가 실제로 막혔는지 좁혀 갈 수 있습니다.
...
## cgroup과 namespace는 컨테이너를 process처럼 보이게 만드는 장치다

컨테이너는 가벼운 VM이 아닙니다. 보통 Linux namespace와 cgroup을 조합해 process에게 독립된 hostname, mount, network, PID view를 보여 주고, CPU/memory/I/O 같은 자원 사용량을 제한합니다. namespace는 "무엇이 보이는가"를 나누고, cgroup은 "얼마나 쓸 수 있는가"를 제한하고 계측합니다.
...
이 구분은 장애 분석에서 중요합니다. 컨테이너 안에서 PID 1로 보이는 process가 host에서는 다른 PID일 수 있습니다. 컨테이너 안의 `/proc` 값이 namespace/cgroup view에 맞춰 보일 수 있고, host 전체 자원과 container limit이 다를 수 있습니다. memory limit을 넘으면 host 전체 memory가 남아 있어도 cgroup OOM이 날 수 있습니다. CPU quota가 낮으면 process는 runnable인데 quota가 소진되어 다음 period까지 기다릴 수 있습니다.

Kafka broker를 컨테이너에서 돌릴 때 page cache와 cgroup memory accounting을 잘못 이해하면 heap을 안전하게 잡았다고 생각해도 OOM을 맞을 수 있습니다. Cassandra는 디스크와 메모리를 많이 쓰는 워크로드라 cgroup I/O와 memory limit이 compaction/flush에 직접 영향을 줍니다. Spark executor 컨테이너는 heap, overhead memory, off-heap, Python worker, shuffle buffer를 함께 제한받습니다.
...
## 현실 시나리오: 컨테이너 메모리 제한 안에서 Spark executor가 죽는다
...
이 장의 기억 지도는 `공유 상태 -> 대기 주체 -> 관측 증거`입니다. 공유 상태를 찾고, 그 상태 때문에 누가 기다리는지 확인하고, 마지막으로 어떤 도구가 그 대기를 보여 주는지 연결해야 합니다.
...
동시성 문제를 모두 "락 문제"라고 부르면 해결 방향이 흐려집니다. Deadlock은 서로가 가진 자원을 기다리며 아무도 앞으로 나아가지 못하는 상태입니다. 전형적인 조건은 mutual exclusion, hold and wait, no preemption, circular wait입니다. Thread A가 lock X를 잡고 Y를 기다리고, thread B가 Y를 잡고 X를 기다리면 두 thread 모두 진행하지 못합니다.
...
프로세스 격리(process isolation)는 한 프로세스가 다른 프로세스의 메모리를 마음대로 읽지 못하게 합니다. 하지만 실제 시스템은 프로세스 사이 통신이 필요합니다. IPC(inter-process communication)는 이 격리를 유지하면서 데이터나 signal을 주고받는 통로입니다. Pipe, Unix domain socket, TCP socket, shared memory, signal, file lock, message queue가 모두 IPC에 속합니다. 각각 copy 비용, 순서 보장(ordering), backpressure, 보안 경계, 수명이 다릅니다.
...
컨테이너(container)는 작은 VM이 아닙니다. 일반적으로 같은 커널(kernel)을 공유하면서 네임스페이스(namespace), cgroup, capability, seccomp, 파일 시스템 마운트, 네트워크 설정을 조합해 프로세스가 보는 세계를 제한합니다. 네임스페이스는 프로세스가 보는 이름 공간을 분리합니다. PID 네임스페이스는 프로세스 id의 보이는 범위를 바꾸고, mount 네임스페이스는 파일 시스템 트리를 다르게 보이게 하며, network 네임스페이스는 네트워크 인터페이스와 라우팅 테이블을 분리합니다. Cgroup은 CPU, 메모리, I/O 같은 자원 사용량을 기록하고 제한합니다.
...
Kafka broker가 컨테이너 안에서 파일 디스크립터 제한이나 `mmap` 개수 제한에 걸리고, Cassandra가 메모리 잠금이나 `perf` counter 권한 문제를 만나고, Spark executor가 cgroup 메모리 제한 때문에 종료되는 것은 모두 격리와 자원 제어가 애플리케이션 동작으로 올라온 사례입니다. 컨테이너를 설명할 때 "격리한다"에서 멈추지 말고, "무엇이 다르게 보이고, 무엇이 제한되며, 어떤 시스템 콜이나 커널 기능이 막히는가"를 말해야 합니다.

VM(virtual machine)은 컨테이너와 다른 경계를 만듭니다. 하이퍼바이저(hypervisor)는 guest OS에게 가상 CPU, 메모리, 장치를 제공하고, guest kernel은 자기 안의 프로세스를 관리합니다. 컨테이너는 host kernel을 공유하지만 네임스페이스와 cgroup으로 프로세스를 제한합니다. VM은 커널 경계가 더 강하지만 실행 비용과 운영 단위가 다르고, 컨테이너는 가볍지만 host kernel 공격 표면과 자원 사용량 계산을 더 조심해야 합니다. Kubernetes 클러스터에서는 둘이 함께 쓰이기도 합니다.
...
이 장의 답변은 이렇게 시작할 수 있습니다. "동시성은 여러 실행 흐름이 공유 상태를 어떤 순서로 보고 바꾸는가의 문제입니다. Lock과 atomic은 correctness를 지키지만, contention이 생기면 futex wait와 scheduler sleep으로 latency를 만들 수 있습니다. Memory ordering과 cache coherence는 CPU core 사이에서 값과 순서가 어떻게 보이는지를 다룹니다. 프로세스 격리는 namespace, cgroup, capability, seccomp, VM/컨테이너 경계와 만나 운영상의 권한과 자원 제한으로 드러납니다."

꼬리 질문이 관측으로 오면 "증상을 kernel 질문으로 바꿉니다"라고 말하면 됩니다. CPU 문제인지 보려면 on-CPU profile과 run queue를 봅니다. Lock 문제인지 보려면 thread dump, futex wait, monitor contention을 봅니다. Disk 문제인지 보려면 page cache, dirty writeback, block I/O를 봅니다. Network 문제인지 보려면 socket queue, retransmission, packet capture를 봅니다. 컨테이너 문제인지 보려면 cgroup event와 namespace/capability/seccomp 경계를 봅니다. 제품 문제인지 보려면 Kafka lag/ISR, Cassandra compaction/repair, Spark shuffle/stage metric을 OS 관측과 나란히 놓습니다.
...
shared state or external resource
  ...
  -> container boundary limits what the process can see and use
...
이 trace를 검증할 때는 반증 질문을 같이 둡니다.

| 증상 | 관측 metric | OS 상태 후보 | product/distributed 상태 후보 | 반증 확인 |
| --- | --- | --- | --- | --- |
| Kafka consumer lag 증가 | lag, poll/process time, commit latency | CPU quota, socket queue, GC, disk I/O | partition skew, rebalance, broker fetch latency | thread는 idle인데 broker fetch latency만 높은가? 반대로 broker는 빠른데 downstream DB가 느린가? |
| Cassandra read timeout | read latency, pending tasks, dropped messages | disk queue, page cache miss, network retransmission, cgroup throttle | coordinator/replica path, CL, repair/hinted handoff 상태 | replica 한쪽만 느린가? 모든 replica가 느린가? consistency level을 낮춰도 같은가? |
| Spark straggler | task duration, shuffle read/write, GC time | executor GC, local spill disk, scheduler delay | skewed partition, remote block fetch, executor loss | 같은 input size task만 느린가? 특정 executor나 host에만 몰리는가? |
