...
과부하는 단순 성능 저하가 아닙니다. Queue가 커지면 timeout이 늘고, timeout은 retry를 만들고, retry는 더 많은 load를 만듭니다. 이 feedback loop가 닫히지 않으면 시스템은 실제 처리보다 재시도와 대기열 관리에 더 많은 자원을 씁니다. Backpressure는 producer가 consumer의 처리 능력에 맞게 속도를 줄이게 하는 장치입니다. Admission control은 애초에 처리할 수 없는 요청을 일찍 거절해 내부 queue를 보호합니다.
