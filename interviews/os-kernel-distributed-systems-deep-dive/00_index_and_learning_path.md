...
이 코퍼스의 중심 질문은 하나입니다. 백엔드 시스템에서 "느리다", "유실됐다", "중복 처리됐다", "일관성이 깨졌다", "확장하면 될 줄 알았는데 더 느려졌다" 같은 증상이 보일 때, 제품 이름을 외우지 않고 어떤 상태가 어느 층에서 밀렸는지 설명할 수 있는지입니다.
...
처음 보는 독자는 아래 표를 먼저 붙잡으면 됩니다.

| 보이는 문제 | 대표 메커니즘 | 이 메커니즘이 소유하거나 바꾸는 상태 |
| --- | --- | --- |
| 애플리케이션이 파일이나 socket 같은 공유 자원을 직접 만질 수 없다 | syscall | user request를 커널 객체 조회와 권한 검사로 바꾼다 |
| 실행할 thread가 많은데 CPU core는 제한되어 있다 | scheduler | runnable task, run queue, context switch 시점을 고른다 |
| `write()`는 빨리 끝났는데 저장 장치는 아직 느리다 | page cache / dirty page | file page를 메모리에 남기고 나중에 writeback한다 |
| packet은 도착했는데 request handler가 아직 실행되지 않았다 | socket buffer / epoll | TCP byte와 readiness 이벤트를 application thread 앞에 쌓는다 |
| 여러 머신이 같은 변경을 같은 순서로 보아야 한다 | log | 변경 기록과 replay 위치를 남긴다 |
| replica 일부가 느리거나 실패해도 read/write를 판단해야 한다 | quorum | 어떤 replica 응답 집합을 성공 증거로 볼지 정한다 |
...
각 OS 상세 문서는 독립적으로 읽을 수 있게 쓰되, 모두 같은 읽기 습관을 요구합니다.
분량 자체보다 trace를 붙잡으세요.
예를 들어 `01d`에서는 packet이 NIC RX ring, NAPI, TCP, socket buffer, epoll, scheduler를 지나 request가 되는지 말할 수 있어야 합니다.
`01c`에서는 `write()` 성공, dirty page, writeback, `fsync()`, Kafka ack, Cassandra CL success를 서로 다른 계층의 성공으로 나눌 수 있어야 합니다.

이 단계의 기억 지도는 아래처럼 압축됩니다.

- 실행 흐름: process와 thread는 CPU를 소유하지 않고 scheduler의 선택을 기다립니다.
- 주소 공간: process가 보는 주소는 page table과 MMU를 지나 physical memory와 연결됩니다.
- 파일 쓰기: `write()` 성공은 dirty page 생성과 이어질 수 있지만, storage 내구성은 별도 동기화와 복구 계약을 요구합니다.
- 네트워크: packet 도착은 request 처리 완료가 아니며, NIC queue, TCP socket buffer, epoll, scheduler를 차례로 지납니다.
- 관측: "느리다"는 증상은 run queue, page fault, writeback, socket queue, futex wait 같은 더 낮은 상태 질문으로 바꿔야 합니다.

이 지도를 잡은 뒤에는 다음 질문으로 replay해 봅니다.
