...
- [4. Virtual Memory, Page Table, Page Fault는 주소를 어떻게 속이는가](#4-virtual-memory-page-table-page-fault는-주소를-어떻게-속이는가)
- [5. File I/O와 Page Cache는 성능과 내구성을 동시에 만든다](#5-file-io와-page-cache는-성능과-내구성을-동시에-만든다)
- [6. Network Stack과 Socket Buffer는 distributed latency의 첫 관문이다](#6-network-stack과-socket-buffer는-distributed-latency의-첫-관문이다)
...
- [Exceptional control flow를 한 장의 지도처럼 읽기](#exceptional-control-flow를-한-장의-지도처럼-읽기)
...
파일 I/O에서 page cache는 성능의 친구이자 내구성 오해의 원인입니다. 같은 파일을 두 번 읽을 때 두 번째가 빠른 이유는 애플리케이션 코드가 갑자기 똑똑해져서가 아니라, 첫 번째 read가 파일 page를 커널 메모리에 올려 두었기 때문일 수 있습니다. 반대로 write가 빠르게 반환되는 이유도 디스크가 즉시 모든 byte를 물리적으로 기록해서가 아니라, 커널이 dirty page로 받아 둔 뒤 이후 writeback할 수 있기 때문일 수 있습니다.
...
운영체제 공부에서 boot와 executable format은 너무 앞쪽에 있어서 실무와 멀어 보일 수 있습니다. 하지만 "프로세스가 실행된다"라는 말을 정확히 하려면, 커널이 언제 처음 control을 잡고, 실행 파일을 어떤 규칙으로 주소 공간에 올리며, 동적 링커와 런타임이 어떤 상태에서 시작하는지 알아야 합니다. Kafka broker도, Cassandra node도, Spark executor도 결국 하나의 process image로 시작합니다. 프로세스 시작 지점을 흐리게 이해하면 signal, file descriptor limit, shared library loading, container entrypoint 문제를 모두 애플리케이션 프레임워크 문제처럼 오해하기 쉽습니다.

전원이 들어온 직후의 CPU는 JVM이나 셸을 알지 못합니다. firmware가 최소한의 hardware 초기화를 하고, boot loader가 kernel image와 부팅 인자를 메모리에 올리고, kernel은 매우 작은 초기 실행 환경에서 memory manager, interrupt table, scheduler, device driver, filesystem, network stack 같은 공통 기반을 차례로 세웁니다. 이 초기화는 이후 모든 process가 기대는 약속을 만드는 일입니다. syscall entry가 준비되어야 사용자 코드가 커널에게 요청할 수 있고, page table과 protection 설정이 준비되어야 사용자 mode와 kernel mode를 나눌 수 있으며, driver와 interrupt controller가 준비되어야 disk와 NIC가 kernel object로 연결됩니다.
...
실행 파일 포맷도 같은 맥락입니다. 리눅스의 ELF 같은 포맷은 기계어 byte 묶음만이 아니라, text segment, data segment, BSS, dynamic linking 정보, entry point, relocation 정보를 담습니다. `execve()`는 기존 process의 사용자 주소 공간을 새 program image로 바꾸는 system call입니다. 커널은 파일 header를 읽고, 필요한 segment를 mapping하고, stack 위에 argument와 environment를 준비하고, instruction pointer를 entry point로 맞춘 뒤 사용자 mode로 돌아갑니다. 그 뒤 dynamic linker가 필요한 shared object를 매핑하고 symbol을 resolve하며, runtime startup code가 `main` 또는 해당 런타임 진입점으로 이어집니다.
...
ABI는 이 장면의 낮은 약속입니다. ABI(application binary interface)는 컴파일된 코드가 OS와 라이브러리, 다른 object file과 맞추는 이진 수준의 계약입니다. 함수 인자를 어느 register나 stack 위치에 놓는지, return value는 어디로 받는지, stack alignment는 어떻게 맞추는지, system call number와 arguments는 어떤 규칙으로 전달하는지 같은 내용이 포함됩니다. Calling convention은 그중 함수 호출과 복귀의 세부 규칙입니다. 이 규칙이 깨지면 소스 코드가 논리적으로 맞아도 실행 중 register와 stack 해석이 어긋납니다.

이 지식은 면접에서 "자바는 VM 위에서 도는데 OS 실행 파일과 무슨 관련이 있나요?" 같은 꼬리 질문을 받았을 때 중요합니다. JVM은 OS를 건너뛰는 마법이 아니라 OS process로 올라온 runtime입니다. `java` launcher 자체가 native executable이고, JVM shared library와 native library를 load하며, classpath나 jar를 파일로 읽고, thread와 heap을 커널에게서 받은 process 자원 위에 만듭니다. 그래서 `ulimit -n`이 낮으면 Kafka broker가 file/socket을 충분히 열지 못하고, container PID 1 signal 처리가 잘못되면 graceful shutdown이 깨지고, native library 경로가 틀리면 compression이나 JNI 기능이 시작부터 실패합니다.

## Exceptional control flow를 한 장의 지도처럼 읽기

프로그램은 보통 다음 instruction으로 진행하지만, 실제 시스템은 계속 흐름이 꺾입니다. 함수 호출은 같은 권한 수준 안에서 caller와 callee 사이를 오갑니다. syscall은 사용자 mode에서 kernel mode로 들어갔다가 다시 사용자 코드로 돌아옵니다. page fault는 현재 instruction이 접근한 주소를 지금 사용할 수 없어서 CPU가 커널 handler로 흐름을 넘깁니다. interrupt는 장치나 timer가 바깥에서 CPU의 흐름을 끊습니다. signal은 커널이 사용자 process에게 비정상 사건이나 외부 요청을 전달하는 방식입니다.
...
이들을 한데 묶어 보는 이유는 모두 "현재 흐름이 직선으로 가지 않는다"는 사실을 보여 주기 때문입니다. 하지만 누가 시작했는지, 복귀 위치가 어디인지, 같은 thread로 돌아오는지, scheduler가 다른 task를 고를 수 있는지는 서로 다릅니다. interrupt는 현재 application이 원하지 않아도 올 수 있습니다. syscall은 application이 의도적으로 요청합니다. page fault는 현재 instruction이 만든 조건이고, fault가 정상적으로 해결되면 같은 instruction이 다시 실행될 수 있습니다. timer interrupt 뒤에는 같은 thread로 돌아올 수도 있고, scheduler가 다른 thread를 고를 수도 있습니다.

이 구분은 성능 해석으로 이어집니다. System call이 많으면 user-kernel 전환, 인자 검증, 커널 lock, buffer copy가 늘 수 있습니다. Page fault가 많으면 주소 공간은 커 보여도 실제 physical page나 file-backed page가 준비되지 않아 실행이 끊깁니다. Interrupt와 softirq 처리가 특정 CPU에 몰리면 application thread가 받을 CPU 시간이 줄어듭니다. Signal 처리나 process 종료는 평범한 return 흐름이 아니라 커널이 사용자 process의 제어 흐름에 끼어드는 일입니다.

Kafka fetch request 하나를 예로 들어 보겠습니다. NIC가 packet을 받으면 interrupt 또는 interrupt moderation 뒤 NAPI polling이 시작됩니다. kernel network stack이 TCP byte stream을 socket receive buffer에 넣습니다. `epoll_wait()` 중이던 broker network thread는 fd가 readable하다는 사실을 보고 깨어납니다. scheduler가 그 thread에 CPU를 줍니다. thread는 `read()` system call로 bytes를 가져오고, request를 parse하고, 필요한 log segment를 page cache에서 찾거나 disk I/O를 일으킵니다. 이 한 요청 안에 interrupt, syscall, scheduler wakeup, page cache lookup, file I/O가 모두 들어 있습니다.
...
문서를 덮고 다시 말할 때는 용어를 줄줄 외우기보다 이렇게 시작하면 됩니다. "CPU 흐름은 함수 호출만으로 움직이지 않습니다. 외부 장치가 interrupt를 만들고, 현재 instruction이 page fault를 만들고, 사용자 code가 syscall로 kernel에게 요청하고, kernel은 signal과 scheduler로 사용자 흐름에 다시 영향을 줍니다." 그 뒤 Kafka, Cassandra, Spark의 request가 이 꺾임들을 어떻게 지나가는지 붙이면 OS 기초가 제품 진단 언어로 바뀝니다.
