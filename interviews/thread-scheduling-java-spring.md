...
1. 지금 실행 가능한가
    ...
    CPU를 받을 수 있는 스레드는 실행 가능 상태입니다. CPU 코어 수보다 실행 가능한 스레드가 많으면, 일부는 대기열에 서서 자기 차례를 기다립니다. Linux CFS 문서는 runnable task를 실행 시간 순서의 자료구조에 두고 다음 실행 대상을 고르는 방식으로 설명합니다.[^linux-cfs]
