    ...
    - [HTTP/gRPC와 스트리밍](#httpgrpc와-스트리밍)
        - [HTTP 요청이 일반적인 TCP 연결 방식과 Keep-Alive TCP 연결 방식 이루어지는 경우](#http-요청이-일반적인-tcp-연결-방식과-keep-alive-tcp-연결-방식-이루어지는-경우)
            - [원문: HTTP 요청이 일반적인 TCP 연결 방식과 Keep-Alive TCP 연결 방식 이루어지는 경우](#원문-http-요청이-일반적인-tcp-연결-방식과-keep-alive-tcp-연결-방식-이루어지는-경우)
                - [HTTP 요청이 일반적인 TCP 연결 방식과 Keep-Alive TCP 연결 방식 이루어지는 경우](#http-요청이-일반적인-tcp-연결-방식과-keep-alive-tcp-연결-방식-이루어지는-경우-1)
        ...
        - [네트워크를 통한 스트리밍 방식](#네트워크를-통한-스트리밍-방식)
            - [원문: 네트워크를 통한 스트리밍 방식](#원문-네트워크를-통한-스트리밍-방식)
                - [네트워크를 통한 스트리밍 방식](#네트워크를-통한-스트리밍-방식-1)
        - [서버에서 100GB 파일을 스트리밍 방식으로 읽는 방법 (CPU 4코어, RAM 16GB, SWAP 16GB 조건에서)](#서버에서-100gb-파일을-스트리밍-방식으로-읽는-방법-cpu-4코어-ram-16gb-swap-16gb-조건에서)
            - [원문: 서버에서 100GB 파일을 스트리밍 방식으로 읽는 방법 (CPU 4코어, RAM 16GB, SWAP 16GB 조건에서)](#원문-서버에서-100gb-파일을-스트리밍-방식으로-읽는-방법-cpu-4코어-ram-16gb-swap-16gb-조건에서)
                - [서버에서 100GB 파일을 스트리밍 방식으로 읽는 방법 (CPU 4코어, RAM 16GB, SWAP 16GB 조건에서)](#서버에서-100gb-파일을-스트리밍-방식으로-읽는-방법-cpu-4코어-ram-16gb-swap-16gb-조건에서-1)
    - [Proxy와 L7 진입 경로](#proxy와-l7-진입-경로)
        - [proxy와 reverse proxy](#proxy와-reverse-proxy)
            - [원문: proxy와 reverse proxy](#원문-proxy와-reverse-proxy)
                - [proxy와 reverse proxy](#proxy와-reverse-proxy-1)
    - [TCP/IP와 소켓 통신](#tcpip와-소켓-통신)
        - [IP 구조](#ip-구조)
            - [원문: IP 구조](#원문-ip-구조)
                - [IP 구조](#ip-구조-1)
        - [NAT(Network Address Translation)](#natnetwork-address-translation)
            - [원문: NAT(Network Address Translation)](#원문-natnetwork-address-translation)
                - [NAT(Network Address Translation)](#natnetwork-address-translation-1)
        - [TCP 컨제스천 컨트롤](#tcp-컨제스천-컨트롤)
            - [원문: TCP 컨제스천 컨트롤](#원문-tcp-컨제스천-컨트롤)
                - [TCP 컨제스천 컨트롤](#tcp-컨제스천-컨트롤-1)
        - [클라이언트와 서버 간의 소켓 통신 과정](#클라이언트와-서버-간의-소켓-통신-과정)
            - [원문: 클라이언트와 서버 간의 소켓 통신 과정](#원문-클라이언트와-서버-간의-소켓-통신-과정)
                - [클라이언트와 서버 간의 소켓 통신 과정](#클라이언트와-서버-간의-소켓-통신-과정-1)
