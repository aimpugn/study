...
이 절의 주제는 `클라이언트 요청은 DNS에서 DB 응답까지 어떤 순서로 왕복하는가`이다. 요청은 하나의 선이 아니라 여러 번의 주소 결정, 연결 생성, 암호 채널 수립, HTTP 파싱, 프록시 재요청, 런타임 호출, DB 왕복, 응답 쓰기가 이어진 경로다. 기준 장면은 `사용자가 브라우저에서 주문 조회 API를 호출하고, Spring 서비스가 DB에서 행을 읽어 JSON 응답을 돌려주는 장면`이다. 여기서는 다음 대상을 따라간다: 도메인 이름, TCP 세그먼트, TLS 레코드, HTTP 메시지, SQL 결과 행. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


이 절에서 따라갈 대상은 도메인 이름, TCP 세그먼트, TLS 레코드, HTTP 메시지, SQL 결과 행이다.
이 대상들은 같은 요청 안에서 여러 번 이름을 바꾸며 서로 다른 계층을 지난다.
...
아래 trace는 이 절의 주제를 `사용자가 브라우저에서 주문 조회 API를 호출하고, Spring 서비스가 DB에서 행을 읽어 JSON 응답을 돌려주는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `클라이언트 요청은 DNS에서 DB 응답까지 어떤 순서로 왕복하는가`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
...
| 계층 | 이 절에서 맡는 일 | 직접 소유하는 상태 | 이 계층이 하지 않는 일 |
| --- | --- | --- | --- |
...
같은 "느림"이라도 소유 상태가 다르면 확인 명령과 수리 방법이 완전히 달라진다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `DNS가 끝나면 HTTP가 바로 간다고 생각하거나, Nginx와 Tomcat 사이의 upstream 연결을 같은 TCP 연결로 합쳐 버리는 설명`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
높은 층의 이름만 기억하면 API 호출 순서만 남지만, 낮은 층의 상태를 함께 보면 왜 그 API가 그런 모양인지 이해할 수 있다.

`클라이언트 요청은 DNS에서 DB 응답까지 어떤 순서로 왕복하는가`의 공식 자료 고정점은 [RFC 9293 TCP](https://www.rfc-editor.org/rfc/rfc9293), [Linux tcp(7)](https://www.man7.org/linux/man-pages/man7/tcp.7.html), [Spring WebFlux](https://docs.spring.io/spring-framework/reference/web/webflux.html), [Tomcat architecture](https://tomcat.apache.org/tomcat-10.1-doc/architecture/overview.html)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `도메인 이름, TCP 세그먼트, TLS 레코드, HTTP 메시지, SQL 결과 행`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`도메인 이름, TCP 세그먼트, TLS 레코드, HTTP 메시지, SQL 결과 행`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`클라이언트 요청은 DNS에서 DB 응답까지 어떤 순서로 왕복하는가`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
...
`클라이언트 요청은 DNS에서 DB 응답까지 어떤 순서로 왕복하는가`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
...
FAIL 신호는 `도메인 이름, TCP 세그먼트, TLS 레코드, HTTP 메시지, SQL 결과 행`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `DNS가 끝나면 HTTP가 바로 간다고 생각하거나, Nginx와 Tomcat 사이의 upstream 연결을 같은 TCP 연결로 합쳐 버리는 설명`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
...
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.

#### 자주 틀리는 설명과 장애 해석

첫 번째 위험은 계층을 건너뛰는 설명이다.
DNS가 끝나면 HTTP가 바로 간다고 생각하거나, Nginx와 Tomcat 사이의 upstream 연결을 같은 TCP 연결로 합쳐 버리는 설명은 실제 현장에서 꽤 자주 나온다.
...
따라서 이 절에서는 숫자 하나를 보는 대신 `도메인 이름, TCP 세그먼트, TLS 레코드, HTTP 메시지, SQL 결과 행`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 비교축을 적용하면 이 절의 주제를 설명할 때 불필요한 단정이 줄어든다.
...
규칙: 이 절에서 설명한 계층의 소유 상태와 처리 규칙
출력: 다음 계층이 소비할 fd readiness, event, request object, SQL result, response byte
```

이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `도메인 이름, TCP 세그먼트, TLS 레코드, HTTP 메시지, SQL 결과 행`를 먼저 말하고, 그다음 `요청은 하나의 선이 아니라 여러 번의 주소 결정, 연결 생성, 암호 채널 수립, HTTP 파싱, 프록시 재요청, 런타임 호출, DB 왕복, 응답 쓰기가 이어진 경로다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `사용자가 브라우저에서 주문 조회 API를 호출하고, Spring 서비스가 DB에서 행을 읽어 JSON 응답을 돌려주는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
이 순서가 유지되면 `DNS가 끝나면 HTTP가 바로 간다고 생각하거나, Nginx와 Tomcat 사이의 upstream 연결을 같은 TCP 연결로 합쳐 버리는 설명` 같은 설명이 왜 부족한지도 자연스럽게 드러난다.

이 절을 읽은 뒤 남겨야 할 자기 점검 질문은 하나다.
...
가설은 나쁘지 않지만, 가설을 결론처럼 말하면 운영에서도 면접에서도 약해진다.
...
이 절의 대상인 도메인 이름, TCP 세그먼트, TLS 레코드, HTTP 메시지, SQL 결과 행 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`클라이언트 요청은 DNS에서 DB 응답까지 어떤 순서로 왕복하는가`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `사용자가 브라우저에서 주문 조회 API를 호출하고, Spring 서비스가 DB에서 행을 읽어 JSON 응답을 돌려주는 장면`에서는 도메인 이름, TCP 세그먼트, TLS 레코드, HTTP 메시지, SQL 결과 행 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
...
따라서 `DNS가 끝나면 HTTP가 바로 간다고 생각하거나, Nginx와 Tomcat 사이의 upstream 연결을 같은 TCP 연결로 합쳐 버리는 설명`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
이 절의 주제는 `한 요청 경로에서 커널, 프록시, 런타임, 애플리케이션, DB의 책임은 어떻게 나뉘는가`이다. 책임 경계를 나누면 관측 지표가 정리된다. 커널은 연결과 버퍼, 프록시는 HTTP 경계와 upstream, 런타임은 스레드와 이벤트 루프, 애플리케이션은 비즈니스 흐름, DB는 쿼리와 저장소 상태를 맡는다. 기준 장면은 `같은 지연 시간을 두고 커널 큐가 찼는지, 프록시가 buffering 중인지, Tomcat worker가 모자란지, DB가 잠긴 것인지 좁혀 가는 장면`이다. 여기서는 다음 대상을 따라간다: 요청 바이트와 그 바이트를 해석한 HTTP request object, Java 객체, SQL result set. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


이 절에서 따라갈 대상은 요청 바이트와 그 바이트를 해석한 HTTP request object, Java 객체, SQL result set이다.
이 대상들은 같은 요청 안에서 여러 번 이름을 바꾸며 서로 다른 계층을 지난다.
브라우저에서 시작한 것은 처음에는 도메인 이름이지만, DNS 뒤에는 IP 후보가 되고, TCP 뒤에는 커널이 관리하는 연결 상태가 되며, TLS 뒤에는 암호화된 바이트 흐름이 된다.
Nginx가 TLS를 끝내면 같은 사용자의 요청은 다시 upstream HTTP 요청으로 만들어지고, Tomcat이나 Netty는 그 바이트를 런타임의 요청 객체나 이벤트로 바꾼다.
애플리케이션은 그 요청을 컨트롤러와 서비스 호출로 읽고, DB 접근이 필요하면 또 다른 TCP 연결과 DB 프로토콜 위에서 SQL을 보낸다.
...
아래 trace는 이 절의 주제를 `같은 지연 시간을 두고 커널 큐가 찼는지, 프록시가 buffering 중인지, Tomcat worker가 모자란지, DB가 잠긴 것인지 좁혀 가는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `한 요청 경로에서 커널, 프록시, 런타임, 애플리케이션, DB의 책임은 어떻게 나뉘는가`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `모든 지연을 애플리케이션 코드 탓으로 보거나, 모든 연결 실패를 방화벽 탓으로 보는 설명`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`한 요청 경로에서 커널, 프록시, 런타임, 애플리케이션, DB의 책임은 어떻게 나뉘는가`의 공식 자료 고정점은 [RFC 9293 TCP](https://www.rfc-editor.org/rfc/rfc9293), [Linux tcp(7)](https://www.man7.org/linux/man-pages/man7/tcp.7.html), [Spring WebFlux](https://docs.spring.io/spring-framework/reference/web/webflux.html), [Tomcat architecture](https://tomcat.apache.org/tomcat-10.1-doc/architecture/overview.html)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `요청 바이트와 그 바이트를 해석한 HTTP request object, Java 객체, SQL result set`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`요청 바이트와 그 바이트를 해석한 HTTP request object, Java 객체, SQL result set`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`한 요청 경로에서 커널, 프록시, 런타임, 애플리케이션, DB의 책임은 어떻게 나뉘는가`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`한 요청 경로에서 커널, 프록시, 런타임, 애플리케이션, DB의 책임은 어떻게 나뉘는가`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `요청 바이트와 그 바이트를 해석한 HTTP request object, Java 객체, SQL result set`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `모든 지연을 애플리케이션 코드 탓으로 보거나, 모든 연결 실패를 방화벽 탓으로 보는 설명`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
모든 지연을 애플리케이션 코드 탓으로 보거나, 모든 연결 실패를 방화벽 탓으로 보는 설명은 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `요청 바이트와 그 바이트를 해석한 HTTP request object, Java 객체, SQL result set`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `요청 바이트와 그 바이트를 해석한 HTTP request object, Java 객체, SQL result set`를 먼저 말하고, 그다음 `책임 경계를 나누면 관측 지표가 정리된다. 커널은 연결과 버퍼, 프록시는 HTTP 경계와 upstream, 런타임은 스레드와 이벤트 루프, 애플리케이션은 비즈니스 흐름, DB는 쿼리와 저장소 상태를 맡는다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `같은 지연 시간을 두고 커널 큐가 찼는지, 프록시가 buffering 중인지, Tomcat worker가 모자란지, DB가 잠긴 것인지 좁혀 가는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 요청 바이트와 그 바이트를 해석한 HTTP request object, Java 객체, SQL result set 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`한 요청 경로에서 커널, 프록시, 런타임, 애플리케이션, DB의 책임은 어떻게 나뉘는가`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `같은 지연 시간을 두고 커널 큐가 찼는지, 프록시가 buffering 중인지, Tomcat worker가 모자란지, DB가 잠긴 것인지 좁혀 가는 장면`에서는 요청 바이트와 그 바이트를 해석한 HTTP request object, Java 객체, SQL result set 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `모든 지연을 애플리케이션 코드 탓으로 보거나, 모든 연결 실패를 방화벽 탓으로 보는 설명`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
이 절의 주제는 `프로세스, 스레드, 파일 디스크립터`이다. 백엔드 서버는 프로세스 하나가 아니라 fd table, native thread, heap, direct buffer, socket 객체가 붙어 있는 실행 단위다. 기준 장면은 `Spring Boot 프로세스 하나가 8080 포트를 열고 worker thread와 DB connection을 함께 유지하는 장면`이다. 여기서는 다음 대상을 따라간다: 프로세스의 fd table entry, 커널 open file description, socket 객체, Java Thread. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


리눅스 실행 기반을 볼 때는 Java나 Spring 이름을 잠시 내려놓고 프로세스가 커널에게 빌려 쓰는 자원을 먼저 본다.
이 절에서 따라갈 대상은 프로세스의 fd table entry, 커널 open file description, socket 객체, Java Thread이며, 이 대상은 애플리케이션 코드 안에만 있는 값이 아니라 프로세스의 descriptor table, 커널 socket 객체, scheduler queue, memory accounting, service manager 상태와 연결된다.
...
아래 trace는 이 절의 주제를 `Spring Boot 프로세스 하나가 8080 포트를 열고 worker thread와 DB connection을 함께 유지하는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `프로세스, 스레드, 파일 디스크립터`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `파일 디스크립터를 실제 파일 경로로만 이해해서 socket, pipe, epoll fd, eventfd까지 같은 한도에 걸린다는 점을 놓치는 설명`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`프로세스, 스레드, 파일 디스크립터`의 공식 자료 고정점은 [Linux tcp(7)](https://www.man7.org/linux/man-pages/man7/tcp.7.html), [Linux bind(2)](https://man7.org/linux/man-pages/man2/bind.2.html), [Linux listen(2)](https://man7.org/linux/man-pages/man2/listen.2.html), [Linux accept(2)](https://man7.org/linux/man-pages/man2/accept.2.html), [Linux cgroup v2](https://www.kernel.org/doc/html/latest/admin-guide/cgroup-v2.html)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `프로세스의 fd table entry, 커널 open file description, socket 객체, Java Thread`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`프로세스의 fd table entry, 커널 open file description, socket 객체, Java Thread`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`프로세스, 스레드, 파일 디스크립터`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`프로세스, 스레드, 파일 디스크립터`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `프로세스의 fd table entry, 커널 open file description, socket 객체, Java Thread`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `파일 디스크립터를 실제 파일 경로로만 이해해서 socket, pipe, epoll fd, eventfd까지 같은 한도에 걸린다는 점을 놓치는 설명`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
파일 디스크립터를 실제 파일 경로로만 이해해서 socket, pipe, epoll fd, eventfd까지 같은 한도에 걸린다는 점을 놓치는 설명은 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `프로세스의 fd table entry, 커널 open file description, socket 객체, Java Thread`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `프로세스의 fd table entry, 커널 open file description, socket 객체, Java Thread`를 먼저 말하고, 그다음 `백엔드 서버는 프로세스 하나가 아니라 fd table, native thread, heap, direct buffer, socket 객체가 붙어 있는 실행 단위다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `Spring Boot 프로세스 하나가 8080 포트를 열고 worker thread와 DB connection을 함께 유지하는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 프로세스의 fd table entry, 커널 open file description, socket 객체, Java Thread 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`프로세스, 스레드, 파일 디스크립터`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `Spring Boot 프로세스 하나가 8080 포트를 열고 worker thread와 DB connection을 함께 유지하는 장면`에서는 프로세스의 fd table entry, 커널 open file description, socket 객체, Java Thread 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `파일 디스크립터를 실제 파일 경로로만 이해해서 socket, pipe, epoll fd, eventfd까지 같은 한도에 걸린다는 점을 놓치는 설명`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
이 절의 주제는 `사용자 공간과 커널 공간, 시스템 콜`이다. Java 메서드 호출과 네트워크 전송 사이에는 시스템 콜 경계가 있다. 이 경계에서 복사, blocking, errno, signal interruption, buffer pressure가 드러난다. 기준 장면은 `Tomcat worker가 `read()`로 요청 body 일부를 가져오고 `write()`로 response chunk를 밀어 넣는 장면`이다. 여기서는 다음 대상을 따라간다: 사용자 공간 버퍼의 byte, syscall 인자, 커널 socket buffer의 byte. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


리눅스 실행 기반을 볼 때는 Java나 Spring 이름을 잠시 내려놓고 프로세스가 커널에게 빌려 쓰는 자원을 먼저 본다.
이 절에서 따라갈 대상은 사용자 공간 버퍼의 byte, syscall 인자, 커널 socket buffer의 byte이며, 이 대상은 애플리케이션 코드 안에만 있는 값이 아니라 프로세스의 descriptor table, 커널 socket 객체, scheduler queue, memory accounting, service manager 상태와 연결된다.
운영 장애에서 이 계층을 놓치면 로그에는 `Connection reset`, `Broken pipe`, `OutOfMemoryError`, `too many open files`처럼 서로 다른 문구가 찍히는데, 실제 공통 원인은 fd 한도, cgroup memory limit, thread scheduling 지연, signal 처리 실패일 수 있다.
따라서 리눅스 실행 기반은 "서버가 돌아가는 배경"이 아니라 요청 처리의 첫 번째 런타임이다.

#### 값이 움직이는 순서

아래 trace는 이 절의 주제를 `Tomcat worker가 `read()`로 요청 body 일부를 가져오고 `write()`로 response chunk를 밀어 넣는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `사용자 공간과 커널 공간, 시스템 콜`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
한 요청은 계층을 모두 지난다.
다만 각 계층이 소유한 상태를 분리해야 질문이 선명해진다.
예를 들어 `Tomcat worker가 `read()`로 요청 body 일부를 가져오고 `write()`로 response chunk를 밀어 넣는 장면`에서 response가 느리다면 애플리케이션 method 시간이 길 수도 있고, DB connection pool에서 기다렸을 수도 있으며, Nginx가 upstream response를 buffer에 모으고 있을 수도 있고, client가 느려 send buffer가 차 있을 수도 있다.
같은 "느림"이라도 소유 상태가 다르면 확인 명령과 수리 방법이 완전히 달라진다.
...
2. `Tomcat worker가 `read()`로 요청 body 일부를 가져오고 `write()`로 response chunk를 밀어 넣는 장면`의 그 지점 주인은 커널, 프록시, JVM 런타임, 애플리케이션, DB 중 누구인가.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `JVM 메서드가 곧바로 NIC에 쓴다고 생각하거나, kernel이 Java 객체를 안다고 설명하는 경우`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`사용자 공간과 커널 공간, 시스템 콜`의 공식 자료 고정점은 [Linux tcp(7)](https://www.man7.org/linux/man-pages/man7/tcp.7.html), [Linux bind(2)](https://man7.org/linux/man-pages/man2/bind.2.html), [Linux listen(2)](https://man7.org/linux/man-pages/man2/listen.2.html), [Linux accept(2)](https://man7.org/linux/man-pages/man2/accept.2.html), [Linux cgroup v2](https://www.kernel.org/doc/html/latest/admin-guide/cgroup-v2.html)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `사용자 공간 버퍼의 byte, syscall 인자, 커널 socket buffer의 byte`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`사용자 공간 버퍼의 byte, syscall 인자, 커널 socket buffer의 byte`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`사용자 공간과 커널 공간, 시스템 콜`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`사용자 공간과 커널 공간, 시스템 콜`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `사용자 공간 버퍼의 byte, syscall 인자, 커널 socket buffer의 byte`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `JVM 메서드가 곧바로 NIC에 쓴다고 생각하거나, kernel이 Java 객체를 안다고 설명하는 경우`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
JVM 메서드가 곧바로 NIC에 쓴다고 생각하거나, kernel이 Java 객체를 안다고 설명하는 경우는 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
세 번째 위험은 `Tomcat worker가 `read()`로 요청 body 일부를 가져오고 `write()`로 response chunk를 밀어 넣는 장면`에서 정상 상태와 장애 상태를 구분하지 못하는 것이다.
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `사용자 공간 버퍼의 byte, syscall 인자, 커널 socket buffer의 byte`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
| 비교축 | 왼쪽을 볼 때 | 오른쪽을 볼 때 | 판단 질문 |
| --- | --- | --- | --- |
| `사용자 공간과 커널 공간, 시스템 콜`의 커널 상태 vs 애플리케이션 상태 | `사용자 공간 버퍼의 byte, syscall 인자, 커널 socket buffer의 byte` 중 fd, TCP state, buffer, route, cgroup처럼 OS가 직접 소유한 상태 | request object, transaction, business object, controller log처럼 앱이 만든 상태 | `Tomcat worker가 `read()`로 요청 body 일부를 가져오고 `write()`로 response chunk를 밀어 넣는 장면`에서 증상은 연결이 만들어지기 전인가, 앱 handler가 시작된 뒤인가 |
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `사용자 공간 버퍼의 byte, syscall 인자, 커널 socket buffer의 byte`를 먼저 말하고, 그다음 `Java 메서드 호출과 네트워크 전송 사이에는 시스템 콜 경계가 있다. 이 경계에서 복사, blocking, errno, signal interruption, buffer pressure가 드러난다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `Tomcat worker가 `read()`로 요청 body 일부를 가져오고 `write()`로 response chunk를 밀어 넣는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
`Tomcat worker가 `read()`로 요청 body 일부를 가져오고 `write()`로 response chunk를 밀어 넣는 장면`에서 한 설정을 키우면 병목이 사라지는 것이 아니라 다음 좁은 지점으로 이동할 수 있다.
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 사용자 공간 버퍼의 byte, syscall 인자, 커널 socket buffer의 byte 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`사용자 공간과 커널 공간, 시스템 콜`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `Tomcat worker가 `read()`로 요청 body 일부를 가져오고 `write()`로 response chunk를 밀어 넣는 장면`에서는 사용자 공간 버퍼의 byte, syscall 인자, 커널 socket buffer의 byte 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `JVM 메서드가 곧바로 NIC에 쓴다고 생각하거나, kernel이 Java 객체를 안다고 설명하는 경우`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.

`사용자 공간과 커널 공간, 시스템 콜`에서 대기 지점을 찾을 때는 누가 기다리는지부터 분리한다.
`Tomcat worker가 `read()`로 요청 body 일부를 가져오고 `write()`로 response chunk를 밀어 넣는 장면`에서는 호출한 thread가 커널 안에서 기다릴 수도 있고, event loop가 readiness를 기다릴 수도 있으며, DB나 외부 시스템이 자기 wait 상태를 남길 수도 있다.
...
`Tomcat worker가 `read()`로 요청 body 일부를 가져오고 `write()`로 response chunk를 밀어 넣는 장면`에서 메모리나 지연이 늘어난다면 어느 버퍼가 늘었는지 확인해야 한다.
커널 버퍼가 찬 문제를 controller 코드만 고쳐 해결할 수 없고, proxy buffering 문제를 JVM heap 크기만 키워 해결할 수도 없다.
...
이 절의 주제는 `서버 프로세스는 포트를 어떻게 열고 연결을 받을 준비를 하는가`이다. 서버는 요청이 온 뒤 포트를 여는 것이 아니라 시작할 때 로컬 주소와 포트에 socket을 묶고, listening state로 바꾼 뒤 accept 가능한 연결을 기다린다. 기준 장면은 `Spring Boot 내장 Tomcat이 `server.port=8080`을 읽고 리스닝 소켓을 준비하는 장면`이다. 여기서는 다음 대상을 따라간다: socket fd가 `socket -> bind -> listen -> accept` 상태로 바뀌는 흐름. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


리눅스 실행 기반을 볼 때는 Java나 Spring 이름을 잠시 내려놓고 프로세스가 커널에게 빌려 쓰는 자원을 먼저 본다.
이 절에서 따라갈 대상은 socket fd가 `socket -> bind -> listen -> accept` 상태로 바뀌는 흐름이며, 이 대상은 애플리케이션 코드 안에만 있는 값이 아니라 프로세스의 descriptor table, 커널 socket 객체, scheduler queue, memory accounting, service manager 상태와 연결된다.
운영 장애에서 이 계층을 놓치면 로그에는 `Connection reset`, `Broken pipe`, `OutOfMemoryError`, `too many open files`처럼 서로 다른 문구가 찍히는데, 실제 공통 원인은 fd 한도, cgroup memory limit, thread scheduling 지연, signal 처리 실패일 수 있다.
따라서 리눅스 실행 기반은 "서버가 돌아가는 배경"이 아니라 요청 처리의 첫 번째 런타임이다.

#### 값이 움직이는 순서

아래 trace는 이 절의 주제를 `Spring Boot 내장 Tomcat이 `server.port=8080`을 읽고 리스닝 소켓을 준비하는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `서버 프로세스는 포트를 어떻게 열고 연결을 받을 준비를 하는가`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
한 요청은 계층을 모두 지난다.
다만 각 계층이 소유한 상태를 분리해야 질문이 선명해진다.
예를 들어 `Spring Boot 내장 Tomcat이 `server.port=8080`을 읽고 리스닝 소켓을 준비하는 장면`에서 response가 느리다면 애플리케이션 method 시간이 길 수도 있고, DB connection pool에서 기다렸을 수도 있으며, Nginx가 upstream response를 buffer에 모으고 있을 수도 있고, client가 느려 send buffer가 차 있을 수도 있다.
같은 "느림"이라도 소유 상태가 다르면 확인 명령과 수리 방법이 완전히 달라진다.
...
2. `Spring Boot 내장 Tomcat이 `server.port=8080`을 읽고 리스닝 소켓을 준비하는 장면`의 그 지점 주인은 커널, 프록시, JVM 런타임, 애플리케이션, DB 중 누구인가.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `클라이언트마다 서버 local port가 새로 생긴다고 설명하는 경우`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`서버 프로세스는 포트를 어떻게 열고 연결을 받을 준비를 하는가`의 공식 자료 고정점은 [Linux tcp(7)](https://www.man7.org/linux/man-pages/man7/tcp.7.html), [Linux bind(2)](https://man7.org/linux/man-pages/man2/bind.2.html), [Linux listen(2)](https://man7.org/linux/man-pages/man2/listen.2.html), [Linux accept(2)](https://man7.org/linux/man-pages/man2/accept.2.html), [Linux cgroup v2](https://www.kernel.org/doc/html/latest/admin-guide/cgroup-v2.html)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `socket fd가 `socket -> bind -> listen -> accept` 상태로 바뀌는 흐름`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`socket fd가 `socket -> bind -> listen -> accept` 상태로 바뀌는 흐름`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`서버 프로세스는 포트를 어떻게 열고 연결을 받을 준비를 하는가`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`서버 프로세스는 포트를 어떻게 열고 연결을 받을 준비를 하는가`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `socket fd가 `socket -> bind -> listen -> accept` 상태로 바뀌는 흐름`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `클라이언트마다 서버 local port가 새로 생긴다고 설명하는 경우`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
클라이언트마다 서버 local port가 새로 생긴다고 설명하는 경우는 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
세 번째 위험은 `Spring Boot 내장 Tomcat이 `server.port=8080`을 읽고 리스닝 소켓을 준비하는 장면`에서 정상 상태와 장애 상태를 구분하지 못하는 것이다.
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `socket fd가 `socket -> bind -> listen -> accept` 상태로 바뀌는 흐름`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `socket fd가 `socket -> bind -> listen -> accept` 상태로 바뀌는 흐름`를 먼저 말하고, 그다음 `서버는 요청이 온 뒤 포트를 여는 것이 아니라 시작할 때 로컬 주소와 포트에 socket을 묶고, listening state로 바꾼 뒤 accept 가능한 연결을 기다린다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `Spring Boot 내장 Tomcat이 `server.port=8080`을 읽고 리스닝 소켓을 준비하는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
이 절의 기준 장면인 `Spring Boot 내장 Tomcat이 `server.port=8080`을 읽고 리스닝 소켓을 준비하는 장면`에서는 socket fd가 `socket -> bind -> listen -> accept` 상태로 바뀌는 흐름 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `클라이언트마다 서버 local port가 새로 생긴다고 설명하는 경우`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.

`서버 프로세스는 포트를 어떻게 열고 연결을 받을 준비를 하는가`에서 대기 지점을 찾을 때는 누가 기다리는지부터 분리한다.
`Spring Boot 내장 Tomcat이 `server.port=8080`을 읽고 리스닝 소켓을 준비하는 장면`에서는 호출한 thread가 커널 안에서 기다릴 수도 있고, event loop가 readiness를 기다릴 수도 있으며, DB나 외부 시스템이 자기 wait 상태를 남길 수도 있다.
...
`Spring Boot 내장 Tomcat이 `server.port=8080`을 읽고 리스닝 소켓을 준비하는 장면`에서 메모리나 지연이 늘어난다면 어느 버퍼가 늘었는지 확인해야 한다.
커널 버퍼가 찬 문제를 controller 코드만 고쳐 해결할 수 없고, proxy buffering 문제를 JVM heap 크기만 키워 해결할 수도 없다.
...
`Spring Boot 내장 Tomcat이 `server.port=8080`을 읽고 리스닝 소켓을 준비하는 장면`에서 한 설정을 키우면 병목이 사라지는 것이 아니라 다음 좁은 지점으로 이동할 수 있다.
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 socket fd가 `socket -> bind -> listen -> accept` 상태로 바뀌는 흐름 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.
...
이 절의 주제는 `소켓 버퍼와 파일 디스크립터`이다. fd는 커널 객체를 가리키는 손잡이고, 소켓 버퍼는 그 객체 안에서 네트워크 byte를 임시로 보관하는 공간이다. 기준 장면은 `10MB upload body가 여러 TCP segment로 들어오고 애플리케이션이 조금씩 읽는 장면`이다. 여기서는 다음 대상을 따라간다: NIC가 받은 byte, 커널 receive buffer, 사용자 공간 ByteBuffer 또는 ServletInputStream buffer. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


리눅스 실행 기반을 볼 때는 Java나 Spring 이름을 잠시 내려놓고 프로세스가 커널에게 빌려 쓰는 자원을 먼저 본다.
이 절에서 따라갈 대상은 NIC가 받은 byte, 커널 receive buffer, 사용자 공간 ByteBuffer 또는 ServletInputStream buffer이며, 이 대상은 애플리케이션 코드 안에만 있는 값이 아니라 프로세스의 descriptor table, 커널 socket 객체, scheduler queue, memory accounting, service manager 상태와 연결된다.
운영 장애에서 이 계층을 놓치면 로그에는 `Connection reset`, `Broken pipe`, `OutOfMemoryError`, `too many open files`처럼 서로 다른 문구가 찍히는데, 실제 공통 원인은 fd 한도, cgroup memory limit, thread scheduling 지연, signal 처리 실패일 수 있다.
따라서 리눅스 실행 기반은 "서버가 돌아가는 배경"이 아니라 요청 처리의 첫 번째 런타임이다.

#### 값이 움직이는 순서

아래 trace는 이 절의 주제를 `10MB upload body가 여러 TCP segment로 들어오고 애플리케이션이 조금씩 읽는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `소켓 버퍼와 파일 디스크립터`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `Content-Length 전체가 한 번에 애플리케이션 메모리에 올라온 뒤 처리된다고 보는 설명`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`소켓 버퍼와 파일 디스크립터`의 공식 자료 고정점은 [Linux tcp(7)](https://www.man7.org/linux/man-pages/man7/tcp.7.html), [Linux bind(2)](https://man7.org/linux/man-pages/man2/bind.2.html), [Linux listen(2)](https://man7.org/linux/man-pages/man2/listen.2.html), [Linux accept(2)](https://man7.org/linux/man-pages/man2/accept.2.html), [Linux cgroup v2](https://www.kernel.org/doc/html/latest/admin-guide/cgroup-v2.html)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `NIC가 받은 byte, 커널 receive buffer, 사용자 공간 ByteBuffer 또는 ServletInputStream buffer`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`NIC가 받은 byte, 커널 receive buffer, 사용자 공간 ByteBuffer 또는 ServletInputStream buffer`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`소켓 버퍼와 파일 디스크립터`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`소켓 버퍼와 파일 디스크립터`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `NIC가 받은 byte, 커널 receive buffer, 사용자 공간 ByteBuffer 또는 ServletInputStream buffer`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `Content-Length 전체가 한 번에 애플리케이션 메모리에 올라온 뒤 처리된다고 보는 설명`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
Content-Length 전체가 한 번에 애플리케이션 메모리에 올라온 뒤 처리된다고 보는 설명은 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `NIC가 받은 byte, 커널 receive buffer, 사용자 공간 ByteBuffer 또는 ServletInputStream buffer`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `NIC가 받은 byte, 커널 receive buffer, 사용자 공간 ByteBuffer 또는 ServletInputStream buffer`를 먼저 말하고, 그다음 `fd는 커널 객체를 가리키는 손잡이고, 소켓 버퍼는 그 객체 안에서 네트워크 byte를 임시로 보관하는 공간이다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `10MB upload body가 여러 TCP segment로 들어오고 애플리케이션이 조금씩 읽는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 NIC가 받은 byte, 커널 receive buffer, 사용자 공간 ByteBuffer 또는 ServletInputStream buffer 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`소켓 버퍼와 파일 디스크립터`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `10MB upload body가 여러 TCP segment로 들어오고 애플리케이션이 조금씩 읽는 장면`에서는 NIC가 받은 byte, 커널 receive buffer, 사용자 공간 ByteBuffer 또는 ServletInputStream buffer 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `Content-Length 전체가 한 번에 애플리케이션 메모리에 올라온 뒤 처리된다고 보는 설명`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
이 절의 주제는 `메모리, page cache, OOM, cgroup`이다. Linux 메모리 압박은 JVM heap만 보아서는 닫히지 않는다. page cache, direct buffer, native thread stack, cgroup limit, OOM killer 판정이 함께 움직인다. 기준 장면은 `컨테이너 안의 Spring 서비스가 파일 스트리밍과 JSON 직렬화를 동시에 하다가 OOMKilled 되는 장면`이다. 여기서는 다음 대상을 따라간다: anonymous heap, direct memory, page cache, cgroup memory.current, OOM victim. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


리눅스 실행 기반을 볼 때는 Java나 Spring 이름을 잠시 내려놓고 프로세스가 커널에게 빌려 쓰는 자원을 먼저 본다.
이 절에서 따라갈 대상은 anonymous heap, direct memory, page cache, cgroup memory.current, OOM victim이며, 이 대상은 애플리케이션 코드 안에만 있는 값이 아니라 프로세스의 descriptor table, 커널 socket 객체, scheduler queue, memory accounting, service manager 상태와 연결된다.
운영 장애에서 이 계층을 놓치면 로그에는 `Connection reset`, `Broken pipe`, `OutOfMemoryError`, `too many open files`처럼 서로 다른 문구가 찍히는데, 실제 공통 원인은 fd 한도, cgroup memory limit, thread scheduling 지연, signal 처리 실패일 수 있다.
따라서 리눅스 실행 기반은 "서버가 돌아가는 배경"이 아니라 요청 처리의 첫 번째 런타임이다.

#### 값이 움직이는 순서

아래 trace는 이 절의 주제를 `컨테이너 안의 Spring 서비스가 파일 스트리밍과 JSON 직렬화를 동시에 하다가 OOMKilled 되는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `메모리, page cache, OOM, cgroup`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `컨테이너 OOM을 Java `OutOfMemoryError`와 같은 사건으로만 설명하는 경우`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`메모리, page cache, OOM, cgroup`의 공식 자료 고정점은 [Linux tcp(7)](https://www.man7.org/linux/man-pages/man7/tcp.7.html), [Linux bind(2)](https://man7.org/linux/man-pages/man2/bind.2.html), [Linux listen(2)](https://man7.org/linux/man-pages/man2/listen.2.html), [Linux accept(2)](https://man7.org/linux/man-pages/man2/accept.2.html), [Linux cgroup v2](https://www.kernel.org/doc/html/latest/admin-guide/cgroup-v2.html)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `anonymous heap, direct memory, page cache, cgroup memory.current, OOM victim`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`anonymous heap, direct memory, page cache, cgroup memory.current, OOM victim`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`메모리, page cache, OOM, cgroup`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`메모리, page cache, OOM, cgroup`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `anonymous heap, direct memory, page cache, cgroup memory.current, OOM victim`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `컨테이너 OOM을 Java `OutOfMemoryError`와 같은 사건으로만 설명하는 경우`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
컨테이너 OOM을 Java `OutOfMemoryError`와 같은 사건으로만 설명하는 경우는 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `anonymous heap, direct memory, page cache, cgroup memory.current, OOM victim`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `anonymous heap, direct memory, page cache, cgroup memory.current, OOM victim`를 먼저 말하고, 그다음 `Linux 메모리 압박은 JVM heap만 보아서는 닫히지 않는다. page cache, direct buffer, native thread stack, cgroup limit, OOM killer 판정이 함께 움직인다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `컨테이너 안의 Spring 서비스가 파일 스트리밍과 JSON 직렬화를 동시에 하다가 OOMKilled 되는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 anonymous heap, direct memory, page cache, cgroup memory.current, OOM victim이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`메모리, page cache, OOM, cgroup`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `컨테이너 안의 Spring 서비스가 파일 스트리밍과 JSON 직렬화를 동시에 하다가 OOMKilled 되는 장면`에서는 anonymous heap, direct memory, page cache, cgroup memory.current, OOM victim이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `컨테이너 OOM을 Java `OutOfMemoryError`와 같은 사건으로만 설명하는 경우`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
이 절의 주제는 `스케줄링, load average, iowait`이다. load average는 CPU 사용률이 아니라 runnable 또는 일부 uninterruptible 상태 task의 평균 압박을 보여 주는 신호다. 기준 장면은 `CPU 사용률은 낮은데 load average가 높고 API p99가 튀는 장면`이다. 여기서는 다음 대상을 따라간다: runnable thread, run queue, blocked I/O task, CPU time slice. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


리눅스 실행 기반을 볼 때는 Java나 Spring 이름을 잠시 내려놓고 프로세스가 커널에게 빌려 쓰는 자원을 먼저 본다.
이 절에서 따라갈 대상은 runnable thread, run queue, blocked I/O task, CPU time slice이며, 이 대상은 애플리케이션 코드 안에만 있는 값이 아니라 프로세스의 descriptor table, 커널 socket 객체, scheduler queue, memory accounting, service manager 상태와 연결된다.
운영 장애에서 이 계층을 놓치면 로그에는 `Connection reset`, `Broken pipe`, `OutOfMemoryError`, `too many open files`처럼 서로 다른 문구가 찍히는데, 실제 공통 원인은 fd 한도, cgroup memory limit, thread scheduling 지연, signal 처리 실패일 수 있다.
따라서 리눅스 실행 기반은 "서버가 돌아가는 배경"이 아니라 요청 처리의 첫 번째 런타임이다.

#### 값이 움직이는 순서

아래 trace는 이 절의 주제를 `CPU 사용률은 낮은데 load average가 높고 API p99가 튀는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `스케줄링, load average, iowait`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `load average가 8이면 CPU 800% 사용이라고 단정하는 설명`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`스케줄링, load average, iowait`의 공식 자료 고정점은 [Linux tcp(7)](https://www.man7.org/linux/man-pages/man7/tcp.7.html), [Linux bind(2)](https://man7.org/linux/man-pages/man2/bind.2.html), [Linux listen(2)](https://man7.org/linux/man-pages/man2/listen.2.html), [Linux accept(2)](https://man7.org/linux/man-pages/man2/accept.2.html), [Linux cgroup v2](https://www.kernel.org/doc/html/latest/admin-guide/cgroup-v2.html)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `runnable thread, run queue, blocked I/O task, CPU time slice`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`runnable thread, run queue, blocked I/O task, CPU time slice`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`스케줄링, load average, iowait`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`스케줄링, load average, iowait`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `runnable thread, run queue, blocked I/O task, CPU time slice`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `load average가 8이면 CPU 800% 사용이라고 단정하는 설명`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
load average가 8이면 CPU 800% 사용이라고 단정하는 설명은 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `runnable thread, run queue, blocked I/O task, CPU time slice`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `runnable thread, run queue, blocked I/O task, CPU time slice`를 먼저 말하고, 그다음 `load average는 CPU 사용률이 아니라 runnable 또는 일부 uninterruptible 상태 task의 평균 압박을 보여 주는 신호다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `CPU 사용률은 낮은데 load average가 높고 API p99가 튀는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 runnable thread, run queue, blocked I/O task, CPU time slice 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`스케줄링, load average, iowait`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `CPU 사용률은 낮은데 load average가 높고 API p99가 튀는 장면`에서는 runnable thread, run queue, blocked I/O task, CPU time slice 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `load average가 8이면 CPU 800% 사용이라고 단정하는 설명`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
이 절의 주제는 `systemd, signal, 로그, ulimit`이다. 운영에서 서버 프로세스는 단순히 `java -jar`로 떠 있는 것이 아니라 service manager, signal, file limit, stdout/stderr 수집, restart policy 안에서 산다. 기준 장면은 `배포 중 `systemctl restart`가 Java 프로세스에 SIGTERM을 보내고 graceful shutdown이 시작되는 장면`이다. 여기서는 다음 대상을 따라간다: service unit, process signal, journal log, resource limit. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


리눅스 실행 기반을 볼 때는 Java나 Spring 이름을 잠시 내려놓고 프로세스가 커널에게 빌려 쓰는 자원을 먼저 본다.
이 절에서 따라갈 대상은 service unit, process signal, journal log, resource limit이며, 이 대상은 애플리케이션 코드 안에만 있는 값이 아니라 프로세스의 descriptor table, 커널 socket 객체, scheduler queue, memory accounting, service manager 상태와 연결된다.
운영 장애에서 이 계층을 놓치면 로그에는 `Connection reset`, `Broken pipe`, `OutOfMemoryError`, `too many open files`처럼 서로 다른 문구가 찍히는데, 실제 공통 원인은 fd 한도, cgroup memory limit, thread scheduling 지연, signal 처리 실패일 수 있다.
따라서 리눅스 실행 기반은 "서버가 돌아가는 배경"이 아니라 요청 처리의 첫 번째 런타임이다.

#### 값이 움직이는 순서

이 절은 client request body가 이동하는 경로보다 서버 프로세스의 생명주기와 운영 한도가 어떻게 요청 처리에 영향을 주는지를 먼저 본다.
기준 장면은 `배포 중 `systemctl restart`가 Java 프로세스에 SIGTERM을 보내고 graceful shutdown이 시작되는 장면`이고, 따라갈 대상은 service unit, process signal, journal log, resource limit이다.
...
그래서 이 절의 trace는 요청 byte의 흐름이 아니라 `service manager -> signal -> JVM shutdown/drain -> resource limit -> journal evidence` 흐름으로 읽어야 한다.
...
한 요청은 계층을 모두 지난다.
다만 각 계층이 소유한 상태를 분리해야 질문이 선명해진다.
예를 들어 `배포 중 `systemctl restart`가 Java 프로세스에 SIGTERM을 보내고 graceful shutdown이 시작되는 장면`에서 response가 느리다면 애플리케이션 method 시간이 길 수도 있고, DB connection pool에서 기다렸을 수도 있으며, Nginx가 upstream response를 buffer에 모으고 있을 수도 있고, client가 느려 send buffer가 차 있을 수도 있다.
같은 "느림"이라도 소유 상태가 다르면 확인 명령과 수리 방법이 완전히 달라진다.
...
2. `배포 중 `systemctl restart`가 Java 프로세스에 SIGTERM을 보내고 graceful shutdown이 시작되는 장면`의 그 지점 주인은 커널, 프록시, JVM 런타임, 애플리케이션, DB 중 누구인가.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `프로세스 종료를 항상 kill -9로 보거나, too many open files를 애플리케이션 예외만으로 보는 경우`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`systemd, signal, 로그, ulimit`의 공식 자료 고정점은 [systemd.service(5)](https://www.freedesktop.org/software/systemd/man/latest/systemd.service.html), [Linux signal(7)](https://www.man7.org/linux/man-pages/man7/signal.7.html), [Linux getrlimit(2)](https://www.man7.org/linux/man-pages/man2/getrlimit.2.html), [Linux accept(2)](https://man7.org/linux/man-pages/man2/accept.2.html)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `service unit, process signal, journal log, resource limit`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`service unit, process signal, journal log, resource limit`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`systemd, signal, 로그, ulimit`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`systemd, signal, 로그, ulimit`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `service unit, process signal, journal log, resource limit`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `프로세스 종료를 항상 kill -9로 보거나, too many open files를 애플리케이션 예외만으로 보는 경우`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
프로세스 종료를 항상 kill -9로 보거나, too many open files를 애플리케이션 예외만으로 보는 경우는 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
세 번째 위험은 `배포 중 `systemctl restart`가 Java 프로세스에 SIGTERM을 보내고 graceful shutdown이 시작되는 장면`에서 정상 상태와 장애 상태를 구분하지 못하는 것이다.
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `service unit, process signal, journal log, resource limit`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
| 비교축 | 왼쪽을 볼 때 | 오른쪽을 볼 때 | 판단 질문 |
| --- | --- | --- | --- |
| `systemd, signal, 로그, ulimit`의 커널 상태 vs 애플리케이션 상태 | `service unit, process signal, journal log, resource limit` 중 fd, TCP state, buffer, route, cgroup처럼 OS가 직접 소유한 상태 | request object, transaction, business object, controller log처럼 앱이 만든 상태 | `배포 중 `systemctl restart`가 Java 프로세스에 SIGTERM을 보내고 graceful shutdown이 시작되는 장면`에서 증상은 연결이 만들어지기 전인가, 앱 handler가 시작된 뒤인가 |
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `service unit, process signal, journal log, resource limit`를 먼저 말하고, 그다음 `운영에서 서버 프로세스는 단순히 `java -jar`로 떠 있는 것이 아니라 service manager, signal, file limit, stdout/stderr 수집, restart policy 안에서 산다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `배포 중 `systemctl restart`가 Java 프로세스에 SIGTERM을 보내고 graceful shutdown이 시작되는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
`배포 중 `systemctl restart`가 Java 프로세스에 SIGTERM을 보내고 graceful shutdown이 시작되는 장면`에서 한 설정을 키우면 병목이 사라지는 것이 아니라 다음 좁은 지점으로 이동할 수 있다.
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 service unit, process signal, journal log, resource limit 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`systemd, signal, 로그, ulimit`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `배포 중 `systemctl restart`가 Java 프로세스에 SIGTERM을 보내고 graceful shutdown이 시작되는 장면`에서는 service unit, process signal, journal log, resource limit 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `프로세스 종료를 항상 kill -9로 보거나, too many open files를 애플리케이션 예외만으로 보는 경우`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.

`systemd, signal, 로그, ulimit`에서 대기 지점을 찾을 때는 누가 기다리는지부터 분리한다.
`배포 중 `systemctl restart`가 Java 프로세스에 SIGTERM을 보내고 graceful shutdown이 시작되는 장면`에서는 호출한 thread가 커널 안에서 기다릴 수도 있고, event loop가 readiness를 기다릴 수도 있으며, DB나 외부 시스템이 자기 wait 상태를 남길 수도 있다.
...
`배포 중 `systemctl restart`가 Java 프로세스에 SIGTERM을 보내고 graceful shutdown이 시작되는 장면`에서 메모리나 지연이 늘어난다면 어느 버퍼가 늘었는지 확인해야 한다.
커널 버퍼가 찬 문제를 controller 코드만 고쳐 해결할 수 없고, proxy buffering 문제를 JVM heap 크기만 키워 해결할 수도 없다.
...
이 절의 주제는 `DNS, IP, routing, NAT`이다. DNS는 이름을 주소 후보로 바꾸고, routing은 그 주소로 나갈 인터페이스와 next hop을 고르며, NAT는 패킷의 주소/포트 일부를 경계에서 바꾼다. 기준 장면은 `Pod에서 외부 결제 API로 나가는 요청이 NAT 게이트웨이를 지나고 응답이 돌아오는 장면`이다. 여기서는 다음 대상을 따라간다: 도메인 이름, destination IP, route table entry, NAT 변환된 5-tuple. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


네트워크 실행 경로는 프로토콜 이름을 외우는 문서가 아니라, 커널이 어떤 상태를 만들고 어떤 큐에 무엇을 놓는지 따라가는 문서여야 한다.
이 절에서 따라갈 대상은 도메인 이름, destination IP, route table entry, NAT 변환된 5-tuple이며, 이 대상은 TCP/IP stack, routing table, NAT table, socket buffer, proxy upstream 연결 사이를 지난다.
...
아래 trace는 이 절의 주제를 `Pod에서 외부 결제 API로 나가는 요청이 NAT 게이트웨이를 지나고 응답이 돌아오는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `DNS, IP, routing, NAT`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `DNS, routing, NAT 문제를 모두 timeout이라는 한 단어로 뭉개는 설명`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`DNS, IP, routing, NAT`의 공식 자료 고정점은 [RFC 9293 TCP](https://www.rfc-editor.org/rfc/rfc9293), [Linux tcp(7)](https://www.man7.org/linux/man-pages/man7/tcp.7.html), [Nginx reverse proxy](https://docs.nginx.com/nginx/admin-guide/web-server/reverse-proxy), [RFC 8446 TLS 1.3](https://www.rfc-editor.org/rfc/rfc8446)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `도메인 이름, destination IP, route table entry, NAT 변환된 5-tuple`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`도메인 이름, destination IP, route table entry, NAT 변환된 5-tuple`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`DNS, IP, routing, NAT`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`DNS, IP, routing, NAT`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `도메인 이름, destination IP, route table entry, NAT 변환된 5-tuple`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `DNS, routing, NAT 문제를 모두 timeout이라는 한 단어로 뭉개는 설명`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
DNS, routing, NAT 문제를 모두 timeout이라는 한 단어로 뭉개는 설명은 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `도메인 이름, destination IP, route table entry, NAT 변환된 5-tuple`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `도메인 이름, destination IP, route table entry, NAT 변환된 5-tuple`를 먼저 말하고, 그다음 `DNS는 이름을 주소 후보로 바꾸고, routing은 그 주소로 나갈 인터페이스와 next hop을 고르며, NAT는 패킷의 주소/포트 일부를 경계에서 바꾼다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `Pod에서 외부 결제 API로 나가는 요청이 NAT 게이트웨이를 지나고 응답이 돌아오는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
이 절의 기준 장면인 `Pod에서 외부 결제 API로 나가는 요청이 NAT 게이트웨이를 지나고 응답이 돌아오는 장면`에서는 도메인 이름, destination IP, route table entry, NAT 변환된 5-tuple 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `DNS, routing, NAT 문제를 모두 timeout이라는 한 단어로 뭉개는 설명`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 도메인 이름, destination IP, route table entry, NAT 변환된 5-tuple 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.
...
이 절의 주제는 `bind, listen, accept는 각각 어느 계층의 일을 하는가`이다. `bind`는 이름 붙이기, `listen`은 새 연결 입구 만들기, `accept`는 완성 연결을 사용자 공간으로 꺼내는 단계다. 기준 장면은 `Nginx worker가 443 포트에서 listen하고 Tomcat connector가 8080 포트에서 accept하는 장면`이다. 여기서는 다음 대상을 따라간다: socket fd, local endpoint binding, listening queue, connected socket fd. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


네트워크 실행 경로는 프로토콜 이름을 외우는 문서가 아니라, 커널이 어떤 상태를 만들고 어떤 큐에 무엇을 놓는지 따라가는 문서여야 한다.
이 절에서 따라갈 대상은 socket fd, local endpoint binding, listening queue, connected socket fd이며, 이 대상은 TCP/IP stack, routing table, NAT table, socket buffer, proxy upstream 연결 사이를 지난다.
각 계층은 자기보다 위의 HTTP 의미를 모른 채 자기 계약만 지킨다.
서버 운영자는 이 분리를 알아야 `connection refused`, `timeout`, `CLOSE_WAIT`, `TIME_WAIT`, `SSL handshake failure`, `502`, `504` 같은 증상을 보고 어느 계층부터 확인할지 정할 수 있다.

#### 값이 움직이는 순서

아래 trace는 이 절의 주제를 `Nginx worker가 443 포트에서 listen하고 Tomcat connector가 8080 포트에서 accept하는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `bind, listen, accept는 각각 어느 계층의 일을 하는가`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. ``accept`가 TCP handshake를 직접 수행한다고 말하거나 `listen`이 요청 body를 읽는다고 말하는 경우`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`bind, listen, accept는 각각 어느 계층의 일을 하는가`의 공식 자료 고정점은 [RFC 9293 TCP](https://www.rfc-editor.org/rfc/rfc9293), [Linux tcp(7)](https://www.man7.org/linux/man-pages/man7/tcp.7.html), [Nginx reverse proxy](https://docs.nginx.com/nginx/admin-guide/web-server/reverse-proxy), [RFC 8446 TLS 1.3](https://www.rfc-editor.org/rfc/rfc8446)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `socket fd, local endpoint binding, listening queue, connected socket fd`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`socket fd, local endpoint binding, listening queue, connected socket fd`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`bind, listen, accept는 각각 어느 계층의 일을 하는가`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`bind, listen, accept는 각각 어느 계층의 일을 하는가`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `socket fd, local endpoint binding, listening queue, connected socket fd`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 ``accept`가 TCP handshake를 직접 수행한다고 말하거나 `listen`이 요청 body를 읽는다고 말하는 경우`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
`accept`가 TCP handshake를 직접 수행한다고 말하거나 `listen`이 요청 body를 읽는다고 말하는 경우는 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `socket fd, local endpoint binding, listening queue, connected socket fd`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `socket fd, local endpoint binding, listening queue, connected socket fd`를 먼저 말하고, 그다음 ``bind`는 이름 붙이기, `listen`은 새 연결 입구 만들기, `accept`는 완성 연결을 사용자 공간으로 꺼내는 단계다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `Nginx worker가 443 포트에서 listen하고 Tomcat connector가 8080 포트에서 accept하는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 socket fd, local endpoint binding, listening queue, connected socket fd 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`bind, listen, accept는 각각 어느 계층의 일을 하는가`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `Nginx worker가 443 포트에서 listen하고 Tomcat connector가 8080 포트에서 accept하는 장면`에서는 socket fd, local endpoint binding, listening queue, connected socket fd 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 ``accept`가 TCP handshake를 직접 수행한다고 말하거나 `listen`이 요청 body를 읽는다고 말하는 경우`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
이 절의 주제는 `TCP 3-way handshake, backlog, accept queue`이다. 3-way handshake는 커널 TCP 스택의 연결 상태를 맞추는 절차이고, backlog와 accept queue는 그 연결을 애플리케이션이 가져가기 전까지 보관하는 완충 지점이다. 기준 장면은 `부하 순간 새 연결이 몰리고 일부 클라이언트가 connect timeout 또는 refused를 보는 장면`이다. 여기서는 다음 대상을 따라간다: SYN, SYN-ACK, ACK, SYN backlog, accept queue entry. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


네트워크 실행 경로는 프로토콜 이름을 외우는 문서가 아니라, 커널이 어떤 상태를 만들고 어떤 큐에 무엇을 놓는지 따라가는 문서여야 한다.
이 절에서 따라갈 대상은 SYN, SYN-ACK, ACK, SYN backlog, accept queue entry이며, 이 대상은 TCP/IP stack, routing table, NAT table, socket buffer, proxy upstream 연결 사이를 지난다.
각 계층은 자기보다 위의 HTTP 의미를 모른 채 자기 계약만 지킨다.
서버 운영자는 이 분리를 알아야 `connection refused`, `timeout`, `CLOSE_WAIT`, `TIME_WAIT`, `SSL handshake failure`, `502`, `504` 같은 증상을 보고 어느 계층부터 확인할지 정할 수 있다.

#### 값이 움직이는 순서

아래 trace는 이 절의 주제를 `부하 순간 새 연결이 몰리고 일부 클라이언트가 connect timeout 또는 refused를 보는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `TCP 3-way handshake, backlog, accept queue`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `backlog를 HTTP request queue로만 설명하는 경우`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`TCP 3-way handshake, backlog, accept queue`의 공식 자료 고정점은 [RFC 9293 TCP](https://www.rfc-editor.org/rfc/rfc9293), [Linux tcp(7)](https://www.man7.org/linux/man-pages/man7/tcp.7.html), [Nginx reverse proxy](https://docs.nginx.com/nginx/admin-guide/web-server/reverse-proxy), [RFC 8446 TLS 1.3](https://www.rfc-editor.org/rfc/rfc8446)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `SYN, SYN-ACK, ACK, SYN backlog, accept queue entry`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`SYN, SYN-ACK, ACK, SYN backlog, accept queue entry`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`TCP 3-way handshake, backlog, accept queue`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`TCP 3-way handshake, backlog, accept queue`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `SYN, SYN-ACK, ACK, SYN backlog, accept queue entry`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `backlog를 HTTP request queue로만 설명하는 경우`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
backlog를 HTTP request queue로만 설명하는 경우는 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `SYN, SYN-ACK, ACK, SYN backlog, accept queue entry`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `SYN, SYN-ACK, ACK, SYN backlog, accept queue entry`를 먼저 말하고, 그다음 `3-way handshake는 커널 TCP 스택의 연결 상태를 맞추는 절차이고, backlog와 accept queue는 그 연결을 애플리케이션이 가져가기 전까지 보관하는 완충 지점이다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `부하 순간 새 연결이 몰리고 일부 클라이언트가 connect timeout 또는 refused를 보는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 SYN, SYN-ACK, ACK, SYN backlog, accept queue entry 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`TCP 3-way handshake, backlog, accept queue`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `부하 순간 새 연결이 몰리고 일부 클라이언트가 connect timeout 또는 refused를 보는 장면`에서는 SYN, SYN-ACK, ACK, SYN backlog, accept queue entry 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `backlog를 HTTP request queue로만 설명하는 경우`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
이 절의 주제는 `TCP 상태: ESTABLISHED, CLOSE_WAIT, TIME_WAIT`이다. TCP 상태는 커널이 각 연결의 생애를 추적하는 이름이고, 상태마다 누가 다음 close를 해야 하는지가 다르다. 기준 장면은 `운영 서버에서 `ss -ant` 결과 CLOSE_WAIT 또는 TIME_WAIT가 많이 보이는 장면`이다. 여기서는 다음 대상을 따라간다: TCP state machine entry, FIN, ACK, socket close ownership. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


네트워크 실행 경로는 프로토콜 이름을 외우는 문서가 아니라, 커널이 어떤 상태를 만들고 어떤 큐에 무엇을 놓는지 따라가는 문서여야 한다.
이 절에서 따라갈 대상은 TCP state machine entry, FIN, ACK, socket close ownership이며, 이 대상은 TCP/IP stack, routing table, NAT table, socket buffer, proxy upstream 연결 사이를 지난다.
각 계층은 자기보다 위의 HTTP 의미를 모른 채 자기 계약만 지킨다.
서버 운영자는 이 분리를 알아야 `connection refused`, `timeout`, `CLOSE_WAIT`, `TIME_WAIT`, `SSL handshake failure`, `502`, `504` 같은 증상을 보고 어느 계층부터 확인할지 정할 수 있다.

#### 값이 움직이는 순서

아래 trace는 이 절의 주제를 `운영 서버에서 `ss -ant` 결과 CLOSE_WAIT 또는 TIME_WAIT가 많이 보이는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `TCP 상태: ESTABLISHED, CLOSE_WAIT, TIME_WAIT`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `TIME_WAIT가 보이면 무조건 장애라고 보거나 CLOSE_WAIT를 네트워크 장비 탓으로 돌리는 설명`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`TCP 상태: ESTABLISHED, CLOSE_WAIT, TIME_WAIT`의 공식 자료 고정점은 [RFC 9293 TCP](https://www.rfc-editor.org/rfc/rfc9293), [Linux tcp(7)](https://www.man7.org/linux/man-pages/man7/tcp.7.html), [Nginx reverse proxy](https://docs.nginx.com/nginx/admin-guide/web-server/reverse-proxy), [RFC 8446 TLS 1.3](https://www.rfc-editor.org/rfc/rfc8446)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `TCP state machine entry, FIN, ACK, socket close ownership`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`TCP state machine entry, FIN, ACK, socket close ownership`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`TCP 상태: ESTABLISHED, CLOSE_WAIT, TIME_WAIT`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`TCP 상태: ESTABLISHED, CLOSE_WAIT, TIME_WAIT`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `TCP state machine entry, FIN, ACK, socket close ownership`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `TIME_WAIT가 보이면 무조건 장애라고 보거나 CLOSE_WAIT를 네트워크 장비 탓으로 돌리는 설명`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
TIME_WAIT가 보이면 무조건 장애라고 보거나 CLOSE_WAIT를 네트워크 장비 탓으로 돌리는 설명은 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `TCP state machine entry, FIN, ACK, socket close ownership`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `TCP state machine entry, FIN, ACK, socket close ownership`를 먼저 말하고, 그다음 `TCP 상태는 커널이 각 연결의 생애를 추적하는 이름이고, 상태마다 누가 다음 close를 해야 하는지가 다르다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `운영 서버에서 `ss -ant` 결과 CLOSE_WAIT 또는 TIME_WAIT가 많이 보이는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 TCP state machine entry, FIN, ACK, socket close ownership 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`TCP 상태: ESTABLISHED, CLOSE_WAIT, TIME_WAIT`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `운영 서버에서 `ss -ant` 결과 CLOSE_WAIT 또는 TIME_WAIT가 많이 보이는 장면`에서는 TCP state machine entry, FIN, ACK, socket close ownership 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `TIME_WAIT가 보이면 무조건 장애라고 보거나 CLOSE_WAIT를 네트워크 장비 탓으로 돌리는 설명`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
이 절의 주제는 `TLS와 HTTP keep-alive`이다. keep-alive는 HTTP 요청을 하나 더 보내기 위해 이미 만든 TCP/TLS 연결을 살려 두는 운영 선택이고, handshake 비용과 TIME_WAIT 부담을 줄인다. 기준 장면은 `클라이언트가 같은 서버에 여러 API를 호출하면서 TCP/TLS 연결을 재사용하는 장면`이다. 여기서는 다음 대상을 따라간다: TCP connection, TLS session, HTTP request/response pair, idle keep-alive socket. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


네트워크 실행 경로는 프로토콜 이름을 외우는 문서가 아니라, 커널이 어떤 상태를 만들고 어떤 큐에 무엇을 놓는지 따라가는 문서여야 한다.
이 절에서 따라갈 대상은 TCP connection, TLS session, HTTP request/response pair, idle keep-alive socket이며, 이 대상은 TCP/IP stack, routing table, NAT table, socket buffer, proxy upstream 연결 사이를 지난다.
각 계층은 자기보다 위의 HTTP 의미를 모른 채 자기 계약만 지킨다.
서버 운영자는 이 분리를 알아야 `connection refused`, `timeout`, `CLOSE_WAIT`, `TIME_WAIT`, `SSL handshake failure`, `502`, `504` 같은 증상을 보고 어느 계층부터 확인할지 정할 수 있다.

#### 값이 움직이는 순서

아래 trace는 이 절의 주제를 `클라이언트가 같은 서버에 여러 API를 호출하면서 TCP/TLS 연결을 재사용하는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `TLS와 HTTP keep-alive`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `TLS handshake와 TCP 4-way close를 같은 종류의 handshake로 설명하는 경우`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`TLS와 HTTP keep-alive`의 공식 자료 고정점은 [RFC 9293 TCP](https://www.rfc-editor.org/rfc/rfc9293), [Linux tcp(7)](https://www.man7.org/linux/man-pages/man7/tcp.7.html), [Nginx reverse proxy](https://docs.nginx.com/nginx/admin-guide/web-server/reverse-proxy), [RFC 8446 TLS 1.3](https://www.rfc-editor.org/rfc/rfc8446)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `TCP connection, TLS session, HTTP request/response pair, idle keep-alive socket`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`TCP connection, TLS session, HTTP request/response pair, idle keep-alive socket`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`TLS와 HTTP keep-alive`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`TLS와 HTTP keep-alive`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `TCP connection, TLS session, HTTP request/response pair, idle keep-alive socket`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `TLS handshake와 TCP 4-way close를 같은 종류의 handshake로 설명하는 경우`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
TLS handshake와 TCP 4-way close를 같은 종류의 handshake로 설명하는 경우는 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `TCP connection, TLS session, HTTP request/response pair, idle keep-alive socket`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `TCP connection, TLS session, HTTP request/response pair, idle keep-alive socket`를 먼저 말하고, 그다음 `keep-alive는 HTTP 요청을 하나 더 보내기 위해 이미 만든 TCP/TLS 연결을 살려 두는 운영 선택이고, handshake 비용과 TIME_WAIT 부담을 줄인다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `클라이언트가 같은 서버에 여러 API를 호출하면서 TCP/TLS 연결을 재사용하는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 TCP connection, TLS session, HTTP request/response pair, idle keep-alive socket 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`TLS와 HTTP keep-alive`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `클라이언트가 같은 서버에 여러 API를 호출하면서 TCP/TLS 연결을 재사용하는 장면`에서는 TCP connection, TLS session, HTTP request/response pair, idle keep-alive socket 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `TLS handshake와 TCP 4-way close를 같은 종류의 handshake로 설명하는 경우`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
이 절의 주제는 `Nginx, L4/L7 load balancer, reverse proxy`이다. L4는 보통 연결과 주소/포트 수준에서 분산하고, L7 reverse proxy는 HTTP 메시지를 이해해 header, path, buffering, upstream 정책을 바꾼다. 기준 장면은 `ALB가 TLS를 종료하고 Nginx가 upstream Tomcat으로 요청을 보내는 장면`이다. 여기서는 다음 대상을 따라간다: client TCP 연결, proxy HTTP request, upstream TCP 연결, response buffering. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


네트워크 실행 경로는 프로토콜 이름을 외우는 문서가 아니라, 커널이 어떤 상태를 만들고 어떤 큐에 무엇을 놓는지 따라가는 문서여야 한다.
이 절에서 따라갈 대상은 client TCP 연결, proxy HTTP request, upstream TCP 연결, response buffering이며, 이 대상은 TCP/IP stack, routing table, NAT table, socket buffer, proxy upstream 연결 사이를 지난다.
각 계층은 자기보다 위의 HTTP 의미를 모른 채 자기 계약만 지킨다.
서버 운영자는 이 분리를 알아야 `connection refused`, `timeout`, `CLOSE_WAIT`, `TIME_WAIT`, `SSL handshake failure`, `502`, `504` 같은 증상을 보고 어느 계층부터 확인할지 정할 수 있다.

#### 값이 움직이는 순서

아래 trace는 이 절의 주제를 `ALB가 TLS를 종료하고 Nginx가 upstream Tomcat으로 요청을 보내는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `Nginx, L4/L7 load balancer, reverse proxy`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `로드밸런서와 reverse proxy를 모두 단순 port forward로 설명하는 경우`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`Nginx, L4/L7 load balancer, reverse proxy`의 공식 자료 고정점은 [RFC 9293 TCP](https://www.rfc-editor.org/rfc/rfc9293), [Linux tcp(7)](https://www.man7.org/linux/man-pages/man7/tcp.7.html), [Nginx reverse proxy](https://docs.nginx.com/nginx/admin-guide/web-server/reverse-proxy), [RFC 8446 TLS 1.3](https://www.rfc-editor.org/rfc/rfc8446)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `client TCP 연결, proxy HTTP request, upstream TCP 연결, response buffering`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`client TCP 연결, proxy HTTP request, upstream TCP 연결, response buffering`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`Nginx, L4/L7 load balancer, reverse proxy`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`Nginx, L4/L7 load balancer, reverse proxy`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `client TCP 연결, proxy HTTP request, upstream TCP 연결, response buffering`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `로드밸런서와 reverse proxy를 모두 단순 port forward로 설명하는 경우`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
로드밸런서와 reverse proxy를 모두 단순 port forward로 설명하는 경우는 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `client TCP 연결, proxy HTTP request, upstream TCP 연결, response buffering`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `client TCP 연결, proxy HTTP request, upstream TCP 연결, response buffering`를 먼저 말하고, 그다음 `L4는 보통 연결과 주소/포트 수준에서 분산하고, L7 reverse proxy는 HTTP 메시지를 이해해 header, path, buffering, upstream 정책을 바꾼다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `ALB가 TLS를 종료하고 Nginx가 upstream Tomcat으로 요청을 보내는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
이 절의 기준 장면인 `ALB가 TLS를 종료하고 Nginx가 upstream Tomcat으로 요청을 보내는 장면`에서는 client TCP 연결, proxy HTTP request, upstream TCP 연결, response buffering 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `로드밸런서와 reverse proxy를 모두 단순 port forward로 설명하는 경우`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 client TCP 연결, proxy HTTP request, upstream TCP 연결, response buffering 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.
...
이 절의 주제는 `패킷이 NIC에서 애플리케이션 버퍼까지 가는 길`이다. NIC는 HTTP를 모르고, 커널은 TCP byte stream을 복원하며, 애플리케이션은 read 계열 호출로 가능한 byte를 가져온다. 기준 장면은 `NIC가 받은 frame이 커널 네트워크 스택을 지나 Java 프로세스의 buffer로 복사되는 장면`이다. 여기서는 다음 대상을 따라간다: Ethernet frame, sk_buff, TCP segment, socket receive buffer, ByteBuffer. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


네트워크 실행 경로는 프로토콜 이름을 외우는 문서가 아니라, 커널이 어떤 상태를 만들고 어떤 큐에 무엇을 놓는지 따라가는 문서여야 한다.
이 절에서 따라갈 대상은 Ethernet frame, sk_buff, TCP segment, socket receive buffer, ByteBuffer이며, 이 대상은 TCP/IP stack, routing table, NAT table, socket buffer, proxy upstream 연결 사이를 지난다.
각 계층은 자기보다 위의 HTTP 의미를 모른 채 자기 계약만 지킨다.
서버 운영자는 이 분리를 알아야 `connection refused`, `timeout`, `CLOSE_WAIT`, `TIME_WAIT`, `SSL handshake failure`, `502`, `504` 같은 증상을 보고 어느 계층부터 확인할지 정할 수 있다.

#### 값이 움직이는 순서

아래 trace는 이 절의 주제를 `NIC가 받은 frame이 커널 네트워크 스택을 지나 Java 프로세스의 buffer로 복사되는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `패킷이 NIC에서 애플리케이션 버퍼까지 가는 길`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `packet 하나가 Java 객체 하나로 바로 바뀐다고 설명하는 경우`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`패킷이 NIC에서 애플리케이션 버퍼까지 가는 길`의 공식 자료 고정점은 [RFC 9293 TCP](https://www.rfc-editor.org/rfc/rfc9293), [Linux tcp(7)](https://www.man7.org/linux/man-pages/man7/tcp.7.html), [Nginx reverse proxy](https://docs.nginx.com/nginx/admin-guide/web-server/reverse-proxy), [RFC 8446 TLS 1.3](https://www.rfc-editor.org/rfc/rfc8446)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `Ethernet frame, sk_buff, TCP segment, socket receive buffer, ByteBuffer`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`Ethernet frame, sk_buff, TCP segment, socket receive buffer, ByteBuffer`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`패킷이 NIC에서 애플리케이션 버퍼까지 가는 길`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`패킷이 NIC에서 애플리케이션 버퍼까지 가는 길`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `Ethernet frame, sk_buff, TCP segment, socket receive buffer, ByteBuffer`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `packet 하나가 Java 객체 하나로 바로 바뀐다고 설명하는 경우`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
packet 하나가 Java 객체 하나로 바로 바뀐다고 설명하는 경우는 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `Ethernet frame, sk_buff, TCP segment, socket receive buffer, ByteBuffer`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `Ethernet frame, sk_buff, TCP segment, socket receive buffer, ByteBuffer`를 먼저 말하고, 그다음 `NIC는 HTTP를 모르고, 커널은 TCP byte stream을 복원하며, 애플리케이션은 read 계열 호출로 가능한 byte를 가져온다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `NIC가 받은 frame이 커널 네트워크 스택을 지나 Java 프로세스의 buffer로 복사되는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 Ethernet frame, sk_buff, TCP segment, socket receive buffer, ByteBuffer 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`패킷이 NIC에서 애플리케이션 버퍼까지 가는 길`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `NIC가 받은 frame이 커널 네트워크 스택을 지나 Java 프로세스의 buffer로 복사되는 장면`에서는 Ethernet frame, sk_buff, TCP segment, socket receive buffer, ByteBuffer 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `packet 하나가 Java 객체 하나로 바로 바뀐다고 설명하는 경우`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
이 절의 주제는 `요청 수신 경로와 응답 송신 경로는 커널 안에서 어떻게 다르게 보이는가`이다. 수신은 NIC에서 socket receive buffer로 쌓이고 애플리케이션이 당겨 가는 흐름이며, 송신은 애플리케이션이 send buffer에 밀어 넣고 커널이 전송 가능할 때 내보내는 흐름이다. 기준 장면은 `클라이언트 upload는 느린데 서버 response도 동시에 나가야 하는 장면`이다. 여기서는 다음 대상을 따라간다: receive queue, user read, user write, send queue, congestion/window update. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


네트워크 실행 경로는 프로토콜 이름을 외우는 문서가 아니라, 커널이 어떤 상태를 만들고 어떤 큐에 무엇을 놓는지 따라가는 문서여야 한다.
이 절에서 따라갈 대상은 receive queue, user read, user write, send queue, congestion/window update이며, 이 대상은 TCP/IP stack, routing table, NAT table, socket buffer, proxy upstream 연결 사이를 지난다.
각 계층은 자기보다 위의 HTTP 의미를 모른 채 자기 계약만 지킨다.
서버 운영자는 이 분리를 알아야 `connection refused`, `timeout`, `CLOSE_WAIT`, `TIME_WAIT`, `SSL handshake failure`, `502`, `504` 같은 증상을 보고 어느 계층부터 확인할지 정할 수 있다.

#### 값이 움직이는 순서

아래 trace는 이 절의 주제를 `클라이언트 upload는 느린데 서버 response도 동시에 나가야 하는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `요청 수신 경로와 응답 송신 경로는 커널 안에서 어떻게 다르게 보이는가`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `read와 write가 같은 큐 하나를 공유한다고 보는 설명`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`요청 수신 경로와 응답 송신 경로는 커널 안에서 어떻게 다르게 보이는가`의 공식 자료 고정점은 [RFC 9293 TCP](https://www.rfc-editor.org/rfc/rfc9293), [Linux tcp(7)](https://www.man7.org/linux/man-pages/man7/tcp.7.html), [Nginx reverse proxy](https://docs.nginx.com/nginx/admin-guide/web-server/reverse-proxy), [RFC 8446 TLS 1.3](https://www.rfc-editor.org/rfc/rfc8446)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `receive queue, user read, user write, send queue, congestion/window update`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`receive queue, user read, user write, send queue, congestion/window update`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`요청 수신 경로와 응답 송신 경로는 커널 안에서 어떻게 다르게 보이는가`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`요청 수신 경로와 응답 송신 경로는 커널 안에서 어떻게 다르게 보이는가`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `receive queue, user read, user write, send queue, congestion/window update`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `read와 write가 같은 큐 하나를 공유한다고 보는 설명`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
read와 write가 같은 큐 하나를 공유한다고 보는 설명은 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `receive queue, user read, user write, send queue, congestion/window update`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `receive queue, user read, user write, send queue, congestion/window update`를 먼저 말하고, 그다음 `수신은 NIC에서 socket receive buffer로 쌓이고 애플리케이션이 당겨 가는 흐름이며, 송신은 애플리케이션이 send buffer에 밀어 넣고 커널이 전송 가능할 때 내보내는 흐름이다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `클라이언트 upload는 느린데 서버 response도 동시에 나가야 하는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 receive queue, user read, user write, send queue, congestion/window update 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`요청 수신 경로와 응답 송신 경로는 커널 안에서 어떻게 다르게 보이는가`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `클라이언트 upload는 느린데 서버 response도 동시에 나가야 하는 장면`에서는 receive queue, user read, user write, send queue, congestion/window update 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `read와 write가 같은 큐 하나를 공유한다고 보는 설명`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
이 절의 주제는 `방화벽, security group, routing 문제는 connection refused와 timeout을 어떻게 갈라놓는가`이다. refused는 상대 커널이나 중간 장치가 거절 신호를 돌려준 쪽에 가깝고, timeout은 응답이 오지 않아 재시도 끝에 포기하는 쪽에 가깝다. 기준 장면은 `배포 후 새 서버 포트가 열리지 않아 어떤 클라이언트는 refused, 어떤 클라이언트는 timeout을 보는 장면`이다. 여기서는 다음 대상을 따라간다: SYN, RST, ICMP, dropped packet, route lookup result. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


네트워크 실행 경로는 프로토콜 이름을 외우는 문서가 아니라, 커널이 어떤 상태를 만들고 어떤 큐에 무엇을 놓는지 따라가는 문서여야 한다.
이 절에서 따라갈 대상은 SYN, RST, ICMP, dropped packet, route lookup result이며, 이 대상은 TCP/IP stack, routing table, NAT table, socket buffer, proxy upstream 연결 사이를 지난다.
각 계층은 자기보다 위의 HTTP 의미를 모른 채 자기 계약만 지킨다.
서버 운영자는 이 분리를 알아야 `connection refused`, `timeout`, `CLOSE_WAIT`, `TIME_WAIT`, `SSL handshake failure`, `502`, `504` 같은 증상을 보고 어느 계층부터 확인할지 정할 수 있다.

#### 값이 움직이는 순서

아래 trace는 이 절의 주제를 `배포 후 새 서버 포트가 열리지 않아 어떤 클라이언트는 refused, 어떤 클라이언트는 timeout을 보는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `방화벽, security group, routing 문제는 connection refused와 timeout을 어떻게 갈라놓는가`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `connection refused와 timeout을 같은 네트워크 불안정으로 묶는 설명`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`방화벽, security group, routing 문제는 connection refused와 timeout을 어떻게 갈라놓는가`의 공식 자료 고정점은 [RFC 9293 TCP](https://www.rfc-editor.org/rfc/rfc9293), [Linux tcp(7)](https://www.man7.org/linux/man-pages/man7/tcp.7.html), [Nginx reverse proxy](https://docs.nginx.com/nginx/admin-guide/web-server/reverse-proxy), [RFC 8446 TLS 1.3](https://www.rfc-editor.org/rfc/rfc8446)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `SYN, RST, ICMP, dropped packet, route lookup result`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`SYN, RST, ICMP, dropped packet, route lookup result`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`방화벽, security group, routing 문제는 connection refused와 timeout을 어떻게 갈라놓는가`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`방화벽, security group, routing 문제는 connection refused와 timeout을 어떻게 갈라놓는가`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `SYN, RST, ICMP, dropped packet, route lookup result`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `connection refused와 timeout을 같은 네트워크 불안정으로 묶는 설명`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
connection refused와 timeout을 같은 네트워크 불안정으로 묶는 설명은 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `SYN, RST, ICMP, dropped packet, route lookup result`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `SYN, RST, ICMP, dropped packet, route lookup result`를 먼저 말하고, 그다음 `refused는 상대 커널이나 중간 장치가 거절 신호를 돌려준 쪽에 가깝고, timeout은 응답이 오지 않아 재시도 끝에 포기하는 쪽에 가깝다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `배포 후 새 서버 포트가 열리지 않아 어떤 클라이언트는 refused, 어떤 클라이언트는 timeout을 보는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 SYN, RST, ICMP, dropped packet, route lookup result 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`방화벽, security group, routing 문제는 connection refused와 timeout을 어떻게 갈라놓는가`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `배포 후 새 서버 포트가 열리지 않아 어떤 클라이언트는 refused, 어떤 클라이언트는 timeout을 보는 장면`에서는 SYN, RST, ICMP, dropped packet, route lookup result 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `connection refused와 timeout을 같은 네트워크 불안정으로 묶는 설명`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
이 절의 주제는 `select와 poll은 왜 한계가 생겼는가`이다. select와 poll은 여러 fd를 기다릴 수 있게 했지만, 현대적인 대규모 연결 서버에서는 fd 집합 복사와 전체 scan, API 한계가 비용이 된다. 기준 장면은 `연결 수가 수만 개로 늘어났는데 매 loop마다 관심 fd 전체를 다시 훑는 서버 장면`이다. 여기서는 다음 대상을 따라간다: fd_set, pollfd array, watched descriptor list. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


이벤트 모델을 이해할 때 가장 먼저 버려야 할 오해는 이벤트 API가 데이터를 대신 읽어 준다는 생각이다.
이 절에서 따라갈 대상은 fd_set, pollfd array, watched descriptor list이며, 이 대상은 준비 상태 또는 완료 상태를 알려 주는 신호이지 애플리케이션 프로토콜 경계를 대신 판단하는 값이 아니다.
...
아래 trace는 이 절의 주제를 `연결 수가 수만 개로 늘어났는데 매 loop마다 관심 fd 전체를 다시 훑는 서버 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `select와 poll은 왜 한계가 생겼는가`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `select/poll이 느린 이유를 단순히 오래된 API라서라고만 설명하는 경우`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`select와 poll은 왜 한계가 생겼는가`의 공식 자료 고정점은 [Linux select(2)](https://man7.org/linux/man-pages/man2/select.2.html), [Linux poll(2)](https://man7.org/linux/man-pages/man2/poll.2.html), [Linux epoll(7)](https://www.man7.org/linux/man-pages/man7/epoll.7.html), [Linux io_uring(7)](https://man7.org/linux/man-pages/man7/io_uring.7.html), [FreeBSD kqueue(2)](https://man.freebsd.org/kqueue%282%29)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `fd_set, pollfd array, watched descriptor list`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`fd_set, pollfd array, watched descriptor list`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`select와 poll은 왜 한계가 생겼는가`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`select와 poll은 왜 한계가 생겼는가`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `fd_set, pollfd array, watched descriptor list`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `select/poll이 느린 이유를 단순히 오래된 API라서라고만 설명하는 경우`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
select/poll이 느린 이유를 단순히 오래된 API라서라고만 설명하는 경우는 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `fd_set, pollfd array, watched descriptor list`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `fd_set, pollfd array, watched descriptor list`를 먼저 말하고, 그다음 `select와 poll은 여러 fd를 기다릴 수 있게 했지만, 현대적인 대규모 연결 서버에서는 fd 집합 복사와 전체 scan, API 한계가 비용이 된다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `연결 수가 수만 개로 늘어났는데 매 loop마다 관심 fd 전체를 다시 훑는 서버 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 fd_set, pollfd array, watched descriptor list 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`select와 poll은 왜 한계가 생겼는가`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `연결 수가 수만 개로 늘어났는데 매 loop마다 관심 fd 전체를 다시 훑는 서버 장면`에서는 fd_set, pollfd array, watched descriptor list 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `select/poll이 느린 이유를 단순히 오래된 API라서라고만 설명하는 경우`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
이 절의 주제는 `epoll은 readiness를 어떤 방식으로 관리하는가`이다. epoll은 관심 fd를 커널 안 epoll instance에 등록해 두고, 준비된 fd 목록을 기다리는 Linux 전용 readiness 모델이다. 기준 장면은 `Nginx worker나 Netty event loop가 많은 socket fd 중 읽을 수 있는 fd만 받아 처리하는 장면`이다. 여기서는 다음 대상을 따라간다: epoll instance, interest list, ready list, epoll_wait result. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


이벤트 모델을 이해할 때 가장 먼저 버려야 할 오해는 이벤트 API가 데이터를 대신 읽어 준다는 생각이다.
이 절에서 따라갈 대상은 epoll instance, interest list, ready list, epoll_wait result이며, 이 대상은 준비 상태 또는 완료 상태를 알려 주는 신호이지 애플리케이션 프로토콜 경계를 대신 판단하는 값이 아니다.
readiness 기반 모델에서는 커널이 "지금 읽거나 쓸 수 있다"고 알려 주고, 애플리케이션이 실제 read/write를 수행한다.
completion 기반 모델에서는 애플리케이션이 "이 read/write를 해 달라"고 작업을 제출하고, 나중에 완료 결과를 받는다.
두 모델의 차이는 API 이름보다 소유권 차이에 있다.
누가 작업을 시작하고, 누가 반복하며, 누가 완료와 오류를 책임지는지가 달라진다.
...
아래 trace는 이 절의 주제를 `Nginx worker나 Netty event loop가 많은 socket fd 중 읽을 수 있는 fd만 받아 처리하는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `epoll은 readiness를 어떤 방식으로 관리하는가`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `epoll_wait가 byte를 읽어 준다고 설명하는 경우`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`epoll은 readiness를 어떤 방식으로 관리하는가`의 공식 자료 고정점은 [Linux select(2)](https://man7.org/linux/man-pages/man2/select.2.html), [Linux poll(2)](https://man7.org/linux/man-pages/man2/poll.2.html), [Linux epoll(7)](https://www.man7.org/linux/man-pages/man7/epoll.7.html), [Linux io_uring(7)](https://man7.org/linux/man-pages/man7/io_uring.7.html), [FreeBSD kqueue(2)](https://man.freebsd.org/kqueue%282%29)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `epoll instance, interest list, ready list, epoll_wait result`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`epoll instance, interest list, ready list, epoll_wait result`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`epoll은 readiness를 어떤 방식으로 관리하는가`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`epoll은 readiness를 어떤 방식으로 관리하는가`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `epoll instance, interest list, ready list, epoll_wait result`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `epoll_wait가 byte를 읽어 준다고 설명하는 경우`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
epoll_wait가 byte를 읽어 준다고 설명하는 경우는 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `epoll instance, interest list, ready list, epoll_wait result`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `epoll instance, interest list, ready list, epoll_wait result`를 먼저 말하고, 그다음 `epoll은 관심 fd를 커널 안 epoll instance에 등록해 두고, 준비된 fd 목록을 기다리는 Linux 전용 readiness 모델이다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `Nginx worker나 Netty event loop가 많은 socket fd 중 읽을 수 있는 fd만 받아 처리하는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
이 절의 기준 장면인 `Nginx worker나 Netty event loop가 많은 socket fd 중 읽을 수 있는 fd만 받아 처리하는 장면`에서는 epoll instance, interest list, ready list, epoll_wait result 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `epoll_wait가 byte를 읽어 준다고 설명하는 경우`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 epoll instance, interest list, ready list, epoll_wait result 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.
...
이 절의 주제는 `kqueue는 epoll과 어떤 관점에서 비교해야 하는가`이다. kqueue는 BSD 계열의 kernel event queue이고, socket뿐 아니라 vnode, signal, process 같은 다양한 filter를 같은 이벤트 큐에 올릴 수 있다. 기준 장면은 `macOS에서 Java NIO나 Netty native transport를 볼 때 epoll이 아니라 kqueue/kevent가 관측되는 장면`이다. 여기서는 다음 대상을 따라간다: kqueue fd, kevent filter, EVFILT_READ/WRITE event. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


이벤트 모델을 이해할 때 가장 먼저 버려야 할 오해는 이벤트 API가 데이터를 대신 읽어 준다는 생각이다.
이 절에서 따라갈 대상은 kqueue fd, kevent filter, EVFILT_READ/WRITE event이며, 이 대상은 준비 상태 또는 완료 상태를 알려 주는 신호이지 애플리케이션 프로토콜 경계를 대신 판단하는 값이 아니다.
readiness 기반 모델에서는 커널이 "지금 읽거나 쓸 수 있다"고 알려 주고, 애플리케이션이 실제 read/write를 수행한다.
completion 기반 모델에서는 애플리케이션이 "이 read/write를 해 달라"고 작업을 제출하고, 나중에 완료 결과를 받는다.
두 모델의 차이는 API 이름보다 소유권 차이에 있다.
누가 작업을 시작하고, 누가 반복하며, 누가 완료와 오류를 책임지는지가 달라진다.
...
이 절은 Linux 기본 경로가 아니라 BSD/macOS 비교 경로를 따로 떼어 보는 부분이다.
...
이 절의 목적은 kqueue를 Linux 기능처럼 소개하는 것이 아니라, Java NIO와 Netty가 운영체제별 커널 이벤트 모델 위에 올라갈 때 어떤 부분이 공통 추상화이고 어떤 부분이 플랫폼 고유 구현인지 구분하는 것이다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `Linux에서 kqueue 설정을 찾거나, macOS에서 epoll fdinfo를 찾는 설명`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`kqueue는 epoll과 어떤 관점에서 비교해야 하는가`의 공식 자료 고정점은 [Linux select(2)](https://man7.org/linux/man-pages/man2/select.2.html), [Linux poll(2)](https://man7.org/linux/man-pages/man2/poll.2.html), [Linux epoll(7)](https://www.man7.org/linux/man-pages/man7/epoll.7.html), [Linux io_uring(7)](https://man7.org/linux/man-pages/man7/io_uring.7.html), [FreeBSD kqueue(2)](https://man.freebsd.org/kqueue%282%29)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `kqueue fd, kevent filter, EVFILT_READ/WRITE event`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`kqueue fd, kevent filter, EVFILT_READ/WRITE event`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
따라서 이 절의 재현 명령은 Linux production 서버에 그대로 붙이는 명령이 아니라 macOS/BSD 개발 환경에서 플랫폼 차이를 확인하는 명령으로 읽어야 한다.
...
PASS 신호는 `kqueue` 또는 `kevent` 호출이 BSD/macOS 커널 이벤트 큐를 통해 준비 상태를 돌려주는 모습을 확인하는 것이다.
Linux 환경에서는 같은 목적을 `epoll_wait`와 `/proc/<pid>/fdinfo/<epoll-fd>` 같은 Linux 전용 표면으로 확인한다.
FAIL 신호는 두 플랫폼의 관측 표면을 섞는 경우다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
Linux에서 kqueue 설정을 찾거나, macOS에서 epoll fdinfo를 찾는 설명은 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `kqueue fd, kevent filter, EVFILT_READ/WRITE event`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `kqueue fd, kevent filter, EVFILT_READ/WRITE event`를 먼저 말하고, 그다음 `kqueue는 BSD 계열의 kernel event queue이고, socket뿐 아니라 vnode, signal, process 같은 다양한 filter를 같은 이벤트 큐에 올릴 수 있다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `macOS에서 Java NIO나 Netty native transport를 볼 때 epoll이 아니라 kqueue/kevent가 관측되는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 kqueue fd, kevent filter, EVFILT_READ/WRITE event 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`kqueue는 epoll과 어떤 관점에서 비교해야 하는가`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `macOS에서 Java NIO나 Netty native transport를 볼 때 epoll이 아니라 kqueue/kevent가 관측되는 장면`에서는 kqueue fd, kevent filter, EVFILT_READ/WRITE event 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `Linux에서 kqueue 설정을 찾거나, macOS에서 epoll fdinfo를 찾는 설명`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
이 절의 주제는 `io_uring은 readiness가 아니라 completion을 어떻게 다루는가`이다. io_uring은 fd가 준비됐는지 묻는 대신 수행할 I/O 작업을 제출하고 완료 항목을 회수하는 모델이다. 기준 장면은 `애플리케이션이 read 요청 자체를 큐에 제출하고 완료 결과를 나중에 받는 장면`이다. 여기서는 다음 대상을 따라간다: SQE, submission queue, kernel operation, CQE. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


이벤트 모델을 이해할 때 가장 먼저 버려야 할 오해는 이벤트 API가 데이터를 대신 읽어 준다는 생각이다.
이 절에서 따라갈 대상은 SQE, submission queue, kernel operation, CQE이며, 이 대상은 준비 상태 또는 완료 상태를 알려 주는 신호이지 애플리케이션 프로토콜 경계를 대신 판단하는 값이 아니다.
readiness 기반 모델에서는 커널이 "지금 읽거나 쓸 수 있다"고 알려 주고, 애플리케이션이 실제 read/write를 수행한다.
completion 기반 모델에서는 애플리케이션이 "이 read/write를 해 달라"고 작업을 제출하고, 나중에 완료 결과를 받는다.
두 모델의 차이는 API 이름보다 소유권 차이에 있다.
누가 작업을 시작하고, 누가 반복하며, 누가 완료와 오류를 책임지는지가 달라진다.
...
아래 trace는 이 절의 주제를 `애플리케이션이 read 요청 자체를 큐에 제출하고 완료 결과를 나중에 받는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `io_uring은 readiness가 아니라 completion을 어떻게 다루는가`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `io_uring을 epoll의 빠른 버전으로만 설명하는 경우`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`io_uring은 readiness가 아니라 completion을 어떻게 다루는가`의 공식 자료 고정점은 [Linux select(2)](https://man7.org/linux/man-pages/man2/select.2.html), [Linux poll(2)](https://man7.org/linux/man-pages/man2/poll.2.html), [Linux epoll(7)](https://www.man7.org/linux/man-pages/man7/epoll.7.html), [Linux io_uring(7)](https://man7.org/linux/man-pages/man7/io_uring.7.html), [FreeBSD kqueue(2)](https://man.freebsd.org/kqueue%282%29)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `SQE, submission queue, kernel operation, CQE`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`SQE, submission queue, kernel operation, CQE`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`io_uring은 readiness가 아니라 completion을 어떻게 다루는가`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`io_uring은 readiness가 아니라 completion을 어떻게 다루는가`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `SQE, submission queue, kernel operation, CQE`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `io_uring을 epoll의 빠른 버전으로만 설명하는 경우`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
io_uring을 epoll의 빠른 버전으로만 설명하는 경우는 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `SQE, submission queue, kernel operation, CQE`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `SQE, submission queue, kernel operation, CQE`를 먼저 말하고, 그다음 `io_uring은 fd가 준비됐는지 묻는 대신 수행할 I/O 작업을 제출하고 완료 항목을 회수하는 모델이다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `애플리케이션이 read 요청 자체를 큐에 제출하고 완료 결과를 나중에 받는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 SQE, submission queue, kernel operation, CQE 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`io_uring은 readiness가 아니라 completion을 어떻게 다루는가`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `애플리케이션이 read 요청 자체를 큐에 제출하고 완료 결과를 나중에 받는 장면`에서는 SQE, submission queue, kernel operation, CQE 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `io_uring을 epoll의 빠른 버전으로만 설명하는 경우`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
이 절의 주제는 `readiness 기반 모델과 completion 기반 모델은 서버 설계를 어떻게 바꾸는가`이다. readiness 모델은 '지금 해도 되는가'를 받고 사용자가 직접 수행하며, completion 모델은 '이 일을 해 달라'고 맡기고 완료를 받는다. 기준 장면은 `같은 파일 전송 서버를 epoll 스타일과 io_uring 스타일로 설계할 때 코드가 달라지는 장면`이다. 여기서는 다음 대상을 따라간다: ready fd, user read loop, submitted operation, completion result. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


이벤트 모델을 이해할 때 가장 먼저 버려야 할 오해는 이벤트 API가 데이터를 대신 읽어 준다는 생각이다.
이 절에서 따라갈 대상은 ready fd, user read loop, submitted operation, completion result이며, 이 대상은 준비 상태 또는 완료 상태를 알려 주는 신호이지 애플리케이션 프로토콜 경계를 대신 판단하는 값이 아니다.
readiness 기반 모델에서는 커널이 "지금 읽거나 쓸 수 있다"고 알려 주고, 애플리케이션이 실제 read/write를 수행한다.
completion 기반 모델에서는 애플리케이션이 "이 read/write를 해 달라"고 작업을 제출하고, 나중에 완료 결과를 받는다.
두 모델의 차이는 API 이름보다 소유권 차이에 있다.
누가 작업을 시작하고, 누가 반복하며, 누가 완료와 오류를 책임지는지가 달라진다.
...
아래 trace는 이 절의 주제를 `같은 파일 전송 서버를 epoll 스타일과 io_uring 스타일로 설계할 때 코드가 달라지는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `readiness 기반 모델과 completion 기반 모델은 서버 설계를 어떻게 바꾸는가`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `모델 차이를 성능 숫자 하나로만 비교하고 코드 소유권과 backpressure 지점을 놓치는 설명`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`readiness 기반 모델과 completion 기반 모델은 서버 설계를 어떻게 바꾸는가`의 공식 자료 고정점은 [Linux select(2)](https://man7.org/linux/man-pages/man2/select.2.html), [Linux poll(2)](https://man7.org/linux/man-pages/man2/poll.2.html), [Linux epoll(7)](https://www.man7.org/linux/man-pages/man7/epoll.7.html), [Linux io_uring(7)](https://man7.org/linux/man-pages/man7/io_uring.7.html), [FreeBSD kqueue(2)](https://man.freebsd.org/kqueue%282%29)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `ready fd, user read loop, submitted operation, completion result`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`ready fd, user read loop, submitted operation, completion result`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`readiness 기반 모델과 completion 기반 모델은 서버 설계를 어떻게 바꾸는가`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`readiness 기반 모델과 completion 기반 모델은 서버 설계를 어떻게 바꾸는가`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `ready fd, user read loop, submitted operation, completion result`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `모델 차이를 성능 숫자 하나로만 비교하고 코드 소유권과 backpressure 지점을 놓치는 설명`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
모델 차이를 성능 숫자 하나로만 비교하고 코드 소유권과 backpressure 지점을 놓치는 설명은 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `ready fd, user read loop, submitted operation, completion result`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `ready fd, user read loop, submitted operation, completion result`를 먼저 말하고, 그다음 `readiness 모델은 '지금 해도 되는가'를 받고 사용자가 직접 수행하며, completion 모델은 '이 일을 해 달라'고 맡기고 완료를 받는다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `같은 파일 전송 서버를 epoll 스타일과 io_uring 스타일로 설계할 때 코드가 달라지는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 ready fd, user read loop, submitted operation, completion result 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`readiness 기반 모델과 completion 기반 모델은 서버 설계를 어떻게 바꾸는가`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `같은 파일 전송 서버를 epoll 스타일과 io_uring 스타일로 설계할 때 코드가 달라지는 장면`에서는 ready fd, user read loop, submitted operation, completion result 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `모델 차이를 성능 숫자 하나로만 비교하고 코드 소유권과 backpressure 지점을 놓치는 설명`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
이 절의 주제는 `Java NIO는 blocking socket 모델과 무엇이 다른가`이다. Java NIO는 socket을 non-blocking channel로 바꾸고 selector에 등록해 여러 연결의 준비 상태를 한 흐름에서 다룰 수 있게 한다. 기준 장면은 `스레드 하나가 여러 SocketChannel의 OP_READ readiness를 처리하는 echo server 장면`이다. 여기서는 다음 대상을 따라간다: SocketChannel, Selector, SelectionKey, ByteBuffer. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


JVM 런타임 계층은 Linux fd readiness를 Java 객체와 callback, signal, worker thread로 번역한다.
이 절에서 따라갈 대상은 SocketChannel, Selector, SelectionKey, ByteBuffer이며, 이 대상은 커널에서 직접 WebFlux handler로 순간 이동하지 않는다.
...
아래 trace는 이 절의 주제를 `스레드 하나가 여러 SocketChannel의 OP_READ readiness를 처리하는 echo server 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `Java NIO는 blocking socket 모델과 무엇이 다른가`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `NIO를 Java가 자동으로 비동기 작업을 끝까지 수행해 주는 모델로 설명하는 경우`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`Java NIO는 blocking socket 모델과 무엇이 다른가`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`Java NIO는 blocking socket 모델과 무엇이 다른가`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `SocketChannel, Selector, SelectionKey, ByteBuffer`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `NIO를 Java가 자동으로 비동기 작업을 끝까지 수행해 주는 모델로 설명하는 경우`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
NIO를 Java가 자동으로 비동기 작업을 끝까지 수행해 주는 모델로 설명하는 경우는 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `SocketChannel, Selector, SelectionKey, ByteBuffer`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `SocketChannel, Selector, SelectionKey, ByteBuffer`를 먼저 말하고, 그다음 `Java NIO는 socket을 non-blocking channel로 바꾸고 selector에 등록해 여러 연결의 준비 상태를 한 흐름에서 다룰 수 있게 한다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `스레드 하나가 여러 SocketChannel의 OP_READ readiness를 처리하는 echo server 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 SocketChannel, Selector, SelectionKey, ByteBuffer 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`Java NIO는 blocking socket 모델과 무엇이 다른가`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `스레드 하나가 여러 SocketChannel의 OP_READ readiness를 처리하는 echo server 장면`에서는 SocketChannel, Selector, SelectionKey, ByteBuffer 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `NIO를 Java가 자동으로 비동기 작업을 끝까지 수행해 주는 모델로 설명하는 경우`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
이 절의 주제는 `Tomcat thread-per-request 모델과 Netty event loop 모델은 어디서 갈라지는가`이다. 전통적 Servlet MVC는 요청 처리 흐름이 worker thread를 점유하는 쪽이고, Netty는 event loop가 readiness event를 받아 handler chain으로 흘려보내는 쪽이다. 기준 장면은 `같은 HTTP 요청을 Spring MVC와 WebFlux/Reactor Netty로 처리할 때 스레드 점유 방식이 갈라지는 장면`이다. 여기서는 다음 대상을 따라간다: Tomcat worker thread, request object, Netty EventLoop, Channel event. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


JVM 런타임 계층은 Linux fd readiness를 Java 객체와 callback, signal, worker thread로 번역한다.
이 절에서 따라갈 대상은 Tomcat worker thread, request object, Netty EventLoop, Channel event이며, 이 대상은 커널에서 직접 WebFlux handler로 순간 이동하지 않는다.
Java NIO Selector가 준비된 channel을 알려 주고, Netty EventLoop가 그 channel event를 처리하며, ChannelPipeline이 byte를 HTTP object나 application message로 바꾸고, Reactor는 그 흐름을 Publisher/Subscriber 계약으로 감싼다.
Spring WebFlux는 이 계약 위에 controller와 handler adapter를 올린다.
이 경로를 알면 "WebFlux니까 빠르다"가 아니라 "event loop를 막지 않고, upstream/downstream도 backpressure를 존중할 때 연결 수와 I/O 대기 비용을 줄인다"고 말할 수 있다.
...
아래 trace는 이 절의 주제를 `같은 HTTP 요청을 Spring MVC와 WebFlux/Reactor Netty로 처리할 때 스레드 점유 방식이 갈라지는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `Tomcat thread-per-request 모델과 Netty event loop 모델은 어디서 갈라지는가`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `Netty를 쓰면 비즈니스 로직이 자동으로 병렬 처리된다고 설명하는 경우`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`Tomcat thread-per-request 모델과 Netty event loop 모델은 어디서 갈라지는가`의 공식 자료 고정점은 [Java NIO channels](https://docs.oracle.com/en/java/javase/24/docs/api/java.base/java/nio/channels/package-summary.html), [Netty user guide](https://netty.io/wiki/user-guide-for-4.x.html), [Reactor backpressure](https://projectreactor.io/docs/core/release/reference/coreFeatures/simple-ways-to-create-a-flux-or-mono-and-subscribe-to-it.html), [Spring WebFlux](https://docs.spring.io/spring-framework/reference/web/webflux.html)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `Tomcat worker thread, request object, Netty EventLoop, Channel event`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`Tomcat worker thread, request object, Netty EventLoop, Channel event`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`Tomcat thread-per-request 모델과 Netty event loop 모델은 어디서 갈라지는가`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`Tomcat thread-per-request 모델과 Netty event loop 모델은 어디서 갈라지는가`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `Tomcat worker thread, request object, Netty EventLoop, Channel event`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `Netty를 쓰면 비즈니스 로직이 자동으로 병렬 처리된다고 설명하는 경우`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
Netty를 쓰면 비즈니스 로직이 자동으로 병렬 처리된다고 설명하는 경우는 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `Tomcat worker thread, request object, Netty EventLoop, Channel event`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `Tomcat worker thread, request object, Netty EventLoop, Channel event`를 먼저 말하고, 그다음 `전통적 Servlet MVC는 요청 처리 흐름이 worker thread를 점유하는 쪽이고, Netty는 event loop가 readiness event를 받아 handler chain으로 흘려보내는 쪽이다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `같은 HTTP 요청을 Spring MVC와 WebFlux/Reactor Netty로 처리할 때 스레드 점유 방식이 갈라지는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
이 절의 기준 장면인 `같은 HTTP 요청을 Spring MVC와 WebFlux/Reactor Netty로 처리할 때 스레드 점유 방식이 갈라지는 장면`에서는 Tomcat worker thread, request object, Netty EventLoop, Channel event 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `Netty를 쓰면 비즈니스 로직이 자동으로 병렬 처리된다고 설명하는 경우`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 Tomcat worker thread, request object, Netty EventLoop, Channel event 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

### Netty는 selector, event loop, channel, pipeline을 어떻게 조립하는가

이 절의 주제는 `Netty는 selector, event loop, channel, pipeline을 어떻게 조립하는가`이다. Netty는 Java NIO/네이티브 transport의 준비 상태 처리를 EventLoop와 Channel 추상화로 묶고, 애플리케이션 프로토콜 처리는 Pipeline의 handler에게 맡긴다. 기준 장면은 `boss event loop가 accept하고 worker event loop가 channelRead를 pipeline으로 전달하는 장면`이다. 여기서는 다음 대상을 따라간다: ServerChannel, child Channel, EventLoop, ByteBuf, ChannelPipeline event. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


JVM 런타임 계층은 Linux fd readiness를 Java 객체와 callback, signal, worker thread로 번역한다.
이 절에서 따라갈 대상은 ServerChannel, child Channel, EventLoop, ByteBuf, ChannelPipeline event이며, 이 대상은 커널에서 직접 WebFlux handler로 순간 이동하지 않는다.
Java NIO Selector가 준비된 channel을 알려 주고, Netty EventLoop가 그 channel event를 처리하며, ChannelPipeline이 byte를 HTTP object나 application message로 바꾸고, Reactor는 그 흐름을 Publisher/Subscriber 계약으로 감싼다.
Spring WebFlux는 이 계약 위에 controller와 handler adapter를 올린다.
이 경로를 알면 "WebFlux니까 빠르다"가 아니라 "event loop를 막지 않고, upstream/downstream도 backpressure를 존중할 때 연결 수와 I/O 대기 비용을 줄인다"고 말할 수 있다.
...
아래 trace는 이 절의 주제를 `boss event loop가 accept하고 worker event loop가 channelRead를 pipeline으로 전달하는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `Netty는 selector, event loop, channel, pipeline을 어떻게 조립하는가`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `ChannelPipeline을 단순 interceptor 목록으로만 보고 ByteBuf 생애와 framing 책임을 놓치는 설명`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`Netty는 selector, event loop, channel, pipeline을 어떻게 조립하는가`의 공식 자료 고정점은 [Java NIO channels](https://docs.oracle.com/en/java/javase/24/docs/api/java.base/java/nio/channels/package-summary.html), [Netty user guide](https://netty.io/wiki/user-guide-for-4.x.html), [Reactor backpressure](https://projectreactor.io/docs/core/release/reference/coreFeatures/simple-ways-to-create-a-flux-or-mono-and-subscribe-to-it.html), [Spring WebFlux](https://docs.spring.io/spring-framework/reference/web/webflux.html)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `ServerChannel, child Channel, EventLoop, ByteBuf, ChannelPipeline event`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`ServerChannel, child Channel, EventLoop, ByteBuf, ChannelPipeline event`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`Netty는 selector, event loop, channel, pipeline을 어떻게 조립하는가`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`Netty는 selector, event loop, channel, pipeline을 어떻게 조립하는가`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `ServerChannel, child Channel, EventLoop, ByteBuf, ChannelPipeline event`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `ChannelPipeline을 단순 interceptor 목록으로만 보고 ByteBuf 생애와 framing 책임을 놓치는 설명`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
ChannelPipeline을 단순 interceptor 목록으로만 보고 ByteBuf 생애와 framing 책임을 놓치는 설명은 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `ServerChannel, child Channel, EventLoop, ByteBuf, ChannelPipeline event`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `ServerChannel, child Channel, EventLoop, ByteBuf, ChannelPipeline event`를 먼저 말하고, 그다음 `Netty는 Java NIO/네이티브 transport의 준비 상태 처리를 EventLoop와 Channel 추상화로 묶고, 애플리케이션 프로토콜 처리는 Pipeline의 handler에게 맡긴다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `boss event loop가 accept하고 worker event loop가 channelRead를 pipeline으로 전달하는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 ServerChannel, child Channel, EventLoop, ByteBuf, ChannelPipeline event 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`Netty는 selector, event loop, channel, pipeline을 어떻게 조립하는가`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `boss event loop가 accept하고 worker event loop가 channelRead를 pipeline으로 전달하는 장면`에서는 ServerChannel, child Channel, EventLoop, ByteBuf, ChannelPipeline event 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `ChannelPipeline을 단순 interceptor 목록으로만 보고 ByteBuf 생애와 framing 책임을 놓치는 설명`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.

### Reactor는 Publisher, Subscriber, backpressure를 어떤 계약으로 묶는가

이 절의 주제는 `Reactor는 Publisher, Subscriber, backpressure를 어떤 계약으로 묶는가`이다. Reactor의 핵심은 데이터가 마음대로 밀려오는 것이 아니라 subscription과 request 신호로 생산량을 조절한다는 계약이다. 기준 장면은 `느린 downstream subscriber가 upstream에게 한 번에 1개만 요청하는 장면`이다. 여기서는 다음 대상을 따라간다: Subscription.request(n), onNext signal, demand counter. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


JVM 런타임 계층은 Linux fd readiness를 Java 객체와 callback, signal, worker thread로 번역한다.
이 절에서 따라갈 대상은 Subscription.request(n), onNext signal, demand counter이며, 이 대상은 커널에서 직접 WebFlux handler로 순간 이동하지 않는다.
Java NIO Selector가 준비된 channel을 알려 주고, Netty EventLoop가 그 channel event를 처리하며, ChannelPipeline이 byte를 HTTP object나 application message로 바꾸고, Reactor는 그 흐름을 Publisher/Subscriber 계약으로 감싼다.
Spring WebFlux는 이 계약 위에 controller와 handler adapter를 올린다.
이 경로를 알면 "WebFlux니까 빠르다"가 아니라 "event loop를 막지 않고, upstream/downstream도 backpressure를 존중할 때 연결 수와 I/O 대기 비용을 줄인다"고 말할 수 있다.
...
아래 trace는 이 절의 주제를 `느린 downstream subscriber가 upstream에게 한 번에 1개만 요청하는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `Reactor는 Publisher, Subscriber, backpressure를 어떤 계약으로 묶는가`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `Flux를 반환하면 TCP와 DB까지 모두 자동으로 속도 조절된다고 말하는 경우`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`Reactor는 Publisher, Subscriber, backpressure를 어떤 계약으로 묶는가`의 공식 자료 고정점은 [Java NIO channels](https://docs.oracle.com/en/java/javase/24/docs/api/java.base/java/nio/channels/package-summary.html), [Netty user guide](https://netty.io/wiki/user-guide-for-4.x.html), [Reactor backpressure](https://projectreactor.io/docs/core/release/reference/coreFeatures/simple-ways-to-create-a-flux-or-mono-and-subscribe-to-it.html), [Spring WebFlux](https://docs.spring.io/spring-framework/reference/web/webflux.html)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `Subscription.request(n), onNext signal, demand counter`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`Subscription.request(n), onNext signal, demand counter`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`Reactor는 Publisher, Subscriber, backpressure를 어떤 계약으로 묶는가`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`Reactor는 Publisher, Subscriber, backpressure를 어떤 계약으로 묶는가`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `Subscription.request(n), onNext signal, demand counter`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `Flux를 반환하면 TCP와 DB까지 모두 자동으로 속도 조절된다고 말하는 경우`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
Flux를 반환하면 TCP와 DB까지 모두 자동으로 속도 조절된다고 말하는 경우는 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `Subscription.request(n), onNext signal, demand counter`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `Subscription.request(n), onNext signal, demand counter`를 먼저 말하고, 그다음 `Reactor의 핵심은 데이터가 마음대로 밀려오는 것이 아니라 subscription과 request 신호로 생산량을 조절한다는 계약이다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `느린 downstream subscriber가 upstream에게 한 번에 1개만 요청하는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 Subscription.request(n), onNext signal, demand counter이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`Reactor는 Publisher, Subscriber, backpressure를 어떤 계약으로 묶는가`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `느린 downstream subscriber가 upstream에게 한 번에 1개만 요청하는 장면`에서는 Subscription.request(n), onNext signal, demand counter이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `Flux를 반환하면 TCP와 DB까지 모두 자동으로 속도 조절된다고 말하는 경우`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
이 절의 주제는 `Spring WebFlux는 Servlet MVC와 요청 처리 모델이 어떻게 다른가`이다. Spring WebFlux는 Servlet API 전용 MVC와 달리 reactive adapter와 non-blocking I/O 경계를 중심으로 요청 처리를 구성한다. 기준 장면은 `컨트롤러가 `Mono<Response>`를 반환하고 실제 응답 쓰기가 subscriber demand와 event loop 위에서 이어지는 장면`이다. 여기서는 다음 대상을 따라간다: DispatcherServlet request, DispatcherHandler exchange, Mono/Flux signal. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


JVM 런타임 계층은 Linux fd readiness를 Java 객체와 callback, signal, worker thread로 번역한다.
이 절에서 따라갈 대상은 DispatcherServlet request, DispatcherHandler exchange, Mono/Flux signal이며, 이 대상은 커널에서 직접 WebFlux handler로 순간 이동하지 않는다.
Java NIO Selector가 준비된 channel을 알려 주고, Netty EventLoop가 그 channel event를 처리하며, ChannelPipeline이 byte를 HTTP object나 application message로 바꾸고, Reactor는 그 흐름을 Publisher/Subscriber 계약으로 감싼다.
Spring WebFlux는 이 계약 위에 controller와 handler adapter를 올린다.
이 경로를 알면 "WebFlux니까 빠르다"가 아니라 "event loop를 막지 않고, upstream/downstream도 backpressure를 존중할 때 연결 수와 I/O 대기 비용을 줄인다"고 말할 수 있다.
...
아래 trace는 이 절의 주제를 `컨트롤러가 `Mono<Response>`를 반환하고 실제 응답 쓰기가 subscriber demand와 event loop 위에서 이어지는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `Spring WebFlux는 Servlet MVC와 요청 처리 모델이 어떻게 다른가`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `WebFlux 컨트롤러 안에서 blocking JDBC를 호출하면서 전체를 non-blocking이라고 부르는 설명`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`Spring WebFlux는 Servlet MVC와 요청 처리 모델이 어떻게 다른가`의 공식 자료 고정점은 [Java NIO channels](https://docs.oracle.com/en/java/javase/24/docs/api/java.base/java/nio/channels/package-summary.html), [Netty user guide](https://netty.io/wiki/user-guide-for-4.x.html), [Reactor backpressure](https://projectreactor.io/docs/core/release/reference/coreFeatures/simple-ways-to-create-a-flux-or-mono-and-subscribe-to-it.html), [Spring WebFlux](https://docs.spring.io/spring-framework/reference/web/webflux.html)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `DispatcherServlet request, DispatcherHandler exchange, Mono/Flux signal`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`DispatcherServlet request, DispatcherHandler exchange, Mono/Flux signal`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`Spring WebFlux는 Servlet MVC와 요청 처리 모델이 어떻게 다른가`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`Spring WebFlux는 Servlet MVC와 요청 처리 모델이 어떻게 다른가`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `DispatcherServlet request, DispatcherHandler exchange, Mono/Flux signal`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `WebFlux 컨트롤러 안에서 blocking JDBC를 호출하면서 전체를 non-blocking이라고 부르는 설명`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
WebFlux 컨트롤러 안에서 blocking JDBC를 호출하면서 전체를 non-blocking이라고 부르는 설명은 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `DispatcherServlet request, DispatcherHandler exchange, Mono/Flux signal`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `DispatcherServlet request, DispatcherHandler exchange, Mono/Flux signal`를 먼저 말하고, 그다음 `Spring WebFlux는 Servlet API 전용 MVC와 달리 reactive adapter와 non-blocking I/O 경계를 중심으로 요청 처리를 구성한다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `컨트롤러가 `Mono<Response>`를 반환하고 실제 응답 쓰기가 subscriber demand와 event loop 위에서 이어지는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 DispatcherServlet request, DispatcherHandler exchange, Mono/Flux signal 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`Spring WebFlux는 Servlet MVC와 요청 처리 모델이 어떻게 다른가`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `컨트롤러가 `Mono<Response>`를 반환하고 실제 응답 쓰기가 subscriber demand와 event loop 위에서 이어지는 장면`에서는 DispatcherServlet request, DispatcherHandler exchange, Mono/Flux signal 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `WebFlux 컨트롤러 안에서 blocking JDBC를 호출하면서 전체를 non-blocking이라고 부르는 설명`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
이 절의 주제는 `WebFlux, Reactor, Netty는 Linux I/O 모델 위에서 어떻게 이어지는가`이다. Linux는 fd readiness를 알려 주고, Netty는 그 이벤트를 channel/pipeline으로 바꾸며, Reactor는 signal과 demand 계약을 제공하고, WebFlux는 HTTP handler 모델을 올린다. 기준 장면은 `client socket에 읽을 byte가 생기고, epoll_wait 결과가 Netty event loop를 깨우며, WebFlux handler가 Mono/Flux로 응답을 만드는 장면`이다. 여기서는 다음 대상을 따라간다: Linux epoll event, Netty Channel event, Reactor signal, WebFlux handler result. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


JVM 런타임 계층은 Linux fd readiness를 Java 객체와 callback, signal, worker thread로 번역한다.
이 절에서 따라갈 대상은 Linux epoll event, Netty Channel event, Reactor signal, WebFlux handler result이며, 이 대상은 커널에서 직접 WebFlux handler로 순간 이동하지 않는다.
Java NIO Selector가 준비된 channel을 알려 주고, Netty EventLoop가 그 channel event를 처리하며, ChannelPipeline이 byte를 HTTP object나 application message로 바꾸고, Reactor는 그 흐름을 Publisher/Subscriber 계약으로 감싼다.
Spring WebFlux는 이 계약 위에 controller와 handler adapter를 올린다.
이 경로를 알면 "WebFlux니까 빠르다"가 아니라 "event loop를 막지 않고, upstream/downstream도 backpressure를 존중할 때 연결 수와 I/O 대기 비용을 줄인다"고 말할 수 있다.
...
아래 trace는 이 절의 주제를 `client socket에 읽을 byte가 생기고, epoll_wait 결과가 Netty event loop를 깨우며, WebFlux handler가 Mono/Flux로 응답을 만드는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `WebFlux, Reactor, Netty는 Linux I/O 모델 위에서 어떻게 이어지는가`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `Reactor가 epoll을 직접 호출한다고 설명하거나 WebFlux가 커널 버퍼를 직접 제어한다고 설명하는 경우`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`WebFlux, Reactor, Netty는 Linux I/O 모델 위에서 어떻게 이어지는가`의 공식 자료 고정점은 [Java NIO channels](https://docs.oracle.com/en/java/javase/24/docs/api/java.base/java/nio/channels/package-summary.html), [Netty user guide](https://netty.io/wiki/user-guide-for-4.x.html), [Reactor backpressure](https://projectreactor.io/docs/core/release/reference/coreFeatures/simple-ways-to-create-a-flux-or-mono-and-subscribe-to-it.html), [Spring WebFlux](https://docs.spring.io/spring-framework/reference/web/webflux.html)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `Linux epoll event, Netty Channel event, Reactor signal, WebFlux handler result`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`Linux epoll event, Netty Channel event, Reactor signal, WebFlux handler result`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`WebFlux, Reactor, Netty는 Linux I/O 모델 위에서 어떻게 이어지는가`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`WebFlux, Reactor, Netty는 Linux I/O 모델 위에서 어떻게 이어지는가`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `Linux epoll event, Netty Channel event, Reactor signal, WebFlux handler result`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `Reactor가 epoll을 직접 호출한다고 설명하거나 WebFlux가 커널 버퍼를 직접 제어한다고 설명하는 경우`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
Reactor가 epoll을 직접 호출한다고 설명하거나 WebFlux가 커널 버퍼를 직접 제어한다고 설명하는 경우는 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `Linux epoll event, Netty Channel event, Reactor signal, WebFlux handler result`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `Linux epoll event, Netty Channel event, Reactor signal, WebFlux handler result`를 먼저 말하고, 그다음 `Linux는 fd readiness를 알려 주고, Netty는 그 이벤트를 channel/pipeline으로 바꾸며, Reactor는 signal과 demand 계약을 제공하고, WebFlux는 HTTP handler 모델을 올린다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `client socket에 읽을 byte가 생기고, epoll_wait 결과가 Netty event loop를 깨우며, WebFlux handler가 Mono/Flux로 응답을 만드는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 Linux epoll event, Netty Channel event, Reactor signal, WebFlux handler result 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`WebFlux, Reactor, Netty는 Linux I/O 모델 위에서 어떻게 이어지는가`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `client socket에 읽을 byte가 생기고, epoll_wait 결과가 Netty event loop를 깨우며, WebFlux handler가 Mono/Flux로 응답을 만드는 장면`에서는 Linux epoll event, Netty Channel event, Reactor signal, WebFlux handler result 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `Reactor가 epoll을 직접 호출한다고 설명하거나 WebFlux가 커널 버퍼를 직접 제어한다고 설명하는 경우`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
이 절의 주제는 `HTTP streaming은 응답을 한 번에 만들지 않고 어떻게 흘려보내는가`이다. streaming은 응답 객체 전체를 완성한 뒤 한 번에 쓰는 대신, header를 먼저 보내고 body 조각을 생산되는 순서대로 흘려보내는 방식이다. 기준 장면은 `서버가 SSE나 NDJSON을 한 줄씩 만들어 client에게 계속 보내는 장면`이다. 여기서는 다음 대상을 따라간다: response header, chunk, flush, TCP send buffer, client read. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


스트리밍과 backpressure는 "응답을 조금씩 보낸다"는 표면만으로는 부족하다.
이 절에서 따라갈 대상은 response header, chunk, flush, TCP send buffer, client read이며, 이 대상은 애플리케이션 buffer, proxy buffer, kernel send buffer, TCP window, client read 속도 사이에서 계속 압력을 주고받는다.
...
아래 trace는 이 절의 주제를 `서버가 SSE나 NDJSON을 한 줄씩 만들어 client에게 계속 보내는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `HTTP streaming은 응답을 한 번에 만들지 않고 어떻게 흘려보내는가`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `Flux를 반환했다는 이유만으로 client가 즉시 조각을 받는다고 단정하는 설명`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`HTTP streaming은 응답을 한 번에 만들지 않고 어떻게 흘려보내는가`의 공식 자료 고정점은 [RFC 9110 HTTP Semantics](https://www.rfc-editor.org/rfc/rfc9110), [Nginx reverse proxy](https://docs.nginx.com/nginx/admin-guide/web-server/reverse-proxy), [Nginx proxy_buffering](https://nginx.org/en/docs/http/ngx_http_proxy_module.html#proxy_buffering), [Reactor backpressure](https://projectreactor.io/docs/core/release/reference/coreFeatures/simple-ways-to-create-a-flux-or-mono-and-subscribe-to-it.html), [RFC 9293 TCP](https://www.rfc-editor.org/rfc/rfc9293)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `response header, chunk, flush, TCP send buffer, client read`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`response header, chunk, flush, TCP send buffer, client read`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`HTTP streaming은 응답을 한 번에 만들지 않고 어떻게 흘려보내는가`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`HTTP streaming은 응답을 한 번에 만들지 않고 어떻게 흘려보내는가`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `response header, chunk, flush, TCP send buffer, client read`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `Flux를 반환했다는 이유만으로 client가 즉시 조각을 받는다고 단정하는 설명`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
Flux를 반환했다는 이유만으로 client가 즉시 조각을 받는다고 단정하는 설명은 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `response header, chunk, flush, TCP send buffer, client read`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `response header, chunk, flush, TCP send buffer, client read`를 먼저 말하고, 그다음 `streaming은 응답 객체 전체를 완성한 뒤 한 번에 쓰는 대신, header를 먼저 보내고 body 조각을 생산되는 순서대로 흘려보내는 방식이다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `서버가 SSE나 NDJSON을 한 줄씩 만들어 client에게 계속 보내는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
이 절의 기준 장면인 `서버가 SSE나 NDJSON을 한 줄씩 만들어 client에게 계속 보내는 장면`에서는 response header, chunk, flush, TCP send buffer, client read 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `Flux를 반환했다는 이유만으로 client가 즉시 조각을 받는다고 단정하는 설명`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 response header, chunk, flush, TCP send buffer, client read 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.
...
이 절의 주제는 `100GB 파일 스트리밍은 page cache, TCP window, backpressure를 어떻게 지나간다`이다. 큰 파일 전송의 핵심은 파일 전체를 heap에 올리지 않고, 커널 page cache와 socket send buffer, TCP window의 압력에 맞춰 조금씩 보내는 것이다. 기준 장면은 `서버가 100GB 백업 파일을 HTTP로 내려 주고 client가 느리게 받는 장면`이다. 여기서는 다음 대상을 따라간다: file page, page cache entry, send buffer, TCP receive window. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


스트리밍과 backpressure는 "응답을 조금씩 보낸다"는 표면만으로는 부족하다.
이 절에서 따라갈 대상은 file page, page cache entry, send buffer, TCP receive window이며, 이 대상은 애플리케이션 buffer, proxy buffer, kernel send buffer, TCP window, client read 속도 사이에서 계속 압력을 주고받는다.
서버가 조각을 만들었다고 client가 곧바로 본다는 보장은 없다.
프록시가 응답을 모을 수 있고, 커널 send buffer가 찰 수 있으며, 느린 client의 receive window가 줄면 송신 쪽 전송이 멈출 수 있다.
따라서 streaming 설명은 HTTP API 모양과 운영 경로를 함께 봐야 한다.
...
아래 trace는 이 절의 주제를 `서버가 100GB 백업 파일을 HTTP로 내려 주고 client가 느리게 받는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `100GB 파일 스트리밍은 page cache, TCP window, backpressure를 어떻게 지나간다`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `streaming이면 메모리를 전혀 쓰지 않는다고 설명하거나, page cache를 애플리케이션 heap과 섞는 설명`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`100GB 파일 스트리밍은 page cache, TCP window, backpressure를 어떻게 지나간다`의 공식 자료 고정점은 [RFC 9110 HTTP Semantics](https://www.rfc-editor.org/rfc/rfc9110), [Nginx reverse proxy](https://docs.nginx.com/nginx/admin-guide/web-server/reverse-proxy), [Nginx proxy_buffering](https://nginx.org/en/docs/http/ngx_http_proxy_module.html#proxy_buffering), [Reactor backpressure](https://projectreactor.io/docs/core/release/reference/coreFeatures/simple-ways-to-create-a-flux-or-mono-and-subscribe-to-it.html), [RFC 9293 TCP](https://www.rfc-editor.org/rfc/rfc9293)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `file page, page cache entry, send buffer, TCP receive window`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`file page, page cache entry, send buffer, TCP receive window`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`100GB 파일 스트리밍은 page cache, TCP window, backpressure를 어떻게 지나간다`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`100GB 파일 스트리밍은 page cache, TCP window, backpressure를 어떻게 지나간다`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `file page, page cache entry, send buffer, TCP receive window`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `streaming이면 메모리를 전혀 쓰지 않는다고 설명하거나, page cache를 애플리케이션 heap과 섞는 설명`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
streaming이면 메모리를 전혀 쓰지 않는다고 설명하거나, page cache를 애플리케이션 heap과 섞는 설명은 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `file page, page cache entry, send buffer, TCP receive window`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `file page, page cache entry, send buffer, TCP receive window`를 먼저 말하고, 그다음 `큰 파일 전송의 핵심은 파일 전체를 heap에 올리지 않고, 커널 page cache와 socket send buffer, TCP window의 압력에 맞춰 조금씩 보내는 것이다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `서버가 100GB 백업 파일을 HTTP로 내려 주고 client가 느리게 받는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 file page, page cache entry, send buffer, TCP receive window 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`100GB 파일 스트리밍은 page cache, TCP window, backpressure를 어떻게 지나간다`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `서버가 100GB 백업 파일을 HTTP로 내려 주고 client가 느리게 받는 장면`에서는 file page, page cache entry, send buffer, TCP receive window 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `streaming이면 메모리를 전혀 쓰지 않는다고 설명하거나, page cache를 애플리케이션 heap과 섞는 설명`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
이 절의 주제는 `proxy buffering은 streaming 응답의 의미를 어떻게 바꿀 수 있는가`이다. proxy buffering이 켜져 있으면 upstream이 보낸 조각이 client에게 즉시 보이지 않을 수 있고, 서버 관측과 client 관측이 갈라진다. 기준 장면은 `Spring WebFlux SSE는 1초마다 emit하지만 Nginx 뒤 client는 한참 뒤 묶어서 받는 장면`이다. 여기서는 다음 대상을 따라간다: upstream response chunk, Nginx proxy buffer, temporary file, client-facing chunk. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


스트리밍과 backpressure는 "응답을 조금씩 보낸다"는 표면만으로는 부족하다.
이 절에서 따라갈 대상은 upstream response chunk, Nginx proxy buffer, temporary file, client-facing chunk이며, 이 대상은 애플리케이션 buffer, proxy buffer, kernel send buffer, TCP window, client read 속도 사이에서 계속 압력을 주고받는다.
서버가 조각을 만들었다고 client가 곧바로 본다는 보장은 없다.
프록시가 응답을 모을 수 있고, 커널 send buffer가 찰 수 있으며, 느린 client의 receive window가 줄면 송신 쪽 전송이 멈출 수 있다.
따라서 streaming 설명은 HTTP API 모양과 운영 경로를 함께 봐야 한다.
...
아래 trace는 이 절의 주제를 `Spring WebFlux SSE는 1초마다 emit하지만 Nginx 뒤 client는 한참 뒤 묶어서 받는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `proxy buffering은 streaming 응답의 의미를 어떻게 바꿀 수 있는가`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `애플리케이션 로그의 emit 시각을 client 수신 시각으로 착각하는 설명`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `upstream response chunk, Nginx proxy buffer, temporary file, client-facing chunk`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`upstream response chunk, Nginx proxy buffer, temporary file, client-facing chunk`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`proxy buffering은 streaming 응답의 의미를 어떻게 바꿀 수 있는가`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`proxy buffering은 streaming 응답의 의미를 어떻게 바꿀 수 있는가`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `upstream response chunk, Nginx proxy buffer, temporary file, client-facing chunk`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `애플리케이션 로그의 emit 시각을 client 수신 시각으로 착각하는 설명`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
애플리케이션 로그의 emit 시각을 client 수신 시각으로 착각하는 설명은 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `upstream response chunk, Nginx proxy buffer, temporary file, client-facing chunk`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `upstream response chunk, Nginx proxy buffer, temporary file, client-facing chunk`를 먼저 말하고, 그다음 `proxy buffering이 켜져 있으면 upstream이 보낸 조각이 client에게 즉시 보이지 않을 수 있고, 서버 관측과 client 관측이 갈라진다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `Spring WebFlux SSE는 1초마다 emit하지만 Nginx 뒤 client는 한참 뒤 묶어서 받는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 upstream response chunk, Nginx proxy buffer, temporary file, client-facing chunk 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`proxy buffering은 streaming 응답의 의미를 어떻게 바꿀 수 있는가`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `Spring WebFlux SSE는 1초마다 emit하지만 Nginx 뒤 client는 한참 뒤 묶어서 받는 장면`에서는 upstream response chunk, Nginx proxy buffer, temporary file, client-facing chunk 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `애플리케이션 로그의 emit 시각을 client 수신 시각으로 착각하는 설명`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
이 절의 주제는 `느린 클라이언트는 서버 스레드, 메모리, 소켓 버퍼에 어떤 압력을 주는가`이다. 느린 client는 송신 쪽 버퍼와 연결 점유 시간을 늘리고, 모델에 따라 worker thread, event loop task queue, proxy buffer, 메모리 사용량을 압박한다. 기준 장면은 `모바일 client가 매우 느린 네트워크에서 대용량 응답을 받는 동안 서버 연결이 오래 유지되는 장면`이다. 여기서는 다음 대상을 따라간다: slow client receive window, server send buffer, worker/event-loop scheduling, heap buffer. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


스트리밍과 backpressure는 "응답을 조금씩 보낸다"는 표면만으로는 부족하다.
이 절에서 따라갈 대상은 slow client receive window, server send buffer, worker/event-loop scheduling, heap buffer이며, 이 대상은 애플리케이션 buffer, proxy buffer, kernel send buffer, TCP window, client read 속도 사이에서 계속 압력을 주고받는다.
서버가 조각을 만들었다고 client가 곧바로 본다는 보장은 없다.
프록시가 응답을 모을 수 있고, 커널 send buffer가 찰 수 있으며, 느린 client의 receive window가 줄면 송신 쪽 전송이 멈출 수 있다.
따라서 streaming 설명은 HTTP API 모양과 운영 경로를 함께 봐야 한다.
...
아래 trace는 이 절의 주제를 `모바일 client가 매우 느린 네트워크에서 대용량 응답을 받는 동안 서버 연결이 오래 유지되는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `느린 클라이언트는 서버 스레드, 메모리, 소켓 버퍼에 어떤 압력을 주는가`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `서버가 빠르게 응답을 생성하면 느린 client가 서버 자원을 더 쓰지 않는다고 보는 설명`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`느린 클라이언트는 서버 스레드, 메모리, 소켓 버퍼에 어떤 압력을 주는가`의 공식 자료 고정점은 [RFC 9110 HTTP Semantics](https://www.rfc-editor.org/rfc/rfc9110), [Nginx reverse proxy](https://docs.nginx.com/nginx/admin-guide/web-server/reverse-proxy), [Nginx proxy_buffering](https://nginx.org/en/docs/http/ngx_http_proxy_module.html#proxy_buffering), [Reactor backpressure](https://projectreactor.io/docs/core/release/reference/coreFeatures/simple-ways-to-create-a-flux-or-mono-and-subscribe-to-it.html), [RFC 9293 TCP](https://www.rfc-editor.org/rfc/rfc9293)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `slow client receive window, server send buffer, worker/event-loop scheduling, heap buffer`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`slow client receive window, server send buffer, worker/event-loop scheduling, heap buffer`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`느린 클라이언트는 서버 스레드, 메모리, 소켓 버퍼에 어떤 압력을 주는가`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`느린 클라이언트는 서버 스레드, 메모리, 소켓 버퍼에 어떤 압력을 주는가`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `slow client receive window, server send buffer, worker/event-loop scheduling, heap buffer`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `서버가 빠르게 응답을 생성하면 느린 client가 서버 자원을 더 쓰지 않는다고 보는 설명`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
서버가 빠르게 응답을 생성하면 느린 client가 서버 자원을 더 쓰지 않는다고 보는 설명은 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `slow client receive window, server send buffer, worker/event-loop scheduling, heap buffer`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `slow client receive window, server send buffer, worker/event-loop scheduling, heap buffer`를 먼저 말하고, 그다음 `느린 client는 송신 쪽 버퍼와 연결 점유 시간을 늘리고, 모델에 따라 worker thread, event loop task queue, proxy buffer, 메모리 사용량을 압박한다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `모바일 client가 매우 느린 네트워크에서 대용량 응답을 받는 동안 서버 연결이 오래 유지되는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 slow client receive window, server send buffer, worker/event-loop scheduling, heap buffer 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`느린 클라이언트는 서버 스레드, 메모리, 소켓 버퍼에 어떤 압력을 주는가`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `모바일 client가 매우 느린 네트워크에서 대용량 응답을 받는 동안 서버 연결이 오래 유지되는 장면`에서는 slow client receive window, server send buffer, worker/event-loop scheduling, heap buffer 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `서버가 빠르게 응답을 생성하면 느린 client가 서버 자원을 더 쓰지 않는다고 보는 설명`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
이 절의 주제는 `장애 분석은 증상에서 커널, 런타임, 애플리케이션, 외부 시스템 중 어디로 좁혀 가는가`이다. 장애 분석은 가장 그럴듯한 원인을 찍는 일이 아니라, 증상을 계층별 신호로 나누고 반증 가능한 순서로 줄여 가는 일이다. 기준 장면은 `API timeout 알림 하나에서 네트워크, JVM, DB, 외부 API 중 원인을 좁히는 장애 대응 장면`이다. 여기서는 다음 대상을 따라간다: 증상, 관측 지표, 계층별 가설, 검증 결과. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


장애 상황에서는 개념을 따로 외우는 방식이 거의 도움이 되지 않는다.
이 절에서 따라갈 대상은 증상, 관측 지표, 계층별 가설, 검증 결과이다.
...
아래 trace는 이 절의 주제를 `API timeout 알림 하나에서 네트워크, JVM, DB, 외부 API 중 원인을 좁히는 장애 대응 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `장애 분석은 증상에서 커널, 런타임, 애플리케이션, 외부 시스템 중 어디로 좁혀 가는가`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `p99 지연을 애플리케이션 로그 하나만 보고 결론 내리는 설명`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`장애 분석은 증상에서 커널, 런타임, 애플리케이션, 외부 시스템 중 어디로 좁혀 가는가`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`장애 분석은 증상에서 커널, 런타임, 애플리케이션, 외부 시스템 중 어디로 좁혀 가는가`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `증상, 관측 지표, 계층별 가설, 검증 결과`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `p99 지연을 애플리케이션 로그 하나만 보고 결론 내리는 설명`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
p99 지연을 애플리케이션 로그 하나만 보고 결론 내리는 설명은 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `증상, 관측 지표, 계층별 가설, 검증 결과`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `증상, 관측 지표, 계층별 가설, 검증 결과`를 먼저 말하고, 그다음 `장애 분석은 가장 그럴듯한 원인을 찍는 일이 아니라, 증상을 계층별 신호로 나누고 반증 가능한 순서로 줄여 가는 일이다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `API timeout 알림 하나에서 네트워크, JVM, DB, 외부 API 중 원인을 좁히는 장애 대응 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 증상, 관측 지표, 계층별 가설, 검증 결과 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`장애 분석은 증상에서 커널, 런타임, 애플리케이션, 외부 시스템 중 어디로 좁혀 가는가`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `API timeout 알림 하나에서 네트워크, JVM, DB, 외부 API 중 원인을 좁히는 장애 대응 장면`에서는 증상, 관측 지표, 계층별 가설, 검증 결과 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `p99 지연을 애플리케이션 로그 하나만 보고 결론 내리는 설명`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
이 절의 주제는 `connection refused와 timeout은 어디가 다른가`이다. refused는 연결 시도에 대해 명시적 거절 신호가 돌아온 쪽이고, timeout은 연결 시도에 대한 충분한 응답을 받지 못한 쪽이다. 기준 장면은 `새 인스턴스 배포 후 health check가 refused를 보거나 timeout을 보는 장면`이다. 여기서는 다음 대상을 따라간다: SYN, RST, dropped SYN, retry timer. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


장애 상황에서는 개념을 따로 외우는 방식이 거의 도움이 되지 않는다.
이 절에서 따라갈 대상은 SYN, RST, dropped SYN, retry timer이다.
그 대상을 시간 순서로 놓고 어느 계층에서 멈췄는지 좁혀야 한다.
커널은 TCP 상태와 fd, cgroup memory, scheduler 상태를 남기고, 런타임은 thread dump와 GC log, event loop delay를 남기며, 애플리케이션은 요청 로그와 business timing을 남긴다.
DB와 외부 API는 query plan, wait event, connection pool, upstream timeout으로 자기 신호를 남긴다.
좋은 장애 분석은 이 신호들을 한 타임라인에 맞춰 보고, 반증 가능한 순서로 가설을 줄인다.
...
아래 trace는 이 절의 주제를 `새 인스턴스 배포 후 health check가 refused를 보거나 timeout을 보는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `connection refused와 timeout은 어디가 다른가`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `둘을 모두 서버 다운으로만 설명하는 경우`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`connection refused와 timeout은 어디가 다른가`의 공식 자료 고정점은 [Linux tcp(7)](https://www.man7.org/linux/man-pages/man7/tcp.7.html), [Linux cgroup v2](https://www.kernel.org/doc/html/latest/admin-guide/cgroup-v2.html), [Reactor Netty reference](https://docs.spring.io/projectreactor/reactor-netty/docs/current/reference/html/), [Tomcat architecture](https://tomcat.apache.org/tomcat-10.1-doc/architecture/overview.html), [PostgreSQL EXPLAIN](https://www.postgresql.org/docs/current/sql-explain.html), [PostgreSQL monitoring stats](https://www.postgresql.org/docs/current/monitoring-stats.html)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `SYN, RST, dropped SYN, retry timer`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`SYN, RST, dropped SYN, retry timer`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`connection refused와 timeout은 어디가 다른가`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`connection refused와 timeout은 어디가 다른가`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `SYN, RST, dropped SYN, retry timer`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `둘을 모두 서버 다운으로만 설명하는 경우`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
둘을 모두 서버 다운으로만 설명하는 경우는 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `SYN, RST, dropped SYN, retry timer`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `SYN, RST, dropped SYN, retry timer`를 먼저 말하고, 그다음 `refused는 연결 시도에 대해 명시적 거절 신호가 돌아온 쪽이고, timeout은 연결 시도에 대한 충분한 응답을 받지 못한 쪽이다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `새 인스턴스 배포 후 health check가 refused를 보거나 timeout을 보는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
이 절의 기준 장면인 `새 인스턴스 배포 후 health check가 refused를 보거나 timeout을 보는 장면`에서는 SYN, RST, dropped SYN, retry timer 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `둘을 모두 서버 다운으로만 설명하는 경우`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 SYN, RST, dropped SYN, retry timer 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.
...
이 절의 주제는 `CLOSE_WAIT가 쌓이면 무엇을 의심하는가`이다. CLOSE_WAIT는 상대의 FIN을 받은 뒤 로컬 애플리케이션이 아직 socket을 닫지 않은 상태이므로, 로컬 close 경로와 resource release를 먼저 본다. 기준 장면은 `상대가 연결을 닫았는데 Java 프로세스에 CLOSE_WAIT socket이 계속 남는 장면`이다. 여기서는 다음 대상을 따라간다: peer FIN, local ACK, application close 누락, fd retention. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


장애 상황에서는 개념을 따로 외우는 방식이 거의 도움이 되지 않는다.
이 절에서 따라갈 대상은 peer FIN, local ACK, application close 누락, fd retention이다.
그 대상을 시간 순서로 놓고 어느 계층에서 멈췄는지 좁혀야 한다.
커널은 TCP 상태와 fd, cgroup memory, scheduler 상태를 남기고, 런타임은 thread dump와 GC log, event loop delay를 남기며, 애플리케이션은 요청 로그와 business timing을 남긴다.
DB와 외부 API는 query plan, wait event, connection pool, upstream timeout으로 자기 신호를 남긴다.
좋은 장애 분석은 이 신호들을 한 타임라인에 맞춰 보고, 반증 가능한 순서로 가설을 줄인다.
...
아래 trace는 이 절의 주제를 `상대가 연결을 닫았는데 Java 프로세스에 CLOSE_WAIT socket이 계속 남는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `CLOSE_WAIT가 쌓이면 무엇을 의심하는가`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `CLOSE_WAIT를 네트워크 장비가 계속 붙잡고 있는 상태로 설명하는 경우`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`CLOSE_WAIT가 쌓이면 무엇을 의심하는가`의 공식 자료 고정점은 [Linux tcp(7)](https://www.man7.org/linux/man-pages/man7/tcp.7.html), [Linux cgroup v2](https://www.kernel.org/doc/html/latest/admin-guide/cgroup-v2.html), [Reactor Netty reference](https://docs.spring.io/projectreactor/reactor-netty/docs/current/reference/html/), [Tomcat architecture](https://tomcat.apache.org/tomcat-10.1-doc/architecture/overview.html), [PostgreSQL EXPLAIN](https://www.postgresql.org/docs/current/sql-explain.html), [PostgreSQL monitoring stats](https://www.postgresql.org/docs/current/monitoring-stats.html)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `peer FIN, local ACK, application close 누락, fd retention`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`peer FIN, local ACK, application close 누락, fd retention`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`CLOSE_WAIT가 쌓이면 무엇을 의심하는가`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`CLOSE_WAIT가 쌓이면 무엇을 의심하는가`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `peer FIN, local ACK, application close 누락, fd retention`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `CLOSE_WAIT를 네트워크 장비가 계속 붙잡고 있는 상태로 설명하는 경우`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
CLOSE_WAIT를 네트워크 장비가 계속 붙잡고 있는 상태로 설명하는 경우는 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `peer FIN, local ACK, application close 누락, fd retention`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `peer FIN, local ACK, application close 누락, fd retention`를 먼저 말하고, 그다음 `CLOSE_WAIT는 상대의 FIN을 받은 뒤 로컬 애플리케이션이 아직 socket을 닫지 않은 상태이므로, 로컬 close 경로와 resource release를 먼저 본다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `상대가 연결을 닫았는데 Java 프로세스에 CLOSE_WAIT socket이 계속 남는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 peer FIN, local ACK, application close 누락, fd retention 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`CLOSE_WAIT가 쌓이면 무엇을 의심하는가`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `상대가 연결을 닫았는데 Java 프로세스에 CLOSE_WAIT socket이 계속 남는 장면`에서는 peer FIN, local ACK, application close 누락, fd retention 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `CLOSE_WAIT를 네트워크 장비가 계속 붙잡고 있는 상태로 설명하는 경우`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
이 절의 주제는 `TIME_WAIT가 많으면 항상 문제인가`이다. TIME_WAIT는 정상 종료 뒤 늦은 packet과 마지막 ACK 재전송을 위해 남는 안전 상태라서, 많다는 사실만으로 장애는 아니다. 기준 장면은 `짧은 HTTP 연결이 매우 많아 `ss`에 TIME_WAIT가 많이 보이는 장면`이다. 여기서는 다음 대상을 따라간다: active close, final ACK, 2MSL wait, ephemeral port reuse. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


장애 상황에서는 개념을 따로 외우는 방식이 거의 도움이 되지 않는다.
이 절에서 따라갈 대상은 active close, final ACK, 2MSL wait, ephemeral port reuse이다.
그 대상을 시간 순서로 놓고 어느 계층에서 멈췄는지 좁혀야 한다.
커널은 TCP 상태와 fd, cgroup memory, scheduler 상태를 남기고, 런타임은 thread dump와 GC log, event loop delay를 남기며, 애플리케이션은 요청 로그와 business timing을 남긴다.
DB와 외부 API는 query plan, wait event, connection pool, upstream timeout으로 자기 신호를 남긴다.
좋은 장애 분석은 이 신호들을 한 타임라인에 맞춰 보고, 반증 가능한 순서로 가설을 줄인다.
...
아래 trace는 이 절의 주제를 `짧은 HTTP 연결이 매우 많아 `ss`에 TIME_WAIT가 많이 보이는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `TIME_WAIT가 많으면 항상 문제인가`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
한 요청은 계층을 모두 지난다.
다만 각 계층이 소유한 상태를 분리해야 질문이 선명해진다.
예를 들어 `짧은 HTTP 연결이 매우 많아 `ss`에 TIME_WAIT가 많이 보이는 장면`에서 response가 느리다면 애플리케이션 method 시간이 길 수도 있고, DB connection pool에서 기다렸을 수도 있으며, Nginx가 upstream response를 buffer에 모으고 있을 수도 있고, client가 느려 send buffer가 차 있을 수도 있다.
같은 "느림"이라도 소유 상태가 다르면 확인 명령과 수리 방법이 완전히 달라진다.
...
2. `짧은 HTTP 연결이 매우 많아 `ss`에 TIME_WAIT가 많이 보이는 장면`의 그 지점 주인은 커널, 프록시, JVM 런타임, 애플리케이션, DB 중 누구인가.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `TIME_WAIT를 leak으로만 보고 무조건 커널 파라미터부터 줄이는 설명`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`TIME_WAIT가 많으면 항상 문제인가`의 공식 자료 고정점은 [Linux tcp(7)](https://www.man7.org/linux/man-pages/man7/tcp.7.html), [Linux cgroup v2](https://www.kernel.org/doc/html/latest/admin-guide/cgroup-v2.html), [Reactor Netty reference](https://docs.spring.io/projectreactor/reactor-netty/docs/current/reference/html/), [Tomcat architecture](https://tomcat.apache.org/tomcat-10.1-doc/architecture/overview.html), [PostgreSQL EXPLAIN](https://www.postgresql.org/docs/current/sql-explain.html), [PostgreSQL monitoring stats](https://www.postgresql.org/docs/current/monitoring-stats.html)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `active close, final ACK, 2MSL wait, ephemeral port reuse`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`active close, final ACK, 2MSL wait, ephemeral port reuse`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.

이 구분은 `짧은 HTTP 연결이 매우 많아 `ss`에 TIME_WAIT가 많이 보이는 장면` 같은 성능 설명에서도 중요하다.
하나의 latency 숫자 안에는 DNS lookup, TCP connect, TLS handshake, proxy upstream wait, runtime scheduling, DB connection wait, query execution, response write, client receive delay가 섞일 수 있다.
`TIME_WAIT가 많으면 항상 문제인가`를 제대로 설명하려면 total latency 하나를 크게 말하기보다, 어떤 계층의 시간이 늘었고 어떤 명령이나 로그가 그 판단을 지지하는지 함께 말해야 한다.

#### 직접 확인할 수 있는 관측 지점

`TIME_WAIT가 많으면 항상 문제인가`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`TIME_WAIT가 많으면 항상 문제인가`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `active close, final ACK, 2MSL wait, ephemeral port reuse`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `TIME_WAIT를 leak으로만 보고 무조건 커널 파라미터부터 줄이는 설명`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
TIME_WAIT를 leak으로만 보고 무조건 커널 파라미터부터 줄이는 설명은 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
세 번째 위험은 `짧은 HTTP 연결이 매우 많아 `ss`에 TIME_WAIT가 많이 보이는 장면`에서 정상 상태와 장애 상태를 구분하지 못하는 것이다.
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `active close, final ACK, 2MSL wait, ephemeral port reuse`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
| 비교축 | 왼쪽을 볼 때 | 오른쪽을 볼 때 | 판단 질문 |
| --- | --- | --- | --- |
| `TIME_WAIT가 많으면 항상 문제인가`의 커널 상태 vs 애플리케이션 상태 | `active close, final ACK, 2MSL wait, ephemeral port reuse` 중 fd, TCP state, buffer, route, cgroup처럼 OS가 직접 소유한 상태 | request object, transaction, business object, controller log처럼 앱이 만든 상태 | `짧은 HTTP 연결이 매우 많아 `ss`에 TIME_WAIT가 많이 보이는 장면`에서 증상은 연결이 만들어지기 전인가, 앱 handler가 시작된 뒤인가 |
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `active close, final ACK, 2MSL wait, ephemeral port reuse`를 먼저 말하고, 그다음 `TIME_WAIT는 정상 종료 뒤 늦은 packet과 마지막 ACK 재전송을 위해 남는 안전 상태라서, 많다는 사실만으로 장애는 아니다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `짧은 HTTP 연결이 매우 많아 `ss`에 TIME_WAIT가 많이 보이는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
`짧은 HTTP 연결이 매우 많아 `ss`에 TIME_WAIT가 많이 보이는 장면`에서 메모리나 지연이 늘어난다면 어느 버퍼가 늘었는지 확인해야 한다.
커널 버퍼가 찬 문제를 controller 코드만 고쳐 해결할 수 없고, proxy buffering 문제를 JVM heap 크기만 키워 해결할 수도 없다.
...
`짧은 HTTP 연결이 매우 많아 `ss`에 TIME_WAIT가 많이 보이는 장면`에서 한 설정을 키우면 병목이 사라지는 것이 아니라 다음 좁은 지점으로 이동할 수 있다.
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 active close, final ACK, 2MSL wait, ephemeral port reuse 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`TIME_WAIT가 많으면 항상 문제인가`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `짧은 HTTP 연결이 매우 많아 `ss`에 TIME_WAIT가 많이 보이는 장면`에서는 active close, final ACK, 2MSL wait, ephemeral port reuse 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `TIME_WAIT를 leak으로만 보고 무조건 커널 파라미터부터 줄이는 설명`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.

`TIME_WAIT가 많으면 항상 문제인가`에서 대기 지점을 찾을 때는 누가 기다리는지부터 분리한다.
`짧은 HTTP 연결이 매우 많아 `ss`에 TIME_WAIT가 많이 보이는 장면`에서는 호출한 thread가 커널 안에서 기다릴 수도 있고, event loop가 readiness를 기다릴 수도 있으며, DB나 외부 시스템이 자기 wait 상태를 남길 수도 있다.
...
이 절의 주제는 `too many open files는 왜 네트워크 장애처럼 보이는가`이다. 네트워크 연결도 fd를 쓰기 때문에 fd 한도 초과는 새 socket, 파일, epoll 등록, 로그 파일 열기까지 함께 실패시키며 네트워크 장애처럼 보인다. 기준 장면은 `새 연결이 들어오는데 서버가 `EMFILE` 때문에 accept하지 못하고 client는 연결 실패를 보는 장면`이다. 여기서는 다음 대상을 따라간다: fd allocation failure, accept failure, DB socket creation failure, log file open failure. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


장애 상황에서는 개념을 따로 외우는 방식이 거의 도움이 되지 않는다.
이 절에서 따라갈 대상은 fd allocation failure, accept failure, DB socket creation failure, log file open failure이다.
그 대상을 시간 순서로 놓고 어느 계층에서 멈췄는지 좁혀야 한다.
커널은 TCP 상태와 fd, cgroup memory, scheduler 상태를 남기고, 런타임은 thread dump와 GC log, event loop delay를 남기며, 애플리케이션은 요청 로그와 business timing을 남긴다.
DB와 외부 API는 query plan, wait event, connection pool, upstream timeout으로 자기 신호를 남긴다.
좋은 장애 분석은 이 신호들을 한 타임라인에 맞춰 보고, 반증 가능한 순서로 가설을 줄인다.
...
아래 trace는 이 절의 주제를 `새 연결이 들어오는데 서버가 `EMFILE` 때문에 accept하지 못하고 client는 연결 실패를 보는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `too many open files는 왜 네트워크 장애처럼 보이는가`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
한 요청은 계층을 모두 지난다.
다만 각 계층이 소유한 상태를 분리해야 질문이 선명해진다.
예를 들어 `새 연결이 들어오는데 서버가 `EMFILE` 때문에 accept하지 못하고 client는 연결 실패를 보는 장면`에서 response가 느리다면 애플리케이션 method 시간이 길 수도 있고, DB connection pool에서 기다렸을 수도 있으며, Nginx가 upstream response를 buffer에 모으고 있을 수도 있고, client가 느려 send buffer가 차 있을 수도 있다.
같은 "느림"이라도 소유 상태가 다르면 확인 명령과 수리 방법이 완전히 달라진다.
...
2. `새 연결이 들어오는데 서버가 `EMFILE` 때문에 accept하지 못하고 client는 연결 실패를 보는 장면`의 그 지점 주인은 커널, 프록시, JVM 런타임, 애플리케이션, DB 중 누구인가.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `open files라는 문구 때문에 파일 I/O 문제로만 한정하는 설명`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`too many open files는 왜 네트워크 장애처럼 보이는가`의 공식 자료 고정점은 [Linux tcp(7)](https://www.man7.org/linux/man-pages/man7/tcp.7.html), [Linux cgroup v2](https://www.kernel.org/doc/html/latest/admin-guide/cgroup-v2.html), [Reactor Netty reference](https://docs.spring.io/projectreactor/reactor-netty/docs/current/reference/html/), [Tomcat architecture](https://tomcat.apache.org/tomcat-10.1-doc/architecture/overview.html), [PostgreSQL EXPLAIN](https://www.postgresql.org/docs/current/sql-explain.html), [PostgreSQL monitoring stats](https://www.postgresql.org/docs/current/monitoring-stats.html)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `fd allocation failure, accept failure, DB socket creation failure, log file open failure`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`fd allocation failure, accept failure, DB socket creation failure, log file open failure`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.

이 구분은 `새 연결이 들어오는데 서버가 `EMFILE` 때문에 accept하지 못하고 client는 연결 실패를 보는 장면` 같은 성능 설명에서도 중요하다.
하나의 latency 숫자 안에는 DNS lookup, TCP connect, TLS handshake, proxy upstream wait, runtime scheduling, DB connection wait, query execution, response write, client receive delay가 섞일 수 있다.
...
`too many open files는 왜 네트워크 장애처럼 보이는가`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`too many open files는 왜 네트워크 장애처럼 보이는가`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `fd allocation failure, accept failure, DB socket creation failure, log file open failure`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `open files라는 문구 때문에 파일 I/O 문제로만 한정하는 설명`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
open files라는 문구 때문에 파일 I/O 문제로만 한정하는 설명은 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
세 번째 위험은 `새 연결이 들어오는데 서버가 `EMFILE` 때문에 accept하지 못하고 client는 연결 실패를 보는 장면`에서 정상 상태와 장애 상태를 구분하지 못하는 것이다.
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `fd allocation failure, accept failure, DB socket creation failure, log file open failure`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `fd allocation failure, accept failure, DB socket creation failure, log file open failure`를 먼저 말하고, 그다음 `네트워크 연결도 fd를 쓰기 때문에 fd 한도 초과는 새 socket, 파일, epoll 등록, 로그 파일 열기까지 함께 실패시키며 네트워크 장애처럼 보인다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `새 연결이 들어오는데 서버가 `EMFILE` 때문에 accept하지 못하고 client는 연결 실패를 보는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
`새 연결이 들어오는데 서버가 `EMFILE` 때문에 accept하지 못하고 client는 연결 실패를 보는 장면`에서 한 설정을 키우면 병목이 사라지는 것이 아니라 다음 좁은 지점으로 이동할 수 있다.
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 fd allocation failure, accept failure, DB socket creation failure, log file open failure 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`too many open files는 왜 네트워크 장애처럼 보이는가`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `새 연결이 들어오는데 서버가 `EMFILE` 때문에 accept하지 못하고 client는 연결 실패를 보는 장면`에서는 fd allocation failure, accept failure, DB socket creation failure, log file open failure 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `open files라는 문구 때문에 파일 I/O 문제로만 한정하는 설명`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.

`too many open files는 왜 네트워크 장애처럼 보이는가`에서 대기 지점을 찾을 때는 누가 기다리는지부터 분리한다.
`새 연결이 들어오는데 서버가 `EMFILE` 때문에 accept하지 못하고 client는 연결 실패를 보는 장면`에서는 호출한 thread가 커널 안에서 기다릴 수도 있고, event loop가 readiness를 기다릴 수도 있으며, DB나 외부 시스템이 자기 wait 상태를 남길 수도 있다.
...
`새 연결이 들어오는데 서버가 `EMFILE` 때문에 accept하지 못하고 client는 연결 실패를 보는 장면`에서 메모리나 지연이 늘어난다면 어느 버퍼가 늘었는지 확인해야 한다.
커널 버퍼가 찬 문제를 controller 코드만 고쳐 해결할 수 없고, proxy buffering 문제를 JVM heap 크기만 키워 해결할 수도 없다.
...
이 절의 주제는 `p99 latency가 튈 때 CPU, GC, DB, network를 어떻게 가른다`이다. p99는 소수 요청의 꼬리 지연이므로 CPU 포화, GC stop-the-world, DB lock/wait, network retransmission, upstream timeout을 같은 타임라인에 놓고 본다. 기준 장면은 `평균은 괜찮은데 p99만 튀고 일부 요청이 timeout에 걸리는 장면`이다. 여기서는 다음 대상을 따라간다: latency histogram, thread state, GC pause, DB wait, TCP retransmission. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


장애 상황에서는 개념을 따로 외우는 방식이 거의 도움이 되지 않는다.
이 절에서 따라갈 대상은 latency histogram, thread state, GC pause, DB wait, TCP retransmission이다.
그 대상을 시간 순서로 놓고 어느 계층에서 멈췄는지 좁혀야 한다.
커널은 TCP 상태와 fd, cgroup memory, scheduler 상태를 남기고, 런타임은 thread dump와 GC log, event loop delay를 남기며, 애플리케이션은 요청 로그와 business timing을 남긴다.
DB와 외부 API는 query plan, wait event, connection pool, upstream timeout으로 자기 신호를 남긴다.
좋은 장애 분석은 이 신호들을 한 타임라인에 맞춰 보고, 반증 가능한 순서로 가설을 줄인다.
...
아래 trace는 이 절의 주제를 `평균은 괜찮은데 p99만 튀고 일부 요청이 timeout에 걸리는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `p99 latency가 튈 때 CPU, GC, DB, network를 어떻게 가른다`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `평균 latency가 낮다는 이유로 장애를 부정하거나, p99만 보고 무조건 scale-out하는 설명`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`p99 latency가 튈 때 CPU, GC, DB, network를 어떻게 가른다`의 공식 자료 고정점은 [Linux tcp(7)](https://www.man7.org/linux/man-pages/man7/tcp.7.html), [Linux cgroup v2](https://www.kernel.org/doc/html/latest/admin-guide/cgroup-v2.html), [Reactor Netty reference](https://docs.spring.io/projectreactor/reactor-netty/docs/current/reference/html/), [Tomcat architecture](https://tomcat.apache.org/tomcat-10.1-doc/architecture/overview.html), [PostgreSQL EXPLAIN](https://www.postgresql.org/docs/current/sql-explain.html), [PostgreSQL monitoring stats](https://www.postgresql.org/docs/current/monitoring-stats.html)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `latency histogram, thread state, GC pause, DB wait, TCP retransmission`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`latency histogram, thread state, GC pause, DB wait, TCP retransmission`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`p99 latency가 튈 때 CPU, GC, DB, network를 어떻게 가른다`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`p99 latency가 튈 때 CPU, GC, DB, network를 어떻게 가른다`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `latency histogram, thread state, GC pause, DB wait, TCP retransmission`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `평균 latency가 낮다는 이유로 장애를 부정하거나, p99만 보고 무조건 scale-out하는 설명`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
평균 latency가 낮다는 이유로 장애를 부정하거나, p99만 보고 무조건 scale-out하는 설명은 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `latency histogram, thread state, GC pause, DB wait, TCP retransmission`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `latency histogram, thread state, GC pause, DB wait, TCP retransmission`를 먼저 말하고, 그다음 `p99는 소수 요청의 꼬리 지연이므로 CPU 포화, GC stop-the-world, DB lock/wait, network retransmission, upstream timeout을 같은 타임라인에 놓고 본다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `평균은 괜찮은데 p99만 튀고 일부 요청이 timeout에 걸리는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 latency histogram, thread state, GC pause, DB wait, TCP retransmission 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`p99 latency가 튈 때 CPU, GC, DB, network를 어떻게 가른다`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `평균은 괜찮은데 p99만 튀고 일부 요청이 timeout에 걸리는 장면`에서는 latency histogram, thread state, GC pause, DB wait, TCP retransmission 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `평균 latency가 낮다는 이유로 장애를 부정하거나, p99만 보고 무조건 scale-out하는 설명`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
이 절의 주제는 `컨테이너에서 OOMKilled가 났을 때 Linux 관점에서 무엇을 본다`이다. 컨테이너 OOMKilled는 cgroup limit 안에서 커널이 프로세스를 죽인 사건일 수 있으므로 JVM heap 로그뿐 아니라 cgroup memory.events와 container runtime 이벤트를 함께 봐야 한다. 기준 장면은 `Kubernetes Pod가 exit code 137로 재시작되고 Java 로그에는 명확한 OutOfMemoryError가 없는 장면`이다. 여기서는 다음 대상을 따라간다: cgroup memory usage, process RSS, heap/direct/native memory, OOM event. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


장애 상황에서는 개념을 따로 외우는 방식이 거의 도움이 되지 않는다.
이 절에서 따라갈 대상은 cgroup memory usage, process RSS, heap/direct/native memory, OOM event이다.
그 대상을 시간 순서로 놓고 어느 계층에서 멈췄는지 좁혀야 한다.
커널은 TCP 상태와 fd, cgroup memory, scheduler 상태를 남기고, 런타임은 thread dump와 GC log, event loop delay를 남기며, 애플리케이션은 요청 로그와 business timing을 남긴다.
DB와 외부 API는 query plan, wait event, connection pool, upstream timeout으로 자기 신호를 남긴다.
좋은 장애 분석은 이 신호들을 한 타임라인에 맞춰 보고, 반증 가능한 순서로 가설을 줄인다.
...
아래 trace는 이 절의 주제를 `Kubernetes Pod가 exit code 137로 재시작되고 Java 로그에는 명확한 OutOfMemoryError가 없는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `컨테이너에서 OOMKilled가 났을 때 Linux 관점에서 무엇을 본다`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `Java heap이 limit보다 작으면 OOMKilled가 불가능하다고 설명하는 경우`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`컨테이너에서 OOMKilled가 났을 때 Linux 관점에서 무엇을 본다`의 공식 자료 고정점은 [Linux tcp(7)](https://www.man7.org/linux/man-pages/man7/tcp.7.html), [Linux cgroup v2](https://www.kernel.org/doc/html/latest/admin-guide/cgroup-v2.html), [Reactor Netty reference](https://docs.spring.io/projectreactor/reactor-netty/docs/current/reference/html/), [Tomcat architecture](https://tomcat.apache.org/tomcat-10.1-doc/architecture/overview.html), [PostgreSQL EXPLAIN](https://www.postgresql.org/docs/current/sql-explain.html), [PostgreSQL monitoring stats](https://www.postgresql.org/docs/current/monitoring-stats.html)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `cgroup memory usage, process RSS, heap/direct/native memory, OOM event`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`cgroup memory usage, process RSS, heap/direct/native memory, OOM event`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`컨테이너에서 OOMKilled가 났을 때 Linux 관점에서 무엇을 본다`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`컨테이너에서 OOMKilled가 났을 때 Linux 관점에서 무엇을 본다`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `cgroup memory usage, process RSS, heap/direct/native memory, OOM event`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `Java heap이 limit보다 작으면 OOMKilled가 불가능하다고 설명하는 경우`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
Java heap이 limit보다 작으면 OOMKilled가 불가능하다고 설명하는 경우는 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `cgroup memory usage, process RSS, heap/direct/native memory, OOM event`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `cgroup memory usage, process RSS, heap/direct/native memory, OOM event`를 먼저 말하고, 그다음 `컨테이너 OOMKilled는 cgroup limit 안에서 커널이 프로세스를 죽인 사건일 수 있으므로 JVM heap 로그뿐 아니라 cgroup memory.events와 container runtime 이벤트를 함께 봐야 한다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `Kubernetes Pod가 exit code 137로 재시작되고 Java 로그에는 명확한 OutOfMemoryError가 없는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
이 절의 기준 장면인 `Kubernetes Pod가 exit code 137로 재시작되고 Java 로그에는 명확한 OutOfMemoryError가 없는 장면`에서는 cgroup memory usage, process RSS, heap/direct/native memory, OOM event 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `Java heap이 limit보다 작으면 OOMKilled가 불가능하다고 설명하는 경우`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 cgroup memory usage, process RSS, heap/direct/native memory, OOM event 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.
...
이 절의 주제는 `WebFlux나 Netty로 바꿨는데도 느릴 때 무엇을 의심하는가`이다. 비동기 런타임은 I/O 대기 중 스레드 점유를 줄이는 도구이지 CPU 작업, blocking DB, 느린 downstream, 잘못된 buffering을 자동으로 해결하지 않는다. 기준 장면은 `Spring MVC에서 WebFlux로 바꾼 뒤에도 대용량 요청에서 latency가 줄지 않는 장면`이다. 여기서는 다음 대상을 따라간다: event loop task, blocking call, demand signal, DB connection, proxy buffer. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


장애 상황에서는 개념을 따로 외우는 방식이 거의 도움이 되지 않는다.
이 절에서 따라갈 대상은 event loop task, blocking call, demand signal, DB connection, proxy buffer이다.
그 대상을 시간 순서로 놓고 어느 계층에서 멈췄는지 좁혀야 한다.
커널은 TCP 상태와 fd, cgroup memory, scheduler 상태를 남기고, 런타임은 thread dump와 GC log, event loop delay를 남기며, 애플리케이션은 요청 로그와 business timing을 남긴다.
DB와 외부 API는 query plan, wait event, connection pool, upstream timeout으로 자기 신호를 남긴다.
좋은 장애 분석은 이 신호들을 한 타임라인에 맞춰 보고, 반증 가능한 순서로 가설을 줄인다.
...
아래 trace는 이 절의 주제를 `Spring MVC에서 WebFlux로 바꾼 뒤에도 대용량 요청에서 latency가 줄지 않는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `WebFlux나 Netty로 바꿨는데도 느릴 때 무엇을 의심하는가`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `WebFlux 전환을 성능 개선 보증으로 설명하는 경우`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`WebFlux나 Netty로 바꿨는데도 느릴 때 무엇을 의심하는가`의 공식 자료 고정점은 [Linux tcp(7)](https://www.man7.org/linux/man-pages/man7/tcp.7.html), [Linux cgroup v2](https://www.kernel.org/doc/html/latest/admin-guide/cgroup-v2.html), [Reactor Netty reference](https://docs.spring.io/projectreactor/reactor-netty/docs/current/reference/html/), [Tomcat architecture](https://tomcat.apache.org/tomcat-10.1-doc/architecture/overview.html), [PostgreSQL EXPLAIN](https://www.postgresql.org/docs/current/sql-explain.html), [PostgreSQL monitoring stats](https://www.postgresql.org/docs/current/monitoring-stats.html)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `event loop task, blocking call, demand signal, DB connection, proxy buffer`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`event loop task, blocking call, demand signal, DB connection, proxy buffer`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`WebFlux나 Netty로 바꿨는데도 느릴 때 무엇을 의심하는가`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`WebFlux나 Netty로 바꿨는데도 느릴 때 무엇을 의심하는가`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `event loop task, blocking call, demand signal, DB connection, proxy buffer`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `WebFlux 전환을 성능 개선 보증으로 설명하는 경우`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
WebFlux 전환을 성능 개선 보증으로 설명하는 경우는 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `event loop task, blocking call, demand signal, DB connection, proxy buffer`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `event loop task, blocking call, demand signal, DB connection, proxy buffer`를 먼저 말하고, 그다음 `비동기 런타임은 I/O 대기 중 스레드 점유를 줄이는 도구이지 CPU 작업, blocking DB, 느린 downstream, 잘못된 buffering을 자동으로 해결하지 않는다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `Spring MVC에서 WebFlux로 바꾼 뒤에도 대용량 요청에서 latency가 줄지 않는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 event loop task, blocking call, demand signal, DB connection, proxy buffer 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`WebFlux나 Netty로 바꿨는데도 느릴 때 무엇을 의심하는가`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `Spring MVC에서 WebFlux로 바꾼 뒤에도 대용량 요청에서 latency가 줄지 않는 장면`에서는 event loop task, blocking call, demand signal, DB connection, proxy buffer 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `WebFlux 전환을 성능 개선 보증으로 설명하는 경우`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
이 절의 주제는 `커널 파라미터와 애플리케이션 설정은 언제 같이 봐야 하는가`이다. 커널 파라미터와 애플리케이션 설정은 같은 병목 지점을 양쪽에서 제한할 때 함께 봐야 한다. 한쪽만 키우면 다음 좁은 목이 드러난다. 기준 장면은 `Tomcat acceptCount를 키웠는데 여전히 새 연결이 밀리거나, ulimit만 올렸는데 DB가 먼저 죽는 장면`이다. 여기서는 다음 대상을 따라간다: somaxconn, acceptCount, maxConnections, ulimit, pool size, timeout. 이 대상이 계층을 지나며 어떤 이름과 상태로 바뀌는지 보는 것이 핵심이다.


장애 상황에서는 개념을 따로 외우는 방식이 거의 도움이 되지 않는다.
이 절에서 따라갈 대상은 somaxconn, acceptCount, maxConnections, ulimit, pool size, timeout이다.
그 대상을 시간 순서로 놓고 어느 계층에서 멈췄는지 좁혀야 한다.
커널은 TCP 상태와 fd, cgroup memory, scheduler 상태를 남기고, 런타임은 thread dump와 GC log, event loop delay를 남기며, 애플리케이션은 요청 로그와 business timing을 남긴다.
DB와 외부 API는 query plan, wait event, connection pool, upstream timeout으로 자기 신호를 남긴다.
좋은 장애 분석은 이 신호들을 한 타임라인에 맞춰 보고, 반증 가능한 순서로 가설을 줄인다.
...
아래 trace는 이 절의 주제를 `Tomcat acceptCount를 키웠는데 여전히 새 연결이 밀리거나, ulimit만 올렸는데 DB가 먼저 죽는 장면`이라는 실제 요청 흐름 안에 놓고 본 것이다.
표현은 단순화했지만, 움직이는 대상과 책임 경계는 운영에서 그대로 다시 확인할 수 있다.
...
이 trace에서 중요한 점은 `커널 파라미터와 애플리케이션 설정은 언제 같이 봐야 하는가`의 산출물이 다음 계층의 입력이 될 때 이름과 책임이 함께 바뀐다는 것이다.
커널이 남기는 것은 Java 객체가 아니라 fd, 연결 상태, socket buffer, route 결과이고, 프록시가 남기는 것은 kernel packet이 아니라 upstream 요청, header 조정, buffering 정책이다.
Tomcat이나 Netty가 만드는 것도 곧바로 domain object가 아니라 request wrapper, channel event, ByteBuf, decoded HTTP object에 가깝다.
Spring controller와 service가 business 의미를 만들고, DB driver는 그 의미를 다시 SQL과 database protocol byte로 낮춘다.
...
4. 바로 위 계층과 바로 아래 계층의 신호가 같은 시간대에 맞물리는가. `설정 값을 크게 하면 안전하다고 생각하고 downstream 용량과 timeout, pool size를 같이 보지 않는 설명`라는 설명을 반증할 수 있는 adjacent signal은 무엇인가.
...
`커널 파라미터와 애플리케이션 설정은 언제 같이 봐야 하는가`의 공식 자료 고정점은 [Linux tcp(7)](https://www.man7.org/linux/man-pages/man7/tcp.7.html), [Linux cgroup v2](https://www.kernel.org/doc/html/latest/admin-guide/cgroup-v2.html), [Reactor Netty reference](https://docs.spring.io/projectreactor/reactor-netty/docs/current/reference/html/), [Tomcat architecture](https://tomcat.apache.org/tomcat-10.1-doc/architecture/overview.html), [PostgreSQL EXPLAIN](https://www.postgresql.org/docs/current/sql-explain.html), [PostgreSQL monitoring stats](https://www.postgresql.org/docs/current/monitoring-stats.html)에서 잡을 수 있다.
이 자료들은 서로 다른 계층을 설명하지만, 이 절에서는 모두 `somaxconn, acceptCount, maxConnections, ulimit, pool size, timeout`의 소유권을 확인하는 근거로 사용한다.
Linux man page와 RFC는 커널과 프로토콜이 어떤 상태 이름과 오류 신호를 남기는지 보여 주고, JVM/Spring/Netty/Reactor 문서는 그 낮은 층의 상태가 Java 객체, callback, signal, handler 흐름으로 올라오는 방식을 보여 준다.
Nginx나 Tomcat 문서는 user-space 서버가 커널 socket 위에서 별도의 timeout, queue, buffer, worker 정책을 가진다는 점을 확인하게 해 준다.

그래서 이 절의 설명을 면접이나 장애 분석에서 사용할 때는 "무엇이 무엇을 대신한다"가 아니라 "`somaxconn, acceptCount, maxConnections, ulimit, pool size, timeout`가 어떤 계층의 입력이 되는가"라고 말하는 편이 정확하다.
커널 readiness는 런타임 이벤트의 입력이 될 수 있지만 application protocol parsing을 대신하지 않는다.
프록시 upstream response는 client response의 원재료이지만, proxy buffering이나 header rewrite 때문에 byte-for-byte tunnel이라고 단정할 수 없다.
DB result set이나 외부 API response도 Java collection으로 순간 이동하지 않고 driver buffer, pool state, transaction boundary를 거쳐 애플리케이션 의미로 올라온다.
...
`커널 파라미터와 애플리케이션 설정은 언제 같이 봐야 하는가`의 핵심 주장은 아래 명령으로 최소한의 replay가 가능하다.
실제 운영 서버에서 실행할 때는 권한, 개인정보, 트래픽 영향, 로그 보존 정책을 먼저 확인해야 한다.
개발 환경이나 staging에서 작은 재현을 만든 뒤 production에서는 읽기 전용 관측부터 시작하는 편이 안전하다.
...
`커널 파라미터와 애플리케이션 설정은 언제 같이 봐야 하는가`의 PASS 신호는 명령마다 다르지만, 공통적으로는 "예상한 계층의 상태가 실제로 보인다"는 것이다.
예를 들어 `ss`에서는 local/remote endpoint와 TCP state가 보이고, `lsof`나 `/proc/<pid>/fd`에서는 socket도 fd 한도 안의 항목이라는 점이 보인다.
`strace`에서는 Java나 Nginx가 결국 `read`, `write`, `accept4`, `epoll_wait` 같은 시스템 콜로 커널과 만나는 지점을 확인할 수 있다.
`jcmd Thread.print`에서는 Tomcat worker, Reactor event loop, DB pool 대기 같은 JVM 내부 대기 지점이 보인다.
Nginx 설정 출력에서는 proxy buffering, upstream, timeout이 애플리케이션 밖에서 응답 의미를 바꿀 수 있음을 확인한다.

FAIL 신호는 `somaxconn, acceptCount, maxConnections, ulimit, pool size, timeout`를 엉뚱한 계층에서만 찾는 경우다.
예를 들어 `설정 값을 크게 하면 안전하다고 생각하고 downstream 용량과 timeout, pool size를 같이 보지 않는 설명`라는 식으로 설명하면, 이 절에서 확인해야 할 관측 지점이 사라진다.
연결 생성 전 문제는 packet trace, listen socket, route, firewall 표면에서 먼저 닫아야 하고, handler 진입 뒤 문제는 thread dump, event-loop delay, DB wait, application timing으로 좁혀야 한다.
관측 명령은 결론을 대신하지 않는다.
명령이 보여 준 상태가 어느 계층의 상태인지 설명할 수 있을 때 비로소 이 절의 검증이 끝난다.
...
첫 번째 위험은 계층을 건너뛰는 설명이다.
설정 값을 크게 하면 안전하다고 생각하고 downstream 용량과 timeout, pool size를 같이 보지 않는 설명은 실제 현장에서 꽤 자주 나온다.
그럴듯해 보이지만, 한 계층을 지워 버리는 순간 반증할 명령도 사라진다.
네트워크 요청은 "클라이언트가 Spring controller를 호출한다"로 줄일 수 없다.
그 사이에는 DNS, routing, TCP state, TLS endpoint, proxy request, servlet 또는 Netty runtime, application handler, DB protocol이 있다.
...
같은 숫자라도 문맥에 따라 의미가 달라진다.
TIME_WAIT는 정상 종료의 안전 장치일 수 있고, CLOSE_WAIT는 로컬 close 누락 신호일 수 있으며, page cache 증가는 정상 캐시일 수도 cgroup 압박의 일부일 수도 있다.
따라서 이 절에서는 숫자 하나를 보는 대신 `somaxconn, acceptCount, maxConnections, ulimit, pool size, timeout`가 어느 계층에서 누가 소유한 상태인지 먼저 고정한다.
...
이 세 줄을 다른 주제에도 옮길 수 있으면 이해가 남은 것이다.
이 절에서는 `somaxconn, acceptCount, maxConnections, ulimit, pool size, timeout`를 먼저 말하고, 그다음 `커널 파라미터와 애플리케이션 설정은 같은 병목 지점을 양쪽에서 제한할 때 함께 봐야 한다. 한쪽만 키우면 다음 좁은 목이 드러난다.`라는 규칙을 붙인 뒤, 마지막으로 다음 계층이 소비할 상태를 말하면 된다.
예를 들어 `Tomcat acceptCount를 키웠는데 여전히 새 연결이 밀리거나, ulimit만 올렸는데 DB가 먼저 죽는 장면`을 다시 그릴 때는 "무엇이 들어왔는가 -> 누가 그 상태를 소유하는가 -> 다음 계층은 어떤 형태로 소비하는가" 순서가 무너지면 안 된다.
...
예를 들어 listen backlog, acceptCount, maxConnections, worker thread, DB pool, proxy timeout, cgroup memory limit은 서로 다른 계층의 상한이다.
이 절의 대상인 somaxconn, acceptCount, maxConnections, ulimit, pool size, timeout 중 무엇이 어느 상한에 걸렸는지 확인하지 않으면 설정 변경은 효과가 있어 보여도 재발 여지를 남긴다.

`커널 파라미터와 애플리케이션 설정은 언제 같이 봐야 하는가`에서 "연결"이라는 단어를 쓸 때는 먼저 어떤 계층의 연결인지 말해야 한다.
이 절의 기준 장면인 `Tomcat acceptCount를 키웠는데 여전히 새 연결이 밀리거나, ulimit만 올렸는데 DB가 먼저 죽는 장면`에서는 somaxconn, acceptCount, maxConnections, ulimit, pool size, timeout 같은 대상이 서로 다른 연결 의미를 만들 수 있다.
TCP 연결은 커널이 sequence number, window, retransmission timer로 관리하는 상태이고, HTTP keep-alive는 그 TCP 연결 위에 여러 request/response를 싣는 사용 방식이다.
DB connection이나 Reactor subscription은 또 다른 계층의 계약이다.
따라서 `설정 값을 크게 하면 안전하다고 생각하고 downstream 용량과 timeout, pool size를 같이 보지 않는 설명`처럼 말하면 연결의 주인이 사라지고, 확인해야 할 fd, pool, signal, buffer가 함께 흐려진다.
...
아래 명령은 운영 서버에서 바로 실행하라는 처방이 아니라, 각 계층이 실제로 어떤 신호를 남기는지 replay하기 위한 출발점이다.
