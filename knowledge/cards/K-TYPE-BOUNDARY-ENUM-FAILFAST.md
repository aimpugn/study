...
- `잘못된 값은 내부 로직으로 들어가기 전에 실패합니까(fail-fast)?`: fail-fast의 목적은 "엄격함"이 아니라, 장애 원인 위치를 경계로 고정해 디버깅 범위를 줄이는 것입니다.
- `내부 핵심 로직은 enum-only로 고정되어 있습니까?`: 내부가 `String`을 받는 순간, 검증 완료 여부가 호출자에게 달라붙어 재현성이 떨어집니다.
- `분기 누락을 컴파일 단계에서 드러내는 구조(switch expression 등)입니까?`: 새 enum 상수가 추가될 때 빌드가 깨지도록 설계할 수 있어야, 누락 분기가 운영까지 넘어가지 않습니다.
