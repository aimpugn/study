...
배포판이 업스트림 프로젝트(커널, nginx 등)의 어느 시점 버전을 가져와 어떻게 동결·백포트·배포하는지 — 즉 "배포판 저장소의 버전이 공식 latest와 다른 이유"의 전체 그림은 [upstream_to_distro.md](upstream_to_distro.md)에서 다룹니다.
