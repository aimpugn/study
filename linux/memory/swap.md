...
> 스왑 지표(`swpd`, `si`/`so`, `pswpin`/`pswpout`)가 커널의 어느 카운터에서 나오고 도구가 그것을 어떻게 가공하는지는 [memory_metrics.md](./memory_metrics.md) 1.6·2.4 에 있다. 특히 **스왑 사용량(게이지)과 스왑 활동(카운터)의 구분**은 그쪽에서 다룬다.
