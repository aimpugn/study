...
이 저장소(우분투 아카이브)가 어떻게 채워지는지 — 누가 패키지를 올리고, `-updates`/`-security`/`-proposed` 같은 포켓이 무엇이며, 왜 릴리스 수명 동안 패키지 베이스 버전이 올라가지 않는지 — 는 [../upstream_to_distro.md](../upstream_to_distro.md)에서 다룹니다.
