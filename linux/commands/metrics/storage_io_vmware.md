...
이 문서는 VMware 게스트의 스토리지 I/O 사례를 중심에 둔다. 더 넓은 OS·커널·하드웨어 사고의 기본 축은
[실무 엔지니어를 위한 Linux 커널·하드웨어 내부 구조](../../../interviews/linux-kernel-hardware-practical-internals.md)에서 먼저 잡고, 이 문서는 그 사고를 VMware 운영 환경에 대입하는 상세 사례로 읽으면 된다. 두 문서를 잇는 핵심 질문은 하나다. **애플리케이션 요청이 어느 커널 객체, 어느 큐, 어느 캐시, 어느 대기 상태에서 시간을 쓰고 있는가?**
...
이 문서를 읽은 뒤에는 명령 이름보다 아래 네 가지를 자기 말로 설명할 수 있어야 한다.

1. **요청 경로**: 애플리케이션의 `read()`·`write()`·`fsync()`가 VFS, 페이지 캐시, 파일시스템, 블록 계층, 가상 SCSI, ESXi, 데이터스토어, 어레이로 내려가는 흐름.
2. **커널 객체와 카운터**: `await`, `aqu-sz`, `%util`, `wa`, `b`, `si`, `so`, `dirty`가 각각 어떤 누적 카운터나 현재 상태에서 만들어지는지.
3. **문제가 되는 조건**: 단순히 값이 존재하는 것이 아니라, 어떤 값들이 함께 움직일 때 CPU, 메모리, 게스트 블록 계층, ESXi, 스토리지 중 어느 쪽을 의심해야 하는지.
4. **VMware 경계**: 게스트에서 볼 수 있는 `await`와 게스트에서 볼 수 없는 GAVG/KAVG/DAVG, DQLEN, 스냅샷, 데이터스토어 경합의 경계.

비유는 1장을 읽기 쉽게 만드는 발판일 뿐이다. 운영 판단으로 넘어갈 때는 같은 장면을 아래처럼 실제 커널 상태와 화면 값으로 다시 접어야 한다.

| 푸드코트 장면 | 실제 OS/VMware 상태 | 화면에서 보는 값 | 문제가 되는 순간 |
| --- | --- | --- | --- |
| 심부름 한 번이 오래 걸림 | 블록 I/O 요청 1건의 완료 지연 | `await`, `r_await`, `w_await` | 수십 ms 이상이 지속되고, 특히 처리량은 낮은데 지연만 클 때 |
| 주문서 줄이 길어짐 | 블록 계층이나 가상 디스크 앞의 in-flight 요청 누적 | `aqu-sz`, `r/s`, `w/s` | `IOPS × await(초)`와 비슷한 크기로 큐가 커질 때 |
| 요리사가 재료를 기다리며 놂 | CPU가 idle인 동안 I/O 완료를 기다림 | `wa` | 디스크를 보라는 힌트다. CPU가 바쁘면 병목이어도 낮게 보일 수 있다 |
| 직원이 굳어 버림 | `TASK_UNINTERRUPTIBLE` 대기 | `b`, D state, hung task 로그 | 여러 태스크가 계속 D state에 남거나 120초 이상 막힐 때 |
| 선반·설거지통이 밀림 | 페이지 캐시, dirty page, writeback | `cached`, `dirty`, `bo` | 캐시가 줄어 읽기가 디스크로 떨어지거나 dirty가 한도를 넘어 쓰는 프로세스를 멈출 때 |
| 짐을 치우거나 다시 가져옴 | swap-out / swap-in | `so`, `si`, `avail` | `so > 0`이면 현재 압박, `si`만 있고 `avail`이 충분하면 과거 잔재 회수일 수 있다 |
| 건물 복도와 중앙 창고 중 어디가 막힘 | ESXi VMkernel 큐와 어레이 왕복 지연 | GAVG/KAVG/DAVG | 게스트 `await`만으로는 못 가른다. KAVG면 ESXi, DAVG면 스토리지 쪽이다 |

손으로 빠르게 검산해 보면 기억에 남는다. `400 IOPS × 40ms`면 `aqu-sz ≈ 16`이므로 큐가 실제로 쌓인 그림이고, `wa 0% + await 40ms`면 CPU가 바빠서 iowait이 숨어 있을 수 있다. `si 16K/s + so 0 + avail 60%`는 현재 메모리 부족이 아니라 과거 스왑 잔재 회수에 가깝고, `D state 3 + await 40ms`가 지속되면 프로세스들이 I/O 완료를 기다리며 굳은 상태라 호스트·스토리지 경계까지 열어 봐야 한다.
...
SHOW_LV=0 bash vm_os_watch.sh               # LV(마운트별 분해) 행 끄기 — 기본은 자동 표시 (8.4)
...
**LV(논리볼륨) 행 — 물리 합산의 분해.** 물리 디스크 행의 수치는 그 위 모든 LV 의 합산이다. LVM 구성이면 모니터가 dm 장치를 시작 시 자동 인식해(이름·마운트·부모를 sysfs 와 /proc/mounts 에서 해석) **마운트포인트 이름으로, 부모 디스크 아래에 들여서** 함께 보여 준다 (`SHOW_LV=0` 으로 끔). 왜 필요한지는 실제 서버의 lsblk 가 보여 준다.

```text
sda(300G) ── /(root) · [SWAP] · /var · /home    ← OS 디스크에 4개 용도 동거
sdb(500G) ── /jboss                              ← 전용 1:1
sdc(100G) ── /app                                ← 전용 1:1
```

sda 의 await 가 튀었을 때 sda 행만으로는 "스왑이 미는가, /var 로그 폭주인가"를 가를 수 없다 — LV 행이 그걸 가른다.

```text
  장치        r/s     w/s      await
  sda          12    340     38.2ms      ← 합산: "문제가 있다"까지만
   /            2      8      4.1ms
   /var         1    320     41.0ms      ← 범인: /var 쓰기 폭주 (로그·감사 등)
   [SWAP]       0      0      0.0ms      ← 스왑은 결백
   /home        0      0      0.0ms
  sdb           5     60      3.2ms
   /jboss       5     60      3.2ms      ← 전용 디스크라 물리와 같은 값 — 그 자체가 확인
```

읽는 규칙: **물리 행 = "문제가 있는가", LV 행 = "누구의 문제인가."** `[SWAP]` 행의 쓰기는 위 스왑 `so` 와 같은 사건의 디바이스 측 모습이다. 경보 키는 dm 이름이라 안정적이고(`disk_dm-3_await`), 문구에는 라벨이 함께 나간다 — `디스크 /var(dm-3) 평균응답 …`.
...
| `SHOW_LV` | 1 | LV(dm) 행 표시 — 물리 디스크 수치를 마운트별로 분해 (0 = 끔) |
