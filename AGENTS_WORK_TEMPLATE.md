# WORK_YYYYMMDD_TITLE

> study 저장소의 규모가 있는 작업용 기본 WORK 템플릿입니다.
> 이 저장소는 자연어 요청을 기본으로 하며, 이 템플릿은 사용자 입력 형식을 강제하려는 것이 아니라 AI의 집행 ledger를 남기기 위한 것입니다.
> SSOT는 [`AGENTS.md`](AGENTS.md)와 전역 `~/.codex/AGENTS.md`입니다.
> 새 WORK 파일은 `docs/works/WORK_YYYYMMDD_TITLE.md`에 둡니다. 프로젝트 루트에 `WORK_*.md`를 만들지 않습니다.
> 작은 기계적 작업은 축약할 수 있지만, 근거·반박·검증·최종감사라는 핵심 루프는 약화하지 않습니다.

## 0. Meta

- 작업 제목:
- WORK 파일 경로: `docs/works/WORK_YYYYMMDD_TITLE.md`
- 저장소:
- 작업 유형: `research | analysis | design | explain | audit | execute | refactor_docs`
- 작업 깊이: `minimal | standard | full`
- 관련 요청:
- 원문 사용자 요청:
- 대상 경로 / 자산:
- 실행자:
- 시작 일시:
- 종료 일시:
- 현재 상태: `IN_PROGRESS | PARTIAL | BLOCKED | COMPLETE`
- 완료 게이트: `PENDING | ALLOW_COMPLETE | BLOCK_COMPLETE`
- finish: `report | verify | test | test+commit | test+commit+push`

## 1. Request Normalization

- goal:
- refs:
- scope:
- mode:
- run_mode: `normal | dry-run`
- finish:
- must_keep:
- extra_checks:

### 1.1 Explicit Deliverables

- 사용자가 명시한 필수 요구:
- 사용자가 명시한 금지 사항:
- path / naming / format / finish 관련 요구:
- 내가 추가한 누락 방지 항목:

### 1.2 Non-Goals

- 이번 작업의 비범위:
- 지금 하지 않는 이유:

## 2. Root-First Framing

- 근본 문제:
- 왜 이 문제가 지금 중요한가:
- 작업 목표:
- 기대 이점:
- 이점이 닫혔다고 판단할 확인 기준:
- 하드 제약 / 호환성 경계:
- 성공 정의:
- PARTIAL 조건:
- BLOCKED 조건:

## 3. Reader & Internalization Contract

- 주 독자:
- 독자가 이미 알고 있다고 가정하는 것:
- 이 작업 결과를 통해 나중에 스스로 설명할 수 있어야 하는 것:
- teach-back 목표:
- 사용자의 현재 빈칸 / learner gap 진단:
- 예상 독자 수준과 어린아이처럼 대하지 않는 기준:
- 이 설명이 사용자를 더 높은 개발자 판단으로 끌어올려야 하는 지점:
- 사용자가 내재화해야 할 사고 패턴:
- 특히 막아야 하는 오해:
- 오해를 깨기 위한 반례 / 대조쌍:
- 기억 anchor 후보:
- active recall 질문 / 작은 실험 / replay 과제:
- 반드시 거쳐야 하는 추상화 계층:
- 핵심 대조쌍 / 혼동쌍:
- 질문형으로 먼저 답해야 하는 핵심 질문:
- 목차 필요 여부와 이유:
- `개요` 또는 서두에서 먼저 고정할 문장:
- 사용자 / 저장소의 말투, 표기, 목록 형식 선호:
- 이번 문서의 기본 전개 흐름:
- 문서를 어떤 직접 진술형 문장으로 열 것인가:
- 왜 이런 구조나 규약이 생겼는지 먼저 설명할 핵심 배경:
- 초반에 먼저 고정할 공통 메타데이터 / 구조 / artifact:
- 필요하면 실무에서 왜 중요한지 짧게 연결할 지점:
- 본격적인 실행 경로 / 동작 경로 추적이 시작되는 지점:
- 이번 문서에서 특히 지켜야 할 설명 원칙:
- 이번 문서에서 문장 단위로 점검할 판정 질문:
- bad -> better 예시 페어(선택):
- 바로 풀어써야 하는 지시어 / 애매한 연결 표현:
- 초반에 먼저 보여줄 공식 규약 / 실제 구조 / 예시:
- 독자가 자연스럽게 묻게 될 한 단계 아래 질문:
- 독자가 자연스럽게 묻게 될 역사 / 등장 맥락 / 이전 방식 질문:
- 현재 머신 관측과 일반 규칙을 구분해야 하는 지점:
- 먼저 분해해서 설명할 구성요소:
- 그 구성요소들이 어떻게 엮여 상위 메커니즘이 되는가:
- 번호 목록으로 써야 하는 순차 흐름:
- 불릿 목록으로 써야 하는 동등 비교축:
- 한 줄로 닫을 항목과 들여쓴 prose가 필요한 항목:
- 공식 자료를 어떤 인과 흐름으로 설명에 연결할 것인가:
- 공식 자료 인용이 멈추면 FAIL로 볼 판정 지점:
- 이번 작업의 품질 기준 exemplar:
- primary exemplar와 참고할 설명 원리:
- secondary exemplar와 참고할 설명 원리(없으면 N/A):
- 각 exemplar에서 그대로 따라 하지 않을 visible trait:
- exemplar 대비 이번 산출물이 반드시 더 강해야 하는 축:
- 현재 저장소 평균 품질을 기준으로 삼으면 안 되는 이유:
- 속도를 이유로 줄이면 안 되는 조사 / 비교 / 검증 / 설명 단계:

## 4. Depth Decision

- 선택한 깊이:
- 왜 이 깊이가 맞는가:
- 전체 루프를 켜야 하는 트리거:
- 축약 가능한 섹션과 그 근거:

## 5. Project Overlay

- 적용한 로컬 AGENTS 경로:
- 활성화한 프로젝트 규칙:
- 특히 중요한 규칙:
- 전역 규칙과의 충돌 여부 / 해소:

## 6. Topic Analysis

- 현재 이해한 사용자 의도:
- 현재 보이는 문제 구조:
- 핵심 경계:
- 숨은 가정 / 불확실성:
- 성공을 오판하기 쉬운 지점:

## 7. Analysis Critique + Repair

### 7.1 Challenge 1

- 어떤 점이 틀릴 수 있는가:
- 보강안:
- 왜 이 보강안이 더 강한가:

### 7.2 Challenge 2

- 어떤 점이 좁거나 누락될 수 있는가:
- 보강안:
- 왜 이 보강안이 더 강한가:

### 7.3 Challenge 3

- 어떤 점이 설명/학습 목표를 놓칠 수 있는가:
- 보강안:
- 왜 이 보강안이 더 강한가:

### 7.4 Retained Framing

- 최종 채택한 문제 정의:
- 폐기한 문제 정의와 이유:

## 8. Scope Expansion & Impact Sync

- 시작 키워드:
- 확장 키워드(동의어 / 약어 / 원어 / 구명칭 / 관련 에러 / 표준 번호):
- 조사한 경로:
- 함께 점검한 자산:
- 함께 움직여야 하는 표면:
- 한쪽만 바꾸면 깨질 부분:
- 제외 표면과 근거:

## 9. Evidence Gathering

### 9.1 Evidence Ledger

- E-01
  - 주장:
  - 근거 유형: `repo evidence | command result | experiment | official doc | standard | expert source | inference`
  - 자료:
  - 이 자료로 닫힌 것:
  - 아직 비어 있는 것:
- E-02
  - 주장:
  - 근거 유형:
  - 자료:
  - 이 자료로 닫힌 것:
  - 아직 비어 있는 것:
- E-03
  - 주장:
  - 근거 유형:
  - 자료:
  - 이 자료로 닫힌 것:
  - 아직 비어 있는 것:

### 9.2 Source Conflicts / Gaps

- 충돌하는 근거:
- 아직 부족한 근거:
- 추론으로만 남는 항목:

## 10. Evidence Critique + Repair

- 소스 품질 리스크:
- 오래되었을 가능성이 있는 가정:
- 빠진 대안 또는 빠진 근거:
- 근거 세트를 어떻게 보강했는가:
- 보강 후에도 남는 한계:

## 11. Design

- 선택한 접근:
- 왜 이 접근이 근본 문제와 이점에 가장 잘 맞는가:
- 고려한 대안:
- 대안을 채택하지 않은 이유:
- 문서 / 예제 / 자산 구조:
- 설명 뼈대: `질문형 | 계층형 | 비교형 | 시나리오형 | 혼합`
- 계층별 설명 순서:
- 넣을 구체 예시 / 관측 anchor:
- 이 문서를 끌어올릴 목표 수준:
- 실패 모드:
- 검증 경로:

## 12. Design Critique + Repair

### 12.1 Architect View

- 반론:
- 보강 또는 유지 결정:
- 이유:

### 12.2 Domain / API Consumer View

- 반론:
- 보강 또는 유지 결정:
- 이유:

### 12.3 Newcomer / Learner View

- 반론:
- 보강 또는 유지 결정:
- 이유:

### 12.4 Final Design Decision

- 최종 채택:
- 트레이드오프:

## 13. Overall Plan

- 작업 순서:
- 선행 의존성:
- validation order:
- rollback / retry / staging 필요 여부와 이유:

## 14. Plan Critique + Repair

- 계획이 실패할 수 있는 지점:
- 순서상 위험:
- 빠진 prerequisite:
- 보강안:
- 왜 보강된 계획이 더 나은가:

## 15. Detailed Task Plan

- 수정 / 생성 / 검토할 파일:
- 각 파일에서 바꿀 논리 또는 구조:
- 관련 문서 동기화 계획:
- 예제 추가 / 보강 계획:
- 근거 섹션 반영 계획:

### 15.1 Success / Failure / Regression Cases

- 성공 케이스 최소 3개:
  - S1:
  - S2:
  - S3:
- 실패 케이스 최소 3개:
  - F1:
  - F2:
  - F3:
- 회귀 위험:
- 회귀 방지 확인 경로:

### 15.2 Code / Doc Quality Review Points

- 단순성:
- 응집도:
- 확장 여지:
- 과한 일반화 여부:
- 설명 누락 위험:
- 더 빠른 경로가 있었더라도 왜 채택하지 않았는가:
- 속도를 이유로 근거, 비교, 검증, 설명 밀도를 낮추지 않았는가:
- 목차만 보고 설명 범위를 예측할 수 있는가:
- `개요`가 질문과 범위를 바로 고정하는가:
- 문서가 직접 진술형으로 시작하고, 왜 이런 구조가 생겼는지와 먼저 알아야 할 공통 메타데이터를 초반에 고정하는가:
- 필요할 때만 짧은 실무 연결을 넣고, 본론보다 길어지지 않게 통제했는가:
- 실제 실행 경로나 동작 경로 추적이 충분히 준비된 뒤에 시작되는가:
- 메타 진행 멘트보다 직접 진술형 문장이 우세한가:
- 문장 단위 판정 질문을 다시 적용해도 서술자의 이동 / 감상 / 예고만 하는 문장이 남지 않았는가:
- 핵심 차이를 나중으로 미루지 않고 현재 문단에서 닫았는가:
- `그 사이`, `이 과정`, `이것` 같은 지시어가 그대로 남지 않았는가:
- 번호 목록과 들여쓰기가 비교 구조를 더 선명하게 만드는가:
- 순차 흐름에는 번호 목록을, 동등 비교에는 불릿 목록을 사용했는가:
- 짧은 항목은 한 줄로 닫고 긴 항목만 들여쓴 prose로 확장했는가:
- load-bearing artifact를 뒤로 미루지 않고 초반에 실제 예시와 함께 보여 주었는가:
- 구조 예시를 보여 준 뒤 감상형 문장으로 넘기지 않고, 각 구성요소의 역할을 직접 설명했는가:
- 구성요소 설명 뒤에 그것들이 어떻게 연결되어 상위 메커니즘을 만드는지 설명했는가:
- 독자가 자연스럽게 궁금해할 한 단계 아래 질문을 필요한 범위까지 닫았는가:
- 독자가 자연스럽게 궁금해할 역사 / 등장 맥락 / 이전 방식 질문을 필요한 범위까지 닫았는가:
- 공식 자료를 인용한 뒤 출처 요약에서 멈추지 않고, 인과 흐름과 구조 설명으로 자연스럽게 이어졌는가:
- 현재 머신 관측과 플랫폼 일반 규칙을 섞지 않았는가:
- teach-back 목표가 문서 초반 설계와 최종 검수에 실제로 반영되었는가:
- learner gap을 먼저 진단하고 첫 번째 벽돌을 추상 설명보다 이르게 제공했는가:
- 거의 모르는 독자에게 낱낱이 설명하되 어린아이를 대하는 말투나 불필요한 미사여구를 제거했는가:
- active recall, 작은 실험, replay, 손풀이 trace 중 적어도 하나가 실제 학습 전이에 도움이 되는가:
- primary / secondary exemplar의 참고할 설명 원리와 따라 하지 않을 visible trait가 분리되어 있는가:

## 16. Detailed Plan Critique + Repair

- 누락된 케이스:
- fuzzy success criteria:
- scope overreach / under-specification:
- 보강안:
- 최종 상세 계획:

## 17. Frozen Checklist

> 사용자 항목은 최소 바닥선입니다. 삭제, 병합, 완화 금지.

### 17.1 Checklist Draft

- C-01
  - 출처: `사용자 | AI-추가`
  - 내용:
  - PASS 기준:
  - FAIL 기준:
  - 필요한 증거:
- C-02
  - 출처:
  - 내용:
  - PASS 기준:
  - FAIL 기준:
  - 필요한 증거:
- C-03
  - 출처:
  - 내용:
  - PASS 기준:
  - FAIL 기준:
  - 필요한 증거:
- C-04
  - 출처:
  - 내용:
  - PASS 기준:
  - FAIL 기준:
  - 필요한 증거:

### 17.2 Checklist Quality Review

- [ ] 각 항목이 목표, 이점, 불변식에 매핑된다.
- [ ] PASS/FAIL이 관측 가능하다.
- [ ] 필요한 근거 또는 검증 경로가 있다.
- [ ] 사용자 요구가 조용히 약화되지 않았다.
- [ ] 한 항목 실패 시 task가 reopened 되는 구조다.
- 판정:
- 보완 사항:

### 17.3 Freeze

- freeze 시각:
- freeze 버전:
- freeze 이후 추가된 항목과 이유:

## 18. Execution Log

- 실제 조사한 것:
- 실제 수정한 것:
- 실행 중 바뀐 가정:
- earliest affected phase로 되돌아간 이력:
- 버린 접근과 이유:

## 19. Verification

### 19.1 Verification Plan

- 실행 / 확인할 명령:
- 확인 경로:
- PASS 조건:
- FAIL 조건:

### 19.2 Verification Result

- 실제 실행한 검증:
- 결과:
- 실행하지 못한 검증과 이유:
- 예제 / 명령 검증 범위:
- 소스 검증 범위:

## 20. Explanation Quality Review

- 결론이 초반에 드러나는가:
- 질문에 대한 직접 답이 초반에 드러나는가:
- 왜 중요한지가 닫히는가:
- 근거와 제약이 설명에 연결되는가:
- 검증 경로가 보이는가:
- 의미 있는 대안과 트레이드오프가 남는가:
- 구체적인 anchor가 있는가:
- 불확실성이 정직하게 표시되는가:
- 전이 가능한 원리가 남는가:
- 필요한 추상화 계층이 연결되는가:
- 핵심 대조쌍이 대칭적으로 설명되는가:
- 현재 저장소의 낮은 품질 문서를 답습하지 않았는가:
- 선택한 exemplar 수준까지 충분히 끌어올렸는가:
- exemplar를 막연히 흉내 낸 것이 아니라 route에 맞는 설명 원리를 선별했는가:
- 독자가 이 문서로 가까운 문제를 하나 더 풀거나 틀린 설명을 반박할 수 있는가:
- active recall / replay 과제가 실제 핵심 메커니즘을 다시 확인하게 하는가:

## 21. Final Audit & Closure

- intent-fit review:
- expert-perspective review:
- remaining risks:
- 문서 / 예제 / 관련 자산 동기화 상태:

### 21.1 Checklist Re-Judgement

- C-01: `PASS | FAIL | N/A`
- C-02: `PASS | FAIL | N/A`
- C-03: `PASS | FAIL | N/A`
- C-04: `PASS | FAIL | N/A`

### 21.2 Final State

- 최종 상태: `COMPLETE | PARTIAL | BLOCKED`
- 완료 게이트: `ALLOW_COMPLETE | BLOCK_COMPLETE`
- COMPLETE 승격 조건:
- 커밋 해시 / 미커밋 사유:
