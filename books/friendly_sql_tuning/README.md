...
세그먼트가 테이블 스페이스로부터 익스텐트를 추가로 할당받을 수 있지만, 세그먼트에 할당된 모든 익스텐트가 같은 데이터 파일에 위치하는 것은 아닙니다. 오히려 서로 다른 데이터 파일에 위치할 가능성이 높습니다.
...
테이블 풀 스캔을 피해야 한다는 인식과 달리, 인덱스가 SQL 성능을 떨어뜨리는 경우도 상당히 많다고 합니다.
