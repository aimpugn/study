...
1. Index lookup on table1 using table1_idx
    ...
    - `(cost=2246 rows=48656)`
        - `cost=시작비용..최종예상비용`을 의미합니다.
        - 비용은 DBMS 내부의 비용 모델에 따라 CPU, 디스크 읽기, 캐시 유무 등을 기반으로 추정됩니다.
        - 시간(ms)은 아니지만 비교 가능하며, 1..10보다 1..5가 더 빠를 가능성이 높습니다.
        - 2246은 이 노드 실행 시작할 때 필요한 초기 비용입니다.
        - 48656은 이 노드에서 row를 모두 읽을 떄까지의 누적 비용입니다.
    - `(actual time=0.678..12.6 rows=1146 loops=1)`
        - 이 노드가 상위 노드에 의해 한 번 호출(`loops=1`) 되었을 때 반환된 row의 개수가 1146개라는 의미입니다.
            - 가령 `rows=204, loops=1` 경우 총 204개의 row를 처리합니다.
            - 가령 `rows=3, loops=100` 경우 총 300개의 row를 처리합니다.
