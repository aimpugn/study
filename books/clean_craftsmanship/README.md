...
스티브 프리먼과 냇 프라이스가 런던 사람인 데서 그 이름이 유래했습니다.
이 학파는 유연성보다 '확실성'을 선호합니다.

단, '유연성'의 가치를 여전히 중요시한다는 점에 주의합니다.
그저 더 확실하게 하기 위해 어느 정도의 경직성을 감수하는 것 뿐입니다.

그래서 런던 학파의 경우 모의 객체와 스파이를 거리낌 없이 사용합니다.

이러한 자세는 결과보다는 '알고리즘'에 초점을 둡니다.
결과도 중요하지만, '그 결과를 얻는 과정'을 더 중요하게 여깁니다.
이는 '밖에서 안으로'(outside-in) 설계 방식으로 이어집니다.

'밖에서 안으로' 접근해 가는 프로그래머는 사용자 인터페이스에서 시작해 '한 번에 유즈케이스 하나씩' 비즈니스 규칙을 향해 나아가면서 설계합니다.
경계마다 모의 객체와 스파이를 사용해서 '내부와 통신하기 위해 사용하는 알고리즘이 잘 작동하는지 증명'합니다.

- 비즈니스 규칙을 만나면 이를 구현하고,
- 데이터베이스에 연결한 다음,
- 방향을 바꿔 모의 객체와 스파이를 사용해 '사용자 인터페이스로 다시 경로'를 테스트합니다.

'밖에서 안으로' 한 번 왕복하는 작업을 통틀어 '한 번에 하나의 유즈케이스'만 다룹니다.

##### 시카고: 유연성 > 확실성

시카고를 거점으로 했던 소트웍스(ThoughtWorks)에서 이름이 유래했습니다.
마틴 파울러가 소트웍스의 수석 과학자였다고 합니다.
한때 디트로이트 학파로도 불렸다고 합니다.

이 학파는 확실성보다 '유연성'을 선호합니다.

단, '확실성'의 가치를 여전히 중요시한다는 점에 주의합니다.
선택권이 있을 경우 테스트를 좀 더 유연하게 만드는 쪽을 선택합니다.

이러한 자세는 상호작용이나 알고리즘보다는, '결과'에 초점을 둡니다.
이는 '안에서 밖으로'(inside-out) 설계 방식으로 이어집니다.

비즈니스 규칙에서 시작해서 사용자 인터페이스를 향해 바깥쪽으로 나아갑니다.

시카고 학파는 다음 문제에 착수하기 전에 유즈케이스 하나를 처음부터 끝까지 전부 해결하지 않습니다.
'값 테스트'와 '속성 테스트'를 사용해 아무런 사용자 인터페이스 없이 몇 가지 비즈니스 규칙만 구현합니다.
'사용자 인터페이스', 그리고 '사용자 인터페이스와 비즈니스 규칙 사이의 계층'은 필요할 때 그에 맞춰 구현합니다.

비즈니스 규칙을 데이터베이스에 이르는 모든 단계에 곧바로 적용하지 않기도 합니다.
한 번에 유즈케이스를 하나씩 왕복하기보다는, '각 계층 안에서 협력할 수 있는 기회나 중복을 찾습니다'.
입력에서 출력으로 이어지는 전체 경로를 하나의 실로 꿰는 대신, 각 계층 안에서 더 굵은 줄무늬를 만들면서 일합니다.
비즈니스 규칙부터 시작하여 점점 단계적으로 사용자 인터페이스와 데이터베이스를 향해 움직입니다.
계층을 하나씩 탐험하며 추상화나 일반화의 기회와 디자인 패턴을 찾습니다.
