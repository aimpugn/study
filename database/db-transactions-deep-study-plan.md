...
이번 계획은 `multi-agent`, `dialectic-kernel`, `review-kernel`, `study-explanation` 기준으로 먼저 세운 뒤, 사용자의 추가 피드백을 반영해 `database/` 중심 계획으로 재검토한 것입니다.
따라서 아래 내용은 본문 작성 전의 고정 계약이며, 실제 작성 단계에서는 이 계획을 WORK 문서와 함께 다시 읽고 각 단위의 길이, 기존 자료 보존, 품질을 기계적으로 검증해야 합니다.

## 0.1 review-kernel 재검토 결론

추가 피드백 전의 계획은 `interviews/`를 정식 산출 위치로 두고 기존 `database-storage-search-nosql.md`를 주된 DB 원재료로 보았습니다.
하지만 실제 저장소에는 `database/mvcc.md`가 이미 41,000자 이상 정리되어 있고, `database/lock.md`, `database/replication.md`, `database/replication_lag.md`, `database/mysql/*`, `database/postgresql/*` 같은 DB 전용 자산도 같은 축에 모여 있습니다.
따라서 이전 계획의 verdict는 `ALIGN`이 아니라 `REWORK`가 맞습니다.

개선된 계획의 protected purpose는 새 파일을 많이 만드는 것이 아닙니다.
이미 `database/`에 쌓아 둔 사용자의 정리 자산을 최대한 보존하되, 중복, 비유 중심 반복, 근거가 약한 역사/엔진별 구현 주장, PostgreSQL/MySQL/Oracle 차이의 혼선을 공식 자료와 실험으로 다시 닫아 deep study 문서군으로 승격하는 것입니다.

이번 재검토에서 채택한 disposition은 `database/` canonical home + 기존 파일 보존형 재구성입니다.
`interviews/`에는 나중에 필요하면 커리큘럼 링크만 둘 수 있지만, DB 지식의 본문과 검증 단위는 `database/` 아래에 둡니다.
...
- scope: `database/` 루트의 정식 학습 자산. 특히 기존 `database/mvcc.md`는 source reservoir가 아니라 보존하며 재구성할 핵심 대상이다. `interviews/database-storage-search-nosql.md`, `interviews/spring-backend-frameworks.md`, `interviews/distributed-systems-architecture.md`, `jvm/spring/spring_transactional.md`, `knowledge/cards/*`, `domains/firmbanking/*`, `domains/payment/*`, `math/*`는 연결 자산으로 본다.
...
정식 본문은 `database/` 아래에 둡니다.
사용자가 이미 DB 관련 지식을 이 디렉터리에 모아 왔고, `database/mvcc.md`는 MVCC의 역사, 두 구현 흐름, Oracle/PostgreSQL/MySQL 비교, isolation, lock까지 긴 원재료를 가지고 있습니다.
따라서 새 계획은 `interviews/`에 별도 monograph를 만드는 방향을 폐기하고, `database/`의 기존 파일을 살리는 쪽으로 바꿉니다.

권장 파일 구조는 다음입니다.

1. `database/db-transactions-deep-study.md`

    전체 지도, 읽는 순서, 핵심 용어, 각 상세 문서 링크를 담는 허브 문서입니다.
    이 파일은 각 장문 문서를 대체하지 않고, `database/` 안에서 어떤 순서로 읽어야 하는지와 어떤 시나리오에서 어느 문서로 가야 하는지를 고정합니다.

2. `database/transaction-lifecycle-acid.md`

    트랜잭션 생명주기, autocommit, `BEGIN/COMMIT/ROLLBACK/SAVEPOINT`, ACID, 제약조건과 도메인 불변식을 다룹니다.

3. `database/wal-undo-redo.md`

    WAL, redo, undo, crash recovery, commit record, fsync, checkpoint, group commit을 다룹니다.

4. `database/mvcc.md`

    기존 파일을 폐기하지 않고 in-place로 재구성합니다.
    이미 있는 역사, Reed/Bernstein/Stonebraker 계열 배경, MGA와 rollback segment 비교, Oracle/PostgreSQL/MySQL 비교, XMIN/XMAX/undo/read view 설명을 살리되, 중복 문장과 근거가 약한 단정은 줄이고 공식 자료와 2-session 실험으로 보강합니다.

5. `database/isolation-levels.md`

    SQL 표준 격리 수준, anomaly, PostgreSQL과 MySQL InnoDB의 실제 차이를 다룹니다.
    `database/mvcc.md`의 격리 수준 설명은 이 파일과 상호 링크하고, MVCC 문서 안에서는 snapshot visibility와 engine implementation 중심으로 압축합니다.

6. `database/lock.md`

    기존 짧은 `database/lock.md`를 확장합니다.
    row/table/gap/next-key/predicate/advisory lock, 2PL, deadlock, lock wait 관측을 다룹니다.

7. `database/consistency-constraints.md`

    ACID의 C, unique/check/foreign key, optimistic lock, version column, compare-and-swap, 도메인 불변식 검증을 다룹니다.

8. `database/idempotency-duplicate-request.md`

    따닥, 중복 요청, HTTP retry, idempotency key, request hash, DB winner election, response replay, 외부 부작용 재시도를 다룹니다.

9. `database/money-calculation.md`

    금액 표현, `BigDecimal`, scale, rounding, allocation, minor unit, DB/app representation, overflow, 취소/환불 금액을 다룹니다.

10. `database/spring-transaction-boundary.md`

    Spring `@Transactional`, proxy, propagation, rollback rule, resource binding, async/reactive boundary를 DB transaction 관점에서 다룹니다.
    세부 Spring 원재료는 `jvm/spring/spring_transactional.md`에 남기되, 이 문서는 DB transaction 학습 흐름으로 다시 엮습니다.

11. `database/bank-transfer-payment-state-machine.md`

    은행 이체, 결제 승인/매입/취소/환불, 원장, available balance, 상태 전이를 다룹니다.

12. `database/external-response-reconciliation.md`

    외부 은행/PG가 성공했지만 우리 서버가 timeout을 본 경우, 응답 유실, duplicate callback, 결과 파일, 정산, 대사, 수동 보정 기준을 다룹니다.

13. `database/outbox-saga-2pc.md`

    2PC/XA/JTA, saga, transactional outbox/inbox, claim lease, replay, consumer idempotency를 다룹니다.

이 구조의 장점은 세 가지입니다.
첫째, 기존 `database/` 학습 축과 사용자의 정리 흔적을 살립니다.
둘째, `mvcc.md`처럼 이미 큰 원재료가 있는 파일은 대체하지 않고 품질을 끌어올립니다.
셋째, 각 주요 파일을 15,000자 검증 단위로 삼을 수 있어 filler 없이 깊이를 관리하기 쉽습니다.
...
검증 단위는 T id에 대응되는 파일 본문입니다.
각 파일은 첫 `#` 제목 줄을 제외한 Unicode 문자 수가 15,000자 이상이어야 하며, 표와 코드 블록은 본문 문자 수에 포함합니다.
`database/mvcc.md`처럼 기존에 여러 `##`가 있는 파일도 T03 하나의 보존형 재구성 단위로 검증합니다.

| id | 대상 파일 | 본문 제목 후보 | 최소 본문 길이 | 상태 |
|---|---|---|---:|---|
| T01 | `database/transaction-lifecycle-acid.md` | 트랜잭션 생명주기와 ACID | 15,000자 | 계획 개선 완료 |
| T02 | `database/wal-undo-redo.md` | WAL, undo, redo, 지속성 | 15,000자 | 계획 개선 완료 |
| T03 | `database/mvcc.md` | MVCC와 snapshot | 15,000자 | 기존 파일 보존형 재구성 대상 |
| T04 | `database/isolation-levels.md` | 격리 수준과 anomaly | 15,000자 | 계획 개선 완료 |
| T05 | `database/lock.md` | 락, 2PL, deadlock | 15,000자 | 기존 파일 확장 대상 |
| T06 | `database/consistency-constraints.md` | consistency, 제약조건, 도메인 불변식 | 15,000자 | 계획 개선 완료 |
| T07 | `database/idempotency-duplicate-request.md` | 따닥, 중복 요청, 멱등성 | 15,000자 | 계획 개선 완료 |
| T08 | `database/money-calculation.md` | 금액 계산과 반올림 정책 | 15,000자 | 계획 개선 완료 |
| T09 | `database/spring-transaction-boundary.md` | Spring 트랜잭션 경계 | 15,000자 | 계획 개선 완료 |
| T10 | `database/bank-transfer-payment-state-machine.md` | 은행 이체와 결제 상태 머신 | 15,000자 | 계획 개선 완료 |
| T11 | `database/external-response-reconciliation.md` | 외부 응답 유실과 정산/대사 | 15,000자 | 계획 개선 완료 |
| T12 | `database/outbox-saga-2pc.md` | 분산 트랜잭션, saga, outbox | 15,000자 | 계획 개선 완료 |

`database/db-transactions-deep-study.md` 허브 문서는 15,000자 이상 필수 단위가 아닙니다.
...
- `database/mvcc.md`: 기존 MVCC 핵심 원재료. 역사, 등장 배경, MGA/rollback segment, Oracle/PostgreSQL/MySQL 비교, isolation/lock 연결을 최대한 살리되, 중복과 근거 약한 단정은 공식 자료와 실험으로 재검증합니다.
- `database/lock.md`: 기존 lock 원재료. 현재는 짧으므로 T05의 확장 대상입니다.
- `database/replication.md`, `database/replication_lag.md`: commit log, replication, 지연과 트랜잭션 관측을 연결할 때 참고합니다.
- `database/mysql/*`, `database/postgresql/*`: MySQL/PostgreSQL 엔진 차이, introspection, lock 관측의 로컬 원재료로 봅니다.
- `interviews/database-storage-search-nosql.md`: interviews 커리큘럼의 DB 원문 배치본. ACID, MVCC, lock, 2PC/2PL의 연결 문맥으로 참고하되, canonical 본문은 `database/`에 둡니다.
...
- claim: canonical 본문은 `interviews/`가 아니라 `database/`에 두고, 기존 `database/mvcc.md`와 `database/lock.md`를 보존형 재구성 대상으로 삼아야 한다.
- premises: 사용자는 DB 관련 내용을 `database/`에 모아 왔다고 명시했고, 실제 저장소에도 `database/mvcc.md`가 41,000자 이상 존재한다. 이번 주제는 DB 내부, 애플리케이션 경계, 금융 시나리오가 서로 다른 실패 모델을 가진다.
- attack: `interviews/`는 커리큘럼 표면이므로 면접 학습 연결성이 더 좋을 수 있다.
- response lane: ACCEPT_REPAIR.
- repaired claim: 본문과 검증 단위는 `database/`에 두고, `interviews/`에는 나중에 필요한 경우 링크 또는 커리큘럼 안내만 둔다. `database/mvcc.md`는 새 파일로 대체하지 않고 기존 내용을 살려 재구성한다.
- support tier: T1 direct repo evidence for existing `database/mvcc.md` and user feedback; T2 for final file split.
- admission lane: APPLY for planning, later writing에서 재검증.

### C2. 15,000자 단위 claim

- claim: 각 `##` 주요 개념/시나리오를 최소 15,000자 검증 단위로 둔다.
- premises: 이전 긴 문서 작업에서는 `###` subsection length가 검증 단위였지만, 이번에는 T id마다 canonical 파일을 두는 구조이므로 파일 본문이 더 자연스러운 검증 단위다.
...
- response lane: ACCEPT_REPAIR.
- synthesis: T id별 파일 본문을 첫 `#` 제목 줄 제외 15,000자 이상으로 측정하고, length TSV를 남긴다. 기존 `database/mvcc.md`처럼 여러 `##`가 있는 파일은 파일 전체를 한 T 단위로 검증한다.
...
- domain packs/evidence: study-explanation, repo-local study rules, `database/` corpus, 기존 `database/mvcc.md`, DB 공식 문서 후보, Spring/Java/HTTP 공식 문서 후보, 로컬 idempotency/outbox/payment/firmbanking/math 원재료.
- evidence boundary: 이 계획은 source strategy와 registry를 닫은 planning artifact다. `database/` 배치와 `mvcc.md` 존재는 repo/user direct evidence로 닫혔지만, DB 엔진별 사실과 금융 도메인 세부 결론은 다음 writing tranche에서 공식 자료와 실험으로 다시 닫아야 한다.
...
- verdict: `REWORK -> ALIGN for revised planning tranche only`.
- repair disposition: 사용자 피드백으로 드러난 canonical path 오류를 ACCEPT_REPAIR했다. 계획 파일을 `database/`로 옮기고, T03은 `database/mvcc.md` in-place 재구성으로 바꾸며, future targets를 `database/*.md`로 수정했다.
- residual risk: 계획은 수정됐지만 실제 본문 품질은 아직 검증되지 않았다. T01-T12 작성 전 이 계획과 WORK를 다시 읽고 source-to-claim mapping을 새로 채워야 한다. 특히 `database/mvcc.md`는 기존 내용을 지우는 것이 아니라 보존/정리/검증/재배치 ledger를 남겨야 한다.

## 10. 작성 체크리스트

실제 본문 작성 단계는 아래 체크리스트를 freeze한 뒤 시작합니다.

- [ ] `database/db-transactions-deep-study.md` 허브 파일 생성
- [ ] `database/mvcc.md`, `database/lock.md` 기존 내용 보존/재구성 ledger 작성
- [ ] T01-T12 모든 파일 단위 작성 또는 기존 파일 확장
- [ ] 각 T 파일 body 15,000자 이상
...
targets = {
    "T01": Path("database/transaction-lifecycle-acid.md"),
    "T02": Path("database/wal-undo-redo.md"),
    "T03": Path("database/mvcc.md"),
    "T04": Path("database/isolation-levels.md"),
    "T05": Path("database/lock.md"),
    "T06": Path("database/consistency-constraints.md"),
    "T07": Path("database/idempotency-duplicate-request.md"),
    "T08": Path("database/money-calculation.md"),
    "T09": Path("database/spring-transaction-boundary.md"),
    "T10": Path("database/bank-transfer-payment-state-machine.md"),
    "T11": Path("database/external-response-reconciliation.md"),
    "T12": Path("database/outbox-saga-2pc.md"),
}

for unit, path in targets.items():
    text = path.read_text(encoding="utf-8")
    lines = text.splitlines()
    body_lines = lines[1:] if lines and lines[0].startswith("# ") else lines
    body = "\n".join(body_lines).strip()
    print(f"{unit}\t{path}\t{len(body)}")
PY
```

PASS 기준은 T01-T12 각 파일 body length가 15,000 이상인 것입니다.
FAIL 기준은 하나라도 15,000 미만이거나, target file이 누락되거나, 중복 장문 문단/깨진 link/fence 불균형이 발견되는 것입니다.
...
예상 행동은 `database/` 아래 허브와 상세 문서를 작성하고, `database/mvcc.md`와 `database/lock.md`는 기존 내용을 살려 확장/재구성하며, 검증 후 commit하는 것입니다.

되돌림 가능성은 높습니다.
대부분은 새 파일 중심으로 작성하지만, `database/mvcc.md`와 `database/lock.md`는 기존 파일을 직접 바꾸므로 변경 전후 구조 비교와 path-limited diff review가 필요합니다.
다만 잘못된 DB/금융 사실을 장문으로 쓰면 학습 자산에 장기 오류가 남으므로, DB 엔진 차이와 금융 시나리오는 공식 자료와 실험으로 닫아야 합니다.

safer path는 다음입니다.

1. 이 계획 파일을 먼저 review한다.
2. `database/mvcc.md`의 보존할 내용, 이동할 내용, 삭제/축약할 중복을 먼저 ledger로 나눈다.
3. T01-T06에 해당하는 `database/transaction-lifecycle-acid.md`, `database/wal-undo-redo.md`, `database/mvcc.md`, `database/isolation-levels.md`, `database/lock.md`, `database/consistency-constraints.md`를 먼저 작성하고 검증한다.
4. T07-T09 application boundaries를 작성하고 검증한다.
5. T10-T12 financial scenarios를 작성하고 검증한다.
6. 허브 문서를 마지막에 연결한다.
...
- achieved closure scope: finite concept/scenario registry, `database/` canonical topology, `database/mvcc.md` 보존형 재구성 방침, teaching spine, evidence strategy, validation strategy, critic/sentinel/user-feedback repairs.
- whole-request completion verdict: REQUEST_PARTIAL.
- remaining executable count: 12 primary writing units plus hub creation.
- next immediate target: `database/mvcc.md` 보존/재구성 ledger 작성 후 T01-T06 core concepts 작성.
