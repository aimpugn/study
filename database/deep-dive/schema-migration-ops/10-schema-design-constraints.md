...
마지막으로, online DDL과 migration 도구가 있어도 "큰 테이블 변경"은 여전히 별도 설계 대상이다. 수억 행 테이블에 `NOT NULL` 제약을 추가하거나 타입을 바꾸거나 primary key를 재구성하는 작업은 단순 DDL보다 데이터 복사 프로젝트에 가깝다. 이때는 shadow table, dual write, trigger 기반 동기화, gh-ost/pt-online-schema-change 같은 online schema change 도구, 또는 애플리케이션 레벨 분할 migration을 검토할 수 있다. 이 문서의 registry는 MySQL online DDL과 Flyway를 공식 source로 요구하지만, 실무 판단에서는 DBMS 내장 online DDL이 충분한지, 외부 online schema change 도구가 필요한지, 아니면 도메인 모델을 나눠 단계적으로 옮겨야 하는지 비교해야 한다. 내장 DDL이 충분하다는 결론도 staging 리허설과 관측 지표가 있어야 한다.
