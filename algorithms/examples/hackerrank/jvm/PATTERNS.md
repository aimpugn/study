...
**재귀·DFS·메모/DP는 호출 트리를 세지 말고 상태 장부를 세라** — visited/캐시가 있으면 각 상태의 본문은 딱 1번 돌고, 나머지 호출은 O(1)로 반사된다.
...
- bottom-up DP: 표 크기 x 칸당 비용 (for가 장부를 그대로 노출해서 세기 쉬운 것 — 메모 재귀도 같은 장부다)
