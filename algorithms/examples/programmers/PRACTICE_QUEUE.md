...
여기에 카카오, PCCP, 최근 Lv.2~Lv.4 문제를 섞어 실전 감각을 보강합니다.

이 큐의 앞 80문제는 공식 난이도 `Lv.1~Lv.3` 중심입니다.
목표는 어려운 문제만 몰아치는 것이 아니라, `Lv.2`를 안정적으로 풀고 `Lv.3`에 들어가도 무너지지 않는 저점을 만드는 것입니다.
마지막 20문제는 모두 공식 `Lv.4`로 채웠습니다.
이 구간은 바로 합격 컷처럼 취급하기보다, `어려운 문제는 어떤 생각의 단위로 쪼개야 하는가`를 배우는 브리지 구간입니다.
...
그래도 앞 구간은 손풀이와 기본 자료구조, 중간 구간은 문제 변환과 최적화 감각, 마지막 구간은 어려운 문제를 분해하는 설계력을 더 많이 요구하도록 배치했습니다.

## 실력이 쌓이는 흐름

처음에는 문제를 많이 풀어도 `봤던 문제만 푸는 느낌`이 들 수 있습니다.
그래서 이 큐는 유형을 무작정 섞기보다, 같은 생각을 조금씩 다른 얼굴로 다시 만나도록 배치했습니다.
해시로 개수를 세는 문제를 지나면 정렬로 이웃만 보는 문제가 나오고, 단순 큐 시뮬레이션을 지나면 우선순위 큐와 시간 흐름을 함께 보는 문제가 나옵니다.
이렇게 해야 풀이가 한 문제 안에 갇히지 않고, 다음 문제에서 다시 꺼낼 수 있는 판단 문장으로 남습니다.

`1~40`은 손을 다시 깨우는 구간입니다.
여기서 중요한 것은 화려한 알고리즘 이름보다 `입력을 어떤 형태로 바꿔 들고 있을까`를 빨리 정하는 힘입니다.
`41~80`은 문제를 다른 형태로 바꾸는 구간입니다.
전체를 매번 다시 보지 않기 위해 전처리를 하거나, 답을 직접 찾지 않고 가능한지 판정하거나, 위치 하나가 아니라 방향과 비용까지 상태로 묶습니다.
`81~100`은 문제를 설계 단위로 쪼개는 구간입니다.
어려운 Lv.4는 대개 코드 한 줄이 어려운 것이 아니라, 어떤 상태를 들고 가야 하는지와 어떤 검사를 미리 끝내야 하는지가 잘 보이지 않아서 어렵습니다.

그래서 마지막 20문제는 `풀 수 있느냐`보다 `무엇이 떠올라야 하는지`를 남기는 용도로 쓰면 좋습니다.
예를 들어 `사칙연산`은 구간 DP에서 최댓값과 최솟값을 같이 들고 가야 한다는 감각을 주고, `호텔 방 배정`은 해시맵이 단순 저장소가 아니라 다음 후보를 압축하는 포인터처럼 쓰일 수 있음을 보여 줍니다.
`동굴 탐험`과 `미로 탈출`부터는 그래프 문제가 단순 방문 문제가 아니라 선행 조건, 잠긴 상태, 토글된 상태를 함께 다루는 모델링 문제로 바뀝니다.
마지막에 가까운 문제들은 거의 Lv.5처럼 느껴질 수 있지만, 그만큼 `상태 정의 -> 불변식 -> 검증 가능한 전이`라는 큰 틀을 각인시키기에 좋습니다.
