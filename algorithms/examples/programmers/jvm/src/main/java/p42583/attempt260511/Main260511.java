...
class Solution {
    ...
    public int solution(int bridge_length, int weight, int[] truck_weights) {
        ...
        //
        // 여기서 ArrayDeque를 쓰는 이유도 이 FIFO 성질 때문입니다.
        // ArrayList는 내부 배열을 리스트처럼 쓰게 해 주는 구조라서 get(0), get(1)처럼
        // 인덱스로 읽을 때는 편하지만, 맨 앞 원소를 remove(0)로 빼면 뒤 원소들을 한 칸씩 당겨야 합니다.
        // 이 문제처럼 "앞에서 꺼내고 뒤에 넣는" 작업을 계속하면 그 당김 비용이 반복됩니다.
        //
        // LinkedList는 말씀하신 것처럼 [] - [] - [] 형태의 노드 연결 구조입니다.
        // 맨 앞과 맨 뒤를 가리키는 포인터가 있어서 앞에서 빼고 뒤에 넣는 큐 작업은 잘 맞습니다.
        // 다만 각 값마다 별도 노드 객체가 생기고, 다음 노드를 따라가야 하므로 배열처럼 가까운 메모리를
        // 순서대로 읽는 장점은 약합니다. 이 문제는 중간 노드를 자르거나 삽입할 일이 없으니,
        // 노드 구조의 유연함보다 더 단순한 양끝 삽입/삭제 구조가 잘 맞습니다.
        //
        // ArrayDeque는 이름 그대로 양쪽 끝에서 넣고 빼는 Deque를 배열로 구현한 구조입니다.
        // Java 21 기준 내부에는 Object[] elements, head, tail이 있고, head는 다음에 빠질 칸,
        // tail은 다음에 들어갈 칸을 가리킵니다. 배열 끝에 닿으면 다시 0번 칸으로 돌아가며 쓰기 때문에
        // "원형 배열"처럼 동작합니다.
        //
        // 예를 들어 7, 4가 들어 있고 다음에 앞에서 빠질 값이 7이라면 감각은 이렇습니다.
        // index:     0  1  2  3  4
        // elements: [ _, 7, 4, _, _ ]
        //               ^head  ^tail, 다음에 쓸 칸
        // poll()은 head의 7을 꺼내고 head를 한 칸 옮깁니다.
        // offer(5)는 tail 자리에 5를 넣고 tail을 한 칸 옮깁니다.
        // 그래서 이 문제의 대기 큐처럼 "순서를 지킨다 -> 맨 앞만 본다 -> 맨 앞을 뺀다"는 흐름을
        // 추가 노드 없이, 대부분의 양끝 작업을 상수 시간에 가깝게 처리할 수 있습니다.
        //
        // 비슷한 선택지를 문제 성질로 나누면 이렇게 기억하면 됩니다.
        // ArrayList: 인덱스로 자주 읽고, 주로 뒤에만 추가할 때 편합니다. 큐의 앞 제거에는 약합니다.
        // LinkedList: Deque 구현체이기도 해서 큐로 쓸 수 있지만, 노드 객체와 포인터 이동 비용이 붙습니다.
        // ArrayDeque: 단일 스레드 알고리즘에서 FIFO 큐나 LIFO 스택이 필요할 때 보통 가장 먼저 고를 후보입니다.
        // PriorityQueue: 우선순위가 높은 값을 먼저 꺼내는 힙이라서, 이 문제처럼 도착 순서를 지켜야 할 때는 틀린 모델입니다.
        // ConcurrentLinkedQueue: 여러 스레드가 동시에 넣고 빼는 상황을 위한 큐라서, 단일 스레드 풀이에는 동시성 의미가 과합니다.
        //
        // ArrayDeque 하나로도 세 가지 사용법이 가능합니다.
        // offer()/poll()/peek()처럼 쓰면 FIFO 큐이고,
        // push()/pop()/peek()처럼 쓰면 LIFO 스택이며,
        // offerFirst()/offerLast(), pollFirst()/pollLast()를 섞으면 양끝 큐입니다.
        // 이 문제에서는 대기 트럭과 다리 위 트럭 모두 "먼저 들어간 것이 먼저 나온다"가 불변식이므로
        // offer()로 뒤에 넣고 poll()/peek()로 앞에서 보는 FIFO 큐 용도로만 사용합니다.
