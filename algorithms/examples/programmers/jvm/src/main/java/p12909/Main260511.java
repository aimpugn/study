...
class Solution {
     ...
     * 이 문제에서는 이 조건을 "각 접두사에서 열린 괄호 수가 음수가 되지 않는 불변식"으로 설명하는 편이 자연스럽습니다.
     * prefix는 문자열 알고리즘 문맥에서 보통 접두사 또는 접두어로 옮기고, invariant는 알고리즘 정당성 설명에서 불변식으로 옮깁니다.
     * 다만 "접두사 불변식" 자체가 널리 굳은 독립 용어라기보다는, 이 풀이의 조건을 짧게 가리키는 설명용 표현에 가깝습니다.
     * <p>
    ...
    public boolean solution(String s) {
        ...
        // 끝까지 접두사마다 열린 괄호 수가 음수가 되지 않았더라도, 아직 닫히지 않은 `(`가 남아 있으면 실패입니다.
        return parenCnt == 0;
    }
}
