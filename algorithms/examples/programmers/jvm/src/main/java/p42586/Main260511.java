 ...
 * - 첫 접근: 각 작업의 완료일을 먼저 만들고, 앞 작업의 배포일 기준으로 묶습니다.
 * <p>
 * 이 문제의 Stack/Queue 분류는 "반드시 Queue를 써야 한다"는 뜻이라기보다,
 * 먼저 들어온 기능이 뒤 기능의 배포를 막는 선입선출 흐름을 읽어야 한다는 신호에 가깝습니다.
 * 배열 인덱스로 풀어도 같은 흐름을 표현할 수 있고, Queue로 풀면 그 흐름이 자료구조 이름으로 드러납니다.
 */
class Solution {
    /*
     * 이 문제는 테스트 케이스의 모양을 따라가면 흔들리기 쉽습니다.
     * 먼저 문제 문장에서 변하지 않는 규칙을 뽑고, 그 규칙을 코드 상태로 바꾸는 편이 안전합니다.
     *
     * 문제 분석은 이렇게 시작합니다.
     * 1. 반환값은 "각 기능의 완료일"이 아니라 "각 배포마다 함께 나가는 기능 수"입니다.
     * 2. 뒤 기능이 먼저 끝날 수 있지만, 앞 기능이 배포되기 전에는 뒤 기능도 먼저 배포되지 못합니다.
     * 3. 따라서 비교 기준은 직전 기능 하나가 아니라 "현재 배포 묶음을 이끄는 앞 기능의 완료일"입니다.
     * 4. 이 구조는 먼저 온 기능이 뒤 기능을 막는 선입선출 흐름이라 Queue 패턴으로 읽을 수 있습니다.
     *
     * 여기서 세울 수 있는 루프 불변식(loop invariant)은 풀이 방식마다 조금 다릅니다.
     * 루프 불변식은 반복문이 한 바퀴 돌기 전과 돈 뒤에도 계속 참이어야 하는 조건입니다.
     *
     * - 인덱스 풀이: `start`보다 앞의 기능들은 정확히 한 번씩 배포 묶음에 들어갔고,
     *   `start`는 아직 처리하지 않은 첫 기능입니다.
     * - Queue 풀이: Queue에는 아직 배포 묶음에 넣지 않은 기능들의 완료일만 원래 순서대로 남아 있고,
     *   Queue의 맨 앞 값이 이번 배포 묶음의 기준일입니다.
     *
     * 이 불변식이 있으면 `테스트 1은 [2, 1]이니까 이렇게 세면 되겠다`가 아니라,
     * `이번 묶음을 닫는 순간 다음 시작점은 어디인가`를 먼저 묻게 됩니다.
     * 그 질문이 바로 기존 풀이에서 `i = j - 1`이 필요했던 이유이고,
     * 개선 풀이에서는 `start = next` 또는 `poll()`로 더 직접 표현됩니다.
     *
     * 조건과 루프는 다음 순서로 정합니다.
     * 1. 바깥 반복의 한 단위는 "배포 묶음 하나를 완성하는 일"입니다.
     *    그래서 바깥 루프는 아직 처리하지 않은 기능이 남아 있는 동안만 반복합니다.
     * 2. 안쪽 반복의 한 단위는 "다음 기능이 현재 배포 묶음에 들어갈 수 있는지 확인하는 일"입니다.
     *    그래서 안쪽 루프 조건은 `다음 기능이 존재한다`와 `그 기능의 완료일이 기준일보다 늦지 않다`가 됩니다.
     * 3. 안쪽 루프가 멈추는 순간은 현재 묶음이 닫히는 순간입니다.
     *    이때 멈춘 위치가 다음 배포 묶음의 시작점입니다.
     *
     * 자료구조는 이 규칙을 어떻게 표현하고 싶은지에 따라 고릅니다.
     * - 배열 인덱스는 `start`, `next`로 다음 시작 위치를 직접 보여 주기 좋습니다.
     ...
     *
     * start=0, 기준일=5
     *   next=1의 10은 5보다 늦게 끝납니다.
     *   이번 묶음은 [5] 하나이고, 다음 start는 1입니다.
     *
     * start=1, 기준일=10
     *   next=2,3,4의 1은 10보다 늦지 않으므로 같은 묶음입니다.
     *   next=5의 20은 10보다 늦게 끝납니다.
     *   이번 묶음은 [10, 1, 1, 1] 네 개이고, 다음 start는 5입니다.
     *
     * start=5, 기준일=20
     *   남은 기능은 [20] 하나입니다.
     *
     ...
     * - 배포는 하루에 한 번만 가능하고, 하루의 끝에 이루어진다고 가정합니다.
     ...
     * 바깥 for문의 자동 증가에 기대는 `i = j - 1` 보정을 없앤 버전입니다.
     *
     * 여기서 가장 중요한 불변식은 다음 한 문장입니다.
     * `start`는 아직 어떤 배포 묶음에도 넣지 않은 첫 번째 기능을 가리킵니다.
     * 이 불변식이 보이면 `total` 같은 별도 개수 보정 없이도 다음 시작 위치를 직접 정할 수 있습니다.
     */
    public int[] solutionByExplicitStartIndex(int[] progresses, int[] speeds) {
        ...
        // 바깥 루프의 반복 단위는 "배포 묶음 하나 만들기"입니다.
        // start는 아직 처리하지 않은 첫 기능이고, 한 묶음을 만들 때마다 다음 묶음의 첫 위치로 이동합니다.
        int start = 0;
        while (start < remained.length) {
            // 배포 묶음의 기준일은 그 묶음에서 가장 앞에 있는 기능의 완료일입니다.
            // 뒤 기능이 더 빨리 끝나도 이 기준일 전에는 먼저 배포될 수 없습니다.
            int deployDay = remained[start];

            // next는 현재 묶음에 들어갈 수 있는지 검사할 후보 위치입니다.
            // start는 묶음의 대표 기능으로 이미 포함됐으므로 바로 다음 칸에서 시작합니다.
            int next = start + 1;

            // 안쪽 루프의 조건은 두 부분으로 나뉩니다.
            // next < remained.length: 검사할 후보 기능이 실제로 남아 있어야 합니다.
            // remained[next] <= deployDay: 후보 기능이 기준일보다 늦게 끝나지 않아야 같은 묶음에 들어갑니다.
            // 조건이 깨지는 순간 next는 이번 묶음에 들어가지 못하는 첫 위치, 즉 다음 묶음의 시작점입니다.
            while (next < remained.length && remained[next] <= deployDay) {
                next++;
            }

            // start는 묶음의 첫 위치, next는 묶음이 끝난 다음 위치입니다.
            // 따라서 next - start가 이번 배포에 포함된 기능 수가 됩니다.
            answer.add(next - start);

            // 다음 반복은 아직 묶지 않은 첫 기능에서 시작합니다.
            // 이 줄이 기존 풀이의 `i = j - 1`이 표현하려던 의미를 더 직접 드러냅니다.
            start = next;
        }
     ...
     * Queue를 사용한 풀이입니다.
     *
     * Queue는 먼저 들어온 값이 먼저 나가는 자료구조입니다. 이 문제에서는 앞 기능이 아직 배포되지 않으면
     * 뒤 기능도 먼저 나갈 수 없으므로, 완료일을 앞에서부터 꺼내며 배포 묶음을 만드는 흐름과 잘 맞습니다.
     * 다만 Queue가 배열 인덱스 풀이보다 시간복잡도를 낮추지는 않습니다. 둘 다 O(n)이고,
     * Queue 버전은 완료일을 담는 별도 자료구조 때문에 메모리를 조금 더 씁니다.
     */
    public int[] solutionByQueue(int[] progresses, int[] speeds) {
        // 각 기능의 완료일을 배포 대기열에 넣습니다.
        ...
        // 바깥 루프의 반복 단위는 인덱스 풀이와 같습니다.
        // Queue가 비지 않았다는 것은 아직 어떤 배포 묶음에도 넣지 않은 기능이 남아 있다는 뜻입니다.
        while (!waitingDays.isEmpty()) {
            // 맨 앞 기능은 이번 배포 묶음의 기준이 됩니다.
            // poll은 값을 꺼내면서 Queue에서 제거하므로, 같은 기능을 다시 세지 않습니다.
            int deployDay = waitingDays.poll();

            // 기준 기능 하나는 이미 이번 배포에 포함됐습니다.
            int count = 1;

            // 안쪽 루프는 Queue의 맨 앞 기능이 현재 묶음에 들어갈 수 있는 동안만 반복합니다.
            // !waitingDays.isEmpty()를 먼저 확인해야 peek 결과가 없는 상태를 피할 수 있습니다.
            // waitingDays.peek() <= deployDay는 "앞 기능이 배포될 때 같이 나갈 수 있는가"라는 문제 규칙을 그대로 옮긴 조건입니다.
            while (!waitingDays.isEmpty() && waitingDays.peek() <= deployDay) {
                waitingDays.poll();
                count++;
            }

            // 기준일보다 더 늦게 끝나는 기능을 만나면 이번 묶음은 닫힙니다.
            answer.add(count);
        }
