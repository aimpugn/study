...
**모델은 도구를 실행하지 않습니다.** 어떤 도구를 어떤 인자로 써야 하는지를 담은 출력을 만들고 멈출 뿐이고, 실행은 그 출력을 받은 쪽이 합니다. 누가 받느냐는 도구 종류에 따라 갈리는데, 그 구분은 바로 아래에서 정리합니다.
...
Anthropic은 2024년 5월 30일 Claude의 tool use를 정식 기능으로 발표했습니다. 이 API는 모델의 선택과 외부 프로그램의 실행을 `tool_use`와 `tool_result`라는 블록으로 연결했습니다.

그 실행 주체는 두 갈래입니다. 내가 정의하고 내 프로그램에서 돌리는 도구를 **client tool**이라 부릅니다 — 사내 API 조회나 파일 쓰기 같은 것으로, 모델이 요청하면 내 코드가 실행합니다. 반대로 웹 검색이나 코드 실행처럼 Anthropic 서버에서 돌아가는 것을 **server tool**이라 합니다. 실행 주체는 다르지만 모델이 직접 함수를 돌리지 않는다는 경계는 양쪽 모두 같습니다.

이 문서는 **client tool의 한 차례 왕복을 주 경로**로 설명하고, server tool은 경계가 달라지는 지점에서 따로 표시합니다.
...
일부 `tool_use_id`의 결과를 빠뜨리면 요청이 거절됩니다. 결과를 여러 user 메시지로 쪼개 보내는 것은 공식 문서가 잘못된 예로 드는 형식입니다. 모든 결과를 한 번에 돌려줘야 모델이 이후에도 병렬로 호출한다고 안내합니다. 오해하기 쉬운데 이건 가중치를 다시 학습시킨다는 뜻이 아니라, **API 계약과 지금 이 대화의 문맥을 함께 지키는 규칙**입니다. 쪼개 보낸 기록이 대화에 남으면 모델이 그 패턴을 따라갑니다.
