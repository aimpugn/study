...
- [java/resource_management_and_leaks.md](./java/resource_management_and_leaks.md) — GC가 *무엇을* 책임지나(힙 도달성)와 *못 치우는 것*(외부 자원). 이 문서가 "*어떤* GC를 고르나"라면, 그쪽은 "GC의 책임 범위는 어디까지인가".
- [java/graalvm_native_image.md](./java/graalvm_native_image.md) — native-image가 쓸 수 있는 GC(에디션별로 Serial/G1 등 제한).
