...
- [metaspace_leak_diagnosis.md](./metaspace_leak_diagnosis.md) — 여기 없는 진단용 JDK 도구(`jcmd`/`jstat`/`jmap`)를 누수 추적에 쓰는 실전 레시피.
- [imports_linking_and_loading.md](./imports_linking_and_loading.md) — `javac`가 만든 `.class`를 `javap`로 들여다보기, 그리고 클래스가 런타임에 로드되는 방식.
